...
Connector 需要法智 API Key。TRAE 负责收集并在连接 MCP 时将
`ACCESS_TOKEN` 注入 `open-authorization` 请求头，Skill 不应读取、
展示或要求用户在对话中发送凭证。
...
> **注意**：在调用 MCP 工具前，必须先通过 `LS` 读取 MCP 服务器目录下的工具描述文件，确认工具名称和参数 schema，再使用 `run_mcp` 调用。实际 `server_name` 以 TRAE 运行时注册的 MCP 服务器标识符为准。
...
法规详情页链接格式：`https://www.fazhi.law/law-saas/lawDetail/{law_id}`
