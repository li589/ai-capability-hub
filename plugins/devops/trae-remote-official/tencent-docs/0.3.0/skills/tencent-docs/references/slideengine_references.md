...
| 调用方式 | 直接调用 Trae 加载的 `slide-mcp` 服务                                                    |
...
需要多页信息时，由 Trae 按页调用本工具并只保留后续操作所需字段。只需要总页数或页面列表时先调用 `slide_get_info`，不要加载所有页面详情。
...
优先通过 `tencent-docs` 服务的 `upload_image` 上传图片并取得 `image_id`，再调用 `slide_add_image`。对于体积较小且当前客户端能够安全传递的图片，也可以直接使用 `content` 传入 base64。

不要通过本地脚本读取 connector Token，不要要求用户提供开放平台凭证。若本地图片无法由当前客户端安全传递，请让用户提供可访问的图片 URL，或先在腾讯文档中完成上传。
...
| `image_id` | string | 否 | 通过已授权的 `upload_image` MCP 工具取得的图片 ID，有效期一天；与 `content` 二选一 |
| `content` | string | 否 | 图片的纯 base64 内容，仅适合体积较小且当前客户端能够安全传递的图片；与 `image_id` 二选一 |
...
| `file_id` | string | 否 | 文档ID，与 file_url 二选一 |
| `file_url` | string | 否 | 在线幻灯片的URL链接，与 file_id 二选一 |
| `<addChartArgs>` | object | 否 | 更多参数详见 Trae 当前加载的 slide-mcp 的 schema |
...
| `file_id` | string | 否 | 文档ID，与 file_url 二选一 |
| `file_url` | string | 否 | 在线幻灯片的URL链接，与 file_id 二选一 |
| `<chartTitleArgs>` | object | 否 | 更多参数详见 Trae 当前加载的 slide-mcp 的 schema |
...
| `file_id` | string | 否 | 文档ID，与 file_url 二选一 |
| `file_url` | string | 否 | 在线幻灯片的URL链接，与 file_id 二选一 |
| `<chartLegendArgs>` | object | 否 | 更多参数详见 Trae 当前加载的 slide-mcp 的 schema |
...
| `file_id` | string | 否 | 文档ID，与 file_url 二选一 |
| `file_url` | string | 否 | 在线幻灯片的URL链接，与 file_id 二选一 |
| `<chartAxisArgs>` | object | 否 | 更多参数详见 Trae 当前加载的 slide-mcp 的 schema |
...
| `file_id` | string | 否 | 文档ID，与 file_url 二选一 |
| `file_url` | string | 否 | 在线幻灯片的URL链接，与 file_id 二选一 |
| `<chartSeriesStyleArgs>` | object | 否 | 更多参数详见 Trae 当前加载的 slide-mcp 的 schema |
...
| `file_id` | string | 否 | 文档ID，与 file_url 二选一 |
| `file_url` | string | 否 | 在线幻灯片的URL链接，与 file_id 二选一 |
| `<chartDataLabelsArgs>` | object | 否 | 更多参数详见 Trae 当前加载的 slide-mcp 的 schema |
...
| `file_id` | string | 否 | 文档ID，与 file_url 二选一 |
| `file_url` | string | 否 | 在线幻灯片的URL链接，与 file_id 二选一 |
| `<groupPropertyArgs>` | object | 否 | 更多参数详见 Trae 当前加载的 slide-mcp 的 schema |
...
| `file_id` | string | 否 | 文档ID，与 file_url 二选一 |
| `file_url` | string | 否 | 在线幻灯片的URL链接，与 file_id 二选一 |
| `<commentArgs>` | object | 否 | 更多参数详见 Trae 当前加载的 slide-mcp 的 schema |
...
| `file_id` | string | 否 | 文档ID，与 file_url 二选一 |
| `file_url` | string | 否 | 在线幻灯片的URL链接，与 file_id 二选一 |
| `<commentKeyArgs>` | object | 否 | 更多参数详见 Trae 当前加载的 slide-mcp 的 schema |
...
直接调用 Trae 已加载的 `slide_set_design` 工具，并通过 `design_md` 传入完整 DESIGN 内容。Trae connector 负责授权，不要直连 MCP endpoint 或读取本地 connector 配置。

DESIGN 内容较大时，应先压缩重复说明并保留页面尺寸、色板、字体、版式、素材和质量门禁等必要约束；调用后使用 `slide_get_design` 检查是否完整持久化。
...
| `design_md` | string | 是 | DESIGN.md 全文，包含版式分类、色板、字体、装饰、素材策略和自检条件等设计契约 |
...
- 本文档由 `slideengine_mcp_tools.go` 自动派生（参考脚本 `.tmp/gen_slideengine_md.py`）。如与 `Trae 当前加载的 slide-mcp` 返回的 schema 冲突，以 schema 为准。
