...
| 调用方式 | 直接调用 Trae 加载的 `doc-mcp` 服务                                      |
| 授权     | 与其他腾讯文档 MCP 服务共用同一个 Trae connector                         |
| 文档类型 | 仅支持 Word 文档类型（`doc_type: word`）                                      |

> ⚠️ **所有 `doc.*` 工具均通过 Trae 加载的 `doc-mcp` 独立 MCP 服务调用**，与 `tencent-docs` 主服务平级。四个服务由同一个 connector 授权。
...
优先通过 `tencent-docs` 服务的 `upload_image` 上传图片并取得 `image_id`，再调用 `doc.insert_image`。体积较小且当前客户端能够安全传递时，也可以直接使用 `content` 传入 base64。

不要读取其他产品的本地 MCP 配置，不要要求用户提供 Token 或开放平台凭证。无法安全传递本地图片时，请让用户提供可访问的图片 URL，或先在腾讯文档中上传。
...
- `file_id` (string, 可选): 文档唯一标识符，与 `file_url` 二选一
- `file_url` (string, 可选): 腾讯文档的文档链接，与 `file_id` 二选一
- `content` (string, 可选): 图片的 base64 内容，仅适合体积较小且当前客户端能够安全传递的图片，与 `image_id` 二选一
- `image_id` (string, 可选): 通过已授权的 `upload_image` MCP 工具取得的图片 ID，与 `content` 二选一
- `idx` (integer, 必填): 插入位置的索引，从 0 开始
...
先调用 `doc.get_images` 定位旧图片。新图片优先通过 `upload_image` 获取 `image_id`；体积较小且当前客户端能够安全传递时，也可以使用 `content`。

不要读取其他产品的本地 MCP 配置，不要要求用户提供 Token 或开放平台凭证。
...
- `image_id` (string, 条件必填): 通过已授权的 `upload_image` MCP 工具取得的新图片 ID，与 `content` 二选一（必须提供其一）
- `content` (string, 可选): 新图片的 base64 内容，仅适合体积较小且当前客户端能够安全传递的图片，与 `image_id` 二选一
- `version_info` (object, 可选): 版本参数，详见《通用说明 > 版本参数》
...
直接调用 Trae 已加载的 `doc.create_with_markdown` 工具。Trae connector 会处理授权，不要从本地目录发现 Token，也不要把 Token 写入脚本或环境变量。

Markdown 体积较小时可传 `base64_markdown`。内容较长时，优先使用 `manage.create_file` 创建空 Word 文档，再按段调用 `doc.insert_markdown`，避免单次参数过大。
...
### 参数说明
- `base64_markdown` (string, 必填): base64 编码的 Markdown 内容，适合体积可控的 Markdown。内容过长时改用创建空文档后分段插入
...
2. 使用系统 base64 命令进行编码，并将结果写入工作区目录下的文件（确保 Trae 可读取该工作区文件）：
...
5. 读取工作区文件步骤 2 生成的 encoded_<标题>.txt，拿到 base64 编码后的 Markdown 内容
