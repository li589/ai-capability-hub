...
- 修改某一页时，先生成结构化修改清单，优先使用批量 MCP 工具，避免反复多轮调用。
...
任何 Slide 工作流都先通过 Trae 直接调用以下工具：

1. 调用 `slide_get_info` 获取页数、页面顺序和画布尺寸。
2. 对需要读取或修改的页面调用 `slide_get_page_info`；不要无差别加载所有页面的大 JSON。
3. 写操作前调用 `slide_get_design` 检查 DESIGN。
4. 页数或尺寸为 0 时按空白 PPT 处理；已有页面但没有有效内容时，生成新页后再删除空白占位页。
5. 没有 DESIGN 时先执行第 2 节；已有 DESIGN 时进入对应编辑流程。
6. 工具返回权限错误时，提示用户检查当前腾讯文档账号的查看或编辑权限。
...
当 `slide_get_design` 表明 DESIGN 不存在，或任何写类 MCP 工具需要 DESIGN 时，先完成本节。后续流程默认已经满足这个前置条件，不再重复声明。

执行步骤：

1. 读取 `pptx-generator/references/design-principle.md`。
2. 结合用户需求、当前 PPT 状态、已有内容、材料摘要和搜索事实，在上下文中构思 DESIGN 内容（视觉与落地契约，不含大段叙事）。
3. 直接调用 MCP 工具 `slide_set_design`，将完整内容通过 `design_md` 参数传入。
4. 调用后使用 `slide_get_design` 校验内容是否完整持久化。

DESIGN.md 的结构、字段和质量门禁以 `pptx-generator/references/design-principle.md` 为准；本文件不重复展开。
...
4. 写 JSX 前读取组件语法规范 `pptx-generator/references/component-*.md`。
5. 每页用到什么组件，再读取对应 `pptx-generator/references/component-*.md`，不要一次读全。
...
3. 读取需要使用的组件规范 `pptx-generator/references/component-*.md`。
...
修改某页时，避免“读一次、改一点、再读、再改”的多轮循环：

1. 读取一次 `slide_get_page_info`，整理需要修改的 shape ID 和属性。
2. 三个及以上同类操作优先使用批量工具，例如 `slide_add_shapes`、`slide_add_texts` 或批量属性更新工具。
3. 不存在批量工具时按依赖顺序串行调用，复用上一步返回的版本信息。
4. 全部修改完成后再读取一次目标页校验，不在每次写入后重复读取。
...
| 文档 | 用途 |
|---|---|
| `pptx-generator/references/design-principle.md` | DESIGN.md 编写规范（结构、字段、自检门禁） |
| `pptx-generator/references/component-*.md` | JSX 组件语法规范（box / chart / codeblock / diagram / faicon / image / math / qrcode / slide / svg / table / text 等） |
| `pptx-generator/scripts/doc_image_extractor.py` | 文档图片素材提取（按需） |
