...
| 工具名称 | 功能说明 |
|---------|---------|
| `get_survey` | 获取指定问卷的详细信息（标题、设置、页面、题目、选项 + 纯文本 DSL） |
| `create_survey` | 使用纯文本创建问卷（text 必填，支持指定场景/指定项目） |
| `update_question` | 更新问卷中的某一道题目（需先获取 question_id） |
| `list_answers` | 获取问卷的回答列表（支持游标分页） |
