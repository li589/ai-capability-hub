...
## Text Color — Required Defaults (mixed dark/light)

Huly is a **mixed-theme** system, so headings and body text never have a single safe
default. When you synthesize the deck CSS you **must** set an explicit text color on
the canvas and on every heading helper — do not let text fall back to the browser
default black, or headings will vanish into the dark `#303236` canvas.

- **Set a canvas default:** give the 1280×720 canvas element `color: #ffffff` (its
  dominant surface is the dark Obsidian canvas), so all text inherits white unless a
  light section overrides it.
- **Every heading helper (`.h-display`, `.h-1`, `.h-2`, `.lead`, `.kicker`) must
  declare its own `color`** — never rely on inheritance alone for display type:
  - On dark surfaces (`#303236`/`#090a0c`/`#111111`): `.h-display`/`.h-1`/`.h-2` →
    `#ffffff`; `.lead` → `#a9a9aa` (Ash); `.kicker` → `#5683da` (Electric Iris).
  - On light section bands (`#ffffff`/`#f6f6f6`): scope an override such as
    `.s-light .h-1 { color: #050506; }`, `.s-light .lead { color: #303236; }`.
- **Decide the section mode first** (dark vs light), then apply the matching text
  colors as a block — never leave a heading uncolored on either surface.
