#!/usr/bin/env python3
"""1688-sourcing-inquiry MCP result post-processing CLI.

The `ali1688-buyer` MCP connector owns authentication and API calls. This CLI only
accepts MCP tool result JSON and applies deterministic Python post-processing.
"""
...
def _usage(commands: dict[str, str]) -> None:
    lines = [
        "**1688-sourcing-inquiry 后处理 CLI 用法**",
        "",
        "先调用 MCP 工具 `1688_procurement_digital_human_tool`，再将 MCP 返回 JSON 传给本 CLI：",
        "",
        "```",
    ]
    ...
    lines.extend([
        ...
        "示例：",
        "```",
        "python3 cli.py procurement --offerName \"衣服\" --count \"10\" --demand \"价格便宜\" --mcp-result-file /tmp/procurement.json",
