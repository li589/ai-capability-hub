...
- **危险操作**（`analysis cancel`、`workflow cancel/rm/start`、`terminal close`）：先执行对应 `info`/`ls` 命令确认状态，再向用户确认后执行。
...
| 任务列表 | `dcs workflow ls_task [筛选]`（见 `dcs workflow ls_task --help`） |
