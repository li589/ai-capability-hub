...
function Assert-SeaRunnable([string]$seaPath, [string]$expectedVersion) {
    ...
    try {
        ...
        $output = @(& $seaPath --version 2>$null)
        if ($LASTEXITCODE -ne 0) {
            throw "SEA --version 退出码为 $LASTEXITCODE"
        }
        $actualVersion = (($output | ForEach-Object { [string]$_ }) -join "`n").Trim()
        if (-not $actualVersion) {
            throw "SEA --version 未返回版本号"
        }
        if ($expectedVersion -and $actualVersion -ne $expectedVersion.Trim()) {
            throw "SEA 版本不匹配: expected=$($expectedVersion.Trim()), actual=$actualVersion"
        }
        return $actualVersion
    } catch {
        throw "SEA 二进制验证失败: $($_.Exception.Message)"
    } finally {
...
set "ACTUAL_VERSION_FILE=%PENDING%\actual-version"
set "SL_CLI_SKIP_UPDATE=1"
"%SEA%" --version >"%ACTUAL_VERSION_FILE%" 2>nul
if errorlevel 1 goto :rollback_pending
set "ACTUAL_VERSION="
set /p ACTUAL_VERSION=<"%ACTUAL_VERSION_FILE%"
if not "%ACTUAL_VERSION%"=="%EXPECTED_VERSION%" goto :rollback_pending
...
function Install-InPlaceFromStaged([string]$newDir, [string]$installDir) {
    ...
    try {
        ...
        if (Test-Path $seaBak) { Remove-Item -Force $seaBak -ErrorAction SilentlyContinue }
        $replacedSea = $true
    } catch {
...
function Verify-Install {
    ...
    if (Test-Path $sea) {
        try {
            & (Join-Path $INSTALL_DIR "sl.cmd") --version 2>$null | Out-Null
    ...
    throw "安装验证失败: sl --version 无法正常运行"
}
