...
- 每题答题区域：**5 个分段按钮**，分别对应 完全符合 / 比较符合 / 一般符合 / 比较不符合 / 完全不符合
...
- 题目行之间用浅边框分隔
- **5 级按钮组（关键）**：每题下方一组 5 个等宽按钮（`flex: 1 1 0`，单行不换行），从左到右依次为 完全符合(A) → 比较符合(B) → 一般符合(C) → 比较不符合(D) → 完全不符合(E)，配色按“同意→不同意”语义渐变，**不允许出现无底色（白底/透明）状态**：
  - 未选中：5 个按钮均为灰底（`#E5E7EB` 底 / `#9CA3AF` 字）
  - 选中 `A 完全符合`：该按钮深绿底白字（`#15803D` / `#FFFFFF`）
  - 选中 `B 比较符合`：该按钮绿底白字（`#22C55E` / `#FFFFFF`）
  - 选中 `C 一般符合`：该按钮琥珀底白字（`#F59E0B` / `#FFFFFF`）
  - 选中 `D 比较不符合`：该按钮橙底白字（`#F97316` / `#FFFFFF`）
  - 选中 `E 完全不符合`：该按钮红底白字（`#DC2626` / `#FFFFFF`）
  - 同一题只能有一个按钮带语义色，互斥；点击未选中项时，原带色按钮必须**立即**变回灰底，避免出现双带色或白底残态
...
- **提交按钮交互状态（关键）**：
  - 未答完 85 题前：提交按钮置灰（disabled），不可点击，文案如 `请完成所有题目后再提交（已答 X/85）`
  - 已答完 85 题后：提交按钮高亮可点击，文案如 `提交并生成能力画像`
...
<div class="assessment-card">
  ...
  <div class="question-row">
    ...
    <div class="answer-options">
      <!-- 5 级按钮组：未选=灰底；选中按语义色（A深绿/B绿/C琥珀/D橙/E红）白字；同题互斥，禁止白底 -->
  ...
  <button class="submit-btn" disabled>请完成所有题目后再提交（已答 17/85）</button>
</div>
```
...
- 不允许按钮出现无底色（白底/透明）状态：未选必须灰底，选中必须对应语义色底
- 不允许同一题出现两个带色按钮（必须互斥，切换即时）
- 不允许按钮上显示分数（8/6/4/2/0）
- 不允许没有进度条和题号
...
- **唯一渲染通道仍为 `show_widget`**：生成 HTML/CSS/JS 后必须且只能传入 `show_widget`；严禁把代码以 Markdown 代码块输出、严禁提供 `.html` 下载；`show_widget` 失败则输出纯文本提示，不退化为独立 HTML。
...
- 每条结构：`<维度名>` | `<得分条>` | `<分数>`
- 得分条实现：外层 `<div class="bar-track">`（灰底圆角固定宽度）+ 内层 `<div class="bar-fill" style="width: <score>%">`（语义色填充，`width` 直接取 100 分制 `score` 值）
- 分数值显示在条右侧，100 分制整数（来自 `details[].score`），满分 100
- 得分条填充色按分数语义渐变（同上），低分偏灰，高分偏绿
