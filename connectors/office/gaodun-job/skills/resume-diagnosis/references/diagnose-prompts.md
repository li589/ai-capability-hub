...
`模型内容生成调用` 直接把 `content` 作为用户消息发给按 promptId 配置好 Prompt 的 bot。线上是**每个有值字段各调一次 bot（多条经历多次调用、共用同一份 Prompt）**；本 skill 由 `emit-llm-tasks` 组装成单次批量任务单——每次调用的 (system prompt, user 消息) 二元组逐字不变、条目间互不影响，判定结果与逐条独立调用等价，只是把 N 次串行推理合并为 1 次（提速关键）。
