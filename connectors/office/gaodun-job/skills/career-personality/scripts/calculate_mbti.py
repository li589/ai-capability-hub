...
报告文案来源：本地 references 文件 `references/mbti.md`，文件内按 H2 标题切分为 4 段，每段为一段散文说明 + 一个 ```json 代码块：
  ...
  - ## 职业匹配分          (每型 66 个 jobId + score 职业匹配分)

题库来源：references/mbti.md 的 `## 题库` 段。

文件形态（二选一，均受支持）：
  A. 单文件模式（默认）：`--references-path` 指向一个含全部 4 个 H2 段的 mbti.md；
  B. 多文件拆分模式：可选用 `--questions-path` / `--profiles-path` / `--dimensions-path` /
     `--job-scores-path` 分别指定 4 个独立 md 文件（每个文件内仍须保留对应的 H2 标题与
...
def build_role_job_scores(items):
    """职业匹配分 list（[{role, jobScores}]），按 role 大写建索引字典。"""
    return {item["role"].upper(): item.get("jobScores", []) for item in items}
...
def calculate_scores(answers, questions, role_profiles, dimension_details, role_job_scores):
    """主评分函数。
    ...
    参数：
      ...
      role_job_scores: {TYPE_UPPER: [{jobId, score}]} 每型职业匹配分字典
    """
    ...
    if dominant_type and dominant_type in role_profiles:
        ...
        # 职业匹配分：从 references/mbti.md 的 ## 职业匹配分 段查表（每型 66 个 jobId + score）
        if dominant_type in role_job_scores:
            job_scores = role_job_scores[dominant_type]
...
def main():
    ...
    parser.add_argument("--references-path", default=None,
                        help="（可选）合并后的 references/mbti.md 路径，含全部 4 个 H2 段。省略时自动加载多文件形态：questions.md / profiles.md / dimensions.md / job_scores.md")
    ...
    parser.add_argument("--job-scores-path", default=None,
                        help="多文件拆分模式：仅含 `## 职业匹配分` 段的独立 md 文件路径（可选）")
    ...
    # 4 个 section 的来源优先级：独立拆分路径 > 单文件 --references-path > 默认多文件（形态 B）
    section_sources = [
        ...
        ("jobs", SECTION_ROLE_JOB_SCORES, args.job_scores_path),
    ]
    loaded = {}
    ...
    role_job_scores = build_role_job_scores(loaded["jobs"])

    result = calculate_scores(answers, questions, role_profiles, dimension_details, role_job_scores)
