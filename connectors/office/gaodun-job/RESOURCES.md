...
- `resume-ai-help` 的 prompt 原文**不随包分发**，运行时经 MCP 工具 `resume_prompt_get(prompt_id)` 从服务端获取。

## 三、重复文件说明

`references/resume-module-config.json` 在 resume-diagnosis 与 resume-ai-help 各有一份**逐字相同**的副本：
两个 Skill 需保持自包含（不跨目录引用），故接受此冗余；服务端配置变更时两份同步更新。
