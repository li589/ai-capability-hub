...
3. `generate_image(prompt="...", imageMediaIds=["mediaId_A"])` 创建任务。
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|---|---|---|---|---|
| `modelType` | string | 可选 | `"IMAGE"` | 资源类型（枚举：TEXT/IMAGE/VIDEO）；本 Skill 仅关心 `"IMAGE"` |
...
### `generate_image`
> 注意：上传单文件上限约 10MiB；4K PNG 常超此限。若要把刚生成的图作为参考图传入下一张，先导出 ≤2K 或转 JPEG（≤10MiB）再上传，否则返回 413。
