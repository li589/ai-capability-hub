...
def get_api_key() -> str:
    ...
    print("  1. 访问 https://redfox.hk/?source=workbuddy 注册账号获取 API Key（格式 ak_xxxxxxxx）", file=sys.stderr)
...
def parse_publish_time(raw):
    """将 publishTime（日期字符串或 Unix 时间戳）解析为 (publish_time, publish_date) 元组。"""
    if not raw:
        return "", ""
    rt_str = str(raw).strip()
    # 尝试按日期字符串解析
    ...
    try:
        ts = int(raw)
        if ts > 0:
            return _time.strftime("%Y-%m-%d %H:%M:%S", _time.localtime(ts)), _time.strftime("%Y-%m-%d", _time.localtime(ts))
