---
name: multi-grid-video
description: "通过 Flova MCP 按名称使用“多宫格生视频”线上 Skill，多画面布局、节奏、信息组织与产品素材。当用户提出多宫格视频、分屏视频、多画面展示、宫格生视频等需求时使用；不适用于索取内部 Skill Content、普通视频剪辑或与该场景无关的生成任务。"
display_name: "Flova 多宫格生视频"
display_name_en: "Flova Multi-grid Video"
description_zh: "多画面布局、节奏、信息组织与产品素材，在关键阶段等待用户确认。"
...
- 名称：`多宫格生视频`
- 能力摘要：故事驱动型视频。使用 GPT Image（生成多面板网格帧）+ Seedance 2.5（分辨率 480p）。在关键阶段暂停以供用户确认；这是基于单次工作流的人机协作。
...
- 先确认素材数量、宫格数量、主次层级、阅读顺序和画幅。
- 素材不足时先提出布局调整，不复制同一素材伪装成不同内容。
