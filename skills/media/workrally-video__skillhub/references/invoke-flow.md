...
本文档帮助 AI Agent 正确调用 WorkRally 视频生成 Skill 的两个云端 API（`invoke` / `content`），完成"发起 → 支付 → 取结果"。
...
sequenceDiagram
    ...
    U->>A: 「生成一段……的视频」
    A->>A: 命中 workrally-video
    A->>U: 展示「WorkRally视频生成 6.88 元/次，是否使用？」
    U->>A: 确认
    A->>API: 1. POST /agent-pay/invoke<br/>{product_id:"video-generation", prompt, duration?, single_image_url?}
    ...
    Note over A,API: 视频生成通常 1-3 分钟<br/>需长轮询 generating → delivered
    API-->>A: {status:"delivered", result_url}
    A->>U: 展示生成的视频，并提示「该视频由AI生成」
```
...
| 字段 | 必填 | 说明 |
|------|------|------|
| `product_id` | ✅ | 本 Skill 固定传 `video-generation`，见 [pricing.md](pricing.md) |
| `prompt` | ✅ | 用户的视频描述（中文/英文均可），≤500 字 |
| `aspect_ratio` | ❌ | 默认 `16:9`；可选 `1:1` / `9:16` 等 |
| `duration` | ❌ | 仅当用户明确指定秒数时传入；默认 10 |
| `single_image_url` | ❌ | 单图生视频：本地图片以 `data:image/<ext>;base64,<...>` 内联（云端自动上传）；也可传 WorkRally 产出的短链（如 `result_url`）。纯文生视频省略 |

> 分辨率为 720p，由云端统一设定。调用方无需指定 `model`。
...
`content` 是**异步状态机**：HTTP 200 不代表已出结果，必须读 body 的 `status` 字段判断。**视频生成耗时较长**，可能需要多轮 `generating` 轮询。

| 响应 status | HTTP | body 关键字段 | 含义 | Agent 行为 |
|------|------|------|------|-----------|
| `delivered` | 200 | `result_url` | 已生成 | 展示 `result_url`，提示「该视频由AI生成」，并尽量引导 Agent 下载到本地 |
