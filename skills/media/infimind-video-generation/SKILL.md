...
description: 根据用户提供的文本、图片、视频或音频素材，完成视频生成的预检、单独费用授权、任务创建与状态跟踪，支持文生视频、图生视频、首尾帧、多模态参考和视频编辑。
description_zh: 根据用户提供的文本、图片、视频或音频素材，完成视频生成的预检、单独费用授权、任务创建与状态跟踪，支持文生视频、图生视频、首尾帧、多模态参考和视频编辑。
description_en: "Drive Infimind Video generation from user-supplied text,
  images, video, or audio: preflight the request, obtain separate billing
  authorization, then create the task and track its status. Supports
  text-to-video, image-to-video, first/last-frame, multimodal references, and
  video editing."
category: media
version: 1.0.0
...
User: "Generate a 10-second vertical video from this product image."

Choose `model: SA`, use the user-selected image as `sourceImage`, set `seconds: 10` and `size: 720x1280`, and do not pass `taskMode`, `ratio` or `resolution`. Run `get_user_info` to confirm Token and credits, validate the input file, then call `create_video_task` with `dryRun: true` first to surface `estimatedCredits` and `validationWarnings`. Present both, obtain separate authorization, and only then call again with `dryRun: false`. Persist the returned `taskId` and poll with `get_video_status` until the service returns `completed` or `failed`. Return user-facing results in English, never ask the user to paste their Token into chat, and never invent estimated credits when the preflight does not return one.
