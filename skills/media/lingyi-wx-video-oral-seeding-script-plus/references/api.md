...
脚本生成主流程三个接口，均需 `Authorization: <API_KEY>`：
...
| `script_type` | options | 脚本类型；本 skill 固定传 `oral_seeding`（口播种草） |
...
| `account_brief` | 否 | 对象 |
| `script_type` | 本 skill 固定 | 固定 `oral_seeding`（口播种草），由脚本自动注入 |
| `count` / `script_count` | 否 | 1–3，默认 1 |
| `target_duration_sec` | 否 | 1–1800 |
| `depth` | 否 | 默认 `fast` |
| `additional_requirements` | 否 | 额外要求文本 |
| `origin` / `origin_method` | 否 | 调用来源/方式，本 skill 填 `01workbuddy` / `skill` |
| `source_skill` | 本 skill 固定 | 固定 `oral-script-creator`，技能调用埋点，用于与其它脚本生成入口区分 |

**校验规则**：mode/direction/platform/industry/purpose/script_type/depth 必须用 config 返回的 value（部分中文值兼容转换但不可依赖）；本 skill 固定传 `script_type=oral_seeding`（须为 config 中的合法 value，否则按下面的 400 处置）；`product` 空对象会被拒；二创缺源视频（分享链接）会被拒。body 允许额外字段，会进入生成上下文。
