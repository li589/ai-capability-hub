#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""视频号口播种草脚本生成【零一数科·出品】 —— 联网主脚本

远端付费 API 生成微信视频号脚本，脚本类型固定为口播种草（oral_seeding）。流程：
...
def validate_and_build(args, config_opts, base_url, api_key, insecure):
    ...
    # 脚本类型固定为口播种草（本 skill 专属调用约定，不再由用户/参数选择）。
    body["script_type"] = "oral_seeding"
    if args.depth:
    ...
    body["source_skill"] = "oral-script-creator"  # 技能调用埋点，区分其它脚本生成入口
    return body
...
def create_task(base_url, api_key, body, insecure):
    ...
    log("创建脚本生成任务：mode=%s direction=%s count=%s script_type=%s"
        % (body.get("mode"), body.get("direction"), body.get("count", 1), body.get("script_type")))
    status, text = None, ""
    for attempt in range(1, HTTP_RETRY_TIMES + 1):
        try:
...
# --------------------------------------------------------------------------- main
def main():
    ap = argparse.ArgumentParser(description="视频号口播种草脚本生成—— 远端 API 生成脚本（script_type 固定 oral_seeding）")
