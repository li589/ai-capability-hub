---
name: lingyi-wx-video-oral-seeding-script-plus
display_name: 视频号口播脚本生成【零一数科·出品】
display_name_en: WeChat Channels Oral Seeding Script Generator
description_zh: "【零一数科·出品】视频号口播种草脚本生成器。一条链接二创脚本，丢进爆款视频号链接即以此骨架改写成你的口播种草脚本，或按方向/目的/商品原创多条口播种草脚本（含分段表、口播全文、六维质检评分）。"
description_en: "[Lingyi Tech] WeChat Channels oral-seeding script generator. Remix a viral Channels share link into your oral-seeding script using the same structure, or create multiple original oral-seeding scripts from direction/purpose/product (segment table, full oral text, six-dimension QC scores)."
version: 0.1.0
...
通过远端付费 API 生成微信视频号口播种草脚本，支持**创作**（按方向/目的/商品原创）与**二创**（以指定爆款视频为骨架改写）。脚本类型固定为口播种草（`script_type=oral_seeding`），无需也不允许用户选择。二创源只支持**微信视频号分享链接**（不支持本地视频文件上传）。脚本 [scripts/generate_script.py](scripts/generate_script.py) 负责 config 拉取→创建任务→轮询→落接口响应 JSON，[scripts/render_report.py](scripts/render_report.py) 直接解析响应里的 `data.result.markdown` 字段原样输出为报告。接口契约见 [references/api.md](references/api.md)，取数路径见 [references/field-mapping.md](references/field-mapping.md)。
...
1. **执行前——扣点确认（必须先问，再创建）**：参数收齐、即将跑完整流程前，先告知用户预估扣点并请其确认，例如：
   ...
   > 本次「视频号口播种草脚本生成」任务预估扣点约 **98 点**，实际扣点会根据任务复杂程度略有增减。是否继续执行？
...
4. **中途暂停——预留续传钩子（任务还没跑完就要停下时必须做）**：轮询仍在进行、但本回合必须中断（会话/上下文要结束、或主动让用户择机续传）时，**不能只丢一句“跑着呢”就结束**，必须把「如何后续接着跑」一次性说清，避免用户断了线索。从最新 `===TASK_ID===` 取 `script_task_id`，按下面模板告知用户：

   > ⏳ 任务已在远端正常排队/执行（{状态/阶段，如 RUNNING / content_ops 阶段}）。续传**不会重复扣点**。
   >
   > 当前状态：
   > - 任务 ID：`{script_task_id}`
   > - 进度：视频号口播种草脚本创作执行中（轮询中，最长 {poll_timeout}s）
   > - 生成完成后会自动落盘为报告 Markdown 并附上正文
   >
   > 我这回合先到这。等你想继续时，**直接回我一句「继续/接着跑」**（或带着上面这个任务 ID），我会用 `--task-id` 续传同一任务，不重新创建、不重复扣点，跑完即把报告和实际扣点交付给你。

   要点：①必须把 `script_task_id` 原样给用户；②明确告知续传不重复扣点、续传方式是「续传同一任务」而非重建；③给出一句具体的后续触发语（如「继续」「接着跑」或带 task_id），让用户知道下一轮发什么能接上；④task_id 可被用户复述，agent 续传时应优先用用户给出的 id，与历史 `===TASK_ID===` 核对一致后再跑。
...
2. **拿到用户选择后，映射回英文 value 跑完整流程**：把用户选的中文对应回英文 value，传给 `--mode/--direction/--industry/--purpose/--platform/--depth` 等参数（`script_type` 已固定为 `oral_seeding`，无需传）。
...
| `script_type` | 固定 | 脚本类型固定口播种草（`oral_seeding`），由脚本自动注入，不向用户询问、不接受覆盖 |
...
python3 scripts/generate_script.py \
  ...
  --industry food_beverage --purpose short_video_seeding \
...
可选参数：`--account-brief '<json>'`、`--source-video '<json>'|--share-url <url>`、`--target-duration-sec`、`--additional-requirements`、`--skip-config`（跳过枚举预校验）、`--insecure`、`--poll-interval`、`--poll-timeout`。（`script_type` 固定为 `oral_seeding`，无对应参数。）
...
3. **（无 `--task-id`）组装并创建**：GET config 拉可选值 → 校验枚举入参∈options.value、必填规则、product 非空、源视频、范围 → 组装 body（自动注入 `script_type=oral_seeding`、`origin=01workbuddy`、`origin_method=skill`、`source_skill=oral-script-creator` 埋点）→ POST 创建，取 `script_task_id`。
4. **轮询**：按 `--poll-interval` 轮询，看 `data.status`（completed/success/succeeded/done=完成；failed/error/cancelled=失败；pending/queued/running/processing=执行中）。进度写 stderr。每次轮询把最新响应落 `--response-out`，防超时丢失。**若本回合要在轮询未结束时停下（会话/上下文要结束、或让用户择机续传）**：从最新 `===TASK_ID===` 取 `script_task_id`，按「点数与计费提示」第 4 步预留续传钩子（含 task_id、当前阶段、续传不重复扣点、后续触发语），不要静默结束。
...
| 2 | 缺 API Key | 引导去 `https://claw.lingyishuke.com/webapps/01claw-auth/index.html?source=01workbuddy` 取 key，写入 `config.json` 后重试。 |
| 3 | 参数非法 | 必填缺失 / 枚举值不在 config options / product 空对象 / 二创源视频缺失 → 按 stderr 里的可选值清单补全后重试。 |
| 4 | 余额不足 | 透出 recharge_url（若有），引导充值。 |
| 5 | 轮询超时 | 把 `===TASK_ID===` 给用户，按「点数与计费提示」第 3 步告知网络原因失败、点数已返还、可用 `--task-id <id>` 续传，**不要重新创建**（避免重复计费）。注：若是 agent 在轮询未结束时**主动暂停**（脚本尚未超时退出），改走第 4 步「中途暂停」话术而非此处失败安抚。 |
