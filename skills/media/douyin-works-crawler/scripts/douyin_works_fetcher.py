...
class DouyinWorksFetcher:
    ...
    def query_account(self, account: str) -> Dict:
        ...
        if self._is_chinese(account):
            payload = {"accountName": account, "source": "抖音作品抓取-WorkBuddy"}
            query_mode = "accountName"
        else:
            payload = {"accountId": account, "source": "抖音作品抓取-WorkBuddy"}
    ...
    def sync_account(self, account_id: str, source: str = "抖音作品抓取-WorkBuddy") -> Dict:
        """
        提交账号收录请求（异步同步触发）
