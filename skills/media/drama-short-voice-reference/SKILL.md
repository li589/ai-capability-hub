---
name: drama-short-voice-reference
description: "通过 Flova MCP 按名称使用“剧情短片(音色参考)”线上 Skill，音色参考、对白、角色与叙事一致性。当用户提出剧情短片、音色参考、对白短片、参考声音做剧情视频等需求时使用；不适用于索取内部 Skill Content、普通视频剪辑或与该场景无关的生成任务。"
display_name: "Flova 剧情短片(音色参考)"
display_name_en: "Flova Drama Short with Voice Reference"
description_zh: "音色参考、对白、角色与叙事一致性，在关键阶段等待用户确认。"
...
- 名称：`剧情短片(音色参考)`
- 能力摘要：支持两种启动方式：用户上传剧本，或由 agent 根据用户提供的主题/需求代写剧本。随后使用 Nano Banana + Seedance 2.5（分辨率 480p） 生成视频。在关键阶段暂停以供用户确认；基于一镜到底工作流的人机协作。
...
- 只接受用户有权使用且已获必要授权的音色参考。
- 不克隆未经授权的真实人物声音，不把参考音色宣称为本人演绎。
