...
display_name: B站评论分析
display_name_en: Bilibili Comment Analyzer
description_zh: 输入B站视频链接获取一级评论，支持分页浏览与四维情感分析，生成精美HTML报告
description_en: Fetch Bilibili video comments and run sentiment analysis with HTML reports.
...
A: 前往 [红狐hub](https://redfox.hk/settings/api-keys?source=workbuddy) 注册账号并申请 API Key，按鉴权章节说明配置到本地环境变量。
...
A: 每次调用获取1页（20条）。如需更多数据，通过翻页命令逐页获取，或前往 [红狐平台](https://redfox.hk?source=workbuddy) 联系客服获取批量能力。
...
A: 前往 [红狐平台](https://redfox.hk?source=workbuddy) 联系客服，或查阅技能文档。
