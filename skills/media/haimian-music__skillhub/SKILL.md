...
name: haimian-music
description: >
  海绵音乐官方 AI 音乐生成 Skill，在 AI 助手里完成写歌、歌词成曲、BGM、参考改编、查询和下载。 Use this whenever the user wants 海绵音乐, AI音乐生成, AI写歌, 生成音乐, 歌词成曲, BGM生成, 背景音乐, 参考音频改编.
...
- 第一次生成前如果未登录，Agent 应直接运行 `node <安装目录>/bin/aimv.js init`：CLI 会输出连接页链接 `https://lexuan.club/skills/connect?slug=haimian-music&from=skill`，把链接发给用户；用户网页登录（新用户自动注册）后复制 API Key 贴回对话，Agent 再运行 `node <安装目录>/bin/aimv.js init --api-key <KEY>` 完成连接。Key 长期有效，只需一次。

## 命名规范

- 展示名：海绵音乐 Skill
- Skill slug / 目录名：`haimian-music`
- Powered by：海绵音乐
- Agent 工具名：`aimv`
...
```bash
node <安装目录>/bin/aimv.js song create --brief "写一首轻快的中文流行歌，主题是夏天旅行" --wait
node <安装目录>/bin/aimv.js song create --lyrics "..." --title "夏天旅行" --style mandopop --wait
node <安装目录>/bin/aimv.js bgm create --brief "适合产品发布会开场的电子 BGM" --wait
```
...
- `code=auth_required`：`[error]` 标签携带 `connect_url=`（即 `https://lexuan.club/skills/connect?slug=haimian-music&from=skill`），把链接发给用户；用户登录复制 API Key 后，运行 `node <安装目录>/bin/aimv.js init --api-key <KEY>` 完成连接并重试。
...
```bash
# API Key 来自连接页 https://lexuan.club/skills/connect?slug=haimian-music&from=skill
...
curl -sS -X POST "https://music.68zysm.cn/api/v3/ai-music/generations/from-brief" \
  ...
  -H "X-App-Id: haimian-music" -H "X-AIMV-Skill-Slug: haimian-music" \
