...
description: 对用户主动提供的视频执行结构化拆解，安全调用分析任务并交付脚本、风格、卖点、爆款结构和可复用的视频生成提示词。
description_zh: 对用户主动提供的视频执行结构化拆解，安全调用分析任务并交付脚本、风格、卖点、爆款结构和可复用的视频生成提示词。
description_en: "Run a structured breakdown of a user-supplied video: safely
  call the analysis task and deliver the script, visual style, selling points,
  viral structure, and a reusable video generation prompt, with explicit
  separate billing authorization before any billable call."
category: media
version: 1.0.0
...
permissions:
  ...
  read:
    - 仅限当前对话中用户主动选择的视频
  network:
    - 仅通过已启用的极睿视频 Connector 调用 get_user_info、create_video_analysis_task 与
...
User: "Analyze this product video and give me a reusable prompt."

Apply the same confirmation sequence as the Chinese flow above: confirm the source video and intended use, verify media rights, run `get_user_info` to check Token and credits, disclose that the analysis has no preflight and will consume credits, obtain separate authorization, call `create_video_analysis_task` exactly once, persist the returned `analysisId`, and poll status until `completed` or `failed`. Deliver the result under script, style, selling points, viral structure and reusable prompt. For any field the service does not return, write "this field was not returned by the analysis" instead of inventing content. Never ask the user to paste their Token into chat, and only return fields that the tool actually provided.
