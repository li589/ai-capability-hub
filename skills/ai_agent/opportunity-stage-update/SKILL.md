...
version: 1.4.0
description: "Opportunity Stage Update workflow for foreign-trade teams, with explicit evidence, controlled CRM access, confirmation, and safety boundaries."
description_zh: "核对opportunity当前阶段、目标阶段、salesStageStatus、关键事项和关联记录，确认后通过正式阶段接口执行并同步回查。适用于用户要求商机阶段更新、生成相关结果或检查执行边界时。"
allowed-tools: Bash
---
...
定位：核对opportunity当前阶段、目标阶段、salesStageStatus、关键事项和关联记录，确认后通过正式阶段接口执行并同步回查。边界：不得硬编码阶段或状态；字段/选项未发现时停在预览。
...
> 核对opportunity dataId=5001当前阶段“方案”，目标阶段“谈判”，salesStageStatus和stageId均取动态发现结果；生成dry-run并停在确认门。
...
- coverage: opportunity-stage-write
...
- 使用opportunity-stage命令；确认执行后同步查询opportunity回查阶段，分别报告请求结果与实际字段状态。
