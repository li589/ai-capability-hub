...
def matches_path(rel_path: str, path_patterns: List[str]) -> bool:
    ...
    Also checks whether *rel_path* is a prefix of any pattern's directory
    component — this enables early directory pruning.  For example, pattern
    ``subdir/*.env`` should prune directory ``subdir`` without walking into it.
    """
    for p in path_patterns:
        if fnmatch.fnmatch(rel_path, p):
            return True
        # Prefix check: if the pattern starts with "<rel_path>/", the
        # directory should be pruned because everything inside matches.
        if rel_path and p.startswith(rel_path + "/"):
            return True
    return False
