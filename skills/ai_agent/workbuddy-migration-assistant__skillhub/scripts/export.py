...
def _build_args():
    ...
    ap.add_argument(
        "--format", default="zip", choices=["zip", "tar.gz"],
        help="Output format (default: zip). tar.gz gives better compression for text-heavy data.",
    )
...
def main() -> int:
    ...
    if args.output:
        ...
        out = Path.cwd() / f"workbuddy-assets-{ts}{'.dir' if args.no_archive else ('.tar.gz' if args.format == 'tar.gz' else '.zip')}"
    workspace_out = (
    ...
    with tempfile.TemporaryDirectory(prefix="wb-export-") as td:
        ...
        if src_root.db_path.is_file():
            # v0.2.0: Use SQLite backup API for consistent snapshot (WAL-safe)
            import tempfile as _tmp
            _snap_td = _tmp.mkdtemp(prefix="wb-snapshot-")
            _snap_db = Path(_snap_td) / "workbuddy.db"
            try:
                _src_conn = sqlite3.connect(f"file:{src_root.db_path}?mode=ro", uri=True)
                _snap_conn = sqlite3.connect(str(_snap_db))
                _src_conn.backup(_snap_conn)
                _snap_conn.close()
                _src_conn.close()
                db_result = DB.export_reduced_db(
                    _snap_db, out_db, inventory["db_tables"],
                    exclude_session_ids=exclude_session_ids,
                    user_id_filter=current_uid if not args.all_users else None,
                )
                # Clean up snapshot
                shutil.rmtree(_snap_td, ignore_errors=True)
            except Exception as e:
                shutil.rmtree(_snap_td, ignore_errors=True)
                # Fallback: use direct file if snapshot fails
                rep.warn(f"SQLite consistent snapshot failed ({e}), using direct copy")
                db_result = DB.export_reduced_db(
        ...
        # 4b1. Filter memory files by current user_id
        if current_uid and not args.all_users:
            _filter_memory_by_user(staging, current_uid, rep)
    ...
    for asset in warned_assets:
        ...
        for child in src_path.iterdir():
            if child.is_file():
                ...
                # connectors/<uid>/ are per-user dirs; connectors/skills/ (and any
                # non-uid dir) is handled by file_assets separately — skip it here
                # to avoid copytree collision.
                if not re.fullmatch(r"[0-9a-fA-F-]{32,}", child.name):
                    continue
                shutil.copytree(child, dst_path / child.name)
...
def _filter_memory_by_user(staging: Path, current_uid: str, rep) -> None:
    """Remove memory files belonging to other user_ids."""
    memory_dir = staging / "memory"
    if not memory_dir.is_dir():
        return
    removed = 0
    for mf in list(memory_dir.glob("*_memory.md")):
        # File pattern: <uid>_memory.md
        uid_in_name = mf.stem.rsplit("_memory", 1)[0]
        if uid_in_name != current_uid:
            mf.unlink()
            removed += 1
    if removed:
        rep.add("filter:memory", {"action": "removed", "count": removed, "kept_uid": current_uid})
