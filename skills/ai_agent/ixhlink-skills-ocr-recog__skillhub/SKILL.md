---
name: 智绘文字识别
description: "智能 OCR 文字识别助手，从图片中提取印刷体、手写体、表格、票据、名片等文字内容，支持 URL 与 Base64 图片；当用户要求「OCR」「识别图片文字」「提取文字」「扫图识字」「图片转文字」「读图」「票据识别」「表格识别」时使用。"
tags: [图片, 媒体, OCR]
---
...
| 字段 | 值 |
|------|-----|
| skill_id | `ixhlink-skills-ocr-recog` |
| skill_version | `1.0.0` |
| product_id | `ixhlink-skills-ocr-recog` |
| model_key | `ixhlink-skills-ocr-recog` |
| capability | `ocr` |
...
```json
{"model": "ixhlink-skills-ocr-recog", "capability": "ocr"}
```
...
{
  "capability": "ocr",
  "model": "ixhlink-skills-ocr-recog",
  "payload": {
    "messages": [
      {
        "role": "system",
...
payload = { 'capability': 'ocr', 'model': 'ixhlink-skills-ocr-recog',
  'payload': { 'messages': [
...
| 产品ID | 服务内容 | 单价 |
|--------|----------|------|
| `ixhlink-skills-ocr-recog` | 智绘文字识别 | 0.1 元/次 |
...
{"model":"ixhlink-skills-ocr-recog","capability":"ocr","payload":{...}}
```
