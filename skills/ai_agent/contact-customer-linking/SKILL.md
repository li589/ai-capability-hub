---
name: contact-customer-linking
version: 1.4.0
description: "Contact Customer Linking workflow for foreign-trade teams, with explicit evidence, controlled CRM access, confirmation, and safety boundaries."
description_zh: "新增/编辑contact并按租户实际关联字段绑定customer。适用于用户要求联系人关联、生成相关结果或检查执行边界时。"
allowed-tools: Bash
---
...
定位：新增/编辑contact并按租户实际关联字段绑定customer。边界：不能仅凭同名公司推断关联；必须使用已核验的客户数据ID和字段类型，写入后查询确认。
...
> 更新contact dataId=7001，将已动态确认的客户关联字段apiKey=customer设置为customer dataId=100234；当前关联为空，目标客户已按ID查询确认，执行用户userId=42。生成逐字段dry-run并停在确认门，确认后按contact dataId回查关联字段。
