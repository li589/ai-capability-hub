...
version: 1.4.0
description: Customer Segmentation workflow for foreign-trade teams, with
  explicit evidence, controlled CRM access, confirmation, and safety boundaries.
description_zh: 结合customer及可关联的opportunity、followUpRecord、order生成分层和标签更新建议。适用于用户要求客户分层运营、生成相关结果或检查执行边界时。
allowed-tools: Bash
...
定位：结合customer及可关联的opportunity、followUpRecord、order生成分层和标签更新建议。边界：分层规则需用户确认；标签字段/选项动态发现，批量写回必须预览、确认和回查。
...
- 默认只给建议，不默认写标签。分层维度为最近互动、近12个月订单金额（按币种分组）、商机阶段、资料完整度和风险信号。
- 阈值由企业确认；没有订单、商机或跟进关联时标记“数据缺失”，不自动判为低价值。
- 输出列：customerId｜当前层级｜建议层级｜事实依据｜缺失关联｜标签差异｜确认状态。

## 实战使用卡

### 可直接复制的任务示例

> 对customer dataId=100234、100235执行已确认的标签更新：字段apiKey=customerTier，目标optionId=9001、displayName=A重点客户；分层规则为最近互动30天内且近12个月USD订单额不少于10000。用户明确要求先查询事实、逐条生成dry-run，确认后再写入；缺少订单关联的记录不得更新。
