---
name: crm-object-query
version: 1.4.0
description: "CRM Object Query workflow for foreign-trade teams, with explicit evidence, controlled CRM access, confirmation, and safety boundaries."
description_zh: "调用标准对象查询能力完成分页、排序、公私海及多条件筛选，并返回查询口径和命中数。适用于用户要求CRM对象通用查询、生成相关结果或检查执行边界时。"
allowed-tools: Bash
---
...
定位：调用标准对象查询能力完成分页、排序、公私海及多条件筛选，并返回查询口径和命中数。边界：仅支持文档和租户OpenAPI视图开放的对象/字段；只读，不在查询过程中触发写入。
...
> 在已授权测试租户中查询2026-07-01至2026-07-31的私海lead，负责人userId=42；完整分页后本地筛选，按createdTime降序输出口径和命中数。
