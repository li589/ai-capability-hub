...
version: 1.4.0
description: "Customer 360 Assembly workflow for foreign-trade teams, with explicit evidence, controlled CRM access, confirmation, and safety boundaries."
description_zh: "按customer记录组装contact、opportunity、followUpRecord和order信息，形成分主题摘要与时间线。适用于用户要求客户360组装、生成相关结果或检查执行边界时。"
allowed-tools: Bash
---
...
定位：按customer记录组装contact、opportunity、followUpRecord和order信息，形成分主题摘要与时间线。边界：依赖租户关联字段；找不到可靠关联时输出缺口，不进行模糊强关联。
...
> 组装customer dataId=100234的客户360：先查询该客户，再按已发现的customer关联字段查询contact、opportunity、followUpRecord和order；时间范围2026-07-01至2026-07-31仅用于筛选关联互动和订单，不用于筛选客户主记录；无法可靠关联的对象明确标记缺口。
