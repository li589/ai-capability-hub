---
name: crm-pool-assignment
version: 1.4.0
description: CRM Pool Assignment workflow for foreign-trade teams, with explicit
  evidence, controlled CRM access, confirmation, and safety boundaries.
description_zh: 对已核对的lead/customer执行公海转私海、私海转移或退公海。适用于用户要求公私海批量分配、生成相关结果或检查执行边界时。
allowed-tools: Bash
...
定位：对已核对的lead/customer执行公海转私海、私海转移或退公海。边界：必须dry-run、明确确认后执行；接口code=10000只表示“已受理，待系统通知确认”，不得宣称处理完成。private transfer的transferRelatedObj枚举未明确，因此不开放关联对象转移参数。
...
6. 使用--confirm提交后，code=10000只报告“已受理，待系统通知确认”；不得声称已完成。private transfer不传关联对象转移参数。
...
- 公私海命令在dry-run和明确确认后可执行；code=10000只输出“已受理，待系统通知确认”，不得表述为完成。
- private transfer不得传递或开放transferRelatedObj及关联对象保留参数，直至官方枚举明确。
