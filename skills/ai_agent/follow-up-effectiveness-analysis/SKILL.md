---
name: follow-up-effectiveness-analysis
version: 1.4.0
description: Follow-up Effectiveness Analysis workflow for foreign-trade teams,
  with explicit evidence, controlled CRM access, confirmation, and safety
  boundaries.
description_zh: 基于followUpRecord及可关联的线索、客户和商机分析跟进频次、覆盖和超期风险。适用于用户要求跟进效能分析、生成相关结果或检查执行边界时。
allowed-tools: Bash
...
定位：基于followUpRecord及可关联的线索、客户和商机分析跟进频次、覆盖和超期风险。边界：没有响应时间字段时不输出首次响应速度；不把记录数量等同于跟进质量。
...
- 响应时长从客户入站时间到首次有效人工跟进时间；自动消息不计入，缺任一时间时为N/A。
- 跟进频次按唯一followUpRecord计数；效果判断只描述后续可见事件，不推断因果或成交概率。
- 输出列：负责人｜对象数｜有效跟进数｜中位响应时长｜超时数｜数据缺口｜建议。

## 实战使用卡

### 可直接复制的任务示例

> 分析2026年7月入站线索首次人工跟进时长，自动消息不计；按负责人输出对象数、中位响应时长、24小时超时数和缺失时间记录。
