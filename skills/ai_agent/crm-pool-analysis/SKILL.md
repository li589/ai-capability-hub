---
name: crm-pool-analysis
version: 1.4.0
description: "CRM Pool Analysis workflow for foreign-trade teams, with explicit evidence, controlled CRM access, confirmation, and safety boundaries."
description_zh: "只读查询lead/customer的公私海状态、公海分组、负责人、进入时间和最近跟进情况，用于资源盘点。适用于用户要求公私海查询分析、生成相关结果或检查执行边界时。"
allowed-tools: Bash
---
...
定位：只读查询lead/customer的公私海状态、公海分组、负责人、进入时间和最近跟进情况，用于资源盘点。边界：不执行分配、转移或退回，不扩大到文档未支持对象。
...
> 分别查询lead和customer公海记录，publicFlag=1；统计口径为2026-07-01至2026-07-31进入公海，负责人范围userId=42。进入公海日期字段和负责人字段必须由租户动态发现；任一对象未开放日期字段时单独标记不可计算，不把创建日期替代为进入公海日期。
