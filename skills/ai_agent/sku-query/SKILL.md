...
name: sku-query
version: 1.4.0
description: SKU Query workflow for foreign-trade teams, with explicit evidence,
  controlled CRM access, confirmation, and safety boundaries.
description_zh: 只读查询stockKeepingUnit字段和记录，为产品清单、订单校验和销售分析提供数据。适用于用户要求SKU查询、生成相关结果或检查执行边界时。
allowed-tools: Bash
...
定位：只读查询stockKeepingUnit字段和记录，为产品清单、订单校验和销售分析提供数据。边界：不新增/编辑SKU；不返回租户未开放字段。
...
> 在已授权测试租户中查询stockKeepingUnit：目标SKU编码=SKU-TUMBLER-304-500，产品名称=304 Stainless Steel Tumbler 500ml。先动态发现SKU编码和名称对应的租户字段，再按确切字段筛选；若编码字段未开放则停止并报告缺口，不使用未经发现的日期或负责人字段。
