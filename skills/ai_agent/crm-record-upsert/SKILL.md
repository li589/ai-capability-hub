---
name: crm-record-upsert
version: 1.4.0
description: "CRM Record Upsert workflow for foreign-trade teams, with explicit evidence, controlled CRM access, confirmation, and safety boundaries."
description_zh: "受控新增或编辑lead、customer、contact，执行字段发现、格式校验、查重、预览、确认和回查。适用于用户要求CRM建档更新、生成相关结果或检查执行边界时。"
allowed-tools: Bash
---
...
定位：受控新增或编辑lead、customer、contact，执行字段发现、格式校验、查重、预览、确认和回查。边界：不覆盖未展示字段；不绕过权限和必填校验；不承接商机/订单写入。
...
> 更新lead dataId=3001：email=buyer@example.de，phone=+491701112233，name=Berlin Gift GmbH；目标字段和值均已由业务员确认。先动态发现lead字段并核对当前值和重复记录，再生成逐字段dry-run并停在确认门。
