---
name: sales-operations-review
version: 1.4.0
description: "Sales Operations Review workflow for foreign-trade teams, with explicit evidence, controlled CRM access, confirmation, and safety boundaries."
description_zh: "汇总开放的线索、客户、商机、跟进、订单及负责人数据，形成数量漏斗和经营复盘。适用于用户要求销售经营复盘、生成相关结果或检查执行边界时。"
allowed-tools: Bash
---
...
定位：汇总开放的线索、客户、商机、跟进、订单及负责人数据，形成数量漏斗和经营复盘。边界：跨对象关联不完整时只做分对象统计；不补造广告、网站、报价或成本数据。
...
- 线索、客户、商机、跟进和订单先分对象统计；只有存在可验证关联键时才计算跨对象转化。
- 默认按recordId去重，按负责人归属必须注明取创建人还是当前负责人。
- 金额指标必须按币种分组，未提供汇率来源与日期时禁止跨币种合计。
- 输出：口径卡｜分对象指标表｜可验证漏斗｜异常清单｜下一步动作。

## 实战使用卡

### 可直接复制的任务示例

> 复盘2026年7月线索、客户、商机、跟进和订单；按当前负责人分组，金额按USD/EUR分别汇总，仅在存在关联ID时计算跨对象转化。
