...
version: 1.4.0
description: "Sales Funnel Analysis workflow for foreign-trade teams, with explicit evidence, controlled CRM access, confirmation, and safety boundaries."
description_zh: "汇总lead、customer、opportunity和order，按负责人、国家、产品、来源或时间形成数量漏斗。适用于用户要求销售漏斗分析、生成相关结果或检查执行边界时。"
allowed-tools: Bash
---
...
定位：汇总lead、customer、opportunity和order，按负责人、国家、产品、来源或时间形成数量漏斗。边界：跨对象转化率仅在可靠关联字段存在时计算；否则仅展示独立对象数量。
...
- 同一漏斗必须先声明统计周期、对象、阶段字段和去重键；默认按recordId去重。
- 阶段库存量与期间流入量分开统计，不把不同对象的总数直接当作连续转化漏斗。
- 转化率=进入下一阶段的唯一记录数/进入当前阶段的唯一记录数；分母为0时输出N/A。
- 输出表：阶段｜期初库存｜期间新增｜流向下一阶段｜期末库存｜转化率｜数据缺口。
