...
version: 1.4.0
description: CRM Data Quality workflow for foreign-trade teams, with explicit
  evidence, controlled CRM access, confirmation, and safety boundaries.
description_zh: 只读扫描lead、customer、contact的缺失字段、异常选项、格式不一致和疑似重复，生成修复预览。适用于用户要求数据质量治理、生成相关结果或检查执行边界时。
allowed-tools: Bash
...
定位：只读扫描lead、customer、contact的缺失字段、异常选项、格式不一致和疑似重复，生成修复预览。边界：不自动合并/删除；批量更新必须分批、小样验证、确认并逐批回查。
