...
version: 1.4.0
description: Follow-up Record Query workflow for foreign-trade teams, with
  explicit evidence, controlled CRM access, confirmation, and safety boundaries.
description_zh: 只读查询followUpRecord，并按线索、客户或商机整理时间线、摘要和待办。适用于用户要求跟进记录查询、生成相关结果或检查执行边界时。
allowed-tools: Bash
...
定位：只读查询followUpRecord，并按线索、客户或商机整理时间线、摘要和待办。边界：只使用可读取记录；不把不同对象或同名客户的跟进混合。
...
- 关联键固定为objectApiKey+objectDataId；同名客户或联系人不得合并。
- 时间优先使用业务发生时间；缺失时才使用createdTime并标注替代，统一输出ISO 8601和租户时区。
- 按recordId去重、时间升序形成时间线；空结果只说明当前权限和范围内未发现记录。
- 输出列：时间｜对象类型｜对象ID｜跟进ID｜方向/渠道｜原文摘要｜事实｜推断｜待办。

## 实战使用卡

### 可直接复制的任务示例

> 查询customer dataId=100234在2026-07-01至2026-07-31的followUpRecord，按业务发生时间升序输出时间线，租户时区Asia/Shanghai。
