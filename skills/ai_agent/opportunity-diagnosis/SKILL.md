---
name: opportunity-diagnosis
version: 1.4.0
description: "Opportunity Diagnosis workflow for foreign-trade teams, with explicit evidence, controlled CRM access, confirmation, and safety boundaries."
description_zh: "查询opportunity及关联followUpRecord，识别阶段停滞、金额/预计成交时间缺失和跟进风险。适用于用户要求商机查询诊断、生成相关结果或检查执行边界时。"
allowed-tools: Bash
---
...
定位：查询opportunity及关联followUpRecord，识别阶段停滞、金额/预计成交时间缺失和跟进风险。边界：诊断基于可见字段，不生成无依据的赢单概率或成交日期。
...
- 默认停滞阈值为最近一次有效跟进后14个自然日，用户规则优先；没有跟进时单列“未发现跟进”。
- 金额和预计成交时间缺失只作为资料风险，不生成赢单概率或预测日期。
- 输出列：opportunityId｜当前阶段｜停滞天数｜金额/币种｜预计成交日｜最近跟进｜事实风险｜建议动作。

## 实战使用卡

### 可直接复制的任务示例

> 查询ownerId=42的opportunity，以最近有效跟进后14个自然日为停滞阈值；输出阶段、金额/币种、预计成交日、停滞天数和字段缺口。
