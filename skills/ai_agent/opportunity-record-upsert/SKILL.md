---
name: opportunity-record-upsert
version: 1.4.0
description: "Opportunity Record Upsert workflow for foreign-trade teams, with explicit evidence, controlled CRM access, confirmation, and safety boundaries."
description_zh: "通过objFieldData新增或更新opportunity，执行字段发现、查重、dry-run、确认、写入和同步回查。适用于用户要求商机建档更新、生成相关结果或检查执行边界时。"
allowed-tools: Bash
---
...
定位：通过objFieldData新增或更新opportunity，执行字段发现、查重、dry-run、确认、写入和同步回查。边界：字段、对象类型或关联客户未确认时停在预览。
...
> 根据已确认资料生成opportunity dry-run：customerId=100234，名称“德国500件保温杯”，金额USD2400，预计成交日2026-08-31；停在确认门。
