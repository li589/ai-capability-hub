...
//
// 触发场景（现场已多次出现）：
//   1) Agent 第一次发 `--prompt "分析X的估值水平，重点关注..."`，被沙箱杀进程；
//   2) 误判失败 → 改短 prompt `--prompt "分析X的估值水平"`，再次新建任务；
 ...
 *   2) prefix：较长 prompt 以较短 prompt 开头（"分析X" / "分析X的估值水平"）
...
/** 可比公司分析 replay 预检用的通用词（不作为主体 token）。 */
...
function hasCompsAnalysisIntent(text) {
  const t = String(text);
  if (!t.includes("可比") && !t.includes("估值") && !t.includes("对标")) return false;
  return /报告|分析|公司|倍数|PE|PB|EV/.test(t);
}
...
function isLikelyCompanyCoreName(name) {
  ...
  if (/^(估值|对标|可比|倍数|分析|对比|报告)/.test(name)) return false;
  return true;
}

/**
 * 从可比公司分析 prompt 提取稳定的主体键（跨措辞 submit 锁 / 调度续接用）。
...
export function computeSubjectKey(prompt) {
  ...
  if (!normalized || !hasCompsAnalysisIntent(normalized)) return null;
 ...
 * 同时，对较长段去掉前缀/后缀的停用词后提取核心主体名（如「给我一份泰山石油的可比公司分析」→「泰山石油」）。
...
/**
 * completed 任务 replay 预检用的相似度（比 running 防护略宽，捕获「可比公司报告」vs「可比公司分析报告」等措辞差异）。
 *
...
export function comparePromptsForReplay(a, b) {
  ...
  if (!hasCompsAnalysisIntent(a) || !hasCompsAnalysisIntent(b)) {
...
export function findSimilarRunning(records, prompt, opts = {}) {
  ...
  for (const r of records) {
    ...
    // 与 completed replay 共用 comparePromptsForReplay，捕获「可比公司报告」vs「可比公司分析报告」等同主体措辞差异
