...
if (isHelp) {
  console.log(
    [
      'alice-comps-analysis — 调用万得 Alice Agent「可比公司分析」，对上市公司做可比公司估值对比（PE/PB/EV 倍数、同业排名、估值溢价/折价），用户原话直接透传给服务端',
      "",
      "Usage:",
      "  alice-comps-analysis --prompt <QUESTION> [--download-dir <DIR>] [--new]",
      "  alice-comps-analysis --prompt <QUESTION> --detach                        # 后台跑：立刻返回 PID 与日志路径，进程脱离当前会话",
      "  alice-comps-analysis --prompt <QUESTION> --no-wait                       # 沙箱推荐：CLI 内部自旋直到完成（默认）",
      ...
      "",
      "Windows UTF-8:",
      "  powershell -NoProfile -ExecutionPolicy Bypass -File scripts/acomps.ps1 --prompt <Q> --no-wait",
      "  管道捕获 live 中文乱码时，读 stdout 的 ALICE_SESSION_LOG= 路径（UTF-8 BOM 会话日志）。",
      "",
      "Options:",
      '  --prompt,       -p <QUESTION>   用户提问，例如"帮我做一份宁德时代的可比公司分析？比亚迪有没有短线机会？"（必填）',
      ...
      "  --no-strict                     关闭 strict 模式（默认开启）；未输出 ALICE_COMPS_ANALYSIS_DONE 时",
      ...
      "",
      "退出码:",
      "  0   成功：必定伴随 stdout 含 ALICE_COMPS_ANALYSIS_DONE 行（含 promptHash= / reportFullFile=；Agent 判定任务完成的唯一确定信号）",
      "  1   一般失败 / 任务终态为 failed",
      ...
      '  Set-Location "D:\\path\\to\\alice-comps-analysis"; node scripts/cli.mjs --prompt "帮我做一份宁德时代的可比公司分析？比亚迪有没有短线机会？"',
      '  node "D:\\path\\to\\alice-comps-analysis\\scripts\\cli.mjs" --prompt "贵州茅台在同行业中估值水平如何？PE 倍数偏高还是偏低？"',
      '  alice-comps-analysis -p "隆基绿能和同行业可比公司相比估值如何？"',
      '  alice-comps-analysis -p "比亚迪的可比公司有哪些？各家的 PE/PB 倍数？" --download-dir "D:\\reports\\comps"',
      '  alice-comps-analysis -p "重新做宁德时代的可比公司分析" --new',
      '  alice-comps-analysis -p "医药板块龙头估值对比情况？" --no-wait              # 沙箱：CLI 内部自旋直到完成',
      '  alice-comps-analysis -p "中芯国际相对同行的估值溢价多少？" --no-wait --once     # 脚本：单次探针',
      '  alice-comps-analysis -p "帮我做一份宁德时代的可比公司分析" --detach           # 后台跑，立即返回，IDE 杀终端也不影响',
      "",
    ...
    if (!prompt || typeof prompt !== "string" || !prompt.trim()) {
      ...
      console.log(
        [
          ...
          `  · 轮询结果:    alice-comps-analysis -p "${prompt}" --no-wait`,
          `  · 查落盘路径:  alice-comps-analysis status -p "${prompt}"`,
          "",
          "完成后报告位置：",
