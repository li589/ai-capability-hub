...
import { formatLocalFileLink, sanitizeAgentResultTextForDelivery } from "./agentResultSanitize.js";
import {
...
// 本 CLI 仅承载万得 Alice 的「可比公司分析」技能。
// 服务端通过 prompt 文本前缀 `使用「<Skill 名>」技能：` 识别 Skill。
const ALICE_SKILL_NAME_ZH = "可比公司分析";
...
//        ALICE_COMPS_ANALYSIS_DONE taskId=<id> promptHash=<hash> reportFile=<abs> reportFullFile=<abs>
//      Agent 只需匹配该行就能确认成功，比解析 ALICE_COMPS_ANALYSIS_STATUS=COMPLETED
...
/** 由 printAttachmentFileTag / downloadCollectedFiles 记录非 .md 附件的绝对路径。 */
...
/**
 * 输出 ALICE_COMPS_ANALYSIS_DONE 行。幂等：同一进程内只输出一次。
...
function emitAnalysisDone({ taskId, promptHash } = {}) {
  ...
  const parts = ["ALICE_COMPS_ANALYSIS_DONE"];
  ...
  console.log(
    "[CLI] 上一行 ALICE_COMPS_ANALYSIS_DONE 是本进程任务"
      + "真正完成"
  ...
  console.log(
    "[CLI] 交付用户：将 stdout 中 agentResult.value 原文交给用户（禁止概括）；"
      + "stdout 被截断时读 reportFile= 兜底。禁止读取 reportFullFile= / attachmentFile= / download/ 附件正文向用户展示。",
  );
  console.log(
    "[CLI] reportFullFile= / attachmentFile= 表示附件已静默下载到本地；"
      + "Agent 必须用 present_files 呈现（路径已在工作空间，无需 cp），不是交付正文。",
  );
  ...
  if (lastReportFullFilePath) {
    ...
    console.log(
      "[CLI] Agent 必须用 present_files 呈现上述 reportFullFile= 附件"
        + "（路径已在工作空间，无需 cp）；路径已内联在 agentResult.value 正文中。",
    );
    console.log(
  ...
  for (const attPath of lastAttachmentFilePaths) {
    ...
    console.log(
      `[CLI] Agent 必须用 present_files 呈现上述 attachmentFile= 附件`
        + "（路径已在工作空间，无需 cp）；路径已内联在 agentResult.value 正文中。",
    );
    console.log(
...
function installStrictExitHook() {
  ...
  process.on("exit", () => {
    ...
    try {
      console.error(
        "[CLI][strict] 进程退出但未输出 ALICE_COMPS_ANALYSIS_DONE；强制退出码="
          + EXIT_STRICT_NO_DONE
          + "。",
      );
      console.error(
...
// strict 模式专用退出码：
// - 6 = 进程退出时未输出 ALICE_COMPS_ANALYSIS_DONE，且 exitCode 仍为 0；
...
function replayCompletedArtifacts({ registry, taskId, promptHash, logPrefix = "[CLI]" }) {
  ...
  if (downloadedFiles.length > 0) {
    ...
    const nonMdFiles = downloadedFiles.filter((e) => !/\.md$/i.test(e.path));
    const mdPresent = mdFiles.filter((e) => existsSync(e.path));
    const mdMissing = mdFiles.filter((e) => !existsSync(e.path));
    const nonMdPresent = nonMdFiles.filter((e) => existsSync(e.path));
    const nonMdMissing = nonMdFiles.filter((e) => !existsSync(e.path));

    if (mdPresent.length > 0) {
      console.log(`${logPrefix} 已登记的完整报告附件（跨进程复用，未重新下载）：`);
      for (const { path: absPath, filename } of mdPresent) {
    ...
    if (mdMissing.length > 0) {
      ...
      for (const { path: absPath, filename } of mdMissing) {
        console.log(`  - ${filename ?? absPath} → ${absPath}`);
      }
      console.log(`${logPrefix} 如需完整报告，请加 --new 重新跑任务。`);
    }
    if (nonMdPresent.length > 0) {
      console.log(`${logPrefix} 已登记的数据附件（跨进程复用，未重新下载）：`);
      for (const { path: absPath, filename } of nonMdPresent) {
        console.log(`  - ${filename ?? absPath}`);
        console.log(`    ${absPath}`);
        printReportFullFileTag(absPath, {
          reused: true,
    ...
    if (nonMdMissing.length > 0) {
      console.log(`${logPrefix} 警告：以下数据附件曾下载但本地已不存在：`);
      for (const { path: absPath, filename } of nonMdMissing) {
        console.log(`  - ${filename ?? absPath} → ${absPath}`);
      }
    }
  } else {
...
function clearCompletedRecordForResubmit(registry, promptHash, { logPrefix = "[CLI]" } = {}) {
  ...
  console.log(
    `${logPrefix} 若只需查询历史落盘路径，请用：alice-comps-analysis status -p "<同一问题>"`,
  );
...
function checkSubmitLockAtPath(lockPath) {
  ...
  // 自己持有的锁不算冲突（enforceSubmitLock 可能被同一进程多次调用）
...
function printAttachBanner({ taskId, contextId, startedAt }) {
  ...
  console.log("");
  console.log("ALICE_COMPS_ANALYSIS_STATUS=RUNNING");
...
function printCliRunStartNotice() {
  const lines = [
    "ALICE_COMPS_ANALYSIS_STATUS=RUNNING",
    "[CLI] 可比公司分析已启动：本进程将阻塞运行直至成功（agentResult + [完成]）或失败，通常需 2-15 分钟。",
    "[CLI] [等待中] 心跳期间无新输出属正常；禁止用 check_command_status 轮询替代等待本进程结束。",
    "[CLI] 见到 [任务已受理] 仅表示已提交，不等于报告完成；必须等到 ALICE_COMPS_ANALYSIS_STATUS=COMPLETED。",
  ];
  for (const line of lines) {
    console.error(line);
  }
...
function persistAgentResultIfAny({ registry, taskId, promptHash, prompt }) {
  ...
  if (!text) return null;
  const header = [
    `<!-- alice-comps-analysis：本次 Alice agentResult.value 已落盘 -->`,
    `<!-- 重要：本文件是 Alice 响应正文（agentResult.value），并不一定等于"完整可比公司分析报告"。 -->`,
...
function printCliRunEndNotice({ success, exitCode, elapsed }) {
  const lines = success
    ? [
        "ALICE_COMPS_ANALYSIS_STATUS=COMPLETED",
    ...
    : [
        "ALICE_COMPS_ANALYSIS_STATUS=FAILED",
...
function usage() {
  return [
    "alice-comps-analysis — 调用万得 Alice Agent「可比公司分析」技能，对上市公司做可比公司估值对比（PE/PB/EV 倍数、同业排名、估值溢价/折价）",
    "",
    "Usage:",
    "  alice-comps-analysis --prompt <QUESTION> [--download-dir <DIR>] [--new]",
    "  alice-comps-analysis --prompt <QUESTION> --no-wait                       # 沙箱推荐：CLI 内部自旋 probe 直到完成（默认）",
    "  alice-comps-analysis --prompt <QUESTION> --no-wait --once                # 单次探针：几秒内返回（脚本/调试）",
    "  alice-comps-analysis --prompt <QUESTION> --no-wait \\",
    "      --watch-interval 60 --watch-timeout 1800                              # 自定义轮询节奏",
    "  alice-comps-analysis status --prompt <QUESTION>                        # 查询落盘路径（不访问服务端）",
    "  alice-comps-analysis check-conflict --prompt <QUESTION>                # [已废弃] 主调用 --no-wait 已内置去重，无需预检",
    "  alice-comps-analysis apikey-set <KEY>",
    "  alice-comps-analysis apikey-get",
    "  alice-comps-analysis apikey-clear",
    "  alice-comps-analysis --help",
    "",
    "Options:",
    "  --prompt,       -p <QUESTION>   用户提问，例如\"分析宁德时代的可比公司\"（必填）",
    ...
    "                                  ALICE_COMPS_ANALYSIS_DONE 行，会把 exitCode=0 强制改成 6，",
    ...
    "",
    "退出码:",
    "  0   成功：必定伴随 stdout 含 ALICE_COMPS_ANALYSIS_DONE 行（Agent 判定任务完成的唯一确定信号）",
    "  1   一般失败 / 任务终态为 failed",
    "  2   参数错误 / KEY_MISSING",
    "  4   --no-wait 专用：已达全程自旋上限（默认 60min）仍未完成；Agent 再执行一次相同 --no-wait 续接",
    "  5   --no-wait 专用：--once 且本地无任务记录（默认 --no-wait 会自动提交）",
    "  6   strict 模式兜底：进程退出但未输出 ALICE_COMPS_ANALYSIS_DONE（通常表示进程被沙箱终止）；",
...
async function downloadCollectedFiles(apiKey, override, ctx = {}) {
  ...
  // 所有下载件统一落工作空间 process.cwd()。CLI 先在沙箱内自下载；
  // 仅当自下载失败（沙箱封 HTTP 等）才输出 ALICE_EXTERNAL_DOWNLOAD，交宿主 Agent 在沙箱外 curl。
  console.log("");
  ...
  for (const { url, filename } of items) {
    ...
    if (result.ok) {
      // CLI 沙箱内下载成功，文件已就绪--Agent 无需 curl，直接 present_files
      if (registry && taskId) {
        try {
          registry.appendDownloadedFile(taskId, { url, path: result.path, filename });
        } catch (e) {
      ...
      // CLI 沙箱内下载失败 -> 交宿主 Agent 在沙箱外 curl（此时 Agent 必须执行步骤 4.5 的 curl）
      if (registry && taskId) {
        try {
      ...
      console.log(`  [CLI] 沙箱内下载失败（${result.error}），交由宿主 Agent 在沙箱外下载`);
 ...
 * `ALICE_COMPS_ANALYSIS_STATUS=...`，由调用方（单次 probe / watch 轮询）在合适
...
async function runProbeStep({ registry, taskId, promptHash, prompt, url, headers, apiKey, downloadDir }) {
  ...
  if (probe.kind === "working") {
    ...
    console.log(
      "[CLI] 本进程尚未输出 ALICE_COMPS_ANALYSIS_DONE；logs/ 下其它 .session.log 内的 DONE 行与本 PROMPT_HASH 不一致则无效。",
    );
 ...
 * 调用方读到的 `ALICE_COMPS_ANALYSIS_STATUS=...` 语义一致。
 *
...
function publishFinalStatus(outcome, { suggestExternalPoll = false } = {}) {
  switch (outcome) {
    case "completed":
      console.log("ALICE_COMPS_ANALYSIS_STATUS=COMPLETED");
      emitAnalysisDone();
      break;
    case "failed":
      console.log("ALICE_COMPS_ANALYSIS_STATUS=FAILED");
      break;
    case "no-task":
      console.log("ALICE_COMPS_ANALYSIS_STATUS=NO_TASK");
      break;
    case "working":
    default:
      if (suggestExternalPoll) {
        ...
        console.log(
          "[探针] 禁止读取 download/ 同名报告或 STALE_REPORT_CANDIDATE；必须见到 ALICE_COMPS_ANALYSIS_DONE 且 promptHash= 一致。",
        );
      }
      console.log("ALICE_COMPS_ANALYSIS_STATUS=RUNNING");
      break;
...
async function main() {
  ...
  // analysis 主路径：默认开启 strict 模式（未输出 ALICE_COMPS_ANALYSIS_DONE 即视为
  ...
  if (noWait) {
    ...
    console.log("ALICE_AGENT_REDLINE=禁止在 stdout 出现 ALICE_COMPS_ANALYSIS_DONE 之前 view_files/read download/ 主体报告");
      ...
      mainLoop: while (true) {
        for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
          ...
          if (!startBannerPrinted) {
            ...
            console.log(
              `[开始] 已向 Alice「${ALICE_SKILL_NAME_ZH}」技能发起请求；可比公司分析通常耗时 2-5 分钟，期间会定期播报等待进度，请勿中断。`,
            );
          }
