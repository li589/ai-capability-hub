...
>
> 因此,CLI 通过 `acomps.ps1` 跑完后,服务端返回的 `agentResult.value` 完整结果**会停留在 Bash 工具的 stdout 输出里,被折叠进「已完成」块**。**你不能指望用户去点开那个折叠块**。
>
...
>
> **常见场景**:
> - 用户先「帮我做一份宁德时代的可比公司分析」-> 交付 -> 「**那比亚迪呢**」：同话题延伸，**什么都不加**，自动续接。
> - 「**那 PE 倍数呢 / 估值定位怎么看**」：同话题追加维度，**什么都不加**。
...
| 步 | 动作 | 对用户怎么说 |
|----|------|-------------|
| 0 | **API Key 配置**：从 `agent_md` 读取已保存的 Wind Alice API Key，若存在则执行 `apikey-set`；若不存在则询问用户提供 Key，保存到 `agent_md` 后再执行 `apikey-set` | （已有 Key 时静默执行）；无 Key 时：「使用可比公司分析需要配置一个 Key，你有 Wind Alice 的 API Key 吗？」 |
| 1 | 把用户问题拼成一句自然语言 `--prompt`（**不要**加 `使用「可比公司分析」技能：` 前缀）；**同步判断是否切到完全无关的新话题**（见 [「会话续接判断」专节](#会话续接判断agent-自动执行无需用户显式说)）：同话题追问**什么都不加**（CLI 工作区内自动续接）；完全无关新话题才加 `--new-session` | （无需说话） |
| 2 | 告知用户：通常需 **2–15 分钟** | 「好的，我来帮你分析。可比公司分析通常需要 2–15 分钟，请稍等。」 |
...
| 4.5 | **下载附件（按需 curl）**：CLI 会先在沙箱内自下载附件。若 stdout **无** `ALICE_EXTERNAL_DOWNLOAD` 行，文件已就绪，**跳过本步**直接 present_files。仅当 stdout **含** `ALICE_EXTERNAL_DOWNLOAD url=... target=...`（CLI 沙箱内下载失败），Agent 才对每条用 `curl -k` 下载到 `target` 路径，用 `~/.wind-alice/config.env` 里的 API Key 做 `Authorization: Bearer` 鉴权 | （静默执行，无需对用户解释） |
| 5 | **若 DONE 含 `reportFullFile=` 和/或 `attachmentFile=`**（可比公司分析常见：`.md` 报告 + `.xlsx` 数据表）：步骤 4.5 已把附件下载到当前工作空间（`process.cwd()`），路径必在工作空间内，**无需 `cp`**。① 从 DONE 行提取**所有** `reportFullFile=` / `attachmentFile=` 路径（CLI 会打印 `ALICE_PRESENT_FILES=1 path=...` 供核对）；② 在一条**独立消息**里对每个附件调 `present_files`（直接传该路径，可一次传多个）。**这条消息里不要写分析正文，也禁止任何其它工具调用**（尤其 Edit/Write 写 `.workbuddy/memory` 记忆日志）--否则整条被折叠、卡片被隐藏。记忆日志写入必须在步骤 4.5 回合（下载同回合）完成。无附件时跳过本步 | （静默调用，无需对用户解释） |
...
CLI 会先在沙箱内自下载附件（通常成功，文件已就绪）。**仅当 CLI 沙箱内下载失败时**，stdout 才输出 `ALICE_EXTERNAL_DOWNLOAD url=... target=...` 行--此时 Agent 必须逐条用 `curl -k` 下载。

**判断是否需要 curl**：
- stdout **无** `ALICE_EXTERNAL_DOWNLOAD` 行 -> CLI 已下载成功，文件已就绪，**跳过 curl**，直接进入步骤 5（present_files）。
- stdout **含** `ALICE_EXTERNAL_DOWNLOAD url=... target=...` -> CLI 沙箱内下载失败，Agent 必须 curl。
...
**注意**：
- `reportFullFile=` / `attachmentFile=` 的值就是文件落盘路径（CLI 自下载成功即该路径；失败时是 ALICE_EXTERNAL_DOWNLOAD 的 target）
- 步骤 4.5 必须和步骤 4（核对 DONE）在**同一回合**内完成，不可延迟到后续回合
...
> 2. **`present_files` 与分析正文必须分两条消息**：WorkBuddy 会把**含工具调用的整条消息**折叠成「Tool calls」块、藏住其中的文本。因此 `present_files`（若需）在**独立一条消息**里调用（折叠成卡片可接受）；分析正文（`agentResult.value`）必须在**另一条不含任何工具调用**的纯文本消息里输出。旧版「正文输出之后再调 present_files」会让两者落在同一条消息、整条被折叠--已废弃。 🔴 **`present_files` 那条消息也必须零其它工具调用**--尤其禁止同回合 Edit/Write（写 `.workbuddy/memory` 记忆日志）、Bash、Read；否则整条被折叠、**文件卡片被一起藏起来**（用户看不到 present_file）。记忆日志写入必须在主调用回合（步骤 4.5，curl 下载同回合）完成、先于 present_files。 🔴 **present_files 那条消息里不得有任何文字**（包括道歉、解释、「卡片如下」等）--哪怕是在重新发送卡片（首次没显示要重发）时，道歉/解释也必须单独一条文本消息，present_files 在另一条**只含 present_files** 的消息；文字与 present_files 同消息会整条折叠、连卡片带文字一起藏起来。
> 3. **顺序**：① 静默跑完主调用 -> ②（若 `reportFullFile=` / `attachmentFile=`）在**独立消息**调 `present_files`（**本回合只能有 present_files 一个工具调用（禁 Bash/Read/Edit/Write/文件操作）**--否则卡片被折叠隐藏；这条可折叠成卡片；**含 xlsx 数据表**）-> ③ 在**纯文本消息**里输出 `agentResult.value`（**本条禁任何工具调用**，否则整条被折叠）。
> 4. 🔴 **附件路径已在工作空间**：步骤 4.5 已把所有附件下载到当前工作空间（CLI 自下载；失败时 Agent curl 兜底）（`process.cwd()`），`reportFullFile=` / `attachmentFile=` 路径必在工作空间内，**无需 `cp` 复制**——直接用该路径调 `present_files` 即可。
...
3. **若未保存**：
   - 向用户说：「使用可比公司分析需要配置一个 Key，你有 Wind Alice 的 API Key 吗？」
...
| 「请提供 Wind Alice API Key 以便调用可比公司分析服务端接口。」 | 「使用可比公司分析需要配置一个 Key，你有 Wind Alice 的 API Key 吗？」 |
...
宿主执行主调用 / status 等 Bash 工具调用时，会把你填在工具调用的 `description` 字段里的文字，作为**权限确认框的标题**展示给用户。请把该字段填**简短中文**，不要填英文（如「Call Wind Alice for financial analysis」→ 改填「调用万得 Alice 进行可比公司分析」）。

对应七步流程的推荐 `description` 取值：

| 步骤 | 命令 | `description` 填写 |
|------|------|--------------------|
| 3 | 主调用 `--no-wait` | 调用万得 Alice 进行可比公司分析 |
...
>
> 1. **Windows 必须用 `acomps.ps1`，禁止裸 `node scripts/cli.mjs`**（裸 node 在 PowerShell 5.x 下中文乱码 + 沙箱兼容性问题）。
> 2. **禁止 `Set-Location`/`cd` 到 `<SKILL_DIR>`**。`acomps.ps1` 用绝对路径调用，**不 cd**，让 `process.cwd()` 保持当前工作区。数据目录(tasks.json/session.log/submit-locks)**固定在 `~/.wind-alice/`**（跨会话稳定，不跟 cwd 走），故 cd 不影响状态目录；但仍禁止 cd 到 skill 目录：避免污染 skill 目录、避免 `present_files` 路径错乱。
...
> 4. **禁止用 `Out-File`/`>`/`2>&1 | Out-File` 把 stdout 重定向到文件**。CLI 的完成信号(`ALICE_COMPS_ANALYSIS_DONE`、`agentResult.value`)在 stdout，Agent 必须**直接捕获 stdout**，重定向到文件既把文件写到工作区外(弹窗)、又可能导致 Agent 读不到完成信号。
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\acomps.ps1" --prompt "<USER_QUESTION>" --no-wait -d "<WORKSPACE_DIR>" [--context-id <上一轮contextId>] [--new-session]
```
...
| **上一轮返回了错误公司**（replay 命中但主体不匹配） | 加 `--new-session`（session 已污染）；用明确 Prompt（含公司名）新开会话 |
...
- **代词/指代**:「它 / 这家 / 这只 / 上面那份能不能加 XX」。
- **相同主体追加维度**:上次分析 XX 的 A 维度,本次问 XX 的 B 维度(同一主体、不同角度)。
- **同一可比公司话题延伸**:上文分析「帮我做一份宁德时代的可比公司分析」,本次问「那比亚迪呢 / PE 倍数呢 / 估值定位怎么看」--同属该公司可比分析话题,**必续接**。

**案例（必读）**:
- ✅ 用户先「帮我做一份宁德时代的可比公司分析」-> 你交付 -> 用户「**那比亚迪呢**」：同话题延伸追问，**什么都不加**，自动续接。
- ✅ 用户「宁德时代可比公司分析」-> 「**那 PE 倍数呢**」：同一公司话题追加维度，**什么都不加**。
- ❌ 用户「帮我做一份宁德时代的可比公司分析」-> 「**宁德时代信用怎么样**」：完全无关新主题，加 `--new-session`。

**判定为切换（加 `--new-session`）**:
- 用户明说:「换个话题 / 忘掉刚才 / 新问题」。
- 主体/主题明显无关:上文聊可比公司估值,本次问债券利率或商品期货。
...
6. **禁止在无完成信号时交付** — 无 `ALICE_COMPS_ANALYSIS_DONE` 与 `agentResult.value` 时**不得交付**任何分析结论。
...
- stdout 含 **`ALICE_COMPS_ANALYSIS_DONE`**，且其中 **`promptHash=`** 与本次 CLI 打印的 **`PROMPT_HASH=`** 一致；
- 退出码 **`0`**；
...
2. 若**是**：
   ...
   - 若不涉及 `--context-id`，从 `agentResult.value` 正文中提取标的/同行关键词，与当前会话上下文对比：
   - **主体明显不同**（跨品种/跨视角，如缓存的「帮我做一份宁德时代的可比公司分析」vs 请求的「宁德时代信用分析」；或缓存的「宁德时代」vs 请求的「螺纹钢期货情绪」）→ **静默自动重跑**：用**原 prompt** 加 `--new --no-wait` 重新执行（将消耗积分）。**不向用户提及缓存/主体不匹配/重跑等任何技术细节**——用户不需要知道自己引来了别的主题缓存。重跑后按正常流程交付。
   - **主体一致或高度相似**（同品种不同写法，如「宁德时代」vs「CATL」；同一公司话题的追问）→ 停下来告知用户：「该问题已有最近的分析结果。」然后列出：(A) 查看已有结果 / (B) 重新分析 / (C) 取消。**禁止**未经用户明确选择就交付或 `--new`。
   - **拿不准时**（主体模糊、仅泛化关键词如「公司」「估值」、缩写无法判定）→ 按「主体一致」处理，停下来询问用户。
3. 若**否**：正常交付，按步骤 5 处理。

**关键判断标准**：
- 明显不同：两个公司主体/话题完全不同、无任何字面重叠（如「帮我做一份宁德时代的可比公司分析」→「宁德时代信用」、「宁德时代」→「螺纹钢期货情绪」）。这类情况直接静默重跑，**绝不打扰用户**。
- 需要停下来问：同一公司不同写法（「宁德时代」→「CATL」）、同一公司话题的细化追问、或 Agent 无法判断是否同一主题时。
...
| ✅ **必须** | DONE 行 `reportFullFile=` / `attachmentFile=` → `工作空间/*` | **必须用 `present_files` 呈现**（路径已在工作空间，无需 cp）；路径已内联在 `agentResult.value` 正文中（**保留 markdown 链接格式**）；**禁止加载文件内容展示给用户** |
...
| `reportFullFile=` -> `工作空间/*.md` | 完整报告 Markdown 附件 | **present_files**；路径已内联在正文中 |
| `attachmentFile=` -> `工作空间/*.xlsx` 等 | 数据附件（Excel 等） | **present_files**；路径已内联在正文中 |

若 DONE 行**没有** `reportFullFile=` 且**没有** `attachmentFile=`（服务端未返回可下载附件），**不要**调 `present_files`，只交付 `agentResult.value` 即可。
...
| `78` | 环境受限，无法保存任务状态 | **未向服务端发请求**；告知用户「当前环境无法运行 可比公司分析，请在工具中开启完全访问权限后重试」；禁止换 prompt / `--new` / 反复重试 |
...
可比公司分析通常 **2–15 分钟**。若宿主 `run_command` 只有数十秒～几分钟超时，**第一条 `--no-wait` 会被强杀**（exit `4`/`6`），看起来像「卡住」——其实是终端杀了 CLI，服务端往往仍在跑。此时对用户说：「分析仍在进行中，我继续等待……」，**不要**说「被杀」「超时」等技术细节。
...
powershell ... acomps.ps1 --prompt "<USER_QUESTION>" --no-wait
```
...
| 信号 | 含义 |
|------|------|
| `ALICE_COMPS_ANALYSIS_DONE` | **唯一**完成标记 |
| `ALICE_DELIVER_AS_TEXT=1` | **交付动作提醒**（DONE 前打印，醒目 `⚠️ 交付提醒` 框）：必须把上面 stdout 里的 `agentResult.value` **逐字复制成正文回复**，否则被宿主折叠、用户看不到 |
| `ALICE_PRESENT_FILES=1 path=...` | **present_files 动作提醒**（DONE 前打印，醒目 `⚠️ present_files 提醒` 框）：每个 `reportFullFile=` / `attachmentFile=` 各一行；路径已在工作空间，直接在独立消息调 `present_files` |
...
# ❌ 用裸 node cli.mjs 代替 acomps.ps1（Windows 上会导致中文乱码、沙箱兼容性问题）
...
□ stdout 有 ALICE_COMPS_ANALYSIS_DONE？
...
□ 若 DONE 含 reportFullFile= 和/或 attachmentFile=：路径已在工作空间（CLI 统一下载到 process.cwd()），直接用该路径调 present_files（无需 cp）？**xlsx 数据表也必须 present_files，不能只做路径告知**？
