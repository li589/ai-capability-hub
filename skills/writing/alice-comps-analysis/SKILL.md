---
name: alice-comps-analysis
description: 构建机构级可比公司分析，以 Excel 表格 + 文字报告输出，覆盖经营指标、估值倍数（PE/PB/EV/EBITDA
  等）对比及统计基准分析。适用于可比公司分析、相对估值、行业估值对标、并购/IPO 定价等场景。
description_zh: 万得 Alice 可比公司分析 CLI：构建机构级可比公司分析，覆盖经营指标、估值倍数对比及统计基准分析，输出 Excel 表格 + 文字分析报告。
description_en: Wind Alice Comps Analysis CLI — build institutional-grade
  Comparable Companies Analysis, delivered in Excel workbook and written
  analytical report, covering operating metrics, valuation multiple comparisons,
  and statistical benchmark analysis across peer companies.
version: 1.0.3
author: WindAlice
tags:
  - comps
  ...
  - wind-alice
display_name: 可比公司分析
display_name_en: Comparable Companies Analysis
visibility: public
disable-model-invocation: true
---

# Comps Analysis（万得AI-可比公司分析）

## 技能介绍

### 官方技能：万得AI-可比公司分析

帮你快速构建机构级可比公司分析表（Comps Analysis），自动筛选同行业可比公司，提取关键运营指标、估值倍数（PE、PB、EV/EBITDA 等），并进行统计基准对比分析，输出专业的 Excel 分析表格。

适用于投资分析、并购估值、IPO 定价、行业研究、投资组合评估等场景。支持 A 股、港股、美股等全球主要市场的上市公司。

### 适用场景

| 场景               | 解决的问题                                           |
| ------------------ | ---------------------------------------------------- |
| 投资分析           | 评估标的相对同行的估值水平，辅助买卖判断             |
| 并购估值           | 以可比公司估值倍数为基准，支撑交易定价               |
| IPO 定价           | 参考同行估值区间，为新股发行定价提供依据             |
| 行业研究           | 对比同行业公司的经营效率与估值水平                   |
| 投资组合评估       | 检查组合中标的相对行业的估值偏离度                   |
| 估值对标分析       | 量化标的与同行在 PE/PB/EV/EBITDA 等倍数上的差异      |

### 核心能力

1. **可比公司筛选**
   - 基于行业分类、主营业务、市值规模等维度自动筛选可比公司
   - 支持用户指定可比公司范围
   - 覆盖 A 股、港股、美股等全球主要市场
...
- "比较腾讯和同行业公司的盈利能力"
- "做一个医药行业的估值对标分析"
- "帮我做一份贵州茅台的可比公司分析，重点看 PE 和 EV/EBITDA"
- "比亚迪的估值相对同行是高还是低？"

### 输出形式

- **Agent 向用户展示的内容**：CLI stdout 中的 `agentResult.value` 正文（服务端流式返回）
- 自动生成的结构化内容包含：**核心结论、估值定位、经营指标对比、估值倍数对比、统计基准**
- 输出 Excel 表格 + 文字分析报告，Excel 含可比公司经营指标与估值倍数对比表
...
```powershell
# ✅ 方案 A（唯一推荐）：acomps.ps1 绝对路径，无需 cd，自带 UTF-8 代码页
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\acomps.ps1" --prompt "<USER_QUESTION>" --no-wait -d "<WORKSPACE_DIR>"
```
...
```bash
node /path/to/alice-comps-analysis/scripts/cli.mjs --prompt "<USER_QUESTION>" --no-wait -d "<WORKSPACE_DIR>"
```
...
| 信号 | 含义 / Agent 必须怎么做 |
|------|------------------------|
| stdout 含 `ALICE_COMPS_ANALYSIS_DONE taskId=... promptHash=... reportFile=... reportFullFile=... attachmentFile=...` | **唯一**真正完成信号；**必须先核对 `promptHash=` 与本次 `--prompt` 一致**，再以 stdout 中的 `agentResult.value` 原文交付用户（stdout 被截断时读 `reportFile=` 兜底）；**若有 `reportFullFile=` 必须把绝对路径转告用户；若有 `attachmentFile=` 也必须把绝对路径转告用户** |
...
| 退出码 `78`（环境受限，无法保存任务状态） | 当前环境无法保存任务状态（未开启完全访问权限）；**未向服务端发请求**；告知用户「当前环境无法运行 可比公司分析，请在工具中开启完全访问权限后重试」；禁止换 prompt / `--new` / 反复重试 |
...
5. **交付 agentResult.value + 告知报告路径**——将 CLI stdout 中的 `agentResult.value` **原文**交给用户；**禁止**自行总结、摘录、重写表格或「用自己的话」复述。若 stdout 被沙箱截断，读取 DONE 行 `reportFile=` 落盘正文（跳过 HTML 注释头）。**DONE 含 `reportFullFile=` 时必须告知完整路径**（**保留 CLI 内联的 markdown 链接格式**，如 `[可比公司分析报告.md](file:///<工作空间>/可比公司分析报告.md) (<工作空间>\可比公司分析报告.md)`）——CLI 已把链接内联到 `agentResult.value` 正文中，**原样输出即可**；**禁止**把链接改成行内代码 `` `C:\...\报告.md` ``，行内代码在 Cursor / Trae 等宿主里**不可点击**，用户无法直接打开文件。**DONE 含 `attachmentFile=` 时也必须保留 markdown 链接格式告知完整路径**（如 `[财务模型.xlsx](file:///<工作空间>/财务模型.xlsx) (<工作空间>\财务模型.xlsx)`）。**禁止加载 `reportFullFile=` / `attachmentFile=` 文件内容展示给用户**——`agentResult.value` 已是面向用户的核心分析摘要，完整报告和数据附件供用户本地查阅，不需要 Agent 读取并展示。 **交付 = Agent 亲自打字复述正文**（不折叠），不能只跑完 CLI 让 stdout 显示。**若 DONE 含 `reportFullFile=` 和/或 `attachmentFile=`（含 xlsx 数据表），先在独立一条消息调 `present_files` 呈现每个附件为可点击卡片（文件须在工作空间内，故须先 cp）；随后在另一条纯文本消息里输出 `agentResult.value`（本条禁任何工具调用，否则整条被折叠）**。
...
```powershell
# Windows（方案 A）
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\acomps.ps1" apikey-set <KEY>
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\acomps.ps1" apikey-get
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\acomps.ps1" apikey-clear
```
...
3. **无 Key** → 向用户说「使用可比公司分析需要配置一个 Key，你有 Wind Alice 的 API Key 吗？」→ 用户回复后 → 执行 `apikey-set <KEY>` → `memory_write(file="agent_md", content="增加/更新以下记忆：Wind Alice API Key = <KEY>")` → 继续后续步骤。
...
| 场景 | 发生了什么 |
|------|-----------|
| 首次使用 | Agent：「使用可比公司分析需要配置一个 Key，你有 Wind Alice 的 API Key 吗？」→ 用户回复 → Key 保存到 `agent_md` → 自动配置 → 开始分析 |
...
> **Windows Agent 一律用方案 A**：`acomps.ps1` 写绝对路径，**无需 `cd`**。macOS / Linux 用 `node "<SKILL_DIR>/scripts/cli.mjs"`（绝对路径，无需 `cd`）。
...
```powershell
# ✅ 方案 A（唯一写法）
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\acomps.ps1" --prompt "帮我做一份宁德时代的可比公司分析？比亚迪有没有短线机会？" --no-wait -d "<WORKSPACE_DIR>"
```
...
1. **`scripts/acomps.ps1`**：调用前切 UTF-8 代码页（Windows Agent **优先用它**代替裸 `node`）。
...
```powershell
# Windows（方案 A）
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\acomps.ps1" --prompt "<USER_QUESTION>" --no-wait -d "<WORKSPACE_DIR>" [--new]
```
...
```powershell
# Windows（方案 A）
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\acomps.ps1" -p "帮我做一份宁德时代的可比公司分析？比亚迪有没有短线机会？" --no-wait -d "<WORKSPACE_DIR>"
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\acomps.ps1" -p "贵州茅台在同行业中估值水平如何？PE 倍数偏高还是偏低？" --no-wait -d "<WORKSPACE_DIR>"
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\acomps.ps1" -p "比亚迪的可比公司有哪些？各家的 PE/PB 倍数？" --no-wait -d "D:\reports\comps"
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\acomps.ps1" -p "重新做宁德时代的可比公司分析" --no-wait --new   # 仅用户说"重新跑"才加
```

```bash
# macOS / Linux
node "<SKILL_DIR>/scripts/cli.mjs" -p "帮我做一份宁德时代的可比公司分析？比亚迪有没有短线机会？" --no-wait -d "<WORKSPACE_DIR>"
node "<SKILL_DIR>/scripts/cli.mjs" -p "贵州茅台在同行业中估值水平如何？PE 倍数偏高还是偏低？" --no-wait -d "<WORKSPACE_DIR>"
node "<SKILL_DIR>/scripts/cli.mjs" -p "比亚迪的可比公司有哪些？各家的 PE/PB 倍数？" --no-wait -d "/reports/comps"
node "<SKILL_DIR>/scripts/cli.mjs" -p "重新做宁德时代的可比公司分析" --no-wait --new
```
...
可比公司分析支持多轮对话——同一话题下的连续追问，服务端可继承前一轮的分析上下文。CLI **默认在工作区内自动复用**上次 `contextId`（30 分钟空闲窗口内），**无需手动传 `--context-id`**：
...
2. **若已保存**：提取 Key，执行 `powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\acomps.ps1" apikey-set <KEY>`（macOS/Linux 改用 `node "<SKILL_DIR>/scripts/cli.mjs" apikey-set <KEY>`），**静默完成，不对用户提及**；
3. **若未保存**：向用户说「使用可比公司分析需要配置一个 Key，你有 Wind Alice 的 API Key 吗？」→ 用户回复后 → 执行 `apikey-set <KEY>` → 调用 `memory_write(file="agent_md", content="增加/更新以下记忆：Wind Alice API Key = <KEY>")` 持久化 → 继续。
...
用户问题只要涉及**可比公司分析、估值定位、PE/PB/EV 倍数、同业对比、估值溢价/折价、行业排名**，即应调用本技能。

| 场景 | 典型用户表达 |
|------|-------------|
| 可比公司分析 | 帮我做一份宁德时代的可比公司分析、这家公司的可比公司有哪些 |
| 估值定位 | 在同行业中估值偏高还是偏低、相对同行如何 |
| PE/PB/EV 倍数 | PE 倍数是多少、PB 水平如何、EV/EBITDA 对比 |
| 同业对比 | 和竞争对手相比怎么样、同行业龙头对比 |
| 估值溢价/折价 | 相对同行是溢价还是折价、为什么估值更高 |
| 行业排名 | 在行业中排名第几、市场份额位置 |

不应调用本技能的场景：纯债券利率研判、纯商品期货分析、基金筛选、与可比公司估值无关的通用写作等；除非用户明确要求可比公司分析。

### 2. 构造 prompt

把研究对象和关注维度拼成一句自然语言。**不要加 `使用「可比公司分析」技能：` 前缀，CLI 内部已自动注入。**
...
可比公司分析分析通常 **2-15 分钟**。**调用前必须用一句话告诉用户**这个耗时，并**禁止**中途取消或重复发起。

推荐话术：「好的，我来帮你分析。可比公司分析通常需要 2–15 分钟，请稍等。」
...
```powershell
# Windows（方案 A）
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\acomps.ps1" --prompt "<USER_QUESTION>" --no-wait [-d "<DIR>"]
```
...
**为何像「卡住」**：可比公司分析要 2–15 分钟，而 Trae/Cursor `run_command` 常只有 60–120s 超时。Agent 若用 `check_command_status` 轮询而非阻塞等待，终端会先杀 CLI（exit `4`/`6`），服务端任务却仍在跑——`results/` 尚未生成，Agent 误判为卡住。此时对用户说：「分析仍在进行中，我继续等待……」，**不要**向用户提及超时、被杀等技术细节。
...
```powershell
powershell ... acomps.ps1 --prompt "<USER_QUESTION>" --detach
powershell ... acomps.ps1 --prompt "<和首次完全一致的 prompt>" --no-wait -d "<WORKSPACE_DIR>"
```
...
```powershell
# Windows（方案 A）
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\acomps.ps1" --prompt "<和首次完全一致的 prompt>" --no-wait -d "<WORKSPACE_DIR>"
```
...
2. 若**是**：
   ...
   - 若不涉及 `--context-id`，从 `agentResult.value` 正文中提取标的/同行关键词（如「宁德时代」「比亚迪」「PE 倍数」「估值定位」），与当前会话上下文对比：
     - **主体明显不同**（跨品种/跨视角，如缓存的「帮我做一份宁德时代的可比公司分析」vs 请求的「帮我分析宁德时代信用」；或缓存的「宁德时代」vs 请求的「螺纹钢期货情绪」；主体完全不同、无任何字面重叠）→ **静默自动重跑**：用**原 prompt** 加 `--new --no-wait` 重新执行（将消耗积分）。**不向用户提及缓存/主体不匹配/重跑等任何技术细节**——用户不需要知道。重跑后按正常流程（步骤 8）交付。
     - **主体一致或高度相似**（同品种不同写法，如「宁德时代」vs「CATL」；同一公司话题的追问）→ 停下来告知用户：「该问题已有最近的分析结果。」然后统一列出选项：
...
> **顺序**：① CLI DONE + 并行完成所有内存写入 -> ②（若 `reportFullFile=` / `attachmentFile=`）在**独立消息**调 `present_files`（**本回合只能有 present_files 一个工具调用（禁 Bash/Read/Edit/Write/文件操作）**--否则卡片被折叠隐藏；这条折叠成卡片可接受；**含 xlsx**）-> ③ 在**纯文本消息**里输出 `agentResult.value`（**本条禁任何工具调用**）。两者落在同一条消息会被整条折叠--这是结果被折叠的真正根因。
...
| **必须 present_files** | `reportFullFile=` / `REPORT_FULL_FILE=` -> `工作空间/*.md` | CLI 静默下载的完整 Markdown 附件 | **present_files**；路径已内联在正文中 |
| **必须 present_files** | `attachmentFile=` -> `工作空间/*.xlsx` 等 | CLI 静默下载的数据附件（Excel 等） | **present_files**；路径已内联在正文中 |
| `ALICE_ARTIFACT_GUARD` 等 | stdout 警告 | 防误读提示 | 不是可交付正文；阻塞等到 DONE |

**正确流程（成功场景）**：

1. stdout 出现 `ALICE_COMPS_ANALYSIS_DONE` 且 `promptHash=` 与本次 `PROMPT_HASH=` 一致；
...
#### 8.1 用 `present_files` 把附件呈现为可点击卡片（DONE 含 `reportFullFile=` 和/或 `attachmentFile=` 时必做）

可比公司分析服务端常返回 **Markdown 报告**（`reportFullFile=`）和 **Excel 数据表**（`attachmentFile=`，如 `.xlsx`）。DONE 行含任一路径时，在**独立一条消息**里调用宿主 `present_files` 工具把**每个**附件呈现为可点击卡片--**必须在输出 `agentResult.value` 正文的那条消息之前**单独调用（见下方顺序）。CLI 会在 DONE 前打印 `ALICE_PRESENT_FILES=1 path=...` 逐行列出待呈现文件。
...
- **顺序**：① 静默跑完主调用 + **同一回合内完成所有内存/日志写入** -> ②（若有附件）检查路径 + 必要时复制到工作空间 -> ③ 在**独立消息**调 `present_files`（传工作空间内的路径；**本回合只能有 present_files 一个工具调用（禁 Bash/Read/Edit/Write/文件操作）**--否则卡片被折叠隐藏，用户看不到 present_file；这条可折叠成卡片）-> ④ 在**纯文本消息**里输出 `agentResult.value`（**本条禁任何工具调用**，否则整条被折叠）。旧版「正文输出之后再调 present_files」会让两者落在同一条消息、整条被折叠--已废弃。
- **`present_files` ≠ 加载文件内容**：上面的「禁止加载文件内容展示给用户」指不要把附件正文贴进文本回复；`present_files` 只是给文件一个可点击卡片入口，不把内容塞进正文，两者不冲突，该调用照常。
- **xlsx 也必须 present_files**：`attachmentFile=` 的数据表（Excel）与 `reportFullFile=` 的报告同等对待，**禁止**只做路径告知而跳过 `present_files`。
- 若 DONE **没有** `reportFullFile=` 且**没有** `attachmentFile=`（服务端未返回附件），**不要**调 `present_files`，只交付 `agentResult.value` 正文即可。
...
**CLI 失败时，立即停止；绝不用 WebSearch 或其它信息源拿"近似的可比公司分析"敷衍用户。**
...
- 仍失败：向用户展示完整错误，让用户处理；**绝不**回退到其它信息源伪造"看似合理"的可比公司分析。
...
A：(a) 可以——把多家主体写进同一个 prompt（建议 ≤ 5 家）。(b) 完整报告与 xlsx 数据表会静默落到当前工作空间（`process.cwd()`）；Agent **不要**把附件正文展示给用户，DONE 含 `reportFullFile=` / `attachmentFile=` 时**必须用 present_files 呈现**。(c) `-d` 保留兼容但**不影响下载目录**，附件统一落工作空间。**向用户交付时只用 stdout 的 `agentResult.value` 原文**（截断时读 `reportFile=` 兜底），禁止读附件正文展示。

**Q3：PowerShell 报 `&& 不是有效语句分隔符` / 中文乱码？**

A：(a) PowerShell 5.x 不支持 `&&`——Windows **只能用方案 A**（`acomps.ps1` 绝对路径，见「调用红线」）。(b) 若 live 仍乱码，读 CLI 输出的 `ALICE_SESSION_LOG=` 路径（`Get-Content -Encoding UTF8`），或直接用 Cursor / VS Code 打开 `.log` / `.md` 文件。
...
A：**不能。** `(1)` / `(2)` 后缀只表示同名冲突自动重命名，不代表「本次 prompt 的第 N 次运行」。若 `status -p "<prompt>"` 返回 `ALICE_ARTIFACT_GUARD` 或列出 `SIMILAR_COMPLETED_TASK` / `ORPHAN_DOWNLOAD_CANDIDATE` / `STALE_REPORT_CANDIDATE`，说明本地没有本次 prompt 的登记记录，或存在其它 prompt / 其它会话留下的同名主体文件。**必须**阻塞等待 `ALICE_COMPS_ANALYSIS_DONE`（含匹配的 `promptHash=`），禁止用 `view_files` 扫目录猜报告。
...
3. 旧 log 末尾有 `ALICE_COMPS_ANALYSIS_DONE`，Agent 未核对 `promptHash=` 与本次 `PROMPT_HASH=`；
4. 再 `view_files` 打开 `download/海思科信用分析报告_20260623.md`——磁盘上仍是旧附件；而 `tasks.json` 里本次 `taskId=019ef374-...` 仍是 **running**。

**正确做法**：阻塞等待 CLI 进程结束；stdout 必须含 `ALICE_COMPS_ANALYSIS_DONE` 且 `promptHash=` 与本次 `PROMPT_HASH=` **完全一致**，再交付 stdout 中的 `agentResult.value` 原文。若进程被沙箱 kill（exit `4`/`6`），用**完全相同 prompt** 再发**一条** `--no-wait` 续接——**不要**读 logs/、download/、`tasks.json` 猜正文。
...
1. **没用 `acomps.ps1` 方案 A**——`cd && node` / `Set-Location; node` 在 PowerShell 5.x 易失败；命令末尾拼 `undefined` 是 Agent 工具 bug。
...
**正确做法（单 Agent）**：**一条** `acomps.ps1 ... --no-wait` 阻塞等待 → stdout `ALICE_COMPS_ANALYSIS_DONE` 且 `promptHash=` 一致 → 交付 `agentResult.value` **原文**，禁止展示 `download/` 附件。

**Q15：永辉超市复盘——为什么 Agent 交付的 PD / 评级与真实报告不一致？**

A：Agent 违规组合：(1) 连发多条 `--no-wait` 且用 `check_command_status` 轮询，未阻塞等到 DONE；(2) 读 `results/` 或 `download/` 附件后**自行概括**成表格——定量指标会被改错。**正确做法**：阻塞等到 `ALICE_COMPS_ANALYSIS_DONE` → 将 stdout 的 `agentResult.value` **原文**交给用户（截断时读 `reportFile=` 兜底）→ 禁止概括 → DONE 含 `reportFullFile=` / `attachmentFile=` 时**必须 present_files**（含 xlsx），禁止自行概括改写。
...
A：CLI 会先在沙箱内自下载附件（通常成功，文件已就绪）。仅当沙箱内下载失败时，stdout 才输出 `ALICE_EXTERNAL_DOWNLOAD url=... target=...` 行，宿主 Agent 负责用 `curl -k` 在沙箱外下载兜底（见 AGENT.md 步骤 4.5）。若 Agent 侧 curl 也失败，`agentResult.value` 的核心内容已完整交付，附件是其 Markdown/Excel 存档副本。

## Agent 执行摘要（Skill 注入后**最先读**，再动任何工具）

| 顺序 | 做什么 | 禁止 |
|------|--------|------|
| 0 | 读同目录 **[`AGENT.md`](./AGENT.md)** | 禁止 `search_codebase`、禁止查库猜可比公司估值/财务数据 |
| 1 | **一条** `acomps.ps1 --prompt "<用户原话>" --no-wait -d "<WORKSPACE_DIR>"` [--context-id <上一轮contextId>] [--new-session] | 禁止裸 `node`、禁止 `cd` 到 skill 目录 |
| 2 | `run_command` / Shell **超时 ≥ 1200 秒**；**阻塞等到命令退出** | 禁止秒退后说「后台 PID / 稍后再问继续查」 |
| 3 | stdout 含 `ALICE_COMPS_ANALYSIS_DONE` | 无 DONE = 未完成，禁止编造 |
| 4 | **逐字**交付 `agentResult.value`；DONE 含 `reportFullFile=` / `attachmentFile=` 时先 `present_files` 再交付正文 | 禁止自行概括、禁止读附件正文 |
...
通过 `alice-comps-analysis` 调用万得 Alice Agent 的「可比公司分析」专业技能，对上市公司做可比公司估值对比（PE/PB/EV 倍数、同业排名、估值溢价/折价）。

> **核心原则**：基础设施交给脚本处理；Agent 把用户问题拼成一句自然语言作为 `--prompt` 传入（**不要**加 `使用「可比公司分析」技能：` 前缀，CLI 内部已自动注入）。任务完成后，**只将 CLI stdout 中的 `agentResult.value` 原文原样交给用户**；CLI 静默下载的附件（`reportFullFile=`、`attachmentFile=`）**必须用 `present_files` 呈现，禁止加载文件内容展示给用户**。 **关键（避免结果被折叠）**：`agentResult.value` 必须由 Agent **作为自己的文本回复正文逐字打出来**（`type=text` 才不折叠）。**若 DONE 含 `reportFullFile=` 和/或 `attachmentFile=`（含 xlsx），先在独立一条消息里调 `present_files`；随后在另一条纯文本消息里逐字输出 `agentResult.value`（本条禁任何工具调用）**。
>
...
> **机制（默认自动续接——防串号）**:CLI **默认在工作区内自动复用**上次 `contextId`（30 分钟空闲窗口内），**无需手动传 `--context-id`**。会话状态落在 `<工作区>/.wind-alice/current-session.json`（工作区 = `process.cwd()`），**同一工作区 = 同一会话文件**，不同工作区 / 不同宿主各自独立文件，**物理上不串号**。
>
> **什么时候要你动手**：只有**切到完全无关的新话题**时，加 `--new-session` 强制开新会话，避免无关上文污染。同话题追问（哪怕换标的、加维度）**什么都不用加**，自动续接。
>
> **常见场景**:用户先问「帮我做一份宁德时代的可比公司分析」,你交付后用户问「**那比亚迪呢**」--同话题延伸追问,**什么都不加**,自动续接。「**那 PE 倍数呢 / 估值定位怎么看**」也**什么都不加**。「**宁德时代信用怎么样**」是无关新主题,加 `--new-session`。
>
> **`--context-id` 仍可用但通常不需要**：显式传 `--context-id <id>`（从上一轮 DONE 行取）优先级最高，用于跨工作区 / 精确指定续接；同工作区内同话题续接已由自动机制覆盖。旧的 `--continue-session` / `--session-scope` 已废弃（no-op），可不传。
>
> **判断标准--默认续接,宁滥勿缺**:拿不准时**倾向不加 `--new-session`**（即自动续接）——续接错代价小,误切新会话代价大(丢上下文、答非所问)。只有完全无关的新主题才加 `--new-session`。
>
> 详见 `AGENT.md`「会话续接判断」专节。

## 何时优先调用本技能

当用户问题涉及**可比公司分析、估值定位、PE/PB/EV 倍数、同业对比、估值溢价/折价、行业排名**等场景时，应调用本技能。不应在纯债券利率研判、与可比公司估值无关的宏观问题等场景硬套本技能。
