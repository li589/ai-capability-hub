...
description: 生成横向翻页网页 PPT（单 HTML 文件），含 WebGL 背景、章节幕封、数据大字报、图片网格等模板。提供两种风格：①
  "电子杂志 × 电子墨水"（衬线 + 流体背景 + 暖色） ② "瑞士国际主义"（无衬线 + 网格点阵 +
  IKB/柠檬黄/柠檬绿/安全橙高亮）。当用户需要制作分享 / 演讲 / 发布会风格的网页 PPT，或提到"杂志风 PPT"、"瑞士风 PPT"、"Swiss
  Style"、"horizontal swipe deck"时使用。
description_zh: 歸藏的 PPT Skills，生成高质量网页 PPT、演讲 deck 与多比例封面，内置电子杂志风和瑞士国际主义两套视觉系统。
description_en: 歸藏's PPT Skills — Generate polished single-file HTML slide decks
  and cover pages with editorial magazine and Swiss international design
  systems.
version: 1.0.0
...
permission_notes_zh: 本 Skill 会在用户指定项目目录复制模板、创建 images 目录并写入 HTML
  文件;模板预览时可能加载公开字体、图标或动效 CDN,如需离线可替换为本地资源。
display_name: Magazine Web PPT
display_name_en: Guizang Ppt Skill
visibility: public
icon: https://codebuddy-platform-1258344699.cos.accelerate.myqcloud.com/public/45edac6b-2078-4678-89f3-6f9800cf5e5f/avatar/skill/au_61080f8f-a7f.png
...
- 用普通对话直接询问用户,不要依赖任何特定开发工具的专用问答机制。
- 一次最多问 1-3 个最关键问题;如果信息缺口不影响开工,先做合理假设并在回复里说明。
- 需要用户提供素材时,直接说明文件应放在哪里、如何命名、推荐比例和替换方式。
...
| 用户需要信息图 / 截图 / 证据墙 / 系统关系图等视觉素材承载 | B 也很合适(S22 或 S15/S16 的图片槽位更稳定) |
...
本 WorkBuddy 版本不包含图片生成提示词,也不要求 AI 调用图像模型。图片只作为用户提供或项目已有的本地素材插入 deck。

处理图片时遵守:

- 先确定图片落位和比例,再选择版式;不要先把任意图片硬塞进页面
- 图片风格必须贴合当前 deck 风格:风格 A 用"电子杂志 × 电子墨水";风格 B 用"瑞士国际主义 / Swiss Style"
- 信息图、图表、截图里的文字语言必须跟随用户正在使用的语言;中文 deck 用中文,英文 deck 用英文
- 先看 `references/image-assets.md` 选择图片类型、比例和落位策略
- 图片比例必须匹配最终落位:主视觉 16:9,左文右图 16:10 / 4:3,信息图 16:9 / 16:10,截图 16:10,图文混排小图 3:2 / 3:4,网格图统一高度裁切
- 图片放到 `images/` 下,命名遵守 `{页号}-{语义}.{ext}`
...
- 插入图片前先写 `data-image-slot`:例如 `s22-hero-21x9` / `s15-grid-21x9` / `s16-brief-21x9`
- S22 配图优先使用 21:9 素材,关键主体必须落在中央安全区;照片容器用 `object-position:center 35%`,不要用 `top center`
- 图片容器必须直角、无阴影、无圆角;默认背景用白色 `var(--paper)`,不要用灰底包白底信息图
- 白底信息图/流程图/UI 图默认不要加外框描边,不要随手套 `.swiss-keyline`;需要强调时只用 `.swiss-lined` 的顶部 accent 线
- UI/信息图如果是用户原始截图或文字密集图,才用 `.fit-contain`;如果已按 S15/S16 槽位准备成标准比例素材,必须用 `.frame-img.r-21x9` / `.frame-img.r-16x10` 铺满容器,不要固定 `height:18vh` 后把图缩小
- 多图同组必须统一图片槽位、比例和高度,不能混用
- 图片素材使用 `image-assets.md` 的"风格 B:瑞士国际主义图片素材规则"
...
└── references/
    ...
    ├── image-assets.md       ← 图片素材类型、比例和落位策略(不含生成提示词)
...
5. 如果 deck 使用图片素材,读 `image-assets.md` 挑图片类型、比例和落位策略
