...
displayName: 说人话改写
version: 1.0.0
summary: “检查和清理中英文文本里的 AI 套路，适用于”去 AI 味””说人话””自然一点””别像模板””先标问题”这类改写和审稿需求；按场景控制力度，同时保留事实、术语、语域和责任主体。”
license: MIT
name: shuorenhua
...
pricing:
  ...
  amount_fen: 5
pay_gateway:
  invoke_url: https://pay-skill-nat-4198535-1341112664.ap-shanghai.run.tcloudbase.com/api/skill/invoke
  skill_id: shuorenhua
  skill_version: 1.0.0
---
...
> **本 Skill 为 Pay Skill**：每次改写需先完成微信支付验证，验证通过后按下方规则执行文本改写。
...
向 Pay Skill 网关发起 POST 请求（`invoke_url` 见 frontmatter）：

```http
POST https://pay-skill-nat-4198535-1341112664.ap-shanghai.run.tcloudbase.com/api/skill/invoke
Content-Type: application/json

{
  "query": "用户的原始改写需求（如：帮我把这段话改得自然一点）",
  "skill_id": "shuorenhua",
  "skill_version": "1.0.0",
  "amount_fen": 5
}
...
若收到 HTTP 402，从响应中提取并**保存**以下信息（Header 与 Body 均可能出现）：

- `WeixinPay-Required`：支付码（payment_code）
- `X-Out-Trade-No`：商户订单号

将 `WeixinPay-Required` 的值作为 `paymentCode`，调用 `微信支付官方技能skill（weixinpay-pay），若遇系统繁忙等支付失败，调用mcp__weixinpay__weixinpay_retry_pay 把 paymentCode 回传给系统拉起卡片` 向用户申请支付授权。

### 第三步：支付成功后重新请求（⚠️ 必须执行，不可跳过）

支付成功后，**必须**用与第一步**完全一致**的 JSON body 重新 POST 同一接口，并在 Header 中携带第二步保存的值：
...
### 第四步：确认 access_granted 后执行改写

仅当第三步返回 HTTP 200 且 `"access_granted": true` 时，才继续按下方规则执行文本改写。

若返回 `"code": "NOT_PAID"`，说明支付尚未完成，等待后重试第三步。
若返回 `"code": "REFUNDED"`，向用户说明："抱歉，该服务暂时无法提供内容，已自动退款至原支付账户。"并终止流程，**不要**再次尝试支付或重新请求。
若返回 `"code": "FULFILL_AND_REFUND_FAILED"`，说明服务异常且退款失败，建议用户联系客服。
...
- 必须先通过「付费前置检查」确认 weixinpay 插件已安装，否则不要进入支付流程
- 收到 402 响应时，支付由 weixinpay 插件完成，Skill 无需关心支付细节
- 支付成功后必须主动发起重试请求，body 保持不变，通过 Header `X-Out-Trade-No` 传递订单号
- 如果返回 “NOT_PAID”，说明支付尚未完成，请等待后重试
- 如果返回 “REFUNDED”，说明服务异常已退款，告知用户并终止流程
- 如果返回 “FULFILL_AND_REFUND_FAILED”，说明服务异常且退款失败，建议用户联系客服
