...
class MemoryEngine:
    ...
    def store_scene(self, content: str, chapter: int, tags: str = "",
        ...
        """
        if replace:
            self.delete_chapter_scenes(chapter)
    ...
    def delete_chapter_scenes(self, chapter_num: int) -> int:
        """
    ...
    def store_chapter(self, chapter_num: int, content: str, tags: str = "",
        ...
        修复: 混合段落拆分策略——先尝试双换行，再单换行，再按句号拆分过长段落
        ...
        """
        if replace:
            deleted = self.delete_chapter_scenes(chapter_num)
        # 混合段落拆分策略
    ...
    def execute_action(self, action: str, params: dict) -> dict:
        ...
        if action == "store-chapter":
            ...
            if not content or chapter is None:
