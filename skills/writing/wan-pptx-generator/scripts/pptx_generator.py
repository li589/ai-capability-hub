#!/usr/bin/env python3
"""
PPTX Generator - 使用 DashScope API 生成 PowerPoint 图片
"""

import os
import sys
import re
import json
import argparse
import urllib.request
from http import HTTPStatus
from pathlib import Path
import dashscope
from openai import OpenAI
from datetime import datetime
import requests
# 强制无缓冲输出
os.environ['PYTHONUNBUFFERED'] = '1'
sys.stdout = os.fdopen(sys.stdout.fileno(), 'w', buffering=1)
sys.stderr = os.fdopen(sys.stderr.fileno(), 'w', buffering=1)

# 日志文件路径 - 使用脚本所在目录
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
LOG_FILE = os.path.join(SCRIPT_DIR, "pptx_generator.log")

# 输出目录（可通过 --output-dir 参数覆盖）
OUTPUT_DIR = None


def log_print(*args, **kwargs):
    """同时输出到控制台和日志文件的 print 函数"""
    message = " ".join(str(arg) for arg in args)
    print(message, **kwargs)
    sys.stdout.flush()
    sys.stderr.flush()
    try:
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        log_line = f"[{timestamp}] {message}\n"
        with open(LOG_FILE, 'a', encoding='utf-8') as f:
            f.write(log_line)
            f.flush()
            os.fsync(f.fileno())
    except Exception as e:
        print(f"[Log Error] {e}", file=sys.stderr)


def clear_log():
    """清空日志文件"""
    try:
        with open(LOG_FILE, 'w', encoding='utf-8') as f:
            f.write("")
            f.flush()
            os.fsync(f.fileno())
    except Exception as e:
        print(f"[Clear Log Error] {e}", file=sys.stderr)


def save_outline(outline_text, output_dir):
    """收到大纲后立即保存为 content.md"""
    try:
        filepath = os.path.join(output_dir, "content.md")
        with open(filepath, 'w', encoding='utf-8') as f:
            f.write("## PPT 大纲\n\n")
            f.write(outline_text)
            f.flush()
            os.fsync(f.fileno())
        log_print(f"📝 大纲已保存: {filepath}")
    except Exception as e:
        log_print(f"❌ 保存大纲失败: {e}")


def download_image(page_num, url, output_dir):
    """下载单张图片并保存为 {page_num}.jpg"""
    try:
        filepath = os.path.join(output_dir, f"{page_num}.jpg")
        log_print(f"⬇️  正在下载第 {page_num} 页图片...")
        urllib.request.urlretrieve(url, filepath)
        file_size = os.path.getsize(filepath)
        log_print(f"✅ 第 {page_num} 页已保存: {filepath} ({file_size / 1024:.1f} KB)")
    except Exception as e:
        log_print(f"❌ 下载第 {page_num} 页图片失败: {e}")


def download_pptx(url, output_dir, name="presentation"):
    """下载最终的 PPTX 文件"""
    try:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filepath = os.path.join(output_dir, f"{name}_{timestamp}.pptx")
        log_print(f"⬇️  正在下载 PPTX 文件...")
        urllib.request.urlretrieve(url, filepath)
        file_size = os.path.getsize(filepath)
        log_print(f"✅ PPTX 已保存: {filepath} ({file_size / 1024:.1f} KB)")
        return filepath
    except Exception as e:
        log_print(f"❌ 下载 PPTX 失败: {e}")
        return None


def should_upload_file(file_path):
    """判断文件是否需要上传到 DashScope 进行智能解析"""
    upload_extensions = {
        '.doc', '.docx', '.pdf', '.xls', '.xlsx', '.ppt', '.pptx',
        '.jpg', '.jpeg', '.png', '.gif', '.bmp', '.txt'
    }
    text_extensions = {'.md', '.json', '.csv', '.py', '.js', '.html', '.xml'}
    
    file_ext = os.path.splitext(file_path)[1].lower()
    
    if file_ext in upload_extensions:
        return True
    elif file_ext in text_extensions:
        return False
    else:
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                f.read(100)
            return False
        except:
            return True


def read_file_content(file_path: str):
    """读取文件内容"""
    try:
        log_print(f"📖 正在读取文件内容: {file_path}")
        with open(file_path, 'r', encoding='utf-8') as f:
            content = f.read()
        log_print(f"✅ 文件读取成功，内容长度: {len(content)} 字符")
        return content
    except Exception as e:
        log_print(f"❌ 文件读取失败: {e}")
        return None


def get_upload_policy(api_key, model_name):
    """获取文件上传凭证"""
    url = "https://dashscope.aliyuncs.com/api/v1/uploads"
    headers = {
        "Authorization": f"Bearer {api_key}",
        "Content-Type": "application/json"
    }
    params = {
        "action": "getPolicy",
        "model": model_name
    }
    
    response = requests.get(url, headers=headers, params=params)
    if response.status_code != 200:
        raise Exception(f"Failed to get upload policy: {response.text}")
    
    return response.json()['data']

def upload_file_to_oss(policy_data, file_path):
    """将文件上传到临时存储OSS"""
    file_name = Path(file_path).name
    key = f"{policy_data['upload_dir']}/{file_name}"
    
    with open(file_path, 'rb') as file:
        files = {
            'OSSAccessKeyId': (None, policy_data['oss_access_key_id']),
            'Signature': (None, policy_data['signature']),
            'policy': (None, policy_data['policy']),
            'x-oss-object-acl': (None, policy_data['x_oss_object_acl']),
            'x-oss-forbid-overwrite': (None, policy_data['x_oss_forbid_overwrite']),
            'key': (None, key),
            'success_action_status': (None, '200'),
            'file': (file_name, file)
        }
        
        response = requests.post(policy_data['upload_host'], files=files)
        if response.status_code != 200:
            raise Exception(f"Failed to upload file: {response.text}")
    
    return f"oss://{key}"

def upload_file_and_get_url(api_key, file_path):
    """上传文件并获取URL"""
    # 1. 获取上传凭证，上传凭证接口有限流，超出限流将导致请求失败
    policy_data = get_upload_policy(api_key, "qwen-doc-turbo") 
    # 2. 上传文件到OSS
    oss_url = upload_file_to_oss(policy_data, file_path)
    
    return oss_url



def generate_ppt(content=None, query=None, mode="creative", api_key=None, file_path=None, output_dir=None):
    """
    生成 PPT 图片，实时保存大纲和图片到 output_dir
    """
    dashscope.base_http_api_url = "https://dashscope.aliyuncs.com/api/v1"
    
    if not api_key:
        api_key = os.environ.get("DASHSCOPE_API_KEY")
    if not api_key:
        return {"success": False, "error": "API key not provided."}
    
    # 处理文件输入
    file_url = None
    file_content = None
    
    if file_path:
        log_print(f"📁 检测到文件输入: {file_path}")
        if should_upload_file(file_path):
            file_url = upload_file_and_get_url(api_key, file_path)
            if not file_url:
                return {"success": False, "error": f"文件上传失败: {file_path}"}
        else:
            file_content = read_file_content(file_path)
            if not file_content:
                return {"success": False, "error": f"文件读取失败: {file_path}"}
    
    if not query:
        return {"success": False, "error": "必须提供 query 参数"}
    
    # 构建消息
    messages = [{"role": "system", "content": "You are a helpful assistant."}]
    
    if file_url:
        messages.append({"role": "user", "content": [{"text": query, "type": "text"}, {"type": "doc_url", "doc_url": [file_url], "file_parsing_strategy": "auto"}]})
    elif file_content:
        messages.append({"role": "system", "content": file_content})
        messages.append({"role": "user", "content": query})
    else:
        if not content:
            return {"success": False, "error": "必须提供内容文本或文件路径之一"}
        messages.append({"role": "system", "content": content})
        messages.append({"role": "user", "content": query})
    
    skill = [{"type": "ppt", "mode": mode}]
    
    try:
        log_print(f"📊 开始生成 PPT（模式：{mode}）...")
        log_print(f"💬 用户查询：{query}")
        if file_url:
            log_print(f"📁 使用文件上传模式（file_url: {file_url}）")
        elif file_content:
            log_print(f"📖 使用文件读取模式（内容长度：{len(file_content)} 字符）")
        if content:
            log_print(f"📝 附加内容：{content}")
        log_print()
        
        responses = dashscope.Generation.call(
            api_key=api_key,
            model='qwen-doc-turbo',
            messages=messages,
            skill=skill,
            stream=True,
            incremental_output=True
        )
        
        all_content = []
        all_reasoning = []
        outline_content = ""
        page_images = []
        final_pptx_url = ""
        
        for idx, resp in enumerate(responses, start=1):
            if resp.status_code == HTTPStatus.OK:
                try:
                    choice = resp.output["choices"][0]
                    message = choice["message"]
                    
                    content_chunk = message.get("content", "")
                    reasoning_content = message.get("reasoning_content", "")
                    
                    log_print(f"\n{'─'*60}")
                    log_print(f"第 {idx} 次返回")
                    log_print(f"{'─'*60}")
                    
                    if reasoning_content:
                        log_print("\n🤔 Reasoning Content:")
                        log_print(reasoning_content)
                        all_reasoning.append(reasoning_content)
                        
                        # 第1次返回：大纲 → 立即保存 content.md
                        if idx == 1:
                            outline_content = reasoning_content
                            log_print(f"\n📋 [第1次] 收到大纲和策略内容")
                            if output_dir:
                                save_outline(outline_content, output_dir)
                        
                        # 第2次返回：页面图片链接 → 立即下载每张图片
                        elif "<page-" in reasoning_content:
                            page_matches = re.findall(r'<page-(\d+)>(.*?)</page-\1>', reasoning_content)
                            log_print(f"\n🖼️  收到 {len(page_matches)} 个页面图片链接")
                            for page_num, url in page_matches:
                                page_images.append({"page": int(page_num), "url": url})
                                if output_dir and url.startswith("http"):
                                    download_image(int(page_num), url, output_dir)
                    
                    if content_chunk:
                        log_print("\n✅ Content:")
                        log_print(content_chunk)
                        all_content.append(content_chunk)
                        
                        # 提取 PPT 下载链接 → 立即下载 PPTX
                        url_pattern = r'(https?://[^\s<>"\']+)'
                        url_matches = re.findall(url_pattern, content_chunk)
                        if url_matches:
                            pptx_url = next((u for u in url_matches if 'pptx' in u.lower()), None)
                            if pptx_url:
                                final_pptx_url = pptx_url
                                log_print(f"\n📎 收到完整 PPT 下载链接：{final_pptx_url}")
                                if output_dir:
                                    download_pptx(final_pptx_url, output_dir)
                            elif not final_pptx_url:
                                final_pptx_url = url_matches[0]
                                log_print(f"\n📎 收到下载链接：{final_pptx_url}")
                
                except KeyError as e:
                    log_print(f"❌ 解析响应时出错: {e}")
                    continue
            else:
                log_print(f"❌ API 调用失败: {resp.code} - {resp.message}")
                return {"success": False, "error": f"API 调用失败: {resp.code} - {resp.message}"}
        
        log_print(f"\n{'='*60}")
        log_print("🎉 PPT 生成完成！")
        log_print(f"{'='*60}")
        
        return {
            "success": True,
            "content": "\n".join(all_content),
            "reasoning": "\n".join(all_reasoning),
            "outline": outline_content,
            "page_images": page_images,
            "pptx_download_url": final_pptx_url,
        }
        
    except Exception as e:
        log_print(f"❌ 生成过程中出错: {e}")
        return {"success": False, "error": f"生成过程中出错: {e}"}


def main():
    clear_log()
    
    parser = argparse.ArgumentParser(description="使用 DashScope API 生成 PowerPoint 图片")
    parser.add_argument("-c", "--content", type=str, help="PPT 内容文本")
    parser.add_argument("-f", "--file", type=str, help="输入文件路径")
    parser.add_argument("-q", "--query", type=str, required=True, help="用户查询内容")
    parser.add_argument("-m", "--mode", type=str, choices=["creative"], default="creative", help="生成模式")
    parser.add_argument("-o", "--output-dir", type=str, help="输出目录（保存 content.md、图片、PPTX）")
    parser.add_argument("-k", "--api-key", type=str, help="DashScope API Key")
    
    args = parser.parse_args()
    
    if not args.content and not args.file:
        log_print("❌ 错误：必须提供 --content 或 --file 参数之一")
        sys.exit(1)
    
    api_key = args.api_key or os.environ.get("DASHSCOPE_API_KEY")
    if not api_key:
        log_print("❌ 错误：未设置 DASHSCOPE_API_KEY")
        log_print("请设置环境变量：")
        log_print("echo 'export DASHSCOPE_API_KEY=\"your-api-key-here\"' >> ~/.bashrc && source ~/.bashrc")
        sys.exit(1)
    
    # 设置输出目录
    output_dir = args.output_dir
    if output_dir:
        os.makedirs(output_dir, exist_ok=True)
        log_print(f"📂 输出目录: {output_dir}")
    
    try:
        log_print("🚀 PPTX Generator 启动")
        log_print(f"💬 查询: {args.query}")
        log_print(f"🎨 模式: {args.mode}")
        if args.file:
            log_print(f"📁 文件: {args.file}")
        if args.content:
            log_print(f"📝 内容: {args.content[:100]}{'...' if len(args.content) > 100 else ''}")
        
        result = generate_ppt(
            content=args.content,
            query=args.query,
            mode=args.mode,
            api_key=api_key,
            file_path=args.file,
            output_dir=output_dir
        )
        
        if result["success"]:
            log_print("\n🎉 生成成功！")
        else:
            log_print(f"\n❌ 生成失败: {result['error']}")
            sys.exit(1)
            
    except KeyboardInterrupt:
        log_print("\n\n⚠️  用户中断操作")
        sys.exit(0)
    except Exception as e:
        log_print(f"\n❌ 程序执行出错: {e}")
        sys.exit(1)


if __name__ == "__main__":
    main()