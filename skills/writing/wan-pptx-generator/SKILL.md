...
skill_id: official_ABKRSVG0
enabled_at: 1785348284289
version: 1.0.0
name_zh: 通义 PPT
---
...
运行脚本前，请先检查 `DASHSCOPE_API_KEY` 是否已配置。如果未配置通过以下命令设置（根据系统选择对应命令）：

Linux：echo 'export DASHSCOPE_API_KEY="your-api-key-here"' >> ~/.bashrc && source ~/.bashrc

Mac：echo 'export DASHSCOPE_API_KEY="your-api-key-here"' >> ~/.zshrc && source ~/.zshrc

Windows（CMD）：setx DASHSCOPE_API_KEY "your-api-key-here"

也可在调用时通过 `--api-key` 参数直接传入。
...
| --api-key | -k | 否 | API Key（也可通过环境变量传入） |
...
```bash
# 先检查环境变量是否配置
source ~/.bashrc && echo $DASHSCOPE_API_KEY
...
| 错误 | 原因 | 解决方案 |
|------|------|----------|
| API key not provided | 未设置 API Key | 设置 `DASHSCOPE_API_KEY` 环境变量或传入 `--api-key` |
