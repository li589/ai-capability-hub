...
"""
    ...
    if not api_key:
        ...
        print(f"{RED}║                                                ║{RESET}")
        print(f"{RED}║  注册获取 Key: https://redfox.hk/settings/api-keys?source=workbuddy ║{RESET}")
        print(f"{RED}╚══════════════════════════════════════════════════╝{RESET}")
        sys.exit(1)
    ...
    if not articles:
        ...
        if fallback:
            ...
            warn("如需搜索全量公众号内容，请访问 https://redfox.hk/settings/api-keys?source=workbuddy")
            sys.exit(0)
