...
便于写进交底书：保留专利号、标题、**消化摘要后的**一两句方案概括（有 **`abstract`** 时概括须可追溯至该摘要）；**每条另起一行或表格列给出「来源 URL」**；在 1.1「检索说明」中可简要注明是否已使用 **`cnipa_epub_search.py`**、是否触发 **Google 学术降级**。避免大段抄袭权利要求或整段粘贴官方摘要。
