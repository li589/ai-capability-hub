...
from app.config import SKILLHUB_PUB_KEY_ID, SKILLHUB_PRIVATE_KEY, SKILLHUB_DEVELOPER_ID, MODE
...
async def call_ai_preorder(code_url: str, amount: int = 1, title: str = "五运六气分析") -> str:
    """调用 X402 AI 预下单接口，返回 payment_code"""
    
    # Mock 模式：直接返回模拟 payment_code
    if MODE == "mock":
        mock_code = f"mock_payment_{int(time.time())}_{secrets.token_hex(8)}"
        logger.info(f"[MOCK] AI预下单: payment_code={mock_code}")
        return mock_code
    ...
    if resp.status_code != 200:
        raise Exception(f"AI预下单失败, HTTP {resp.status_code}: {resp.text}")
