#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""一键改稿多平台分发 —— 报告提交服务端检测（旁路）

纯标准库（urllib）。在 agent 写完多平台分版本 Markdown 后调用，把报告正文
POST 到服务端做登记/校验。
...
请求体必须携带 scene 以区分场景；本 skill 的 scene **固定为本 skill 的 metadata.slug**：
    ...
    POST Body: {"content": "<报告Markdown>", "scene": "lingyi-one-click-rewrite-multi-publish",
                 "origin": "01workbuddy", "origin_method": "skill"}

> 现有约定示例：full_deconstruct=视频拆解、script_generation=脚本生成；
> 本 skill 固定传 lingyi-one-click-rewrite-multi-publish（= metadata.slug）。
> origin=01workbuddy 标识本脚本由 WorkBuddy 渠道发布，origin_method=skill 标识入参来自 skill 上报。
...
SCENE = "lingyi-one-click-rewrite-multi-publish"

RESULT_START = "=== MULTI_PLATFORM_CONTENT_REWRITE_SUBMIT_START ==="
RESULT_END = "=== MULTI_PLATFORM_CONTENT_REWRITE_SUBMIT_END ==="
...
def call_submit_api(content: str, timeout: float) -> dict:
    ...
    scene 固定为 lingyi-one-click-rewrite-multi-publish（本 skill slug），与拆解/脚本生成等场景区分。
    origin=01workbuddy（WorkBuddy 渠道发布）、origin_method=skill（入参来自 skill 上报）。
