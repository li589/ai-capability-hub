...
if (isHelp) {
  console.log(
    [
      'alice-broker-top-picks-tracker — 调用万得 Alice Agent「券商金股追踪」，覆盖交易/策略/配置多视角与宏观流动性/曲线/技术情绪五大维度，用户原话直接透传给服务端',
      "",
      "Usage:",
      "  alice-broker-top-picks-tracker --prompt <QUESTION> [--download-dir <DIR>] [--new]",
      "  alice-broker-top-picks-tracker --prompt <QUESTION> --detach                        # 后台跑：立刻返回 PID 与日志路径，进程脱离当前会话",
      "  alice-broker-top-picks-tracker --prompt <QUESTION> --no-wait                       # 沙箱推荐：CLI 内部自旋直到完成（默认）",
      ...
      "  alice-broker-top-picks-tracker check-conflict --prompt <QUESTION>                # [已废弃] 主调用前预检：是否已有同/相似 prompt 的 running 任务",
      ...
      "",
      "Windows UTF-8:",
      "  powershell -NoProfile -ExecutionPolicy Bypass -File scripts/abtpt.ps1 --prompt <Q> --no-wait",
      "  管道捕获 live 中文乱码时，读 stdout 的 ALICE_SESSION_LOG= 路径（UTF-8 BOM 会话日志）。",
      "",
      "Options:",
      '  --prompt,       -p <QUESTION>   用户提问，例如"2026年5月有哪些券商金股？医药生物有没有短线机会？"（必填）',
      ...
      "  --no-strict                     关闭 strict 模式（默认开启）；未输出 ALICE_BROKER_TOP_PICKS_TRACKER_DONE 时",
      ...
      "",
      "退出码:",
      "  0   成功：必定伴随 stdout 含 ALICE_BROKER_TOP_PICKS_TRACKER_DONE 行（含 promptHash= / reportFullFile=；Agent 判定任务完成的唯一确定信号）",
      "  1   一般失败 / 任务终态为 failed",
      ...
      "  11  [已废弃] check-conflict 子命令：检测到同/相似 prompt 的 running 任务（预检功能已下线）",
      "  12  [已废弃] check-conflict 子命令：24h 内已有相似 completed 任务（预检功能已下线）",
      ...
      '  Set-Location "D:\\path\\to\\alice-broker-top-picks-tracker"; node scripts/cli.mjs --prompt "2026年5月有哪些券商金股？医药生物有没有短线机会？"',
      '  node "D:\\path\\to\\alice-broker-top-picks-tracker\\scripts\\cli.mjs" --prompt "最近券商推荐了哪些金股？热度排行如何？"',
      '  alice-broker-top-picks-tracker -p "2026年5月券商金股行业分布如何？"',
      '  alice-broker-top-picks-tracker -p "哪些个股被多家券商同时推荐？" --download-dir "D:\\reports\\broker"',
      '  alice-broker-top-picks-tracker -p "重新统计本月券商金股推荐" --new',
      '  alice-broker-top-picks-tracker -p "今年券商金股推荐趋势怎么看？" --no-wait              # 沙箱：CLI 内部自旋直到完成',
      '  alice-broker-top-picks-tracker -p "上个月券商金股表现如何？" --no-wait --once     # 脚本：单次探针',
      '  alice-broker-top-picks-tracker -p "2026年5月有哪些券商金股？" --detach           # 后台跑，立即返回，IDE 杀终端也不影响',
      "",
    ...
    if (!prompt || typeof prompt !== "string" || !prompt.trim()) {
      ...
      console.log(
        [
          ...
          `  · 轮询结果:    alice-broker-top-picks-tracker -p "${prompt}" --no-wait`,
          `  · 查落盘路径:  alice-broker-top-picks-tracker status -p "${prompt}"`,
          "",
          "完成后报告位置：",
