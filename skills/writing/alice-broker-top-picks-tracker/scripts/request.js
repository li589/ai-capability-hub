...
// 本 CLI 仅承载万得 Alice 的「券商金股追踪」技能。
// 服务端通过 prompt 文本前缀 `使用「<Skill 名>」技能：` 识别 Skill。
const ALICE_SKILL_NAME_ZH = "券商金股追踪";
...
//        ALICE_BROKER_TOP_PICKS_TRACKER_DONE taskId=<id> promptHash=<hash> reportFile=<abs> reportFullFile=<abs>
//      Agent 只需匹配该行就能确认成功，比解析 ALICE_BROKER_TOP_PICKS_TRACKER_STATUS=COMPLETED
...
/**
 * 输出 ALICE_BROKER_TOP_PICKS_TRACKER_DONE 行。幂等：同一进程内只输出一次。
...
function emitAnalysisDone({ taskId, promptHash } = {}) {
  ...
  const parts = ["ALICE_BROKER_TOP_PICKS_TRACKER_DONE"];
  ...
  console.log(
    "[CLI] 上一行 ALICE_BROKER_TOP_PICKS_TRACKER_DONE 是本进程任务"
      + "真正完成"
...
function installStrictExitHook() {
  ...
  process.on("exit", () => {
    ...
    try {
      console.error(
        "[CLI][strict] 进程退出但未输出 ALICE_BROKER_TOP_PICKS_TRACKER_DONE；强制退出码="
          + EXIT_STRICT_NO_DONE
          + "。",
      );
      console.error(
...
// strict 模式专用退出码：
// - 6 = 进程退出时未输出 ALICE_BROKER_TOP_PICKS_TRACKER_DONE，且 exitCode 仍为 0；
...
function clearCompletedRecordForResubmit(registry, promptHash, { logPrefix = "[CLI]" } = {}) {
  ...
  console.log(
    `${logPrefix} 若只需查询历史落盘路径，请用：alice-broker-top-picks-tracker status -p "<同一问题>"`,
  );
...
function printAttachBanner({ taskId, contextId, startedAt }) {
  ...
  console.log("");
  console.log("ALICE_BROKER_TOP_PICKS_TRACKER_STATUS=RUNNING");
...
function printCliRunStartNotice() {
  const lines = [
    "ALICE_BROKER_TOP_PICKS_TRACKER_STATUS=RUNNING",
    "[CLI] 券商金股追踪已启动：本进程将阻塞运行直至成功（agentResult + [完成]）或失败，通常需 2-15 分钟。",
    "[CLI] [等待中] 心跳期间无新输出属正常；禁止用 check_command_status 轮询替代等待本进程结束。",
    "[CLI] 见到 [任务已受理] 仅表示已提交，不等于报告完成；必须等到 ALICE_BROKER_TOP_PICKS_TRACKER_STATUS=COMPLETED。",
  ];
  for (const line of lines) {
    console.error(line);
  }
...
function persistAgentResultIfAny({ registry, taskId, promptHash, prompt }) {
  ...
  if (!text) return null;
  const header = [
    `<!-- alice-broker-top-picks-tracker：本次 Alice agentResult.value 已落盘 -->`,
    `<!-- 重要：本文件是 Alice 响应正文（agentResult.value），并不一定等于"完整金股追踪报告"。 -->`,
...
function printCliRunEndNotice({ success, exitCode, elapsed }) {
  const lines = success
    ? [
        "ALICE_BROKER_TOP_PICKS_TRACKER_STATUS=COMPLETED",
    ...
    : [
        "ALICE_BROKER_TOP_PICKS_TRACKER_STATUS=FAILED",
...
function usage() {
  return [
    "alice-broker-top-picks-tracker — 调用万得 Alice Agent「券商金股追踪」技能，汇总券商月度金股推荐、统计热度排行与行业分布",
    "",
    "Usage:",
    "  alice-broker-top-picks-tracker --prompt <QUESTION> [--download-dir <DIR>] [--new]",
    "  alice-broker-top-picks-tracker --prompt <QUESTION> --no-wait                       # 沙箱推荐：CLI 内部自旋 probe 直到完成（默认）",
    "  alice-broker-top-picks-tracker --prompt <QUESTION> --no-wait --once                # 单次探针：几秒内返回（脚本/调试）",
    "  alice-broker-top-picks-tracker --prompt <QUESTION> --no-wait \\",
    "      --watch-interval 60 --watch-timeout 1800                              # 自定义轮询节奏",
    "  alice-broker-top-picks-tracker status --prompt <QUESTION>                        # 查询落盘路径（不访问服务端）",
    "  alice-broker-top-picks-tracker check-conflict --prompt <QUESTION>                # [已废弃] 主调用 --no-wait 已内置去重，无需预检",
    "  alice-broker-top-picks-tracker apikey-set <KEY>",
    "  alice-broker-top-picks-tracker apikey-get",
    "  alice-broker-top-picks-tracker apikey-clear",
    "  alice-broker-top-picks-tracker --help",
    "",
    "Options:",
    "  --prompt,       -p <QUESTION>   用户提问，例如\"2026年5月有哪些券商金股\"（必填）",
    ...
    "                                  ALICE_BROKER_TOP_PICKS_TRACKER_DONE 行，会把 exitCode=0 强制改成 6，",
    ...
    "",
    "退出码:",
    "  0   成功：必定伴随 stdout 含 ALICE_BROKER_TOP_PICKS_TRACKER_DONE 行（Agent 判定任务完成的唯一确定信号）",
    "  1   一般失败 / 任务终态为 failed",
    "  2   参数错误 / KEY_MISSING",
    "  4   --no-wait 专用：已达全程自旋上限（默认 60min）仍未完成；Agent 再执行一次相同 --no-wait 续接",
    "  5   --no-wait 专用：--once 且本地无任务记录（默认 --no-wait 会自动提交）",
    "  6   strict 模式兜底：进程退出但未输出 ALICE_BROKER_TOP_PICKS_TRACKER_DONE（通常表示进程被沙箱终止）；",
 ...
 * `ALICE_BROKER_TOP_PICKS_TRACKER_STATUS=...`，由调用方（单次 probe / watch 轮询）在合适
...
async function runProbeStep({ registry, taskId, promptHash, prompt, url, headers, apiKey, downloadDir }) {
  ...
  if (probe.kind === "working") {
    ...
    console.log(
      "[CLI] 本进程尚未输出 ALICE_BROKER_TOP_PICKS_TRACKER_DONE；logs/ 下其它 .session.log 内的 DONE 行与本 PROMPT_HASH 不一致则无效。",
    );
 ...
 * 调用方读到的 `ALICE_BROKER_TOP_PICKS_TRACKER_STATUS=...` 语义一致。
 *
...
function publishFinalStatus(outcome, { suggestExternalPoll = false } = {}) {
  switch (outcome) {
    case "completed":
      console.log("ALICE_BROKER_TOP_PICKS_TRACKER_STATUS=COMPLETED");
      emitAnalysisDone();
      break;
    case "failed":
      console.log("ALICE_BROKER_TOP_PICKS_TRACKER_STATUS=FAILED");
      break;
    case "no-task":
      console.log("ALICE_BROKER_TOP_PICKS_TRACKER_STATUS=NO_TASK");
      break;
    case "working":
    default:
      if (suggestExternalPoll) {
        ...
        console.log(
          "[探针] 禁止读取 download/ 同名报告或 STALE_REPORT_CANDIDATE；必须见到 ALICE_BROKER_TOP_PICKS_TRACKER_DONE 且 promptHash= 一致。",
        );
      }
      console.log("ALICE_BROKER_TOP_PICKS_TRACKER_STATUS=RUNNING");
      break;
...
async function main() {
  ...
  // analysis 主路径：默认开启 strict 模式（未输出 ALICE_BROKER_TOP_PICKS_TRACKER_DONE 即视为
  ...
  if (noWait) {
    ...
    console.log("ALICE_AGENT_REDLINE=禁止在 stdout 出现 ALICE_BROKER_TOP_PICKS_TRACKER_DONE 之前 view_files/read download/ 主体报告");
      ...
      mainLoop: while (true) {
        for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
          ...
          if (!startBannerPrinted) {
            ...
            console.log(
              `[开始] 已向 Alice「${ALICE_SKILL_NAME_ZH}」技能发起请求；券商金股追踪通常耗时 2-5 分钟，期间会定期播报等待进度，请勿中断。`,
            );
          }
