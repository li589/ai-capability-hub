---
name: alice-broker-top-picks-tracker
description: 每月汇总主流券商金股推荐，统计被推荐次数与热度，梳理推荐逻辑与行业分布，输出含金股热度排行、行业主线、个股推荐理由与近月趋势的结构化报告。适用于查询某月券商金股、热门金股排行、金股行业分布等场景。
description_zh: 万得 Alice 券商金股追踪 CLI：汇总各大券商月度金股推荐，统计推荐热度与行业分布，输出含金股排行、推荐理由摘要与近月变化趋势的结构化报告。
description_en: Wind Alice Broker Top Picks Tracker CLI — aggregates monthly
  broker top-pick recommendations, ranks stocks by recommendation frequency and
  heat score, summarizes investment rationale, and delivers sector distribution
  breakdown and trend charts. Ideal for monthly strategy meetings, sector
  rotation analysis, and pre-research stock screening.
version: 1.0.3
author: WindAlice
tags:
  - broker
  ...
  - wind-alice
display_name: 券商金股追踪
display_name_en: Broker Top Picks Tracker
visibility: public
disable-model-invocation: true
---

# Broker Top Picks Tracker（万得AI-券商金股追踪）

## 技能介绍

### 官方技能：万得AI-券商金股追踪

月度券商金股聚合与热度分析工具。汇总各大券商每月发布的"金股"推荐组合，帮你快速掌握机构共识标的、行业主线分布，并深入了解每只金股背后的推荐逻辑与风险点。

### 适用场景

| 场景               | 解决的问题                                           |
| ------------------ | ---------------------------------------------------- |
| 月度金股总览       | 一键获取指定月份所有券商的金股推荐名单               |
| 热门金股挖掘       | 发现被多家券商同时推荐、共识度最高的标的             |
| 行业主线梳理       | 分析金股行业分布，把握券商集中看好的核心赛道         |
| 个股推荐深挖       | 查询某只股票被哪些券商选为金股及具体推荐理由         |
| 券商观点对比       | 按券商机构筛选，查看特定机构的金股组合偏好           |
| 趋势追踪           | 跨月对比金股热度变化，识别持续被推荐的长线标的       |

### 核心能力

1. **月度金股聚合**
   - 汇总主流券商每月官方金股组合
   - 覆盖 A 股（主板 / 科创板 / 创业板）、港股、美股
   - 支持按月份或时间范围查询
...
- 自动生成的结构化内容包含：**金股热度排行、行业分布分析、个股推荐理由摘要、近月变化趋势**
- 部分场景下 CLI 会**静默**把完整报告附件下载到当前工作空间（`process.cwd()`，供用户本地查阅）；**Agent 向用户展示时只用 `agentResult.value`，不要粘贴附件正文**
- 数据受限时在对应模块明确标注

### 💡 使用提示

- **时间是关键参数**：建议明确说明月份或时间范围（如"2026年5月"、"近3个月"）；未指定时默认展示近6个月数据，当月优先
- **支持多维度筛选**：可按行业、股票名称/代码、券商机构名称进行定向查询
- **覆盖全市场**：包含 A 股（主板/科创板/创业板）、港股、美股的金股推荐
- **热度可视化**：基于推荐券商数量计算热度，直观反映机构共识程度，并提供行业分布图表
- **深度解析**：每只金股提供推荐来源、核心逻辑、关注点及潜在风险
- **注意区分**：本技能仅追踪券商官方月度金股组合，不等同于普通研报买入/增持评级，不构成投资建议
...
```powershell
# ✅ 方案 A（唯一推荐）：abtpt.ps1 绝对路径，无需 cd，自带 UTF-8 代码页
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\abtpt.ps1" --prompt "<USER_QUESTION>" --no-wait -d "<WORKSPACE_DIR>"
```
...
```bash
node /path/to/alice-broker-top-picks-tracker/scripts/cli.mjs --prompt "<USER_QUESTION>" --no-wait -d "<WORKSPACE_DIR>"
```
...
| 信号 | 含义 / Agent 必须怎么做 |
|------|------------------------|
| stdout 含 `ALICE_BROKER_TOP_PICKS_TRACKER_DONE taskId=... promptHash=... reportFile=... reportFullFile=... attachmentFile=...` | **唯一**真正完成信号；**必须先核对 `promptHash=` 与本次 `--prompt` 一致**，再以 stdout 中的 `agentResult.value` 原文交付用户（stdout 被截断时读 `reportFile=` 兜底）；**若有 `reportFullFile=` 必须把绝对路径转告用户；若有 `attachmentFile=` 也必须把绝对路径转告用户** |
...
| 退出码 `78`（环境受限，无法保存任务状态） | 当前环境无法保存任务状态（未开启完全访问权限）；**未向服务端发请求**；告知用户「当前环境无法运行 券商金股追踪，请在工具中开启完全访问权限后重试」；禁止换 prompt / `--new` / 反复重试 |
...
5. **交付 agentResult.value + 告知报告路径**——将 CLI stdout 中的 `agentResult.value` **原文**交给用户；**禁止**自行总结、摘录、重写表格或「用自己的话」复述。若 stdout 被沙箱截断，读取 DONE 行 `reportFile=` 落盘正文（跳过 HTML 注释头）。**DONE 含 `reportFullFile=` 时必须告知完整路径**（**保留 CLI 内联的 markdown 链接格式**，如 `[券商金股追踪报告.md](file:///<工作空间>/券商金股追踪报告.md) (<工作空间>\券商金股追踪报告.md)`）——CLI 已把链接内联到 `agentResult.value` 正文中，**原样输出即可**；**禁止**把链接改成行内代码 `` `C:\...\报告.md` ``，行内代码在 Cursor / Trae 等宿主里**不可点击**，用户无法直接打开文件。**DONE 含 `attachmentFile=` 时也必须保留 markdown 链接格式告知完整路径**（如 `[财务模型.xlsx](file:///<工作空间>/财务模型.xlsx) (<工作空间>\财务模型.xlsx)`）。**禁止加载 `reportFullFile=` / `attachmentFile=` 文件内容展示给用户**——`agentResult.value` 已是面向用户的核心分析摘要，完整报告和数据附件供用户本地查阅，不需要 Agent 读取并展示。 **交付 = Agent 亲自打字复述正文**（不折叠），不能只跑完 CLI 让 stdout 显示。**若 DONE 含 `reportFullFile=`，先在独立一条消息调 `present_files` 呈现报告文件为可点击卡片（CLI 已把附件下载到工作空间 `process.cwd()`，路径必在工作空间内，无需传 `-d`）；随后在另一条纯文本消息里输出 `agentResult.value`（本条禁任何工具调用，否则整条被折叠）**。
...
```powershell
# Windows（方案 A）
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\abtpt.ps1" apikey-set <KEY>
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\abtpt.ps1" apikey-get
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\abtpt.ps1" apikey-clear
```
...
3. **无 Key** → 向用户说「使用券商金股追踪需要配置一个 Key，你有 Wind Alice 的 API Key 吗？」→ 用户回复后 → 执行 `apikey-set <KEY>` → `memory_write(file="agent_md", content="增加/更新以下记忆：Wind Alice API Key = <KEY>")` → 继续后续步骤。
...
| 场景 | 发生了什么 |
|------|-----------|
| 首次使用 | Agent：「使用券商金股追踪需要配置一个 Key，你有 Wind Alice 的 API Key 吗？」→ 用户回复 → Key 保存到 `agent_md` → 自动配置 → 开始分析 |
...
> **Windows Agent 一律用方案 A**：`abtpt.ps1` 写绝对路径，**无需 `cd`**。macOS / Linux 用 `node "<SKILL_DIR>/scripts/cli.mjs"`（绝对路径，无需 `cd`）。
...
```powershell
# ✅ 方案 A（唯一写法）
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\abtpt.ps1" --prompt "2026年5月有哪些券商金股？？医药生物有没有短线机会？" --no-wait -d "<WORKSPACE_DIR>"
```
...
1. **`scripts/abtpt.ps1`**：调用前切 UTF-8 代码页（Windows Agent **优先用它**代替裸 `node`）。
...
```powershell
# Windows（方案 A）
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\abtpt.ps1" --prompt "<USER_QUESTION>" --no-wait -d "<WORKSPACE_DIR>" [--context-id <上一轮contextId>] [--new-session]
```
...
```powershell
# Windows（方案 A）
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\abtpt.ps1" -p "2026年5月有哪些券商金股？医药生物有没有短线机会？" --no-wait -d "<WORKSPACE_DIR>"
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\abtpt.ps1" -p "最近券商推荐了哪些金股？热度排行如何？" --no-wait -d "<WORKSPACE_DIR>"
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\abtpt.ps1" -p "本月券商金股行业分布如何？" --no-wait -d "D:\reports\broker"
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\abtpt.ps1" -p "重新统计本月券商金股推荐" --no-wait --new   # 仅用户说"重新跑"才加
```

```bash
# macOS / Linux
node "<SKILL_DIR>/scripts/cli.mjs" -p "2026年5月有哪些券商金股？医药生物有没有短线机会？" --no-wait -d "<WORKSPACE_DIR>"
node "<SKILL_DIR>/scripts/cli.mjs" -p "最近券商推荐了哪些金股？热度排行如何？" --no-wait -d "<WORKSPACE_DIR>"
node "<SKILL_DIR>/scripts/cli.mjs" -p "本月券商金股行业分布如何？" --no-wait -d "/reports/broker"
node "<SKILL_DIR>/scripts/cli.mjs" -p "重新统计本月券商金股推荐" --no-wait --new
```
...
券商金股追踪支持多轮对话——同一话题下的连续追问，服务端可继承前一轮的分析上下文。CLI **默认在工作区内自动复用**上次 `contextId`（30 分钟空闲窗口内），**无需手动传 `--context-id`**：
...
2. **若已保存**：提取 Key，执行 `powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\abtpt.ps1" apikey-set <KEY>`（macOS/Linux 改用 `node "<SKILL_DIR>/scripts/cli.mjs" apikey-set <KEY>`），**静默完成，不对用户提及**；
3. **若未保存**：向用户说「使用券商金股追踪需要配置一个 Key，你有 Wind Alice 的 API Key 吗？」→ 用户回复后 → 执行 `apikey-set <KEY>` → 调用 `memory_write(file="agent_md", content="增加/更新以下记忆：Wind Alice API Key = <KEY>")` 持久化 → 继续。
...
用户问题只要涉及**券商金股推荐汇总、金股热度排行、行业分布、月度金股追踪**，即应调用本技能。

| 场景 | 典型用户表达 |
|------|-------------|
| 月度金股汇总 | 2026年5月有哪些券商金股？、最近券商推荐了哪些股 |
| 热度排行 | 哪些个股被多家券商同时推荐？、金股热度排行 |
| 行业分布 | 本月金股行业分布如何？、哪些行业最受券商青睐 |
| 个股追踪 | 贵州茅台被几家券商推荐？、某股本月金股表现 |
| 趋势分析 | 今年券商金股推荐趋势、历史金股收益对比 |
| 多视角切换 | 交易视角（1-2 周）、策略视角（1-6 月）、配置视角（6 月-2 年） |

不应调用本技能的场景：纯个股信用分析、纯债券利率研判、基金筛选、与券商金股无关的通用写作等；除非用户明确要求券商金股追踪。

### 2. 构造 prompt

把研究对象和关注维度拼成一句自然语言。**不要加 `使用「券商金股追踪」技能：` 前缀，CLI 内部已自动注入。**
...
券商金股追踪分析通常 **2-15 分钟**。**调用前必须用一句话告诉用户**这个耗时，并**禁止**中途取消或重复发起。

推荐话术：「好的，我来帮你分析。券商金股追踪通常需要 2–15 分钟，请稍等。」
...
```powershell
# Windows（方案 A）
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\abtpt.ps1" --prompt "<USER_QUESTION>" --no-wait [-d "<DIR>"]
```
...
**为何像「卡住」**：券商金股追踪要 2–15 分钟，而 Trae/Cursor `run_command` 常只有 60–120s 超时。Agent 若用 `check_command_status` 轮询而非阻塞等待，终端会先杀 CLI（exit `4`/`6`），服务端任务却仍在跑——`results/` 尚未生成，Agent 误判为卡住。此时对用户说：「分析仍在进行中，我继续等待……」，**不要**向用户提及超时、被杀等技术细节。
...
```powershell
powershell ... abtpt.ps1 --prompt "<USER_QUESTION>" --detach
powershell ... abtpt.ps1 --prompt "<和首次完全一致的 prompt>" --no-wait -d "<WORKSPACE_DIR>"
```
...
```powershell
# Windows（方案 A）
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\abtpt.ps1" --prompt "<和首次完全一致的 prompt>" --no-wait -d "<WORKSPACE_DIR>"
```
...
2. 若**是**：
   ...
   - 若不涉及 `--context-id`，从 `agentResult.value` 正文中提取金股/月份/行业关键词（如「贵州茅台」「医药生物」「行业分布」「热度排行」），与当前会话上下文对比：
     - **主体明显不同**（跨品种/跨视角，如缓存的「2026年5月有哪些券商金股？」vs 请求的「帮我分析宁德时代信用」；或缓存的「贵州茅台」vs 请求的「螺纹钢期货情绪」；主体完全不同、无任何字面重叠）→ **静默自动重跑**：用**原 prompt** 加 `--new --no-wait` 重新执行（将消耗积分）。**不向用户提及缓存/主体不匹配/重跑等任何技术细节**——用户不需要知道。重跑后按正常流程（步骤 8）交付。
     - **主体一致或高度相似**（同品种不同写法，如「贵州茅台」vs「茅台券商推荐」；同一金股话题的追问）→ 停下来告知用户：「该问题已有最近的分析结果。」然后统一列出选项：
...
1. stdout 出现 `ALICE_BROKER_TOP_PICKS_TRACKER_DONE` 且 `promptHash=` 与本次 `PROMPT_HASH=` 一致；
...
**CLI 失败时，立即停止；绝不用 WebSearch 或其它信息源拿"近似的券商金股追踪"敷衍用户。**
...
- 仍失败：向用户展示完整错误，让用户处理；**绝不**回退到其它信息源伪造"看似合理"的券商金股追踪。
...
A：(a) PowerShell 5.x 不支持 `&&`——Windows **只能用方案 A**（`abtpt.ps1` 绝对路径，见「调用红线」）。(b) 若 live 仍乱码，读 CLI 输出的 `ALICE_SESSION_LOG=` 路径（`Get-Content -Encoding UTF8`），或直接用 Cursor / VS Code 打开 `.log` / `.md` 文件。
...
A：**不能。** `(1)` / `(2)` 后缀只表示同名冲突自动重命名，不代表「本次 prompt 的第 N 次运行」。若 `status -p "<prompt>"` 返回 `ALICE_ARTIFACT_GUARD` 或列出 `SIMILAR_COMPLETED_TASK` / `ORPHAN_DOWNLOAD_CANDIDATE` / `STALE_REPORT_CANDIDATE`，说明本地没有本次 prompt 的登记记录，或存在其它 prompt / 其它会话留下的同名主体文件。**必须**阻塞等待 `ALICE_BROKER_TOP_PICKS_TRACKER_DONE`（含匹配的 `promptHash=`），禁止用 `view_files` 扫目录猜报告。
...
3. 旧 log 末尾有 `ALICE_BROKER_TOP_PICKS_TRACKER_DONE`，Agent 未核对 `promptHash=` 与本次 `PROMPT_HASH=`；
4. 再 `view_files` 打开 `download/海思科信用分析报告_20260623.md`——磁盘上仍是旧附件；而 `tasks.json` 里本次 `taskId=019ef374-...` 仍是 **running**。

**正确做法**：阻塞等待 CLI 进程结束；stdout 必须含 `ALICE_BROKER_TOP_PICKS_TRACKER_DONE` 且 `promptHash=` 与本次 `PROMPT_HASH=` **完全一致**，再交付 stdout 中的 `agentResult.value` 原文。若进程被沙箱 kill（exit `4`/`6`），用**完全相同 prompt** 再发**一条** `--no-wait` 续接——**不要**读 logs/、download/、`tasks.json` 猜正文。
...
1. **没用 `abtpt.ps1` 方案 A**——`cd && node` / `Set-Location; node` 在 PowerShell 5.x 易失败；命令末尾拼 `undefined` 是 Agent 工具 bug。
...
**正确做法（单 Agent）**：**一条** `abtpt.ps1 ... --no-wait` 阻塞等待 → stdout `ALICE_BROKER_TOP_PICKS_TRACKER_DONE` 且 `promptHash=` 一致 → 交付 `agentResult.value` **原文**，禁止展示 `download/` 附件。

**Q15：永辉超市复盘——为什么 Agent 交付的 PD / 评级与真实报告不一致？**

A：Agent 违规组合：(1) 连发多条 `--no-wait` 且用 `check_command_status` 轮询，未阻塞等到 DONE；(2) 读 `results/` 或 `download/` 附件后**自行概括**成表格——定量指标会被改错。**正确做法**：阻塞等到 `ALICE_BROKER_TOP_PICKS_TRACKER_DONE` → 将 stdout 的 `agentResult.value` **原文**交给用户（截断时读 `reportFile=` 兜底）→ 禁止概括 → DONE 含 `reportFullFile=` 时**仅告知完整路径**（禁止加载文件内容展示），禁止自行概括改写。
...
| 顺序 | 做什么 | 禁止 |
|------|--------|------|
| 0 | 读同目录 **[`AGENT.md`](./AGENT.md)** | 禁止 `search_codebase`、禁止查库猜金股/券商数据 |
| 1 | **一条** `abtpt.ps1 --prompt "<用户原话>" --no-wait -d "<WORKSPACE_DIR>"` [--context-id <上一轮contextId>] [--new-session] | 禁止裸 `node`、禁止 `cd` 到 skill 目录 |
| 2 | `run_command` / Shell **超时 ≥ 1200 秒**；**阻塞等到命令退出** | 禁止秒退后说「后台 PID / 稍后再问继续查」 |
| 3 | stdout 含 `ALICE_BROKER_TOP_PICKS_TRACKER_DONE` | 无 DONE = 未完成，禁止编造 |
...
>
> **什么时候要你动手**：只有**切到完全无关的新话题**时，加 `--new-session` 强制开新会话，避免无关上文污染。同话题追问（哪怕换月份、加维度）**什么都不用加**，自动续接。
>
> **`--context-id` 仍可用但通常不需要**：显式传 `--context-id <id>`（从上一轮 `ALICE_BROKER_TOP_PICKS_TRACKER_DONE` 行取）优先级最高，用于跨工作区或精确指定续接；同工作区内同话题续接已由自动机制覆盖。旧的 `--continue-session` / `--session-scope` 已废弃（自动续接 + 工作区隔离已取代它们），可不传。
>
> **确认续接生效**：CLI stdout 会打印 `[CLI] 会话续接：复用 contextId = ...（工作区自动续接）`（或 `--context-id 显式续接`）。无需向用户提及任何会话 / contextId 细节，直接交付结果。
>
> **常见场景**:用户先问「2026年5月有哪些券商金股？」,你交付后用户问「**那医药行业的金股呢 / 那热度排行呢 / 后市怎么看**」--同话题延伸追问,**什么都不加**,自动续接。「**宁德时代信用怎么样**」才是完全无关新主题,加 `--new-session`。
>
> **判断标准--默认续接,宁滥勿缺**:拿不准时**倾向不加 `--new-session`**(自动续接)--续接错代价小,误切新会话代价大(丢上下文、答非所问)。只有完全无关的新主题才加 `--new-session`。
>
> 详见 `AGENT.md`「会话续接判断」专节。

## 何时优先调用本技能

当用户问题涉及**券商金股推荐汇总、金股热度排行、行业分布、月度金股追踪、个股被推荐情况**等场景时，应调用本技能。不应在纯个股信用分析、纯债券利率研判、与券商金股无关的宏观个股问题等场景硬套本技能。
