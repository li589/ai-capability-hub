...
调用接口对纯文本进行拼音校对，直接以 Markdown 展示校对结果，并提供下载链接（不落盘下载）。
...
def load_api_key(explicit_key: Optional[str] = None) -> str:
    ...
    raise MiduProofreadError(
        "未找到鉴权 token。请前往 https://ai.mdata.net 注册获取 MIDU_APP_SECRET，"
...
def call_proofread_api(
    ...
    if str(code).strip() != "0000":
        if code in (401, 403) or "auth" in str(msg).lower() or "token" in str(msg).lower():
            raise MiduProofreadError(
                "鉴权失败或 token 无效。请前往 https://ai.mdata.net 注册获取有效的 MIDU_APP_SECRET，"
                f"并配置到环境变量 {KEY_ENV}。"
                + (f"\ntransactionId={txid}" if txid else "")
            )
...
def proofread_text(
    ...
    # 步骤 2：调用校对接口
    urls = call_proofread_api(proof_text=assembled_text, api_key=key, file_name=file_name)

    try:
        r = requests.get(urls.erratum_md_url, timeout=120)
        r.raise_for_status()
    except requests.RequestException as exc:
    ...
    return ProofreadOutputs(
        urls=urls,
        erratum_md=r.content.decode("utf-8", errors="replace"),
    )
...
def main(argv: Optional[Tuple[str, ...]] = None) -> int:
    ...
    try:
        ...
        if outputs.erratum_md and outputs.erratum_md.strip():
            ...
            output_parts.append("> 本次校对未发现勘误项。")
        output_parts.append("")
        output_parts.append("---")
        output_parts.append("")
