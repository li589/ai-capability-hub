...
> **来源（1:1 还原）**：`PersonalResumeScoreComputeFacadeImpl#doComputeScore` 及其 6 个 `computeXxxScore` 方法、`PersonalResumeScoreFacadeImpl#getScoreFromGpt`、`PersonalResumeConfigHelper#getQualificationScore`。
>
...
所有权重/映射来自 `references/resume-score-config.json`（原样复制自 `src/main/resources/简历/config.json`，即 Nacos `com.gaodun.personal.resume.score.config.json` 的真实配置）。
...
- 学历值 → 分数的映射链（`PersonalResumeConfigHelper.PROFESSION_MAP` + 配置 `qualificationScoreMap`）：
