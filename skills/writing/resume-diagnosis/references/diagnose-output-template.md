# 诊断报告输出（queryScoreDetail 1:1）

> **来源（1:1 还原）**：
> - `PersonalResumeScoreFacadeImpl#queryScoreDetail` → `PersonalResumeScoreHelper#convertToScoreReport`
> - `PersonalResumeScoreHelper#fillReportScore / fillReportDetail / createDiagnoseDTO / convertDiagnoseListToReportModuleDTO`
> - `PersonalResumeScoreFacadeImpl#queryScore` → `PersonalResumeScoreHelper#getResumeScore`（仅分数接口）
>
> 本文件定义**接口级输出结构**与**唯一交付格式（HTML 报告）**。面向用户的渲染必须以此数据为准；额外的"综合评价/求职赛道建议"等属于扩展内容，见文末"扩展节（非接口内容）"。

---

## 1. 接口返回结构（PersonalResumeScoreReportDTO）
...
### 1.1 状态前置判断（convertToScoreReport）

- `runStatus != SUCCESS(1)`（分数未算完）→ 返回**空报告**（score/beatPercent/reportDetails 均不填）。
- `runStatus == SUCCESS` → 填分数（fillReportScore）；`reportStatus == SUCCESS`（诊断完成）→ 再填 `reportDetails`。

### 1.2 分数与击败百分比（fillReportScore）

- `score` = `XjPersonalResumeScore.score`（六维求和结果）。
...
> 前两档（min 为 -2 / -1）服务于 `queryScore` 的状态语义（计算中 -2 / 无记录 -1），见第 2 节。

### 1.3 报告详情装配（fillReportDetail）

1. 取简历的模块列表（`XjPersonalResumeModule`），按 DB 中的 `sort` **升序**遍历。
   > 该 sort 由解析阶段 `analysisResume` 赋值：**显示模块在前（按配置 JSON 数组序）+ 隐藏模块在后**，重新编号 1..N——不是配置里的 `sort` 字段。隐藏模块若有诊断明细（如 NO_DATA 空流 quirk 产生）也会出现在报告中。
2. 诊断明细按 `moduleCode` 分组。
3. 模块配置从 `references/resume-module-config.json` 按当前 `resumeType` 的 code 索引（`moduleConfig[resumeTypeCode]`，`moduleCode → ModuleDTO`）。
4. **跳过**：模块无配置 或 该模块无诊断明细。
5. 模块内每条明细转文案（createDiagnoseDTO）：

| diagnoseType | 文案模板 | 排序值 sort |
|---|---|---|
| NO_DATA(0) | `【{moduleName}】可以为你加分哦，去完善` | `Integer.MIN_VALUE`（最前） |
| NO_FILL(1) | `【{dataFieldName}】是简历中的重要信息，请完善` | 字段在 `moduleConfig.dataFieldList` 中的**下标**（字段不在配置中则**丢弃该条**） |
| DESCRIPTION_SUGGEST(2) | 明细的 `suggestContent` 原文（已是"描述建议N：\n..."格式） | `Integer.MAX_VALUE`（最后） |

6. 模块内按 sort 升序排列文案 → 即 **NO_DATA 最前 → NO_FILL 按字段配置顺序 → DESCRIPTION_SUGGEST 最后**。
7. 模块输出：`{moduleCode, moduleName, diagnoseList, diagnoseNum = diagnoseList.size()}`；`diagnoseList` 为空则不输出该模块。

---

## 2. 仅分数接口（queryScore → getResumeScore）
...
**最终只向用户交付 1 份 HTML 报告**（写入 `简历诊断报告_<resumeId>.html` 后用 `present_files` 展示）。**不渲染 markdown/文本版报告**；中间 JSON（filled/finalized/suggestions/gpt_score/report）不落盘，仅最终 HTML 报告文件落盘。数据仍 1:1 接口——报告只含接口返回的三部分：`score`、`beatPercent`、`reportDetails`（各模块诊断列表）。禁止添加六维得分明细、分数区间评价（title/describe 属于 `queryScore` 接口，不属于本报告）、综合评价等任何接口外信息（扩展内容见第 4 节）。
...
以下内容**不属于** `queryScoreDetail` 接口返回，仅当用户要求"给整改方案/求职建议"时才追加，且必须**追加在 HTML 报告末尾**并标注"以下为扩展分析"：
