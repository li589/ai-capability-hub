...
把 goffer 接口中所有**确定性**逻辑代码化（1:1 还原，规则说明见
references/analysis-resume-rules.md / diagnosis-categories.md / scoring-rubric.md /
diagnose-output-template.md）：

  1. prepare       : 前置合并（提取 .doc/.docx 文本 + 照片检测 idPhoto + 生成填写模板），
                     一次调用替代原先「extract 脚本 + 照片检测 + emit-template」多次往返。
  2. emit-template : 按 resume-module-config.json 生成"逐字段填写模板"，
                     模型唯一的解析职责是把每个字段的值从原文抄进来。
  3. finalize      : 清洗（技能合并/教育前缀/学历枚举归一化/时间归一化）→
                     dataFieldStatus/moduleStatus 赋值（含空模块分支）→
                     模块 sort 重编号 → getShowFields →
                     NO_FILL / NO_DATA 候选 → DESCRIPTION_SUGGEST 目标清单 →
                     getScoreFromGpt 输入串 → 5 个确定性维度评分。
  4. emit-llm-tasks: 把 finalize 结果中的 suggestTargets + scoreGptInput 组装成
                     「单次批量 LLM 任务单」（每个字段只附该字段用到的 Prompt 原文，
                     从 references/diagnose-prompts.json 机器读取，不再让模型整份加载），
                     模型一次推理产出全部 suggestions + score，替代线上 12 次 bot 调用
                     的逐条串行模拟。
  5. build-report  : 合并 LLM 建议 + GPT 模块分 → handleDiagnoseSuggest 编号 →
...
# PersonalResumeConfigHelper.PROFESSION_MAP（学历入库值 -> qualificationScoreMap key）
PROFESSION_MAP = {
    "博士研究生": "doctor",
    "MBA": "mba",
    "硕士研究生": "master",
...
# diagnoseNoDataFields：各 resumeType 的 NO_DATA 检查模块组合
NO_DATA_MODULES = {
...
def months_between(start: str, end: str):
    """DateUtil.betweenMonth(start, end, true) + 1；解析失败返回 None。"""
    try:
...
# ---------------------------------------------------------------------------
# prepare（前置合并：文本提取 + idPhoto 检测 + 填写模板）
# ---------------------------------------------------------------------------
...
def cmd_prepare(args):
    result = {"resumeText": None, "textSource": None, "idPhotoDetected": False}
    ...
    if args.resume_file:
        ...
        if ext == ".docx":
            result["resumeText"] = _extract_docx(data)
            result["textSource"] = "docx-zipfile"
            result["idPhotoDetected"] = _docx_has_media(data) or _count_embedded_images(data) >= 1
        elif ext == ".doc":
            # Spire 的 Document 无 Images 属性，照片检测走魔数（SKILL.md 2026-08-17 实测结论）
            result["idPhotoDetected"] = _count_embedded_images(data) >= 1
            text, source = _extract_doc_via_scripts(path)
            result["resumeText"], result["textSource"] = text, source
        else:
            # .pdf（文本由模型 Read 读取）或其他格式：只做魔数照片检测
            result["idPhotoDetected"] = _count_embedded_images(data) >= 1

    result["idPhoto"] = "<已上传证件照>" if result["idPhotoDetected"] else ""
    result["template"] = _build_template(args.resume_type)
    _write_json(args.out, result)
    _log(f"prepare 完成: 文本={'有(' + result['textSource'] + ')' if result['resumeText'] else '无（模型用 Read 读）'}"
         f"，idPhoto 检测={'含照片' if result['idPhotoDetected'] else '未检出/不适用'}"
...
def cmd_finalize(args):
    ...
    # ---- NO_DATA：组合内逐模块，showFields 全空白即记（空流 allMatch=true quirk 保留）
    no_data = []
    ...
    # ---- getScoreFromGpt 输入串（按 scoreGptConfig.fieldList 顺序，隐藏模块跳过）
    gpt_input_parts = []
...
def cmd_emit_llm_tasks(args):
    ...
    # 按字段分组：同字段多条共用一份 Prompt（线上是多次独立 bot 调用，但每次的
    ...
    L.append("## B. 经历描述评分任务（getScoreFromGpt）")
    L.append("")
...
def cmd_build_report(args):
    ...
    # ---- DESCRIPTION_SUGGEST：handleDiagnoseSuggest 分组编号
...
def main():
    ...
    p = sub.add_parser("prepare",
                       help="前置合并：提取 .doc/.docx 文本 + 照片检测 idPhoto + 生成填写模板")
    p.add_argument("--resume-type", required=True)
    p.add_argument("--resume-file", default=None, help="简历文件路径（粘贴文本时可省略）")
