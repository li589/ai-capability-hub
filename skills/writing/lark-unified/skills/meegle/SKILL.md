...
name: meegle
description: >
  飞书项目（Meegle / Meego，原飞书项目）操作工具。支持查询和管理工作项、节点流转、视图查询、个人待办、排期统计等功能。 Use when
  user needs to work with Feishu/Lark Meego project management — including
  querying work items, creating/updating work items, completing workflow nodes,
  checking views, listing todos, analyzing schedules/workloads, or searching
  with MQL. 关键词：飞书项目、meego、meegle、工作项、需求、任务、缺陷、排期、视图、待办、节点。
version: 1.0.19
parentSkill: lark-unified
...
metadata:
  requires:
    bins:
      - meegle
  cliHelp: meegle --help
  alignedWith: larksuite/meegle-cli v1.0.19
  upstream: https://github.com/larksuite/meegle-cli/tree/main/skills/meegle
...
> **这是 `lark-unified` 飞书套件的按需子技能。** 它使用独立的 `meegle` 二进制和独立的登录凭据，
> 与 `lark-cli`（消息 / 文档 / 表格 / 日历 / 邮件等）互不相通。若当前请求其实属于飞书协作而非
> 飞书项目，先回父技能 [lark-unified 第 0 节](../../SKILL.md) 重新分流。
>
> 相对上游 `larksuite/meegle-cli` 的改动清单见 [NOTICE.md](NOTICE.md)。

> **前置**：`meegle` 未安装时先按父技能第 0.3 节按需安装（务必带 `--no-skills`）。
...
> **CLI 使用指南**（命令结构、参数传递、命令发现、退出码）：见 [references/cli-guide.md](references/cli-guide.md)
...
> ⚠️ **命令名带 `+` 前缀**：真实注册名是 `workitem +batch-get`（别名 `+get-batch`），上游文档里写的
> `workitem batch-get` 不存在。`+` 标记这是客户端封装命令（背后没有 1:1 的 MCP tool），源码见
> `internal/products/meegle/batch.go`。同类还有 `attachment +upload` / `attachment +download`。
