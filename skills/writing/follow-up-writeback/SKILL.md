---
name: follow-up-writeback
version: 1.4.0
description: "Follow-up Record Writeback workflow for foreign-trade teams, with explicit evidence, controlled CRM access, confirmation, and safety boundaries."
description_zh: "通过objFieldData新增或更新followUpRecord，强制核对跟进对象ID、对象数据ID、发生时间和内容，确认后写入并同步回查。适用于用户要求跟进记录写回、生成相关结果或检查执行边界时。"
allowed-tools: Bash
---
...
定位：通过objFieldData新增或更新followUpRecord，强制核对跟进对象ID、对象数据ID、发生时间和内容，确认后写入并同步回查。边界：不猜测同名对象关联。
...
- 必填预览字段：objectApiKey、objectDataId、occurredAt、content、负责人、来源纪要。
- 同名对象不能作为关联依据；对象ID、内容或时间变化后必须重新确认。
- 输出：目标记录｜写入内容｜来源依据｜缺失项｜dry-run差异｜确认状态｜同步回查结果；只有回查一致才可表述写入成功。

## 实战使用卡

### 可直接复制的任务示例

> 为customer dataId=100234生成followUpRecord dry-run：2026-07-18T10:30:00+08:00，内容“客户确认500件样品方案，下周反馈颜色”，负责人userId=42；展示预览并停在确认门。
