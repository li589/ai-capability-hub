...
### P7 使用 [紧箍咒-REPORT] 和 [紧箍咒-ESCALATION]
...
| P8 判定任务需要"想清楚再做" | P8 通过 dispatch_task 派发 P7 | 子 Agent 读取 role-shawujing-pro.md |
...
### P8 向 P7 子Agent 注入 紧箍咒 协议

P8 通过 dispatch_task 派发 P7 时，在 task 末尾注入：

```
## 紧箍咒 行为注入（P7 模式）
1. 用 Read 工具读取 [SKILL_ROOT]/references/role-rulai-pro.md，按其中的三条红线执行
2. 同步读取 [SKILL_ROOT]/references/role-shawujing-pro.md，按 P7 三步工作法执行
...
当无显式指定风格时，P7 默认使用 **🟣 沙悟净模式**。方案驱动 + 影响分析 + 技术深度 = P7 核心能力，旁白与行为约束完全同构。
