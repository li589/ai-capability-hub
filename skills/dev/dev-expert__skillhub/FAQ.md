...
- **十一个"专项 reference"不支持 `@` 显式调用**：`@laravel-development` / `@laravel-testing` / `@java-development` / `@javascript-development` / `@execution-safety` / `@delivery-assurance` / `@error-ledger` / `@style-alignment` / `@architecture-decision` / `@domain-driven-design` / `@distributed-systems` 只作领域协同资料，用关键词触发即可（见 七、Q5 / 十四、Q1）。
...
> **本节要点**：SELF-AUDIT 16 条、收尾报告 5 字段、Graceful Abort、错误册 ERR-XXX、学习捕获三层闭环。

**Q：SELF-AUDIT 执行率自检是什么？什么时候触发？**
A：交付前强制执行（收口于 Step 5），16 条硬约束（E2E 条目/验收标准/安全/质量/性能/GATE-CHECK/轮次/构建/修改 checkpoints/回退/规划/图谱 G1/G2'/G3/架构自检/证伪）。任一 ❌ 回退对应阶段修复→重走 Step 4→复检，最多 3 轮；超出后列未修复项+原因+选项，不阻塞交付。详见 `delivery-assurance.md`「执行率自检」。
