...
def check_frontmatter(content: str, skill_name: str) -> tuple[bool, list[str]]:
    ...
    # 按行处理 frontmatter
    fm_lines = fm_content.splitlines()
    fm_dict = {}
    for line in fm_lines:
        # 匹配 key: value 格式
        line_match = re.match(r'^([\w_]+):\s*(.*)$', line)
        if line_match:
            key = line_match.group(1)
            value = line_match.group(2).strip().strip('"').strip("'").strip()
            fm_dict[key] = value
    ...
    # 检查 title 字段（可选）
    if 'title' in fm_dict:
        title_value = fm_dict['title']
        if not title_value:
            issues.append("title 字段为空")
    
    # 检查 category 字段（可选）
    if 'category' in fm_dict:
        category_value = fm_dict['category']
        if not category_value:
            issues.append("category 字段为空")
