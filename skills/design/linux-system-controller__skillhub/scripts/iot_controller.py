...
Usage examples:
  ...
  python iot_controller.py homeassistant --url http://192.168.1.100:8123 --token YOUR_TOKEN --action call_service --domain light --service turn_on --entity_id light.living_room
  python iot_controller.py http --url http://192.168.1.50/api --action get --path /status
  python iot_controller.py http --url http://192.168.1.50/api --action post --path /command --body '{"command":"on"}'
"""
...
def main():
    ...
    if args.platform == "homeassistant":
        ...
        token = args.token
        if args.action == "list":
            print(ha_list_entities(url, token))
        elif args.action == "state":
            print(ha_get_state(url, token, args.entity_id))
        elif args.action == "call":
            print(ha_call_service(url, token, args.domain, args.service, args.entity_id, args.data))
        elif args.action == "on":
            print(ha_turn_on(url, token, args.entity_id, args.data))
        elif args.action == "off":
            print(ha_turn_off(url, token, args.entity_id))
        elif args.action == "toggle":
            print(ha_toggle(url, token, args.entity_id))
        else:
            p_ha.print_help()
