#!/usr/bin/env python3
"""Deterministic GPT/Gemini bridge between CLIProxyAPI and WorkBuddy.
...
DEFAULT_STATE_RELATIVE = Path(".config/workbuddy-gpt-gemini-bridge")
LEGACY_STATE_RELATIVE = Path(".config/workbuddy-cli-model-bridge")
PORTABLE_ROOT_RELATIVE = Path(".local/share/workbuddy-gpt-gemini-bridge")
...
def active_workbuddy_proxy() -> dict[str, Any]:
    """Report only redacted proxy facts from a running WorkBuddy CLI process."""
    result: dict[str, Any] = {
        "detected": False,
        "proxy": None,
        "loopback": None,
    ...
    if platform.system() != "Darwin":
        return result
    try:
        completed = run_command(["ps", "eww", "-axo", "command="], timeout=15)
    except BridgeError:
        return result
    for line in completed.stdout.splitlines():
        if "codebuddy --serve" not in line or "/WorkBuddy.app/" not in line:
            continue
        match = re.search(r"\bHTTPS?_PROXY=(https?://[^\s]+)", line, flags=re.IGNORECASE)
        if not match:
        ...
        reachable = loopback_port_open(host, port) if host and loopback else None
        return {
            "detected": True,
            "proxy": f"{parsed.scheme}://{host}:{port}" if host else "[invalid]",
            "loopback": loopback,
            "reachable": reachable,
            "stale": bool(loopback and not reachable),
        }
    return result
...
def latest_release_plan() -> dict[str, str]:
    ...
    request = urllib.request.Request(
        ...
        headers={"Accept": "application/vnd.github+json", "User-Agent": "workbuddy-gpt-gemini-bridge"},
    )
    try:
...
def download_file(url: str, destination: Path, *, max_bytes: int) -> None:
    request = urllib.request.Request(url, headers={"User-Agent": "workbuddy-gpt-gemini-bridge"})
    total = 0
    try:
...
def install_windows_release(home: Path, plan: dict[str, str]) -> tuple[str, str]:
    ...
    with tempfile.TemporaryDirectory(prefix="workbuddy-cliproxy-") as temporary:
...
def windows_startup_path(home: Path) -> Path:
    ...
    return root / "Microsoft/Windows/Start Menu/Programs/Startup/workbuddy-gpt-gemini-bridge.cmd"
...
def start_windows_proxy(binary: str, config: Path, home: Path) -> Path:
    ...
    content = f'@echo off\r\nrem Managed by workbuddy-gpt-gemini-bridge\r\nstart "" /min {command}\r\n'
    if startup.is_file() and "Managed by workbuddy-gpt-gemini-bridge" not in startup.read_text(encoding="utf-8", errors="ignore"):
...
def cmd_audit(args: argparse.Namespace) -> int:
    ...
    workbuddy_document = load_json(workbuddy, default=[])
    if isinstance(workbuddy_document, list):
        workbuddy_payload = workbuddy_document
    elif isinstance(workbuddy_document, dict) and isinstance(workbuddy_document.get("models"), list):
        workbuddy_payload = workbuddy_document["models"]
    else:
        workbuddy_payload = []
    managed_ids, state_sources = managed_state(home)
    findings: list[dict[str, str]] = []
    system = platform.system()
    workbuddy_network = active_workbuddy_proxy()
    ...
    elif system != "Windows" and reported_mode(workbuddy) != "0o600":
    ...
    if workbuddy_network.get("stale"):
        findings.append(
            {
                "code": "workbuddy_stale_system_proxy",
                "severity": "error",
                "message": "WorkBuddy is using an unreachable loopback system proxy. Fully quit and reopen WorkBuddy before changing model configuration.",
            }
        )
    emit(
        {
...
def cmd_sync(args: argparse.Namespace) -> int:
    ...
    existing_document = load_json(workbuddy_path)
    if isinstance(existing_document, list):
        ...
        raise BridgeError(
            "invalid_workbuddy_config",
            "WorkBuddy models.json must be an array of model objects or an object containing a models array",
        )
    if not all(isinstance(item, dict) for item in existing):
        raise BridgeError("invalid_workbuddy_config", "WorkBuddy models must contain only model objects")
    ...
    if args.apply and (changes["added"] or changes["updated"]):
        ...
        save_json(workbuddy_path, output_document, mode=0o600)
        save_json(
            state_file,
            {
                "schema_version": SCHEMA_VERSION,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Bridge subscription-backed GPT and Gemini models into WorkBuddy")
