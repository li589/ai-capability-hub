...
tags: [multi-agent, orchestration, role-handoff, review-workflow, long-session, mode-switching]
display_name: "Agent Team Orchestration"
display_name_en: "Agent Team Orchestration"
visibility: "public"
---
...
## Fix → Verify → Confirm Loop

When orchestrating agents for code or configuration fixes, enforce this strict loop to prevent regressions:
1. **Fix**: Before deleting or modifying any file, agents MUST perform a global search for references (e.g., using `Grep` or `Glob`).
2. **Verify**: After applying a fix, agents MUST run tests, compilation, or static checks. Do not proceed to the Review stage if verification fails.
3. **Confirm**: If a regression or new bug is discovered during verification, agents MUST return to the Spec/Planning phase rather than stacking patches on top of broken code.
