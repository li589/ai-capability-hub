---
name: 全能图片理解
description: "全能图片理解助手，支持对单张或多张图片进行内容描述、视觉问答、文字识别与对比分析；当用户要求「识图」「看图」「描述图片」「分析图片」「图中有什么」「提取图片文字」「对比图片」「视觉问答」时使用。"
tags: [图片, 媒体, 视觉]
---
...
| 字段 | 值 |
|------|-----|
| skill_id | `ixhlink-skills-quanneng-image-explain` |
| skill_version | `1.0.0` |
| product_id | `ixhlink-skills-quanneng-image-explain` |
| model_key | `ixhlink-skills-quanneng-image-explain` |
| capability | `vision` |
...
```json
{"model": "ixhlink-skills-quanneng-image-explain", "capability": "vision"}
```
...
{
  "capability": "vision",
  "model": "ixhlink-skills-quanneng-image-explain",
  "payload": {
    "messages": [
      {
        "role": "user",
...
| 产品ID | 服务内容 | 单价 |
|--------|----------|------|
| `ixhlink-skills-quanneng-image-explain` | 全能图片理解 | 0.5 元/次 |
...
{"model":"ixhlink-skills-quanneng-image-explain","capability":"vision","payload":{...}}
```
