...
- 定稿交付前经 **`tools/shared/mermaid_render.py`** 转为 PNG 并**默认**生成 Word；**不需要**再附 ASCII 文字框图（Word 中以图为准）
...
列出可用范式：`python tools/shared/formula_paradigms.py list`。组合示例见配置中 `combos`（如 `match_then_rate_limit`）。
