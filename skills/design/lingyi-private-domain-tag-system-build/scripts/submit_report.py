#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""私域标签体系搭建【零一数科·出品】 —— 报告提交服务端（旁路）

纯标准库（urllib）。在 agent 写完四件套 Markdown 后调用，把报告正文
POST 到服务端做登记/校验。
...
请求体必须携带 scene 以区分场景（服务端用 scene → skill_code 映射做埋点）：

    POST Body: {"content": "<报告Markdown>", "scene": "lingyi-private-domain-tag-system-build", "origin": "01workbuddy", "origin_method": "skill"}

> 现有约定：full_deconstruct=视频拆解、script_generation=脚本生成；
> 本 skill 固定传 lingyi-private-domain-tag-system-build，对应 slug lingyi-private-domain-tag-system-build。

用法:
    python3 submit_report.py <report_md_path> [--timeout 20]
    # 或从 stdin 读报告内容：
    cat report.md | python3 submit_report.py - [--timeout 20]

stdout 输出（始终输出，供 agent 解析）:
    === TAG_SYSTEM_SUBMIT_START ===
    { "ok": true|false, "status": "checked|skipped", "http_status": 200,
      "result": <接口返回的 JSON 或原始文本>, "reason": "失败原因（仅 ok=false 时）" }
    === TAG_SYSTEM_SUBMIT_END ===
...
# 场景标识（入库字段）；服务端 _SCENE_SKILL_CODE 映射到 skill_code。
SCENE = "lingyi-private-domain-tag-system-build"

RESULT_START = "=== TAG_SYSTEM_SUBMIT_START ==="
RESULT_END = "=== TAG_SYSTEM_SUBMIT_END ==="


def log(msg: str) -> None:
    print(f"[submit_report] {msg}", file=sys.stderr, flush=True)
...
def call_submit_api(content: str, timeout: float) -> dict:
    ...
    scene 固定为 lingyi-private-domain-tag-system-build，标识本 skill 场景，与拆解/脚本生成区分。
