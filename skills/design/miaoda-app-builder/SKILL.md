...
display_name: 应用开发（秒哒）
display_name_en: MiaoDa
description: 通过自然语言对话在秒哒平台创建、生成、修改和部署网站、Web应用、安卓App、iOS
  App、数据看板、SaaS产品、内部工具、互动网页、微信小程序及游戏等全栈产品。
description_zh: 通过自然语言对话在秒哒平台创建、生成、修改和部署网站、Web应用、安卓App、iOS
  App、数据看板、SaaS产品、内部工具、互动网页、微信小程序及游戏等全栈产品。
description_en: Create, modify, generate, and deploy websites, web apps, Android
  apps, iOS apps, dashboards, SaaS products, internal tools, interactive web
  pages, Weixin mini program, games on the Miaoda platform using
...
The CLI script auto-loads `MIAODA_API_KEY` from the `.env` file in the skill directory on startup. **Do NOT search the filesystem or check environment variables yourself** — just run the command directly. The script will load the key automatically or report an error if it's missing.

**When to show the guide image** at `references/get_apikey.jpg`:
- A command returns `IAM access key validation failed` or `Error: API key required` (i.e., the key is missing or invalid)
- User asks "how to get an API key" or "how to configure miaoda skill"

![How to get API_KEY and configure to WorkBuddy](references/get_apikey.jpg)

The image covers all 4 steps:

1. **登录秒哒** — Visit miaoda.cn and complete login authorization
2. **进入秒哒 Skill** — Click avatar (top-left) → open menu → click 秒哒 Skill
3. **获取 API_KEY** — Click the "连接 OpenClaw" / "连接 WorkBuddy" tab → copy API_KEY → save it (each user has one valid key)
4. **在 WorkBuddy 中配置秒哒** — Paste API_KEY into WorkBuddy chat to complete configuration

After the user provides their API key, the skill is ready to use.
...
**Important**: The `MIAODA_API_KEY` is auto-loaded from the `.env` file in the skill directory. If a `.env` file exists with `MIAODA_API_KEY=sk_xxxxx`, it is loaded automatically — no manual `export` needed.
...
```bash
export MIAODA_API_KEY="sk_xxxxx"
cd ~/.workbuddy/skills/miaoda-app-builder
...
1. Show the user the API key configuration guide image at `references/get_apikey.jpg` to walk them through obtaining and configuring the key:

   ![How to get API_KEY and configure to WorkBuddy](references/get_apikey.jpg)

2. Or visit the Miaoda(秒哒) official website directly:

   https://www.miaoda.cn

3. After obtaining the key, configure it via one of these methods:
   - Set as an environment variable: `export MIAODA_API_KEY="sk_xxxxx"`
   - Or create a `.env` file in the skill directory: `echo "MIAODA_API_KEY=sk_xxxxx" > ~/.workbuddy/skills/miaoda-app-builder/.env`

The `.env` file is auto-loaded by the CLI script on startup.
