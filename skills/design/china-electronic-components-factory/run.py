"""
china-electronic-components-sourcing - OpenClaw skill for China electronic components industry intelligence
...
def get_industry_overview() -> Dict:
    """Return overview of China's electronic components industry scale and targets."""
    return COMPONENTS_DATA.get("industry_overview", {})

def get_supply_chain_structure() -> Dict:
    """Return the complete electronic components supply chain structure."""
    return COMPONENTS_DATA.get("supply_chain_structure", {})
...
def get_regional_clusters(region: Optional[str] = None) -> Union[List[Dict], Dict]:
    ...
    """
    clusters = COMPONENTS_DATA.get("regional_clusters", [])
...
def get_subsector_info(subsector: str) -> Dict:
    ...
    subsectors = COMPONENTS_DATA.get("subsector_details", {})
    ...
    # Try keyword mapping for common component types
    keyword_mapping = {
        "mlcc": "passive_components",
        "capacitor": "passive_components",
        "resistor": "passive_components",
        "inductor": "passive_components",
        "chip": "semiconductors",
        "ic": "semiconductors",
...
def get_sourcing_guide() -> Dict:
    ...
    guide = COMPONENTS_DATA.get("sourcing_guide", {})
...
def get_glossary(term: Optional[str] = None) -> Union[Dict, str]:
    ...
    glossary = COMPONENTS_DATA.get("glossary", {})
...
def search_data(query: str) -> List[Dict]:
    ...
    for cluster in COMPONENTS_DATA.get("regional_clusters", []):
    ...
    for subsector_name, subsector_data in COMPONENTS_DATA.get("subsector_details", {}).items():
...
def get_metadata() -> Dict:
    ...
    return COMPONENTS_DATA.get("metadata", {})

# --- If run directly, provide interactive example ---
if __name__ == "__main__":
    print("🔧 China Electronic Components Sourcing Skill (CLI Test Mode)")
    print("=" * 60)
    ...
    print('  find_clusters_by_specialization("automotive semiconductors")')