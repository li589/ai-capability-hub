#!/usr/bin/env python3
"""
Zhihuiya Patent Expression Search - LinkFox Skill
调用 /zhihuiya/querySearchPatent 接口。

Usage:
  python zhihuiya_query_search_patent.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python zhihuiya_query_search_patent.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
API_PATH = "/zhihuiya/querySearchPatent"
SLUG = "linkfox-zhihuiya-retrieval-patent-search"
...
def main():
    ...
    if not argv:
        print(
            "Usage: zhihuiya_query_search_patent.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
