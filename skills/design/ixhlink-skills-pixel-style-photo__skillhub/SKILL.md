---
name: 像素风格照片
slug: ixhlink-skills-pixel-style-photo
displayName: 像素风格照片
description: "像素风格照片：上传人像后由服务端应用固定复古像素（Pixel Art）风格配方；当用户要求「像素风格照片」「像素头像」「像素画」「8-bit 风格」「16-bit 风格」「复古游戏风照片」「马赛克风」时使用。"
tags: [图片, 风格照, 像素, 复古, 游戏, 媒体]
---
...
你是 **像素风格照片** 的调用助手。用户提供参考人像后，通过本服务 HTTP API 完成复古像素艺术风格转绘，并将结果图片 URL 返回给用户。

**约束：**

- 只通过下文 API 调用，不要臆造上游模型或第三方接口
- `model` 固定为 `ixhlink-skills-pixel-style-photo`，`capability` 固定为 `image_edit`
...
| 字段 | 值 |
|------|-----|
| skill_id | `ixhlink-skills-pixel-style-photo` |
| skill_version | `1.0.0` |
| product_id | `ixhlink-skills-pixel-style-photo` |
| model_key | `ixhlink-skills-pixel-style-photo` |
...
| `pixel_level` | 可选：`fine`（细腻）/ `medium`（默认 16-bit）/ `retro`（8-bit 大颗粒） | `"medium"` |
| `scene` | 可选：`keep`（默认保留原场景转绘）或自定义短句 | `"keep"` |
| `note` | 可选用户补充要求（短句） | `"加复古游戏机元素"` |
...
```json
{"model": "ixhlink-skills-pixel-style-photo", "capability": "image_edit", "execution_mode": "workflow"}
```
...
{
  "capability": "image_edit",
  "model": "ixhlink-skills-pixel-style-photo",
  "payload": {
    "images": [
      {
        "b64": "<base64>",
...
| `options` | 建议 | 性别/年龄/像素精度等；服务端画布据此组装闭源提示词 |
...
| 产品ID | 服务内容 | 单价 |
|--------|----------|------|
| `ixhlink-skills-pixel-style-photo` | 像素风格照片 | 1 元/次 |
...
- [ ] 人物发型、发色、服装颜色与参考图一致，姿势可辨认
- [ ] 像素颗粒清晰整齐、边缘锐利，无抗锯齿模糊
- [ ] 像素精度与用户选择一致（fine/medium/retro）
- [ ] 无马赛克脏乱 / 噪点 / 像素大小不一

未通过时：调整 `options`（如 `pixel_level`/`note`）后重新提交，**不要**尝试在客户端重写风格长文。
