#!/bin/bash
# =============================================================================
# inject-agent.sh - Inject agents at workflow hook points (3-level support)
# =============================================================================
#
# DESCRIPTION:
#   Manages agent injection at hook points with 3 configuration levels:
...
#
#   Agents are AI sub-agents that can be invoked during workflow stages.
#   Unlike skills (which are prompt-based), agents perform autonomous tasks.
#
# USAGE:
#   ./scripts/inject-agent.sh -r <root> --hook <hook> --agent <agent> [options]
#
# OPTIONS:
...
#   --agent AGENT           Agent name to inject (REQUIRED for inject/remove)
#   --config CONFIG         Agent configuration (JSON string)
#   --required              Make the agent required (blocks on failure)
#   --remove                Remove the agent from the hook
#   --list                  List all injected agents (merged from all levels)
#   --list-scope SCOPE      List agents for specific scope only
#   -h, --help              Show help message
#
# AVAILABLE HOOKS:
#   pre_stage_{STAGE}       Before entering a stage (e.g., pre_stage_ANALYZING)
...
#
# BUILT-IN AGENTS:
#   risk-auditor            PRD/feature risk assessment
#   iron-audit-pm           PRD audit and feature pruning
#   mvp-freeze-architect    MVP scope definition and feature freezing
#   code-reviewer           Code review agent (post-implementation)
#   tech-design-reviewer    Technical design review
#
# CONFIG FILES:
#   Global:   {skill_dir}/hooks.yaml
#   Project:  {project_root}/.trae/workflow/hooks.yaml
#   Workflow: {workflow_dir}/workflow.yaml (agents section under hooks)
#
# EXAMPLES:
#   # Inject risk-auditor at ANALYZING stage (global)
#   ./scripts/inject-agent.sh -r /project --scope global \
#     --hook pre_stage_ANALYZING --agent risk-auditor
#
#   # Inject MVP architect at PLANNING stage
#   ./scripts/inject-agent.sh -r /project --scope global \
#     --hook post_stage_ANALYZING --agent mvp-freeze-architect
#
#   # List all agents
#   ./scripts/inject-agent.sh -r /project --list
#
# =============================================================================
set -euo pipefail
...
show_help() {
  cat << EOF
Usage: $(basename "$0") -r <root> --hook <hook> --agent <agent> [OPTIONS]

Inject agents at workflow hook points with 3-level configuration support.

Agents vs Skills:
    Skills are prompt-based tools that provide context/templates
    Agents are autonomous sub-agents that perform complex tasks
...
Examples:
    $(basename "$0") -r /project --scope global --hook pre_stage_ANALYZING --agent risk-auditor
    $(basename "$0") -r /project --scope global --hook post_stage_ANALYZING --agent mvp-freeze-architect
...
ensure_hooks_file() {
  ...
  if [[ ! -f "$file" ]]; then
    cat > "$file" << 'EOF'
# Hooks Configuration
# Skills and agents to be injected at specific hook points
hooks: {}
agents: {}
EOF
  fi
...
inject_agent_to_file() {
  local target_file="$1"
  local hook="$2"
  local agent="$3"
  local config="${4:-}"
  local required="${5:-false}"
  local is_workflow_file="${6:-false}"
  local timestamp
  ...
  sed 's/^agents: {}$/agents:/' "$target_file" > "$temp_file"

  local agents_found=0
  local hook_found=0
  local agent_inserted=0

  while IFS= read -r line || [[ -n "$line" ]]; do
    if [[ "$line" == "agents:" && $agents_found -eq 0 ]]; then
      agents_found=1
      echo "$line" >> "$output_file"
      continue
    fi
    
    if [[ $agents_found -eq 1 && "$line" == "  ${hook}:" && $agent_inserted -eq 0 ]]; then
      hook_found=1
      echo "$line" >> "$output_file"
      continue
    fi
    
    if [[ $hook_found -eq 1 && $agent_inserted -eq 0 ]]; then
      echo "    - agent: \"$agent\"" >> "$output_file"
  ...
  if [[ $agent_inserted -eq 0 ]]; then
    local final_output
    final_output=$(mktemp)
    local agents_line_found=0
    while IFS= read -r line || [[ -n "$line" ]]; do
      echo "$line" >> "$final_output"
      if [[ "$line" == "agents:" && $agents_line_found -eq 0 ]]; then
        echo "  ${hook}:" >> "$final_output"
        echo "    - agent: \"$agent\"" >> "$final_output"
...
list_agents_from_file() {
  local file="$1"
  local scope="$2"
  local indent="${3:-}"
  
  if [[ ! -f "$file" ]]; then
    echo "${indent}(no agents configured)"
    return
  fi
...
list_all_agents() {
  ...
  echo "═══════════════════════════════════════════════════════"
  echo "🤖 Injected Agents Configuration"
  echo "═══════════════════════════════════════════════════════"
  echo ""
  ...
  echo "-----------------------------------------------------------"
  list_agents_from_file "$global_file" "global" "  "
  echo ""
  ...
  echo "-----------------------------------------------------------"
  list_agents_from_file "$project_file" "project" "  "
  echo ""
  ...
  if [[ -n "$workflow_dir" && -d "$workflow_dir" ]]; then
    ...
    echo "-----------------------------------------------------------"
    list_agents_from_file "$workflow_dir/workflow.yaml" "workflow" "  "
  else
...
list_scope_agents() {
  ...
  echo "═══════════════════════════════════════════════════════"
  echo "🤖 $scope_label Agents"
  echo "═══════════════════════════════════════════════════════"
  echo ""
  list_agents_from_file "$file" "$scope" ""
  echo ""
}
...
main() {
  ...
  if [[ -z "$workflow_dir" ]]; then
    workflow_dir=$(get_active_workflow "$project_root") || true
  fi
  ...
  if [[ -n "$list_scope" ]]; then
    ...
    list_scope_agents "$list_scope" "$project_root" "$workflow_dir"
    exit 0
  fi

  if [[ $list -eq 1 ]]; then
    list_all_agents "$project_root" "$workflow_dir"
    exit 0
  fi
  ...
  if [[ $remove -eq 1 ]]; then
    ...
    echo "   Remove agent '$agent' from hook '$hook'"
    exit 0
  fi

  inject_agent_to_file "$target_file" "$hook" "$agent" "$config" "$required" "$is_workflow_file"
  ...
  echo "✅ Injected agent '$agent' at hook '$hook' (scope: $scope_label)"
