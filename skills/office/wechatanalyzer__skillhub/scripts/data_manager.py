...
class DataManager:
    ...
    def __init__(self, config: Dict[str, Any]):
        self.config = config
        # 默认存到 data/sessions/（package.py 与 gitignore 均忽略，避免运行时 db 混入交付包）
    ...
    def _parse_line(self, line: str) -> Optional[Dict[str, Any]]:
        """解析单行聊天记录"""
        # v2.5.0：先过滤非消息行——纯时间戳行（导出文件分隔行）与分隔线。
        # 必须在格式3之前判断，否则 "2024-01-01 12:30:00" 会被冒号规则
        # 误解析成 sender="2024-01-01 12"。
        ...
        # 格式4: 无发送者的裸文本（按设计约定视为自己的消息）
        # v2.5.0 bugfix：之前 sender 写成 'other'（与注释相反，导致 self/other
        # 统计颠倒、情感/人格分析对象错误）；len(line) < 100 在正则 {1,20}
        # 下永远不可达，一并清理。
        match = re.match(r'^(.{1,100})$', line)
        if match:
            return {
                'timestamp': datetime.now().isoformat(),
                'sender': 'self',
