...
向 IE、ES、FR、BE、NL、DE、IT、SE 与 PL 站点卖家提供终端用户（顾客）数据。数据包含顾客个人信息，包括联系方式、页面浏览（glance view）与订单数据，且仅针对选择向特定卖家共享数据的顾客。顾客数据可按不同报告周期（DAY、WEEK、MONTH）获取。开发者可指定起止日期与报告周期以进行数据获取与聚合。若顾客未选择共享其数据，则报告不生成数据。
