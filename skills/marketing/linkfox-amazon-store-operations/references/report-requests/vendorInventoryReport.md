...
- **原始**：[vendorInventoryReport.json](https://raw.githubusercontent.com/amzn/selling-partner-api-models/main/schemas/reports/vendorInventoryReport.json)
- **目录浏览**：https://github.com/amzn/selling-partner-api-models/tree/main/schemas/reports

## Schema 摘要

本报告以聚合维度（覆盖供应商全部商品目录）和按 ASIN 维度共享供应商商品的库存数据。数据支持不同的日期范围聚合粒度：DAY、WEEK、MONTH、QUARTER、YEAR。请求可跨越多个日期范围周期。例如，若客户指定 `reportPeriod=WEEK` 且 `dataStartTime` 与 `dataEndTime` 相隔 3 周，则报告将包含这 3 个连续周各自的数据。
