...
购物篮分析报告包含在结账时与顾客购物篮（cart）中商品最常被一起购买的商品数据。数据可按不同报告周期获取：DAY、WEEK、MONTH 与 QUARTER。单次请求可跨越多个报告周期。本报告中，`asin` 为卖家目录中的 ASIN，`purchasedWithAsin` 可能是、也可能不是卖家目录中的 ASIN。
