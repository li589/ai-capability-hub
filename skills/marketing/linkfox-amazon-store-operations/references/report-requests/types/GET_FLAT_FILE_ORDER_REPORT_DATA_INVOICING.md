...
- **要点**：本报告可用于在 EU 区域生成税务发票。本报告展示近 60 天的订单。Seller Central 卖家同一时间只能定时一个 GET_ORDER_REPORT_DATA_INVOICING 或 GET_FLAT_FILE_ORDER_REPORT_DATA_INVOICING 报告；若已定时其中一个报告后又定时新报告，原有报告将被取消。📘注意本报告的格式会因定时或按需请求而略有不同。
- **reportOptions（摘录）**：
