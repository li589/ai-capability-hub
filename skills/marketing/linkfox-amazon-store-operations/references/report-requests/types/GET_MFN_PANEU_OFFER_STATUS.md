...
- **站点可用范围**：西班牙、英国、法国、德国和意大利站点。
- **请求/调度**：本报告仅支持按需请求。
- **报告输出类型**：制表符分隔的扁平文件
- **要点**：包含卖家每条自配送 Listing 在泛欧 FBA 计划中的资格信息。自配送 Listing 不得加入泛欧 FBA 计划，本报告可帮助卖家判断是否将自配送 Listing 转为亚马逊配送 Listing 以便加入该计划。更多信息请参见 Seller Central 帮助。
