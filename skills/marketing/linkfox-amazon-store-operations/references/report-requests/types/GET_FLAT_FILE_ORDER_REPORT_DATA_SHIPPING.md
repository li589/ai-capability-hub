...
- **要点**：包含近 60 天的订单。本报告可用于发货 Amazon 订单。Seller Central 卖家同一时间只能定时一个 GET_ORDER_REPORT_DATA_SHIPPING 或 GET_FLAT_FILE_ORDER_REPORT_DATA_SHIPPING 报告；若已定时其中一个报告后又定时新报告，原有报告将被取消。📘注意本报告的格式会因定时或按需请求而略有不同。本报告接受以下 reportOptions 值：
- **reportOptions（摘录）**：
  - ShowSalesChannel：布尔值，指示是否在报告中新增一列以展示销售渠道。默认值：false。示例：`"reportOptions":{"ShowSalesChannel":"true"}`
