...
- **可用范围**：Seller Central 卖家
- **报告输出类型**：制表符分隔的平面文件
- **要点**：结算报告。本报告替代 Flat File Settlement Report 和 XML Settlement Report。价格列被合并为三列通用列：`amount-type` 与 `amount-description` 列包含所产生费用或扣款的类型，并指示不同费用是否同属一类。金额始终填充在单一金额列中，因此无需解析行来查找数字，即使将来引入新的费用或交易类型也是如此。此外，金额以本地货币格式显示（例如欧盟显示为 95,00 而非 95.00）。仅面向 Seller Central 卖家。
