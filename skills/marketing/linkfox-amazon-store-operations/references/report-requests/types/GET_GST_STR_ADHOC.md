...
- **报告输出类型**：逗号分隔的扁平文件
- **站点可用范围**：IN
- **用户类型**：使用亚马逊系统进行亚马逊运营中心（FC）库存调拨（FC 移除或 FC 间调拨）的 FBA 卖家。卖家和供应商申报税务时必需。
- **要点**：提供交易级别的信息，如发票金额、发票号、税率以及商品和服务税识别号（GSTIN）。所有涉及跨邦调拨的供应均视为应税供应。此类供应在起始地缴纳商品和服务税（GST），在目的地获得 GST 抵扣。
