...
- **请求/调度**：调度
- **报告输出类型**：XML
- **要点**：该报告可用于在欧盟地区生成税务发票。报告包含自上次报告以来的所有新订单。对于新报告，包含过去 38 天的订单报告。同一时间只能调度一个 `GET_ORDER_REPORT_DATA_INVOICING` 或 `GET_FLAT_FILE_ORDER_REPORT_DATA_INVOICING` 报告。若已调度其中一个并再次调度新报告，现有报告将被取消。
