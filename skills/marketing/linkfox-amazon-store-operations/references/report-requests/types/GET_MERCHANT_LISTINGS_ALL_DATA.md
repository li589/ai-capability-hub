...
- **要点**：详细的全部 Listing 报告。📘注意每个报告只能指定一个站点。当请求中传入多个 marketplaceIds 时，仅接受列表中的第一个 marketplaceId。本报告接受以下 reportOptions 取值：
- **reportOptions（摘录）**：
  - Custom：布尔值，指示是否返回自定义报告。默认：false。示例："reportOptions":{"custom":"true"}。
