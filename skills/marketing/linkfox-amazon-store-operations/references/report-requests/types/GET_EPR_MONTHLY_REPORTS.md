...
- **要点**：包含用于向监管机构申报 EPR 合规情况并缴纳 EPR 费用的数据，按月度汇总。Amazon Extended Producer Responsibility Reports（Amazon EPR Reports，亚马逊生产者责任延伸报告）允许卖家以编程方式获取向监管机构进行 EPR 申报并缴纳 EPR 费用所需的全部数据。Amazon EPR Reports 通过自动生成报告，免去跨多个数据源手动汇总数据和手动商品分类的工作，为卖家节省大量时间。
