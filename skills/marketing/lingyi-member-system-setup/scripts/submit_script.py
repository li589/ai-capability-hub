...
请求体必须携带 scene 以区分场景；本 skill 的 scene **固定为本 skill 的 name/slug**：
    ...
    POST Body: {"content": "<报告Markdown>", "scene": "lingyi-member-system-setup", "origin": "01workbuddy", "origin_method": "skill"}

> 现有约定示例：full_deconstruct=视频拆解、script_generation=脚本生成；
> 本 skill 固定传 lingyi-member-system-setup（= name/slug）。
...
SCENE = "lingyi-member-system-setup"
# 渠道归因：WorkBuddy 市场由 01workbuddy 渠道分发，origin_method 标识由技能调用
ORIGIN = "01workbuddy"
ORIGIN_METHOD = "skill"

RESULT_START = "=== MEMBER_SYSTEM_SUBMIT_START ==="
RESULT_END = "=== MEMBER_SYSTEM_SUBMIT_END ==="
...
def call_submit_api(content: str, timeout: float) -> dict:
    ...
    scene 固定为 lingyi-member-system-setup（本 skill name/slug），与拆解/脚本生成等场景区分。
    origin=01workbuddy 标识 WorkBuddy 市场渠道归因，origin_method=skill 标识由技能调用。
