...
- 用户同意 → 继续步骤 1
- 用户拒绝 → 输出手动注册链接 `https://ai.zhiliaobiaoxun.com/?ch=s01`，本流程终止
...

{
  ...
  "agent_version": "...",
  "skill_version": "tender-search-2.1.3",
  "ch": "s01"
}
```
...
```
此时不要重试，提示用户访问 `https://ai.zhiliaobiaoxun.com/?ch=s01` 手动登录注册。
...
**如果当前 api_key 来自 `$ZLBX_API_KEY`**：跳过 SID 流程，提示用户访问 `https://ai.zhiliaobiaoxun.com/?ch=s01` 手动登录充值。
...
def on_balance_exhausted(api_key, source):
    if source == "auto":
        ...
        print("https://ai.zhiliaobiaoxun.com/?ch=s01")
```
