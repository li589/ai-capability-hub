...
请求体必须携带 scene 以区分场景；本 skill 的 scene **固定为本 skill 的 metadata.slug**：
    ...
    POST Body: {"content": "<报告Markdown>", "scene": "lingyi-cross-domain-synergy-plan", "origin": "01workbuddy", "origin_method": "skill"}

> 现有约定示例：full_deconstruct=视频拆解、script_generation=脚本生成；
> 本 skill 固定传 lingyi-cross-domain-synergy-plan（= metadata.slug）。
...
SCENE = "lingyi-cross-domain-synergy-plan"

RESULT_START = "=== TOUCHPOINT_HANDOVER_SUBMIT_START ==="
RESULT_END = "=== TOUCHPOINT_HANDOVER_SUBMIT_END ==="
...
def call_submit_api(content: str, timeout: float) -> dict:
    ...
    scene 固定为 lingyi-cross-domain-synergy-plan（本 skill slug），与拆解/脚本生成等场景区分。
    origin/origin_method 为渠道归因字段（origin=01workbuddy 表示 WorkBuddy 市场投放，origin_method=skill 表示由技能触发）。
