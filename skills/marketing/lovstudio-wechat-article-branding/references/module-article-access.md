---
name: "lovstudio-wechat-branding-article-access"
description: "获取微信公众号文章的真实正文、结构、元数据和封面，并提供安全写回入口；用于 Branding 管线中的“读取当前文章”或 \"acquire this WeChat article\" 阶段。"
version: "0.1.0"
author: "lovstudio"
source_type: "git"
...
1. 优先调用已安装的 `lovstudio-wechat-article-operator`。
...
`lovstudio-wechat-article-operator` 优先；缺少时使用当前运行时可用的已登录浏览器自动化能力。
