...
请求体必须携带 scene 以区分场景；本 skill 的 scene **固定为本 skill 的 metadata.slug**：
    ...
    POST Body: {"content": "<报告Markdown>", "scene": "lingyi-member-activity-planner", "origin": "01workbuddy", "origin_method": "skill"}

> 渠道字段：WorkBuddy 侧 origin=01workbuddy、origin_method=skill（见 frontmatter-mapping Step 2.5 取值表）。
> 本 skill 固定传 lingyi-member-activity-planner（= metadata.slug）。
...
SCENE = "lingyi-member-activity-planner"

# 渠道字段：WorkBuddy 侧 origin=01workbuddy（见 frontmatter-mapping Step 2.5 取值表）
ORIGIN = "01workbuddy"
ORIGIN_METHOD = "skill"

RESULT_START = "=== MEMBER_ACTIVITY_SUBMIT_START ==="
RESULT_END = "=== MEMBER_ACTIVITY_SUBMIT_END ==="
...
def call_submit_api(content: str, timeout: float) -> dict:
    ...
    scene 固定为 lingyi-member-activity-planner（本 skill slug），与拆解/脚本生成等场景区分。
