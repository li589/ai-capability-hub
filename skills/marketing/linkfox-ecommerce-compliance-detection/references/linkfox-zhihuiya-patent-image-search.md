...
本技能引导你使用智慧芽专利数据库进行图片相似度检索，帮助用户识别潜在相似的**外观设计专利**。本技能仅支持外观设计专利；实用新型专利检索请使用 `linkfox-zhihuiya-utility-patent-image-search`。参数与响应字段详见 [references/api.md](references/api.md)。
...
- 仅支持外观设计专利（`D`，默认）。
- 支持按受理局国家、洛迦诺分类（LOC）、法律状态、申请人、申请/公开日期等多维筛选。
- 用于侵权排查、外观风险检测、先行技术调研等场景。
...
- **专利类型匹配**：`model` 须与 `patentType` 匹配——本技能仅外观专利（`D`），模型 1-2；实用新型专利（`U`，模型 3-4）请使用 `linkfox-zhihuiya-utility-patent-image-search`。
- **不在范围内**：基于文本的专利检索（关键词/摘要/权利要求检索）；实用新型专利检索；专利法律状态监控或年费管理；专利估值或许可谈判；FTO（自由实施）法律意见；专利族或引证分析。
...
| 类型 | 代码 | 说明 |
|------|------|------|
| 外观设计专利 | `D` | 保护产品的装饰性外观（默认） |

**检索模型**（外观专利）：

| 模型 ID | 专利类型 | 策略 | 推荐 |
|---------|----------|------|------|
| 1 | 外观（`D`） | 智能联想 | 推荐用于外观专利 |
| 2 | 外观（`D`） | 搜索此图 | 精确视觉匹配 |

> 实用新型专利（模型 3/4）不在本技能支持范围，请使用 `linkfox-zhihuiya-utility-patent-image-search`。
...
2. **展示完整信息**：汇总结果或生成报告时，逐条专利包含以下全部字段，不得省略或缩写：申请号（`apno`）、专利名称（中文，使用 `lang: cn` 或提供翻译）、发明人（`inventor`）、命中的专利附图（`url`）、图片列表中的**每一张**专利图片、专利摘要、专利说明书、LOC 分类信息（`loc`）、雷达结果（`radarResult`，如有）、专利规格说明。
...
| 用户说 | 场景 |
|--------|------|
| "检查我的产品设计是否侵权专利" | 外观专利侵权排查 |
| "搜索相似的外观专利" | 外观专利相似度检索 |
| "找和这张图相似的专利" | 视觉专利查询 |
| "我的产品外观有没有相似专利" | 外观风险评估 |
| "检查这个产品在中美的专利风险" | 多国专利排查 |
| "找这个类别下的有效外观专利" | 带筛选的专利检索 |
| "谁持有和这个设计相似的专利" | 竞品专利发现 |
