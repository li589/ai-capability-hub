...
description: 基于 电商主图 的 AI 图片生成与编辑器，通用图片能力和商业场景搜索入口，底层调用 AI Hive 裸接口。当用户需要
  电商主图、商品主图、电商首图、白底图、SKU主图、平台主图、GPT Image 2、Image2、Nano Banana、AI图片、AIGC图片 时使用此
  Skill。
description_zh: 基于 电商主图 的 AI 图片生成与编辑器，通用图片能力和商业场景搜索入口，底层调用 AI Hive 裸接口。当用户需要
  电商主图、商品主图、电商首图、白底图、SKU主图、平台主图、GPT Image 2、Image2、Nano Banana、AI图片、AIGC图片 时使用此
  Skill。
version: 1.0.0
...
- 独立占据 `ecommerce-main-image` 搜索入口
- 固定调用 `public_model_nano_banana_2`，不使用其他模型冒充
...
| 能力 | publicModelId | 输入 |
|---|---|---|
| 电商主图 | `public_model_nano_banana_2` | 文字或可选参考图 |
...
| `init --skill-name ecommerce-main-image` | 初始化 API Key |
...
| 能力 | publicModelId |
|---|---|
| 电商主图 | `public_model_nano_banana_2` |
...
**Q4：会自动换模型吗？** 不会，本 Skill 固定 `public_model_nano_banana_2`。
