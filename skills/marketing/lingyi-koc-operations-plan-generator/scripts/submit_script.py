#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""KOC运营方案生成 —— 报告提交服务端（旁路）

纯标准库（urllib）。在 agent 写完八章方案 Markdown 后调用，把方案正文
POST 到服务端做登记/完整性校验。

设计原则——**绝不阻塞方案交付**：
  - 无论网络异常、超时、接口非 2xx、返回体无法解析，都只打日志并以退出码 0 结束；
  - 提交只是「锦上添花」，失败时方案本身已生成并展示给用户，提交结果可缺省。
...
请求体必须携带 scene 以区分场景；本 skill 的 scene **固定为本 skill 的 metadata.slug**：
    ...
    POST Body: {"content": "<方案Markdown>", "scene": "lingyi-koc-operations-plan-generator", "origin": "01workbuddy", "origin_method": "skill"}

> 现有约定示例：full_deconstruct=视频拆解、script_generation=脚本生成；
> 本 skill 固定传 lingyi-koc-operations-plan-generator（= metadata.slug）。

用法:
    python3 submit_script.py <方案_md_path> [--timeout 20]
    # 或从 stdin 读方案内容：
    cat 方案.md | python3 submit_script.py - [--timeout 20]

stdout 输出（始终输出，供 agent 解析）:
    === KOC_OPS_SUBMIT_START ===
    { "ok": true|false, "status": "checked|skipped", "http_status": 200,
      "result": <接口返回的 JSON 或原始文本>, "reason": "失败原因（仅 ok=false 时）" }
    === KOC_OPS_SUBMIT_END ===

退出码:
    0   总是 0（提交为旁路，不阻塞方案交付）
"""
...
SCENE = "lingyi-koc-operations-plan-generator"
# 渠道字段：WorkBuddy 侧 origin=01workbuddy（见 frontmatter-mapping Step 2.5 取值表）
ORIGIN = "01workbuddy"
ORIGIN_METHOD = "skill"

RESULT_START = "=== KOC_OPS_SUBMIT_START ==="
RESULT_END = "=== KOC_OPS_SUBMIT_END ==="
...
def read_content(src: str) -> "str | None":
    """从文件路径或 stdin（src == '-'）读取方案正文。失败返回 None。"""
    if src == "-":
        try:
...
def call_submit_api(content: str, timeout: float) -> dict:
    ...
    scene 固定为 lingyi-koc-operations-plan-generator（本 skill slug），与拆解/脚本生成等场景区分。
    origin=01workbuddy（WorkBuddy 侧渠道归因）、origin_method=skill。
    任何异常（网络/超时/SSL/非 2xx）都被捕获并转为 {"ok": false, ...}，调用方据此
    降级，绝不抛出——提交是旁路，失败不影响方案交付。
    """
    payload = {"content": content, "scene": SCENE, "origin": ORIGIN, "origin_method": ORIGIN_METHOD}
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="报告提交服务端（旁路，失败不阻塞方案交付）")
    ap.add_argument("report", help="方案 Markdown 文件路径，或 '-' 从 stdin 读取")
