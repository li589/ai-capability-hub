...
class SCRMError(Exception):
    ...
    def __init__(self, message: str, *, details: Optional[dict] = None, error_code: Optional[str] = None) -> None:
...
# call-api 错误类型常量
ERROR_DOC_READ_REQUIRED = "doc_read_required"
ERROR_DOC_VALIDATION_REQUIRED = "doc_validation_required"
ERROR_PATH_PARAM_MISSING = "path_param_missing"
ERROR_AUTH_ERROR = "auth_error"
ERROR_NETWORK_ERROR = "network_error"
ERROR_SERVER_ERROR = "server_error"
ERROR_BUSINESS_ERROR = "business_error"

# 错误类型 → 属性映射
ERROR_TYPE_META = {
    ERROR_DOC_READ_REQUIRED: {"retryable": False, "need_reread_doc": True},
    ERROR_DOC_VALIDATION_REQUIRED: {"retryable": False, "need_reread_doc": True},
    ERROR_PATH_PARAM_MISSING: {"retryable": False, "need_reread_doc": True},
    ERROR_AUTH_ERROR: {"retryable": False, "need_reread_doc": False},
    ERROR_NETWORK_ERROR: {"retryable": True, "need_reread_doc": False},
    ERROR_SERVER_ERROR: {"retryable": True, "need_reread_doc": False},
    ERROR_BUSINESS_ERROR: {"retryable": False, "need_reread_doc": False},
}


class CallApiError(SCRMError):
    """call-api 专用异常，携带标准化的错误类型和详情。"""

    def __init__(self, message, *, error_type, doc_url="", write_operation=False, extra_details=None):
        meta = ERROR_TYPE_META.get(error_type, {})
        details = {
            "retryable": meta.get("retryable", False),
            "need_reread_doc": meta.get("need_reread_doc", False),
            "doc_url": doc_url,
