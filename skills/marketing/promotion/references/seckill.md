...
本文件由 `activity-forms/seckill.json` 转写，用于创建「秒杀」活动时生成活动表单草稿。最终前端工具参数外壳以 `get_web_client_tool_schema(<工具名>)` 返回的 inputSchema 为准。
...
| `secKillPurchaseLimit` | object | 是 |  |  |  | 秒杀购买限制配置 |
| `secKillPurchaseLimit.type` | boolean | 是 | true |  |  | 限购设置: true 每人限购{limitNum}件超出后将无法购买； false 不限购 |
| `secKillPurchaseLimit.limitNum` | integer | 是 |  |  | 最小值: 1<br>最大值: 9999999 | 限制数量 |
