...
本文件由 `activity-forms/multioneprice.json` 转写，用于创建「N元N件」活动时生成活动表单草稿。最终前端工具参数外壳以 `get_web_client_tool_schema(<工具名>)` 返回的 inputSchema 为准。
...
| `multiOnePriceRule` | object | 是 |  |  |  |  |
| `multiOnePriceRule.calculateType` | string | 是 | "1" | "1"<br>"2" |  | 1 阶梯优惠（例：100元3件，200元7件） ;2 循环优惠（例：每满100元任选3件） |
| `multiOnePriceRule.ruleList` | array | 是 |  |  |  | 最多10级，下单时以满足的最高层级优惠为准 |
| `multiOnePriceRule.ruleList[].price` | string | 是 |  |  |  | 价格，以字符串形式存储 |
| `multiOnePriceRule.ruleList[].pieces` | integer | 是 |  |  | 最小值: 1 | 件数 |
