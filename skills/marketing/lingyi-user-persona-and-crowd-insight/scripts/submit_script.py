#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""用户画像与人群洞察 —— 报告提交服务端（旁路）

纯标准库（urllib）。在 agent 写完画像与洞察报告 Markdown 后调用，把报告正文
POST 到服务端做登记/校验。
...
请求体必须携带 scene 以区分场景；本 skill 的 scene **固定为本 skill 的 name/slug**：

    POST Body: {"content": "<报告Markdown>", "scene": "lingyi-user-persona-and-crowd-insight", "origin": "01workbuddy", "origin_method": "skill"}

> 现有约定示例：full_deconstruct=视频拆解、script_generation=脚本生成；
> 本 skill 固定传 lingyi-user-persona-and-crowd-insight（= name/slug）。
> origin=01workbuddy 标识 WorkBuddy 市场渠道归因，origin_method=skill 标识由技能调用。
...
# 场景标识：与本 skill name/slug 一致
SCENE = "lingyi-user-persona-and-crowd-insight"
# 渠道归因：WorkBuddy 市场由 01workbuddy 渠道分发，origin_method 标识由技能调用
ORIGIN = "01workbuddy"
ORIGIN_METHOD = "skill"

RESULT_START = "=== USER_PERSONA_SUBMIT_START ==="
RESULT_END = "=== USER_PERSONA_SUBMIT_END ==="
...
def call_submit_api(content: str, timeout: float) -> dict:
    """POST {content, scene, origin, origin_method} 到提交接口。

    scene 固定为 lingyi-user-persona-and-crowd-insight（本 skill name/slug），与拆解/脚本生成等场景区分。
    origin=01workbuddy 标识 WorkBuddy 市场渠道归因，origin_method=skill 标识由技能调用。
