...
请求体必须携带 scene 以区分场景（与本 skill 的 metadata.slug 一致）：

    POST Body: {"content": "<报告Markdown>", "scene": "lingyi-live-script-creation"}

> 现有约定示例：full_deconstruct=视频拆解、script_generation=脚本生成；
> 本 skill 固定传 lingyi-live-script-creation（= metadata.slug）。
...
SCENE = "lingyi-live-script-creation"

RESULT_START = "=== LIVE_SCRIPT_SUBMIT_START ==="
RESULT_END = "=== LIVE_SCRIPT_SUBMIT_END ==="
...
def call_submit_api(content: str, timeout: float) -> dict:
    """POST {"content": content, "scene": SCENE} 到提交接口。

    scene 固定为 lingyi-live-script-creation（本 skill slug），与拆解/脚本生成等场景区分。
