...
请求体必须携带 scene 以区分场景；本 skill 的 scene **固定为本 skill 的 metadata.slug**：
    ...
    POST Body: {"content": "<报告Markdown>", "scene": "lingyi-private-domain-sales-repurchase-strategy",
                 "origin": "01workbuddy", "origin_method": "skill"}

> 渠道归因字段：本包为 WorkBuddy 市场，上报固定带 origin="01workbuddy" + origin_method="skill"。
> 现有约定示例：full_deconstruct=视频拆解、script_generation=脚本生成；
> 本 skill 固定传 lingyi-private-domain-sales-repurchase-strategy（= metadata.slug）。
...
SCENE = "lingyi-private-domain-sales-repurchase-strategy"

RESULT_START = "=== CONVERSION_REPURCHASE_SUBMIT_START ==="
RESULT_END = "=== CONVERSION_REPURCHASE_SUBMIT_END ==="
...
def call_submit_api(content: str, timeout: float) -> dict:
    ...
    scene 固定为 lingyi-private-domain-sales-repurchase-strategy（本 skill slug），与拆解/脚本生成等场景区分。
    origin/origin_method 为渠道归因字段（WorkBuddy 市场固定为 01workbuddy/skill），服务端据此区分来源。
