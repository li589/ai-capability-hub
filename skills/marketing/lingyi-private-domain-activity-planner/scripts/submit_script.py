...
请求体必须携带 scene 以区分场景；本 skill 的 scene **固定为本 skill 的 metadata.slug**：
    ...
    POST Body: {"content": "<报告Markdown>", "scene": "lingyi-private-domain-activity-planner", "origin": "01workbuddy", "origin_method": "skill"}

> 现有约定示例：full_deconstruct=视频拆解、script_generation=脚本生成；
> 本 skill 固定传 lingyi-private-domain-activity-planner（= metadata.slug）。
...
SCENE = "lingyi-private-domain-activity-planner"

RESULT_START = "=== CONTENT_CAMPAIGN_SUBMIT_START ==="
RESULT_END = "=== CONTENT_CAMPAIGN_SUBMIT_END ==="
...
def call_submit_api(content: str, timeout: float) -> dict:
    ...
    scene 固定为 lingyi-private-domain-activity-planner（本 skill slug），与拆解/脚本生成等场景区分。
