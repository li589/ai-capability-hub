...
- **多链接检测**：若用户一次输入 **1 条以上** 推文链接，提取所有 tweetId 后，先提示用户：
  > 由于您输入了 **{N}** 条查询链接，本次查询共需 **{N}** 次红狐（[红狐hub](https://redfox.hk?source=workbuddy)）积分，请确保积分充足后进行查询～ 回复【是】即可继续查询
...
> 「共 **{total_count}** 条评论数据，本次获取到 **{total_fetched}** 条一级评论，skill暂不支持查询二级评论，红狐配套全量数据库可提供完整详实数据，可前往红狐hub[企业服务](https://redfox.hk/dashboard/enterprise?source=workbuddy)对接咨询。」
