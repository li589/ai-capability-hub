...
2. **open 后等 3–5 秒**再 `exec`/`create`/`upload` 等；若报 **83013**（未就绪）或 **83014**（查状态失败），确认 `workspace_status=Running` 后重试
...
- `analysis consume`：本人任务可用；他人任务会「无权限」
- `analysis start`：调用的是 resume/重投；本人失败任务仍可能「重投失败」，先勿写成稳定可用流程
