...
| 输出 | 含义 | 动作 |
|------|------|------|
| `REFRESH_OK` | STS 刷新成功 | 立刻重试原 tccli 命令一次 |
| `REFRESH_SKIP` | 非 OAuth 凭证无需刷新 | 跳过刷新，重试原 tccli 命令一次 |
| `REFRESH_FAILED` | 刷新失败 | 进入第 2 步交互式登录 |

**第 2 步：交互式登录（自动选择有头/无头）**

先试探浏览器路径（有超时保护，避免无头环境永久阻塞）：

```bash
# 对 timeout 命令，macOS 需用 gtimeout（brew install coreutils）或用 Python 替代
timeout 10 tccli auth login 2>&1
```

| 结果 | 含义 | 动作 |
|------|------|------|
| 10 秒内返回 0 + `登录成功` | 浏览器登录成功 | 凭证已写入，重试原 tccli 命令一次 |
| 10 秒内返回非 0 | 端口冲突等 | 静默回退到无头路径 |
| 10 秒超时 | 无头环境（浏览器打不开） | 静默回退到无头路径 |

**无头路径**（`tccli auth login` 超时或失败后执行）：

阶段 1，获取 OAuth URL：
```bash
tccli auth login --browser no 2>&1
```
从输出中提取 `https://cloud.tencent.com/open/authorize?...` URL。
...
- 输出 `LOGIN_FAILED: <原因>`：进入第 3 步

**第 3 步：收敛与备选提示**

认证修复链路最多执行 1 轮（刷新 1 次 + 登录 1 次）。
若所有 OAuth 路径均失败，停止自动处理，向用户输出错误码 + message + RequestId，并附带以下备选方案：

> OAuth 登录未成功。您还可以通过以下方式配置凭证：
> 1. **环境变量**：设置 `TENCENTCLOUD_SECRET_ID` 和 `TENCENTCLOUD_SECRET_KEY` 后重试
> 2. **永久密钥**：运行 `tccli configure` 交互式输入 SecretId/SecretKey
>
> 以上为备选路径，本 skill 仍推荐 OAuth 方式以确保凭证安全。
