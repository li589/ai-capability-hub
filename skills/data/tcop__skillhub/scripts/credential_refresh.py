...
输出约定：
  ...
  - 跳过：stdout 输出 "REFRESH_SKIP: <原因>"（非 OAuth 凭证无需刷新）
...
仅处理 OAuth 类型凭证。永久密钥（由 tccli configure 写入）不存在租约，
无需刷新，输出 REFRESH_SKIP。
"""
...
def main():
    ...
    if cred.get("type") != "oauth":
        print("REFRESH_SKIP: 凭证类型不是 oauth（永久密钥），无需刷新")
        return 0
