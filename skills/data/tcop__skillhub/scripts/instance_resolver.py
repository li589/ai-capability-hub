...
def _get_credential() -> Any:
    """读取凭证。统一走 ~/.tccli/default.credential（OAuth 单路径）。

    凭证由 tccli auth login 写入，与 tccli 共享同一份文件。
    """
    ...
    if os.path.isfile(tccli_path):
        try:
            ...
            pass
    sys.stderr.write(
        "[FATAL] no credential found. Run `tccli auth login` to authenticate.\n"
        "If OAuth is unavailable, set TENCENTCLOUD_SECRET_ID / TENCENTCLOUD_SECRET_KEY "
        "environment variables or run `tccli configure`.\n"
    )
    sys.exit(3)
