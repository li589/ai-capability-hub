...
description: A股全栈数据工具包 —
  覆盖行情(mootdx+腾讯+百度K线)、研报(东财+同花顺+iwencai)、信号(同花顺热点+北向+龙虎榜+解禁+行业)、资金面(融资融券+大宗交易+股东户数+分红+资金流分钟级+资金流120日)、新闻(东财个股+全球资讯)、基础数据(mootdx财务/F10+东财+新浪三表)、公告(巨潮)七层数据源，内嵌全部调用代码，自包含零依赖外部文件。优先用通达信(mootdx)/腾讯(不封IP)，东财接口已内置限流防封。适用于个股估值、研报检索、题材归因、龙虎榜跟踪、解禁预警、行业轮动、融资融券跟踪、筹码分析、产业链调研、批量筛选等场景。
version: 3.2.3
origin: custom
display_name: A股全栈数据
display_name_en: A-Stock Data Toolkit
description_zh: A 股行情、研报、资金流、公告与财报查询工具包。
description_en: A-share data toolkit for quotes, reports, funds flow,
  announcements, news, and financial analysis.
visibility: public
...
def em_get(url: str, params: dict | None = None, headers: dict | None = None,
           timeout: int = 15, retries: int = 3, **kwargs):
    """东财统一请求入口：自动节流 + 复用 session + 默认 UA + 连接中断重连。
    所有 eastmoney.com 接口都应通过它请求，避免高频被封 IP。
    实测坑：push2.eastmoney.com 主域在某些网络环境会偶发
    'RemoteDisconnected: Remote end closed connection without response'，
    需 close 会话后用新连接重试（见下方 loops）。"""
    ...
    for _ in range(retries):
        try:
            ...
            last_exc = e
            EM_SESSION.close()          # 丢弃被污染的连接，下次新建
...
def industry_comparison(top_n: int = 20) -> dict:
    ...
    """
    # ⚠ 实测坑：push2.eastmoney.com 主域在某些网络环境会偶发被重置连接
    # (RemoteDisconnected)。优先用更稳的备用主机，em_get 已带重连重试；
    # 另：po 必须用 "1"（升序），用 "-1" 会让接口返回 data:null；
    # 排序在 Python 端按涨跌幅降序完成，保证 top=领涨 / bottom=领跌。
    hosts = [
    ...
    for url in hosts:
        try:
            ...
            continue
    if d is None:
        raise last_exc or RuntimeError("行业板块接口全部失败")
    ...
    # 按涨跌幅降序排序，确保 top=领涨、bottom=领跌
    ranked = sorted(rows, key=lambda x: (x["change_pct"] is not None, x["change_pct"] or 0), reverse=True)
...
### Q: 东财行业板块(push2 clist)报 'RemoteDisconnected' / 返回 data:null？
A: 两个独立坑：(1) `push2.eastmoney.com` 主域在部分网络会偶发被重置连接，改用 `82.push2.eastmoney.com` 或 `push2delay.eastmoney.com` 备用主机，并对 `em_get` 加重连重试(`except` 时 `EM_SESSION.close()` 后重试)；(2) `po` 必须用 `"1"`，用 `"-1"` 会让接口返回 `data:null`。排序在 Python 端按 `f3`(涨跌幅)降序完成即可。

### Q: 腾讯财经传指数代码(如 000001)返回的是个股(平安银行)？
A: `tencent_quote` 的前缀规则是按**个股**规则推导的（6/9→sh, 0/3→sz, 8→bj），但**指数代码不能套用**：上证系指数 000001/000300/000688/000016 属 `sh`，深证系 399001/399006/399005 属 `sz`，北证50 899050 属 `bj`；ETF 中 51xxxx/58xxxx 属 `sh`、159xxx 属 `sz`。误用会把指数当个股（如 sz000001=平安银行、sz000688=国城矿业）。拉指数/ETF 时务必显式指定市场前缀，不要复用个股前缀函数。
