...
2. 未配置 → 引导用户前往 [红狐hub](https://redfox.hk/settings/api-keys?source=workbuddy) 获取并配置：
   ```
...
> ⛔ 所有接口调用必须在请求体中携带 `source` 字段，值为 `"X(Twitter)作品搜索-WorkBuddy"`
...
```json
{
  ...
  "cursor": "{分页游标，首次为null}",
  "source": "X(Twitter)作品搜索-WorkBuddy"
}
```
...
| source | string | 是 | 接口来源标识，固定值：`"X(Twitter)作品搜索-WorkBuddy"` |
