---
name: football-bayes
description: 足球竞彩贝叶斯分析：基于公开数据的概率分析（非投注指令），支持多场次分批与断点续写、重复查询重抓对比，处理核心比赛分析及边缘意图边界。
description_zh: 足球竞彩贝叶斯分析：基于公开数据的概率分析（非投注指令），支持多场次分批与断点续写、重复查询重抓对比，处理核心比赛分析及边缘意图边界。
description_en: Football Bayesian analysis based on public data (not betting
  advice), supporting batch processing, duplicate query re-fetching, and edge
  intent handling.
version: 1.0.2
tags:
  - football
  ...
  - duplicate-handling
display_name: 足球贝叶斯分析
display_name_en: Football Bayes
visibility: public
...
> 领域定位：本 Skill 提供基于公开数据的概率分析，属于信息分析用途而非投注指令，输出结果仅供参考不构成投资建议。
...
默认需要联网抓取，按可靠性优先级（数据源优先级清单）：
1. 官方/权威：联赛官网、俱乐部官网、官方伤停公告、官方赛程
2. 数据站：The Odds API（欧赔/亚盘）、FBref/Understat（xG/近况统计）、StatsBomb公开文章、Transfermarkt（伤停/阵容，需交叉验证）
3. 权威媒体：路透/美联社/天空体育/ESPN/主流体育媒体
4. 赔率：多家赔率对比（若仅1家，标注不确定性）

显式降级协议与禁止能力回退：
- 当优先数据源反爬失败或不可用时，必须显式告知用户哪些维度缺失，并征得同意后使用次级数据源或降级推断继续。
- 禁止能力回退：不得因部分数据缺失而直接丢弃核心输出维度（如胜平负/让球/大小球），必须尽最大努力通过替代源补全；若彻底无法获取，对应字段必须标记为 UNAVAILABLE 并给出原因句。
...
## 约束与合规（Constraints）
- **强制工具调用凭证**：每次输出任何数值型数据（概率/赔率/伤停/排名）前，必须先在报告中展示工具调用记录区（包含搜索 query 或 fetch URL）。任何数值必须可追溯到该区。
- **无来源不输出兜底**：禁止在无工具调用证据的情况下输出任何数字型结论。若未找到可靠来源，对应字段必须标记为 UNAVAILABLE 并说明原因，绝不可捏造数据、伤停、首发、赔率。
- 引用数据必须给出来源链接与抓取时间。
