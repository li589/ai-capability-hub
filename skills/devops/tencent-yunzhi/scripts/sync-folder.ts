 ...
 * 支持将本地文件夹增量同步到乐享知识库
 *
 * 与 upload-files.py 的分工（避免混用）：
 *   - upload-files.py：一次性「批量上传」。无状态，每次都把指定文件全部上传，
 *     适合首次导入或明确知道要传哪些文件的场景。
 *   - sync-folder.ts（本脚本）：有状态「增量同步」。通过本地状态文件比对内容 hash，
 *     自动判断 create / update / delete，只处理发生变化的文件，适合需要反复同步的文件夹。
 *   两者都只生成需要执行的 MCP 调用 / HTTP PUT 步骤计划，不直接落库（最终上传由 AI 助手执行 MCP 调用）。
 *
 * 使用方式:
...
function computeFileHash(filePath: string): string {
  try {
    ...
    // 文件权限/读取失败时返回特殊标记，避免整个同步崩溃；该文件本次会被视为变更
    console.warn(`⚠️  无法读取文件用于计算 hash，将跳过其增量判断: ${filePath}`);
...
function loadSyncState(stateFile: string): SyncState {
  const empty: SyncState = { version: '1.0.0', lastSyncAt: '', files: {} };
  ...
  try {
    ...
    // 容错：状态文件损坏或结构不符时回退为空状态（本次按全量同步处理）
    if (!parsed || typeof parsed !== 'object' || typeof parsed.files !== 'object') {
      console.warn(`⚠️  同步状态文件结构异常，将忽略并按全量同步处理: ${stateFile}`);
      return empty;
    }
    return parsed as SyncState;
  } catch (e) {
    console.warn(`⚠️  无法读取/解析同步状态文件，将按全量同步处理: ${stateFile}`);
    return empty;
  }
}
