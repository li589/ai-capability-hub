...
使用方式:
    export LEXIANG_TOKEN=...
    python docs-v1.py query --doc-id xxx
    python docs-v1.py query --url "https://lexiangla.com/docs/xxx"
    python docs-v1.py create --title "标题" --content "<p>内容</p>"
    python docs-v1.py upload --file report.pdf
    python docs-v1.py edit --doc-id xxx --title "新标题"
    python docs-v1.py delete --doc-id xxx

Token 来源优先级:
    1. --token（仅本地临时调试，不推荐）
    2. LEXIANG_TOKEN 环境变量
    3. ~/.workbuddy/mcp.json 或 ~/.mcporter/mcporter.json
"""
...
def resolve_token(cli_token: Optional[str] = None) -> str:
    """Resolve token without requiring secrets in shell history."""
    token = (cli_token or os.environ.get("LEXIANG_TOKEN") or "").strip()
    if token:
    ...
    print("❌ 未找到 LEXIANG_TOKEN。请设置环境变量，或在本地 mcp.json 中配置 lexiang Authorization。")
    sys.exit(1)
...
def upload_single_file(api_host: str, token: str, file_path: str,
    ...
    try:
        ...
        # 流式上传：直接传文件句柄，避免大文件一次性读入内存导致 OOM
        with open(file_path, 'rb') as f:
            resp = requests.put(cos_params["accessUrl"], headers=cos_headers, data=f, timeout=300)
        if resp.status_code != 200:
            print(f"❌ Step 2 失败: HTTP {resp.status_code}")
            return None
    except requests.exceptions.RequestException as e:
        print(f"❌ Step 2 请求失败: {redact(e)}")
        return None
    print("  [2/3] ✅ COS 上传成功")
...
def main():
    ...
    p_query.add_argument('--token', help='MCP Token（仅本地临时调试；优先使用 LEXIANG_TOKEN）')
    ...
    p_create.add_argument('--token', help='MCP Token（仅本地临时调试；优先使用 LEXIANG_TOKEN）')
    ...
    p_upload.add_argument('--token', help='MCP Token（仅本地临时调试；优先使用 LEXIANG_TOKEN）')
    ...
    p_edit.add_argument('--token', help='MCP Token（仅本地临时调试；优先使用 LEXIANG_TOKEN）')
    ...
    p_delete.add_argument('--token', help='MCP Token（仅本地临时调试；优先使用 LEXIANG_TOKEN）')
    ...
    args.token = resolve_token(getattr(args, 'token', None))
