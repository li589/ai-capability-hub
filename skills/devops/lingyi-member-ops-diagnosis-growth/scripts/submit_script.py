#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""会员运营诊断与增长建议 —— 报告提交服务端（旁路）

纯标准库（urllib）。在 agent 写完诊断报告 Markdown 后调用，把报告正文
POST 到服务端做登记/校验。
...
请求体必须携带 scene 以区分场景；本 skill 的 scene **固定为本 skill 的 metadata.slug**；并固定
携带渠道归因 origin + origin_method（WorkBuddy 渠道）：

    POST Body: {"content": "<报告Markdown>", "scene": "lingyi-member-ops-diagnosis-growth", "origin": "01workbuddy", "origin_method": "skill"}

> 现有约定示例：full_deconstruct=视频拆解、script_generation=脚本生成；
> 本 skill 固定传 lingyi-member-ops-diagnosis-growth（= metadata.slug）。
...
SCENE = "lingyi-member-ops-diagnosis-growth"

# 渠道归因：WorkBuddy 侧固定 01workbuddy / skill
ORIGIN = "01workbuddy"
ORIGIN_METHOD = "skill"

RESULT_START = "=== MEMBER_OPS_DIAGNOSIS_SUBMIT_START ==="
RESULT_END = "=== MEMBER_OPS_DIAGNOSIS_SUBMIT_END ==="
...
def call_submit_api(content: str, timeout: float) -> dict:
    ...
    scene 固定为 lingyi-member-ops-diagnosis-growth（本 skill slug），与拆解/脚本生成等场景区分；
    origin/origin_method 固定为 WorkBuddy 渠道归因（01workbuddy / skill）。
