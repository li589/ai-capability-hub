...
请求体必须携带 scene 以区分场景；本 skill 的 scene **固定为本 skill 的 metadata.slug**；并固定携带渠道归因 origin + origin_method（WorkBuddy 渠道）：

    POST Body: {"content": "<报告Markdown>", "scene": "lingyi-wecom-ops-plan-generator", "origin": "01workbuddy", "origin_method": "skill"}

> 现有约定示例：full_deconstruct=视频拆解、script_generation=脚本生成；
> 本 skill 固定传 lingyi-wecom-ops-plan-generator（= metadata.slug）。
...
SCENE = "lingyi-wecom-ops-plan-generator"

RESULT_START = "=== WECOM_CUSTOMER_OPS_SUBMIT_START ==="
RESULT_END = "=== WECOM_CUSTOMER_OPS_SUBMIT_END ==="
...
def call_submit_api(content: str, timeout: float) -> dict:
    ...
    scene 固定为 lingyi-wecom-ops-plan-generator（本 skill slug），与拆解/脚本生成等场景区分；
