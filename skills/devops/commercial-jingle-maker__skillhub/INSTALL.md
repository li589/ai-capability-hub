...
这是一个已经打好的 广告歌生成AI Node CLI 发布包，里面已经包含运行所需代码。
...
广告歌生成AI 的通用核心是 `SKILL.md` + 本地 Node CLI。不同 Agent 需要适配的是“如何发现说明”和“如何执行本地命令”，不是音乐生成逻辑。

通用要求：

- 让 Agent 读到本目录的 `SKILL.md`
- 让 Agent 能执行本目录的 `bin/aimv.js`，命令为 `node`
- 触发语义：广告歌、广告歌生成、品牌歌、商业配乐、品牌音乐、宣传歌、活动主题曲、声音logo
...
发布包内的 `SKILL.md` 已经是 Codex 原生 Skill 格式。复制到 Codex 本地 skills 目录后，重启 Codex 即可自动识别 广告歌生成AI。执行命令时使用 `node <安装目录>/bin/aimv.js`。

macOS / Linux:

```bash
CODEX_SKILL_DIR="$HOME/.codex/skills/commercial-jingle-maker"
...
```powershell
$CodexSkillDir = Join-Path $env:USERPROFILE ".codex\skills\commercial-jingle-maker"
...
{
  "name": "aimv",
  "description": "为品牌、广告、活动和商业内容生成广告歌、品牌歌、宣传歌与声音 logo 方向。",
  "command": "node",
