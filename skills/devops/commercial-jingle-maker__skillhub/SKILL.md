---
name: commercial-jingle-maker
description: >
  为品牌、广告、活动和商业内容生成广告歌、品牌歌、宣传歌与声音 logo 方向。 Use this whenever the user wants 广告歌, 广告歌生成, 品牌歌, 商业配乐, 品牌音乐, 宣传歌, 活动主题曲, 声音logo.
...
- 第一次生成前如果未登录，Agent 应直接运行 `node <安装目录>/bin/aimv.js init`：CLI 会输出连接页链接 `https://lexuan.club/skills/connect?slug=commercial-jingle-maker&from=skill`，把链接发给用户；用户网页登录（新用户自动注册）后复制 API Key 贴回对话，Agent 再运行 `node <安装目录>/bin/aimv.js init --api-key <KEY>` 完成连接。Key 长期有效，只需一次。

## 命名规范

- 展示名：广告歌生成AI
- Skill slug / 目录名：`commercial-jingle-maker`
- Powered by：海绵音乐
- Agent 工具名：`aimv`
...
```bash
node <安装目录>/bin/aimv.js song create --brief "给新咖啡品牌做一首明亮、好记的广告歌" --wait
node <安装目录>/bin/aimv.js song create --brief "年轻、有记忆点的运动饮料广告歌" --style pop energetic --wait
```
...
- `code=auth_required`：`[error]` 标签携带 `connect_url=`（即 `https://lexuan.club/skills/connect?slug=commercial-jingle-maker&from=skill`），把链接发给用户；用户登录复制 API Key 后，运行 `node <安装目录>/bin/aimv.js init --api-key <KEY>` 完成连接并重试。
...
```bash
# API Key 来自连接页 https://lexuan.club/skills/connect?slug=commercial-jingle-maker&from=skill
...
curl -sS -X POST "https://music.68zysm.cn/api/v3/ai-music/generations/from-brief" \
  ...
  -H "X-App-Id: commercial-jingle-maker" -H "X-AIMV-Skill-Slug: commercial-jingle-maker" \
