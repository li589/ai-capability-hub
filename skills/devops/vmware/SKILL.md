...
skill_id: official_OHNoDw2F
enabled_at: 1787232766038
version: 1.0.0
name_zh: VMware 集成
---
...
1. **Create a new connection:**
   ```bash
   membrane search vmware --elementType=connector --json
   ```
...
1. **Check existing connections:**
   ...
   ```
   If a VMware connection exists, note its `connectionId`
...
When the available actions don't cover your use case, you can send requests directly to the VMware API through Membrane's proxy. Membrane automatically appends the base URL to the path you provide and injects the correct authentication headers — including transparent credential refresh if they expire.
