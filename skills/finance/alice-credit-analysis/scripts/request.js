...
import { sanitizeAgentResultTextForDelivery } from "./agentResultSanitize.js";
import {
...
// 本 CLI 仅承载万得 Alice 的「信用分析」技能。
// 服务端通过 prompt 文本前缀 `使用「<Skill 名>」技能：` 识别 Skill。
const ALICE_SKILL_NAME_ZH = "信用分析";
...
//        ALICE_CREDIT_ANALYSIS_DONE taskId=<id> promptHash=<hash> reportFile=<abs> reportFullFile=<abs>
//      Agent 只需匹配该行就能确认成功，比解析 ALICE_CREDIT_ANALYSIS_STATUS=COMPLETED
...
/**
 * 输出 ALICE_CREDIT_ANALYSIS_DONE 行。幂等：同一进程内只输出一次。
...
function emitAnalysisDone({ taskId, promptHash } = {}) {
  ...
  const parts = ["ALICE_CREDIT_ANALYSIS_DONE"];
  ...
  if (lastReportFullFilePath) {
    const presentablePaths = [lastReportFullFilePath];
    for (const line of [
      "================ ⚠️ present_files 提醒（必须执行）================",
      "DONE 行含 reportFullFile=（如 .md 报告）时，",
      "你必须在交付 agentResult.value 正文之前：",
      "· 从 DONE 行提取 reportFullFile= 路径",
      "· 路径已在工作空间（CLI 统一下载到 process.cwd()），无需 cp",
      "· 在【独立一条消息】里调 present_files",
  ...
  console.log(
    "[CLI] 上一行 ALICE_CREDIT_ANALYSIS_DONE 是本进程任务"
      + "真正完成"
  ...
  console.log(
    "[CLI] 交付用户：将 stdout 中 agentResult.value 原文交给用户（禁止概括）；"
      + "stdout 被截断时读 reportFile= 兜底。禁止读取 reportFullFile= / download/ 附件向用户展示。",
  );
  console.log(
    "[CLI] reportFullFile= 仅表示附件已静默下载到本地，供用户自行打开，不是 Agent 交付正文。",
  );
  ...
  if (lastReportFullFilePath) {
    ...
    console.log(
      `ALICE_USER_DOWNLOAD_HINT=完整报告已保存到：${lastReportFullFilePath}`,
    );
  }
}
...
function installStrictExitHook() {
  ...
  process.on("exit", () => {
    ...
    try {
      console.error(
        "[CLI][strict] 进程退出但未输出 ALICE_CREDIT_ANALYSIS_DONE；强制退出码="
          + EXIT_STRICT_NO_DONE
          + "。",
      );
      console.error(
...
// strict 模式专用退出码：
// - 6 = 进程退出时未输出 ALICE_CREDIT_ANALYSIS_DONE，且 exitCode 仍为 0；
...
// IDE --no-wait 频率超限：15 分钟内同一 prompt 已调用 2 次 aca.ps1 --no-wait（Trae detach 模式）。
const EXIT_IDE_WAIT_RATE_LIMITED = 79;
...
export function parseDoneLineFromDetachLog(logPath, expectedPromptHash) {
  ...
  for (let i = lines.length - 1; i >= 0; i--) {
    const line = lines[i];
    if (!line.includes("ALICE_CREDIT_ANALYSIS_DONE")) continue;
...
function deliverFromDetachLogDone(doneLine, promptHash, logPrefix = "[wait]") {
  ...
  if (reportFile && existsSync(reportFile)) {
    ...
    if (body) {
      ...
      console.log(formatValueOutput(body));
      console.log(
 ...
 *
 * --new：清除该 prompt 全部本地记录后新建（并行任务；Agent 须 check-conflict 询问用户）。
 *
 * @returns {Promise<
 *   | { action: "submit" }
...
async function resolveTaskDispatchPlan({
  ...
  if (forceNew) {
    ...
    console.log(
      `${logPrefix} 若已有 running 任务，须先 check-conflict 列给用户并由用户确认后再 --new。`,
    );
...
function clearCompletedRecordForResubmit(registry, promptHash, { logPrefix = "[CLI]" } = {}) {
  ...
  console.log(
    `${logPrefix} 若只需查询历史落盘路径，请用：alice-credit-analysis status -p "<同一问题>"`,
  );
...
// 沙箱（如 Workbuddy）常阻止用户主目录写入但允许临时目录。
...
function reportSubmitLockConflict(existing, prompt, lockKind, subjectKey = null) {
  ...
  console.error(
    ...
    "如需并行新建，须先 check-conflict 询问用户后加 --new。",
  );
  console.error(
...
function printAttachBanner({ taskId, contextId, startedAt }) {
  ...
  console.log("");
  console.log("ALICE_CREDIT_ANALYSIS_STATUS=RUNNING");
...
function printCliRunStartNotice() {
  const lines = [
    "ALICE_CREDIT_ANALYSIS_STATUS=RUNNING",
    "[CLI] 信用分析已启动：本进程将阻塞运行直至成功（agentResult + [完成]）或失败，通常需 2-15 分钟。",
    "[CLI] [等待中] 心跳期间无新输出属正常；禁止用 check_command_status 轮询替代等待本进程结束。",
    "[CLI] 见到 [任务已受理] 仅表示已提交，不等于报告完成；必须等到 ALICE_CREDIT_ANALYSIS_STATUS=COMPLETED。",
  ];
  for (const line of lines) {
    console.error(line);
  }
...
function persistAgentResultIfAny({ registry, taskId, promptHash, prompt }) {
  ...
  if (!text) return null;
  const header = [
    `<!-- alice-credit-analysis：本次 Alice agentResult.value 已落盘 -->`,
    `<!-- 重要：本文件是 Alice 响应正文（agentResult.value），并不一定等于"完整信用报告"。 -->`,
...
export function handleInterruptCleanup({
  ...
  if (taskCreatedOnServer) {
    ...
    log("若要丢弃进行中的旧任务并并行新建，须先 check-conflict 询问用户后加 --new。");
...
function printCliRunEndNotice({ success, exitCode, elapsed }) {
  const lines = success
    ? [
        "ALICE_CREDIT_ANALYSIS_STATUS=COMPLETED",
    ...
    : [
        "ALICE_CREDIT_ANALYSIS_STATUS=FAILED",
...
function usage() {
  return [
    "alice-credit-analysis — 调用万得 Alice Agent「信用分析」技能，对各类企业 / 机构主体流式输出系统化信用研究报告",
    "",
    "Usage:",
    "  alice-credit-analysis --prompt <QUESTION> [--download-dir <DIR>] [--new]",
    "  alice-credit-analysis --prompt <QUESTION> --no-wait                       # 沙箱推荐：CLI 内部自旋 probe 直到完成（默认）",
    "  alice-credit-analysis --prompt <QUESTION> --no-wait --once                # 单次探针：几秒内返回（脚本/调试）",
    "  alice-credit-analysis --prompt <QUESTION> --no-wait \\",
    "      --watch-interval 60 --watch-timeout 1800                              # 自定义轮询节奏",
    "  alice-credit-analysis status --prompt <QUESTION>                        # 查询落盘路径（不访问服务端）",
    "  alice-credit-analysis check-conflict --prompt <QUESTION>                # [已废弃] 主调用 --no-wait 已内置去重，无需预检",
    "  alice-credit-analysis apikey-set <KEY>",
    "  alice-credit-analysis apikey-get",
    "  alice-credit-analysis apikey-clear",
    "  alice-credit-analysis --help",
    "",
    "Options:",
    "  --prompt,       -p <QUESTION>   用户提问，例如\"分析宁德时代的信用资质\"（必填）",
    "  --download-dir, -d <DIR>        保留兼容；不影响下载目录——附件统一落当前工作空间 process.cwd()（见下文）",
    "  --new                     用户明确要求并行新建：清除该 prompt 全部本地记录后新建（须先 check-conflict 询问用户）",
    ...
    "                                  ALICE_CREDIT_ANALYSIS_DONE 行，会把 exitCode=0 强制改成 6，",
    ...
    "",
    "退出码:",
    "  0   成功：必定伴随 stdout 含 ALICE_CREDIT_ANALYSIS_DONE 行（Agent 判定任务完成的唯一确定信号）",
    "  1   一般失败 / 任务终态为 failed",
    "  2   参数错误 / KEY_MISSING",
    "  4   --no-wait 专用：已达全程自旋上限（默认 60min）仍未完成；Agent 再执行一次相同 --no-wait 续接",
    "  5   --no-wait 专用：--once 且本地无任务记录（默认 --no-wait 会自动提交）",
    "  6   strict 模式兜底：进程退出但未输出 ALICE_CREDIT_ANALYSIS_DONE（通常表示进程被沙箱终止）；",
    ...
    "      或在用户明确需要时加 --new 重新发起",
    "  79  IDE 短超时模式：15 分钟内同一 prompt 已调用 2 次 aca.ps1 --no-wait；停止自动重试，等用户说「继续查」",
    "",
    "Env:",
...
function accumulateAgentResultValues(values) {
  ...
  for (const value of values) {
    ...
    if (text) collectedAgentResultValues.push(sanitizeAgentResultTextForDelivery(text, "", resolveInlineLocalPath));
  }
}
...
const DOWNLOAD_404_RETRY_DELAYS_MS = [1500, 2000, 3000, 5000, 8000];
...
async function downloadOneFile({ url, targetPath, apiKey }) {
  for (let attempt = 0; ; attempt++) {
    ...
    try {
      ...
      return { ok: false, error: `网络错误：${formatErrorDetail(e)}` };
    }

    if (response.status === 404 && attempt < DOWNLOAD_404_RETRY_DELAYS_MS.length) {
      try { await response.text(); } catch {}
      const delay = DOWNLOAD_404_RETRY_DELAYS_MS[attempt];
      if (attempt === 0) {
        console.log(`[CLI] 文件暂未在服务端就绪（HTTP 404），${delay}ms 后重试…`);
      }
      await sleep(delay);
      continue;
    }
...
function printReportFullFileTag(absPath, { reused = false, taskId = null, promptHash = null } = {}) {
  ...
  const tagLines = [
    ...
    `ALICE_USER_DOWNLOAD_HINT=完整报告已保存到：${absPath}`,
...
async function downloadCollectedFiles(apiKey, override, ctx = {}) {
  ...
  // 所有下载件（报告附件 / 数据附件 / 图片）统一落工作空间 process.cwd()，
  // 避免 -d 传成工作空间外目录或未传 -d（回落 ~/Downloads）时文件落外部、
  // 再由宿主 cp 到工作空间才能 present_files。
  const targetDirFor = (_filename) => workspaceDir;
  console.log("");
  console.log(`=== 检测到 ${items.length} 个可下载文件，正在下载到工作空间：${workspaceDir} ===`);

  for (const { url, filename } of items) {
    const performDownload = async () => {
      const targetPath = resolveUniqueTargetPath(targetDirFor(filename), filename);
      const result = await downloadOneFile({ url, targetPath, apiKey });
      if (result.ok && registry && taskId) {
        try {
          registry.appendDownloadedFile(taskId, {
            url,
            path: result.path,
            filename,
          });
    ...
    let outcome;
    if (registry && taskId) {
      outcome = await runDownloadWithLock(registry, taskId, url, performDownload);
    } else {
    ...
    if (outcome.kind === "reused") {
      const absPath = outcome.entry.path;
      console.log(`- ${filename}`);
      console.log(`  复用：${absPath}`);
      if (outcome.waited) {
        console.log("  （等待其它 CLI 进程完成下载后跨进程去重命中，未重复 HTTP）");
      }
      printReportFullFileTag(absPath, { reused: true, ...fileTagMeta });
      continue;
    }
    ...
    if (result?.ok) {
      console.log(`- ${filename}`);
      console.log(`  已保存：${result.path}`);
      printReportFullFileTag(result.path, { reused: false, ...fileTagMeta });
    } else {
      console.error(`- ${filename}`);
      console.error(`  下载失败：${result?.error ?? "unknown"}`);
...
export function formatValueOutput(value, resolveLocalPath = null) {
  if (typeof value === "string") {
    return `agentResult.value: ${sanitizeAgentResultTextForDelivery(value, "", resolveLocalPath)}`;
  }
...
function printAgentResultValues(values) {
  for (const value of values) {
    console.log(formatValueOutput(value, resolveInlineLocalPath));
  }
}
 ...
 * `ALICE_CREDIT_ANALYSIS_STATUS=...`，由调用方（单次 probe / watch 轮询）在合适
...
async function runProbeStep({ registry, taskId, promptHash, prompt, url, headers, apiKey, downloadDir }) {
  ...
  if (probe.kind === "working") {
    ...
    console.log(
      "[CLI] 本进程尚未输出 ALICE_CREDIT_ANALYSIS_DONE；logs/ 下其它 .session.log 内的 DONE 行与本 PROMPT_HASH 不一致则无效。",
    );
 ...
 * 调用方读到的 `ALICE_CREDIT_ANALYSIS_STATUS=...` 语义一致。
 *
...
function publishFinalStatus(outcome, { suggestExternalPoll = false } = {}) {
  switch (outcome) {
    case "completed":
      console.log("ALICE_CREDIT_ANALYSIS_STATUS=COMPLETED");
      emitAnalysisDone();
      break;
    case "failed":
      console.log("ALICE_CREDIT_ANALYSIS_STATUS=FAILED");
      break;
    case "no-task":
      console.log("ALICE_CREDIT_ANALYSIS_STATUS=NO_TASK");
      break;
    case "working":
    default:
      if (suggestExternalPoll) {
        ...
        console.log(
          "[探针] 禁止读取 download/ 同名报告或 STALE_REPORT_CANDIDATE；必须见到 ALICE_CREDIT_ANALYSIS_DONE 且 promptHash= 一致。",
        );
      }
      console.log("ALICE_CREDIT_ANALYSIS_STATUS=RUNNING");
      break;
...
function printAttachFallbackBlockedMessage(prompt) {
  console.error("");
  console.error(
    "[CLI][attach-fallback] 已阻止「attach 失败 → 自动新建」路径，避免重复消耗服务端额度。",
  );
  console.error(
    "[CLI][attach-fallback] 正确做法：用**完全相同**的 prompt 加 --no-wait（阻塞等待 CLI 退出）：",
  );
  console.error(
    `  node scripts/cli.mjs --prompt ${JSON.stringify(prompt)} --no-wait`,
  );
  console.error(
    "[CLI][attach-fallback] 禁止再次用无 --no-wait 的普通模式重试——会再次触发 attach 失败并重复扣费。",
  );
  console.error(
    `[CLI][attach-fallback] 退出码 ${EXIT_DUPLICATE_LIKELY} = EXIT_DUPLICATE_LIKELY；未向服务端新建任务。`,
  );
}

/**
 * attach 模式欲回退到新建任务前，用 tasks/get + 本地 registry 确认旧任务确实已失效。
 * 防止沙箱 kill SSE 后误新建第二条仍在服务端 running 的任务（重复扣费）。
 *
 * @returns {Promise<{ allow: true } | { allow: false, kind: string }>}
 */
async function probeBeforeAttachFallbackToNew({
  registry,
  prompt,
  promptHash,
  oldTaskId,
  ...
  url,
  headers,
  logPrefix = "[CLI][attach-fallback]",
}) {
  if (url && headers && oldTaskId && oldContextId) {
    const probe = await probeTaskOnce({ url, headers, taskId: oldTaskId, contextId: oldContextId });
    if (probe.kind === "working") {
      console.error(
        `${logPrefix} tasks/get 确认旧任务仍在执行（state=${probe.state}），**禁止**新建第二条任务。`,
      );
      return { allow: false, kind: "old_task_working" };
    }
    if (probe.kind === "completed") {
      console.error(
        `${logPrefix} tasks/get 确认旧任务已 completed，**禁止**新建；请用相同 prompt 加 --no-wait 重放。`,
      );
      return { allow: false, kind: "old_task_completed" };
    }
    if (probe.kind === "failed") {
      console.log(`${logPrefix} tasks/get 确认旧任务已 failed，允许新建。`);
      return { allow: true };
    }
    if (probe.kind === "error" && isTaskNotFoundProbe(probe)) {
      console.log(`${logPrefix} tasks/get 确认旧任务已不存在，允许新建。`);
      return { allow: true };
    }
    console.error(
      `${logPrefix} tasks/get 暂不可用（${probe.reason ?? probe.kind}）；为防重复扣费，**禁止**新建第二条任务。`,
    );
    return { allow: false, kind: "probe_unavailable" };
  }

  const similarHit = findSimilarRunning(registry.records, prompt, {
    skipPromptHash: promptHash,
    windowMs: STALE_TASK_THRESHOLD_MS,
  });
  if (similarHit) {
    console.error(
      `${logPrefix} 本地仍有相似 running 任务 taskId=${similarHit.record.taskId}，**禁止**新建。`,
    );
    return { allow: false, kind: "similar_running" };
  }
...
/** @internal 单测 */
export async function __probeBeforeAttachFallbackToNewForTesting(opts) {
  return probeBeforeAttachFallbackToNew(opts);
}

/**
 * 阻塞模式（无 --no-wait）检测到 running 任务需续接时，改用 tasks/get 内部自旋，
 * 避免 SSE attach 在沙箱中被杀后 switchToNewTaskMode 误新建第二条任务。
 */
async function runBlockingAttachViaProbeWatch({
  registry,
  attachRecord,
  promptHash,
  prompt,
  ...
  const taskId = attachRecord.taskId;
  console.error(
    `${logPrefix} 同 prompt 已有 running 任务（taskId=${taskId}）；` +
      "无 --no-wait 的阻塞 SSE attach 在沙箱中易被截断并误新建第二条任务。",
  );
  console.error(
    `${logPrefix} 本进程自动降级为 tasks/get 内部自旋续接（ALICE_ATTACH_VIA_PROBE=1），不挂 SSE、不新建任务。`,
  );
  console.log("ALICE_ATTACH_VIA_PROBE=1");
  announceServerUsage({
    action: "attach",
    taskId,
    promptHash,
  ...
  console.log(
    "ALICE_AGENT_REDLINE=禁止在 stdout 出现 ALICE_CREDIT_ANALYSIS_DONE 之前 view_files/read download/ 主体报告",
  );

  await runWatchMode(
    { registry, taskId, promptHash, prompt, url, headers, apiKey, downloadDir },
    {
...
async function main() {
  ...
  // analysis 主路径：默认开启 strict 模式（未输出 ALICE_CREDIT_ANALYSIS_DONE 即视为
  ...
  if (noWait) {
    ...
    console.log("ALICE_AGENT_REDLINE=禁止在 stdout 出现 ALICE_CREDIT_ANALYSIS_DONE 之前 view_files/read download/ 主体报告");
  ...
  const plan = await resolveTaskDispatchPlan({
    ...
    downloadDir,
    logPrefix: "[CLI]",
    // detach 子进程兜底：即使未注入 --no-wait，也不应在 completed 后清除记录再 submit
    preferReplayOnCompleted: Boolean(process.env.ALICE_DETACH_LOG),
  });
  ...
  if (plan.action === "attach") {
    await runBlockingAttachViaProbeWatch({
      registry,
      attachRecord: plan.record,
      promptHash,
      prompt,
      url,
      headers,
  ...
  const switchToNewTaskMode = async (reason) => {
    const fallbackCheck = await probeBeforeAttachFallbackToNew({
      registry,
      prompt,
      promptHash,
      oldTaskId: taskId,
    ...
    if (!fallbackCheck.allow) {
      if (reason) console.log(`[任务复用] ${reason}，但探针确认旧任务仍有效，已阻止自动新建。`);
      printAttachFallbackBlockedMessage(prompt);
      process.exitCode = EXIT_DUPLICATE_LIKELY;
      return false;
    }
      ...
      mainLoop: while (true) {
        for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
          ...
          if (!startBannerPrinted) {
            ...
            console.log(
              `[开始] 已向 Alice「${ALICE_SKILL_NAME_ZH}」技能发起请求；信用分析通常耗时 2-5 分钟，期间会定期播报等待进度，请勿中断。`,
            );
          }
          ...
          if (!response.ok) {
            ...
            if (
              ...
              const switched = await switchToNewTaskMode(
                `服务端返回 ${response.status}，旧任务可能已过期或被清理`,
              );
              if (!switched) return;
