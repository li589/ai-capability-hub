...
>
> 因此,CLI 通过 `aca.ps1` 跑完后,服务端返回的 `agentResult.value` 完整结果**会停留在 Bash 工具的 stdout 输出里,被折叠进「已完成」块**。**你不能指望用户去点开那个折叠块**。
>
...
>
> **常见场景**:
> - 用户先「帮我分析一下宁德时代的信用资质」-> 交付 -> 「**那万科呢**」：同话题延伸，**什么都不加**，自动续接。
> - 「**那违约概率呢 / 后市怎么看**」：同话题追加维度，**什么都不加**。
> - 「**宁德时代股票值得买吗**」：完全无关新主题，加 `--new-session`。
...
| 步 | 动作 | 对用户怎么说 |
|----|------|-------------|
| 0 | **API Key 配置**：从 `agent_md` 读取已保存的 Wind Alice API Key，若存在则执行 `apikey-set`；若不存在则询问用户提供 Key，保存到 `agent_md` 后再执行 `apikey-set` | （已有 Key 时静默执行）；无 Key 时：「使用信用分析需要配置一个 Key，你有 Wind Alice 的 API Key 吗？」 |
| 1 | 把用户问题拼成一句自然语言 `--prompt`（**不要**加 `使用「信用分析」技能：` 前缀）；**同步判断是否切到完全无关的新话题**（见 [「会话续接判断」专节](#会话续接判断agent-自动执行无需用户显式说)）：同话题追问**什么都不加**（CLI 工作区内自动续接）；完全无关新话题才加 `--new-session` | （无需说话） |
| 2 | 告知用户：通常需 **2–15 分钟** | 「好的，我来帮你分析。信用分析通常需要 2–15 分钟，请稍等。」 |
...
3. **若未保存**：
   - 向用户说：「使用信用分析需要配置一个 Key，你有 Wind Alice 的 API Key 吗？」
...
| 「请提供 Wind Alice API Key 以便调用信用分析服务端接口。」 | 「使用信用分析需要配置一个 Key，你有 Wind Alice 的 API Key 吗？」 |
...
宿主执行主调用 / status 等 Bash 工具调用时，会把你填在工具调用的 `description` 字段里的文字，作为**权限确认框的标题**展示给用户。请把该字段填**简短中文**，不要填英文（如「Call Wind Alice for financial analysis」→ 改填「调用万得 Alice 进行信用分析」）。

对应七步流程的推荐 `description` 取值：

| 步骤 | 命令 | `description` 填写 |
|------|------|--------------------|
| 3 | 主调用 `--no-wait` | 调用万得 Alice 进行信用分析 |
...
>
> 1. **Windows 必须用 `aca.ps1`，禁止裸 `node scripts/cli.mjs`**（裸 node 在 PowerShell 5.x 下中文乱码 + 沙箱兼容性问题）。
> 2. **禁止 `Set-Location`/`cd` 到 `<SKILL_DIR>`**。`aca.ps1` 用绝对路径调用，**不 cd**，让 `process.cwd()` 保持当前工作区。数据目录(tasks.json/session.log/submit-locks)**固定在 `~/.wind-alice/`**（跨会话稳定，不跟 cwd 走），故 cd 不影响状态目录；但仍禁止 cd 到 skill 目录：避免污染 skill 目录、避免 `present_files` 路径错乱。
...
> 4. **禁止用 `Out-File`/`>`/`2>&1 | Out-File` 把 stdout 重定向到文件**。CLI 的完成信号(`ALICE_CREDIT_ANALYSIS_DONE`、`agentResult.value`)在 stdout，Agent 必须**直接捕获 stdout**，重定向到文件既把文件写到工作区外(弹窗)、又可能导致 Agent 读不到完成信号。
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\aca.ps1" --prompt "<USER_QUESTION>" --no-wait -d "<WORKSPACE_DIR>" [--context-id <上一轮contextId>] [--new-session]
```
...
- **代词/指代**:「它 / 这家 / 上面那份能不能加 XX」。
- **相同主体追加维度**:上次分析 XX 的 A 维度,本次问 XX 的 B 维度(同一主体、不同角度)。
- **同一信用分析话题延伸**:上文分析「帮我分析一下宁德时代的信用资质」,本次问「那万科呢 / 财务健康度呢 / 后市怎么看」--同属该主体信用话题,**必续接**。

**案例（必读）**:
- ✅ 用户先「帮我分析一下宁德时代的信用资质」-> 你交付 -> 用户「**那万科呢**」：同话题延伸追问，**什么都不加**，自动续接。
- ✅ 用户「宁德时代信用资质」-> 「**那违约概率呢**」：同话题追加维度，**什么都不加**。
- ❌ 用户「帮我分析一下宁德时代的信用资质」-> 「**宁德时代股票值得买吗**」：完全无关新主题，加 `--new-session`。

**判定为切换（加 `--new-session`）**:
- 用户明说:「换个话题 / 忘掉刚才 / 新问题」。
- 主体/主题明显无关:上文聊主体信用,本次问债券利率或商品期货。
...
6. **禁止在无完成信号时交付** — 无 `ALICE_CREDIT_ANALYSIS_DONE` 与 `agentResult.value` 时**不得交付**任何分析结论。
...
- stdout 含 **`ALICE_CREDIT_ANALYSIS_DONE`**，且其中 **`promptHash=`** 与本次 CLI 打印的 **`PROMPT_HASH=`** 一致；
- 退出码 **`0`**；
...
2. 若**是**：从 `agentResult.value` 正文中提取信用主体/维度关键词，与当前会话上下文对比：
   - **主体明显不同**（跨品种/跨视角，如缓存的「帮我分析一下宁德时代的信用资质」vs 请求的「宁德时代信用分析」；或缓存的「宁德时代」vs 请求的「螺纹钢期货情绪」）→ **静默自动重跑**：用**原 prompt** 加 `--new --no-wait` 重新执行（将消耗积分）。**不向用户提及缓存/主体不匹配/重跑等任何技术细节**——用户不需要知道自己引来了别的主题缓存。重跑后按正常流程交付。
   - **主体一致或高度相似**（同品种不同写法，如「宁德时代」vs「CATL」；同一主体信用话题的追问）→ 停下来告知用户：「该问题已有最近的分析结果。」然后列出：(A) 查看已有结果 / (B) 重新分析 / (C) 取消。**禁止**未经用户明确选择就交付或 `--new`。
   - **拿不准时**（主体模糊、仅泛化关键词如「公司」「信用」、缩写无法判定）→ 按「主体一致」处理，停下来询问用户。
3. 若**否**：正常交付，按步骤 5 处理。

**关键判断标准**：
- 明显不同：两个信用主体/话题完全不同、无任何字面重叠（如「帮我分析一下宁德时代的信用资质」→「螺纹钢期货情绪」）。这类情况直接静默重跑，**绝不打扰用户**。
- 需要停下来问：同一主体不同写法（「宁德时代」→「CATL」）、同一主体信用话题的细化追问、或 Agent 无法判断是否同一主题时。
...
| `78` | 环境受限，无法保存任务状态 | **未向服务端发请求**；告知用户「当前环境无法运行 信用分析，请在工具中开启完全访问权限后重试」；禁止换 prompt / `--new` / 反复重试 |
...
信用分析通常 **2–15 分钟**。若宿主 `run_command` 只有数十秒～几分钟超时，**第一条 `--no-wait` 会被强杀**（exit `4`/`6`），看起来像「卡住」——其实是终端杀了 CLI，服务端往往仍在跑。此时对用户说：「分析仍在进行中，我继续等待……」，**不要**说「被杀」「超时」等技术细节。
...
powershell ... aca.ps1 --prompt "<USER_QUESTION>" --no-wait
```
...
| 信号 | 含义 |
|------|------|
| `ALICE_CREDIT_ANALYSIS_DONE` | **唯一**完成标记 |
...
# ❌ 用裸 node cli.mjs 代替 aca.ps1（Windows 上会导致中文乱码、沙箱兼容性问题）
...
□ stdout 有 ALICE_CREDIT_ANALYSIS_DONE？
