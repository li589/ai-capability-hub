---
name: alice-credit-analysis
description: 对各类企业/机构主体做系统化信用研究，覆盖主体信用、行业风险、财务健康度、现金流质量、评级对标、违约概率六大维度，集成 Wind
  风险评分提供更精准违约概率评估。适用于信用资质分析、违约概率评估、信用风险对比等场景。
description_zh: 万得 Alice 信用分析 CLI：对各类企业 / 机构主体做六大维度系统化信用研究，集成 Wind
  风险评分提供更精准违约概率，输出含核心结论、优势、风险、投资建议的结构化报告。
description_en: Wind Alice Credit Analysis CLI — systematic credit research
  across credit profile, industry risk, financial health, cash-flow quality,
  rating benchmarking, and default probability for any corporate / institutional
  entity (LGFV, SOE, private cos, listed cos, financial institutions,
  real-estate, bond issuers, etc.), integrated with Wind risk scoring for
  accurate PD estimation.
version: 1.0.3
author: WindAlice
tags:
  - credit
  ...
  - wind-alice
display_name: 信用分析
display_name_en: Credit Analysis
visibility: public
disable-model-invocation: true
---

# Credit Analysis（万得AI-信用分析）

## 技能介绍

### 官方技能：万得AI-信用分析

系统化信用研究能力，对**各类企业 / 机构主体**（城投、国企、民企、上市公司、金融机构、房地产、债券发行人 等）覆盖**主体信用、行业风险、财务健康度、现金流质量、评级对标、违约概率建模**六大维度，为信用研究、投资决策与风险管理提供定量依据。**集成 Wind 风险评分系统，提供更精准的违约概率评估。**

### 适用场景

| 场景               | 解决的问题                                           |
| ------------------ | ---------------------------------------------------- |
| 债券投资决策       | 评估发行人信用资质，辅助一级 / 二级市场买卖判断      |
| 信用风险管理       | 识别高风险主体，构建预警与监控清单                   |
| 投资组合构建       | 筛选符合风险偏好的标的，做组合信用质量诊断           |
| 信用评级验证       | 内部评级与外部评级机构对标，发现差异并复核           |
| 违约概率建模       | 量化违约风险，输出年化 PD 与置信度                   |
| 财务健康度诊断     | 评估偿债能力、流动性、杠杆趋势                       |
...
5. **评级对标与违约概率**
   - 内部综合评分与内部评级（AAA - BBB 及以下）
   - 与外部评级机构结果对标，标注差异
   - **集成 Wind 风险评分系统**，估算年化违约概率（PD）与置信度
   - 评分历史趋势与黄 / 红色预警
...
- 自动生成的结构化内容包含：**核心结论、主体优势、主要风险、投资 / 配置建议**
- 多维度分析：主体资质、行业风险、财务健康度、现金流质量、内部评级、违约概率
- 部分场景下 CLI 会**静默**把完整报告附件下载到当前工作空间（`process.cwd()`，供用户本地查阅）；**Agent 向用户展示时只用 `agentResult.value`，不要粘贴附件正文**
- 数据受限时在对应模块明确标注

### 💡 使用提示

- 建议**定期跟踪 Wind 评分变化**；**评分下降 > 3 分应触发预警**，需立即复核主体基本面与到期债务。
...
| 顺序 | 做什么 | 禁止 |
|------|--------|------|
| 0 | 读同目录 **[`AGENT.md`](./AGENT.md)** | 禁止 `search_codebase`、禁止查库猜信用数据 |
| 1 | **一条** `aca.ps1 --prompt "<用户原话>" --no-wait -d "<WORKSPACE_DIR>"` [--context-id <上一轮contextId>] [--new-session] | 禁止裸 `node`、禁止 `cd` 到 skill 目录 |
| 2 | `run_command` / Shell **超时 ≥ 1200 秒**；**阻塞等到命令退出** | 禁止秒退后说「后台 PID / 稍后再问继续查」 |
| 3 | stdout 含 `ALICE_CREDIT_ANALYSIS_DONE` | 无 DONE = 未完成，禁止编造 |
| 4 | **逐字**交付 `agentResult.value`；DONE 含 `reportFullFile=` 时先 `present_files` 再交付正文 | 禁止自行概括、禁止读附件正文 |

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\aca.ps1" --prompt "<USER_QUESTION>" --no-wait -d "<WORKSPACE_DIR>"
```
...
>
> **常见场景**:用户先问「帮我分析一下宁德时代的信用资质」,你交付后用户问「**那万科呢**」--同话题延伸追问,**什么都不加**,自动续接。若切到完全无关的新主题（如「宁德时代股票值得买吗」），才加 `--new-session`。
>
> **拿不准时**:倾向**不加 `--new-session`**(即自动续接)。续接错代价小,漏续接代价大(服务端不带上文、答非所问)。`--context-id` 仍可用但通常不需要（显式精确续接，优先级最高）；`--continue-session` / `--session-scope` 已废弃，可不传。
>
> 详见 `AGENT.md`「会话续接判断」专节。

## 何时优先调用本技能

当用户问题涉及**主体信用资质、违约概率、财务健康度、现金流质量、评级对标、风险预警**等场景时，应调用本技能。不应在纯债券利率走势、债市行情、与信用主体无关的宏观问题等场景硬套本技能。
...
```powershell
# ✅ 方案 A（唯一推荐）：aca.ps1 绝对路径，无需 cd，自带 UTF-8 代码页
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\aca.ps1" --prompt "<USER_QUESTION>" --no-wait -d "<WORKSPACE_DIR>"
```
...
```bash
node /path/to/alice-credit-analysis/scripts/cli.mjs --prompt "<USER_QUESTION>" --no-wait -d "<WORKSPACE_DIR>"
```
...
| 信号 | 含义 / Agent 必须怎么做 |
|------|------------------------|
| stdout 含 `ALICE_CREDIT_ANALYSIS_DONE taskId=... promptHash=... reportFile=... reportFullFile=... attachmentFile=...` | **唯一**真正完成信号；**必须先核对 `promptHash=` 与本次 `--prompt` 一致**，再以 stdout 中的 `agentResult.value` 原文交付用户（stdout 被截断时读 `reportFile=` 兜底）；**若有 `reportFullFile=` 必须把绝对路径转告用户；若有 `attachmentFile=` 也必须把绝对路径转告用户** |
...
| 退出码 `78`（环境受限，无法保存任务状态） | 当前环境无法保存任务状态（未开启完全访问权限）；**未向服务端发请求**；告知用户「当前环境无法运行 信用分析，请在工具中开启完全访问权限后重试」；禁止换 prompt / `--new` / 反复重试 |
...
5. **交付 agentResult.value + 告知报告路径**——将 CLI stdout 中的 `agentResult.value` **原文**交给用户；**禁止**自行总结、摘录、重写表格或「用自己的话」复述。若 stdout 被沙箱截断，读取 DONE 行 `reportFile=` 落盘正文（跳过 HTML 注释头）。**DONE 含 `reportFullFile=` 时必须告知完整路径**（**保留 CLI 内联的 markdown 链接格式**，如 `[信用分析报告.md](file:///<工作空间>/信用分析报告.md) (<工作空间>\信用分析报告.md)`）——CLI 已把链接内联到 `agentResult.value` 正文中，**原样输出即可**；**禁止**把链接改成行内代码 `` `C:\...\报告.md` ``，行内代码在 Cursor / Trae 等宿主里**不可点击**，用户无法直接打开文件。**DONE 含 `attachmentFile=` 时也必须保留 markdown 链接格式告知完整路径**（如 `[财务模型.xlsx](file:///<工作空间>/财务模型.xlsx) (<工作空间>\财务模型.xlsx)`）。**禁止加载 `reportFullFile=` / `attachmentFile=` 文件内容展示给用户**——`agentResult.value` 已是面向用户的核心分析摘要，完整报告和数据附件供用户本地查阅，不需要 Agent 读取并展示。 **交付 = Agent 亲自打字复述正文**（不折叠），不能只跑完 CLI 让 stdout 显示。**若 DONE 含 `reportFullFile=`，先在独立一条消息调 `present_files` 呈现报告文件为可点击卡片（CLI 已把附件下载到工作空间 `process.cwd()`，路径必在工作空间内，无需传 `-d`）；随后在另一条纯文本消息里输出 `agentResult.value`（本条禁任何工具调用，否则整条被折叠）**。
...
```powershell
# Windows（方案 A）
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\aca.ps1" apikey-set <KEY>
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\aca.ps1" apikey-get
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\aca.ps1" apikey-clear
```
...
3. **无 Key** → 向用户说「使用信用分析需要配置一个 Key，你有 Wind Alice 的 API Key 吗？」→ 用户回复后 → 执行 `apikey-set <KEY>` → `memory_write(file="agent_md", content="增加/更新以下记忆：Wind Alice API Key = <KEY>")` → 继续后续步骤。
...
| 场景 | 发生了什么 |
|------|-----------|
| 首次使用 | Agent：「使用信用分析需要配置一个 Key，你有 Wind Alice 的 API Key 吗？」→ 用户回复 → Key 保存到 `agent_md` → 自动配置 → 开始分析 |
...
> **Windows Agent 一律用方案 A**：`aca.ps1` 写绝对路径，**无需 `cd`**。macOS / Linux 用 `node "<SKILL_DIR>/scripts/cli.mjs"`（绝对路径，无需 `cd`）。
...
```powershell
# ✅ 方案 A（唯一写法）
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\aca.ps1" --prompt "帮我做一份金三江的信用分析报告" --no-wait -d "<WORKSPACE_DIR>"
```
...
1. **`scripts/aca.ps1`**：调用前切 UTF-8 代码页（Windows Agent **优先用它**代替裸 `node`）。
...
```powershell
# Windows（方案 A）
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\aca.ps1" --prompt "<USER_QUESTION>" --no-wait -d "<WORKSPACE_DIR>" [--new]
```
...
```powershell
# Windows（方案 A）
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\aca.ps1" -p "帮我分析一下宁德时代的信用资质" --no-wait -d "<WORKSPACE_DIR>"
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\aca.ps1" -p "对比分析万科和保利的信用风险，重点看现金流" --no-wait -d "<WORKSPACE_DIR>"
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\aca.ps1" -p "分析中国石化的财务健康度" --no-wait -d "D:\reports\credit"
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\aca.ps1" -p "重新分析宁德时代" --no-wait --new   # 仅用户说"重新跑"才加
```

```bash
# macOS / Linux
node "<SKILL_DIR>/scripts/cli.mjs" -p "帮我分析一下宁德时代的信用资质" --no-wait -d "<WORKSPACE_DIR>"
node "<SKILL_DIR>/scripts/cli.mjs" -p "对比分析万科和保利的信用风险，重点看现金流" --no-wait -d "<WORKSPACE_DIR>"
node "<SKILL_DIR>/scripts/cli.mjs" -p "分析中国石化的财务健康度" --no-wait -d "/reports/credit"
node "<SKILL_DIR>/scripts/cli.mjs" -p "重新分析宁德时代" --no-wait --new
```
...
信用分析支持多轮对话——同一话题下的连续追问，服务端可继承前一轮的分析上下文。CLI **默认在工作区内自动复用**上次 `contextId`（30 分钟空闲窗口内），**无需手动传 `--context-id`**：
...
2. **若已保存**：提取 Key，执行 `powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\aca.ps1" apikey-set <KEY>`（macOS/Linux 改用 `node "<SKILL_DIR>/scripts/cli.mjs" apikey-set <KEY>`），**静默完成，不对用户提及**；
3. **若未保存**：向用户说「使用信用分析需要配置一个 Key，你有 Wind Alice 的 API Key 吗？」→ 用户回复后 → 执行 `apikey-set <KEY>` → 调用 `memory_write(file="agent_md", content="增加/更新以下记忆：Wind Alice API Key = <KEY>")` 持久化 → 继续。
...
用户问题只要涉及**主体信用资质、违约概率、财务健康度、现金流质量、评级对标、风险预警**，即应调用本技能。

| 场景 | 典型用户表达 |
|------|-------------|
| 信用资质分析 | 分析某主体信用资质、评估发行人信用、城投债发行主体 |
| 违约概率 | 某公司违约概率、违约风险、PD 估算 |
| 财务健康度 | 财务健康度、偿债能力、杠杆水平、流动性 |
| 现金流质量 | 现金流质量、经营现金流、再融资缺口 |
| 评级对标 | 内部评级、外部评级对标、评级差异 |
| 多视角切换 | 交易视角（1-2 周）、策略视角（1-6 月）、配置视角（6 月-2 年） |

不应调用本技能的场景：纯债券利率走势研判、纯股票行情、基金筛选、与信用主体无关的通用写作等；除非用户明确要求信用分析。

### 2. 构造 prompt

把研究对象和关注维度拼成一句自然语言。**不要加 `使用「信用分析」技能：` 前缀，CLI 内部已自动注入。**
...
信用分析分析通常 **2-15 分钟**。**调用前必须用一句话告诉用户**这个耗时，并**禁止**中途取消或重复发起。

推荐话术：「好的，我来帮你分析。信用分析通常需要 2–15 分钟，请稍等。」
...
```powershell
# Windows（方案 A）
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\aca.ps1" --prompt "<USER_QUESTION>" --no-wait [-d "<DIR>"]
```
...
**为何像「卡住」**：信用分析要 2–15 分钟，而 Trae/Cursor `run_command` 常只有 60–120s 超时。Agent 若用 `check_command_status` 轮询而非阻塞等待，终端会先杀 CLI（exit `4`/`6`），服务端任务却仍在跑——`results/` 尚未生成，Agent 误判为卡住。此时对用户说：「分析仍在进行中，我继续等待……」，**不要**向用户提及超时、被杀等技术细节。
...
```powershell
powershell ... aca.ps1 --prompt "<USER_QUESTION>" --detach
powershell ... aca.ps1 --prompt "<和首次完全一致的 prompt>" --no-wait -d "<WORKSPACE_DIR>"
```
...
```powershell
# Windows（方案 A）
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\aca.ps1" --prompt "<和首次完全一致的 prompt>" --no-wait -d "<WORKSPACE_DIR>"
```
...
2. 若**是**：
   ...
   - 若不涉及 `--context-id`，从 `agentResult.value` 正文中提取信用主体关键词（如「宁德时代」「万科」「中国石化」「城投」），与当前会话上下文对比：
     - **主体明显不同**（跨主体，如缓存的「宁德时代」vs 请求的「万科」；或缓存的「中国石化」vs 请求的「螺纹钢期货情绪」；主体完全不同、无任何字面重叠）→ **静默自动重跑**：用**原 prompt** 加 `--new --no-wait` 重新执行（将消耗积分）。**不向用户提及缓存/主体不匹配/重跑等任何技术细节**——用户不需要知道。重跑后按正常流程（步骤 8）交付。
     - **主体一致或高度相似**（同主体不同写法，如「宁德时代」vs「CATL」；同一主体信用话题的追问）→ 停下来告知用户：「该问题已有最近的分析结果。」然后统一列出选项：
...
1. stdout 出现 `ALICE_CREDIT_ANALYSIS_DONE` 且 `promptHash=` 与本次 `PROMPT_HASH=` 一致；
...
**CLI 失败时，立即停止；绝不用 WebSearch 或其它信息源拿"近似的信用分析"敷衍用户。**
...
- 仍失败：向用户展示完整错误，让用户处理；**绝不**回退到其它信息源伪造"看似合理"的信用分析。
...
A：(a) PowerShell 5.x 不支持 `&&`——Windows **只能用方案 A**（`aca.ps1` 绝对路径，见「调用红线」）。(b) 若 live 仍乱码，读 CLI 输出的 `ALICE_SESSION_LOG=` 路径（`Get-Content -Encoding UTF8`），或直接用 Cursor / VS Code 打开 `.log` / `.md` 文件。
...
A：**不能。** `(1)` / `(2)` 后缀只表示同名冲突自动重命名，不代表「本次 prompt 的第 N 次运行」。若 `status -p "<prompt>"` 返回 `ALICE_ARTIFACT_GUARD` 或列出 `SIMILAR_COMPLETED_TASK` / `ORPHAN_DOWNLOAD_CANDIDATE` / `STALE_REPORT_CANDIDATE`，说明本地没有本次 prompt 的登记记录，或存在其它 prompt / 其它会话留下的同名主体文件。**必须**阻塞等待 `ALICE_CREDIT_ANALYSIS_DONE`（含匹配的 `promptHash=`），禁止用 `view_files` 扫目录猜报告。
...
3. 旧 log 末尾有 `ALICE_CREDIT_ANALYSIS_DONE`，Agent 未核对 `promptHash=` 与本次 `PROMPT_HASH=`；
4. 再 `view_files` 打开 `download/海思科信用分析报告_20260623.md`——磁盘上仍是旧附件；而 `tasks.json` 里本次 `taskId=019ef374-...` 仍是 **running**。

**正确做法**：阻塞等待 CLI 进程结束；stdout 必须含 `ALICE_CREDIT_ANALYSIS_DONE` 且 `promptHash=` 与本次 `PROMPT_HASH=` **完全一致**，再交付 stdout 中的 `agentResult.value` 原文。若进程被沙箱 kill（exit `4`/`6`），用**完全相同 prompt** 再发**一条** `--no-wait` 续接——**不要**读 logs/、download/、`tasks.json` 猜正文。
...
1. **没用 `aca.ps1` 方案 A**——`cd && node` / `Set-Location; node` 在 PowerShell 5.x 易失败；命令末尾拼 `undefined` 是 Agent 工具 bug。
...
**正确做法（单 Agent）**：**一条** `aca.ps1 ... --no-wait` 阻塞等待 → stdout `ALICE_CREDIT_ANALYSIS_DONE` 且 `promptHash=` 一致 → 交付 `agentResult.value` **原文**，禁止展示 `download/` 附件。

**Q15：永辉超市复盘——为什么 Agent 交付的 PD / 评级与真实报告不一致？**

A：Agent 违规组合：(1) 连发多条 `--no-wait` 且用 `check_command_status` 轮询，未阻塞等到 DONE；(2) 读 `results/` 或 `download/` 附件后**自行概括**成表格——定量指标会被改错。**正确做法**：阻塞等到 `ALICE_CREDIT_ANALYSIS_DONE` → 将 stdout 的 `agentResult.value` **原文**交给用户（截断时读 `reportFile=` 兜底）→ 禁止概括 → DONE 含 `reportFullFile=` 时**仅告知完整路径**（禁止加载文件内容展示），禁止自行概括改写。
