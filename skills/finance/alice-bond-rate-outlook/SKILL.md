---
name: alice-bond-rate-outlook
description: 系统化债券利率走势研判，支持交易/策略/配置多视角切换，覆盖宏观、流动性、供需、曲线、技术情绪五大维度，结合量化评分与压力测试，输出含走势判断、评分、置信度与建议的结构化报告。适用于今天债市怎么看、利率走势、久期配置等场景。
description_zh: "万得 Alice 债券利率走势研判 CLI：支持交易/策略/配置多视角切换，覆盖五大维度系统化研判，输出含利率走势判断、量化评分、交易配置建议的结构化报告。"
description_en: "Wind Alice Bond Rate Outlook CLI — systematic bond market interest rate trend analysis framework, supporting adaptive switching across trading (1-2 weeks), strategy (1-6 months), and allocation (6 months-2 years) perspectives, covering macro fundamentals, liquidity, supply-demand, yield curve structure, and technical sentiment, integrating quantitative scoring and stress testing."
version: 1.0.3
author: WindAlice
tags: [bond, rate, yield-curve, duration, monetary-policy, liquidity, trading, allocation, wind-alice]
display_name: "债券利率走势研判"
display_name_en: "Bond Rate Outlook"
visibility: "public"
---
# Bond Rate Outlook（万得AI-债券利率走势研判）

## 技能介绍

### 官方技能：万得AI-债券利率走势研判

系统化债券市场利率走势分析框架，支持**交易（1-2周）、策略（1-6个月）、配置（6个月-2年）**三种投资视角自适应切换，覆盖**宏观基本面、流动性、供需、曲线结构、技术情绪**五大维度，结合量化评分与压力测试，生成利率走势判断与债券交易/配置策略。

### 适用场景

| 场景 | 解决的问题 |
|------|-----------|
| 利率走势研判 | 判断利率上行/下行方向，输出量化评分与置信度 |
| 久期配置决策 | 明确拉长还是缩短久期，给出配置方向建议 |
| 曲线形态分析 | 判断牛陡/牛平/熊陡/熊平，辅助曲线交易策略 |
| 短线交易机会 | 识别今日/本周债市短线交易窗口与国债期货机会 |
| 货币政策影响 | 评估降准/降息/公开市场操作对债市的传导效应 |
| 年度配置方向 | 跨周期布局，输出中长期债券配置系统性机会判断 |

### 核心能力

1. **宏观基本面分析**：经济增长（GDP、PMI、工业增加值）与通胀（CPI、PPI）趋势；货币政策取向；财政政策与政府债券供给节奏；海外利率与汇率环境传导
2. **流动性监测**：银行间资金面（DR007、R007、Shibor）；央行公开市场操作与 MLF/TMLF 投放；机构杠杆水平与配置需求
3. **供需格局分析**：利率债供给节奏；商业银行、保险、基金等配置盘需求；一级市场发行情况与倍数
4. **收益率曲线结构**：期限利差历史分位数；曲线形态判断（牛陡/牛平/熊陡/熊平）；隐含税率与国开-国债利差
5. **技术情绪与量化评分**：现券与期货技术指标；市场情绪；五大维度加权量化评分；压力测试与情景分析
...
| 顺序 | 做什么 | 禁止 |
|------|--------|------|
| 0 | 读同目录 **[`AGENT.md`](./AGENT.md)** | 禁止 `search_codebase`、禁止查库猜债市数据 |
| 1 | **一条** `abro.ps1 --prompt "<用户原话>" --no-wait -d "<WORKSPACE_DIR>"` [--context-id <上一轮contextId>] [--new-session] | 禁止裸 `node`、禁止 `cd` 到 skill 目录 |
| 2 | `run_command` / Shell **超时 ≥ 1200 秒**；**阻塞等到命令退出** | 禁止秒退后说「后台 PID / 稍后再问继续查」 |
| 3 | stdout 含 `ALICE_BOND_RATE_OUTLOOK_DONE` | 无 DONE = 未完成，禁止编造 |
| 4 | **逐字**交付 `agentResult.value`；DONE 含 `reportFullFile=` 时先 `present_files` 再交付正文 | 禁止自行概括、禁止读附件正文 |

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\abro.ps1" --prompt "<USER_QUESTION>" --no-wait -d "<WORKSPACE_DIR>"
```
...
>
> **什么时候要你动手**：只有**切到完全无关的新话题**时，加 `--new-session` 强制开新会话，避免无关上文污染。同话题追问（哪怕换品种、加维度）**什么都不用加**，自动续接。
>
> **常见场景**:用户先问「今天债市怎么看」,你交付后用户问「**那国债期货呢**」--同话题延伸追问,**什么都不加**,自动续接。「**那曲线形态呢 / 后市怎么看**」也**什么都不加**。「**宁德时代信用怎么样**」是无关新主题,加 `--new-session`。
>
...
当用户问题涉及**利率走势研判、久期配置、曲线形态、货币政策对债市影响、债市短线交易、年度配置方向**等场景时，应调用本技能。不应在纯个股信用分析、与债市无关的宏观个股问题等场景硬套本技能。
...
```powershell
# ✅ 方案 A（唯一推荐）：abro.ps1 绝对路径，无需 cd，自带 UTF-8 代码页
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\abro.ps1" --prompt "<USER_QUESTION>" --no-wait -d "<WORKSPACE_DIR>"
```
...
```bash
node /path/to/alice-bond-rate-outlook/scripts/cli.mjs --prompt "<USER_QUESTION>" --no-wait -d "<WORKSPACE_DIR>"
```
...
| 信号 | 含义 / Agent 必须怎么做 |
|------|------------------------|
| stdout 含 `ALICE_BOND_RATE_OUTLOOK_DONE taskId=... promptHash=... reportFile=... reportFullFile=... attachmentFile=...` | **唯一**真正完成信号；**必须先核对 `promptHash=` 与本次 `--prompt` 一致**，再以 stdout 中的 `agentResult.value` 原文交付用户（stdout 被截断时读 `reportFile=` 兜底）；**若有 `reportFullFile=` 必须把绝对路径转告用户；若有 `attachmentFile=` 也必须把绝对路径转告用户** |
...
| 退出码 `78`（环境受限，无法保存任务状态） | 当前环境无法保存任务状态（未开启完全访问权限）；**未向服务端发请求**；告知用户「当前环境无法运行 债券利率走势研判，请在工具中开启完全访问权限后重试」；禁止换 prompt / `--new` / 反复重试 |
...
5. **交付 agentResult.value + 告知报告路径**——将 CLI stdout 中的 `agentResult.value` **原文**交给用户；**禁止**自行总结、摘录、重写表格或「用自己的话」复述。若 stdout 被沙箱截断，读取 DONE 行 `reportFile=` 落盘正文（跳过 HTML 注释头）。**DONE 含 `reportFullFile=` 时必须告知完整路径**（**保留 CLI 内联的 markdown 链接格式**，如 `[债市利率走势研判报告.md](file:///<工作空间>/债市利率走势研判报告.md) (<工作空间>\债市利率走势研判报告.md)`）——CLI 已把链接内联到 `agentResult.value` 正文中，**原样输出即可**；**禁止**把链接改成行内代码 `` `C:\...\报告.md` ``，行内代码在 Cursor / Trae 等宿主里**不可点击**，用户无法直接打开文件。**DONE 含 `attachmentFile=` 时也必须保留 markdown 链接格式告知完整路径**（如 `[财务模型.xlsx](file:///<工作空间>/财务模型.xlsx) (<工作空间>\财务模型.xlsx)`）。**禁止加载 `reportFullFile=` / `attachmentFile=` 文件内容展示给用户**——`agentResult.value` 已是面向用户的核心分析摘要，完整报告和数据附件供用户本地查阅，不需要 Agent 读取并展示。 **交付 = Agent 亲自打字复述正文**（不折叠），不能只跑完 CLI 让 stdout 显示。**若 DONE 含 `reportFullFile=`，先在独立一条消息调 `present_files` 呈现报告文件为可点击卡片（CLI 已把附件下载到工作空间 `process.cwd()`，路径必在工作空间内，无需传 `-d`）；随后在另一条纯文本消息里输出 `agentResult.value`（本条禁任何工具调用，否则整条被折叠）**。
...
```powershell
# Windows（方案 A）
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\abro.ps1" apikey-set <KEY>
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\abro.ps1" apikey-get
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\abro.ps1" apikey-clear
```
...
3. **无 Key** → 向用户说「使用债券利率走势研判需要配置一个 Key，你有 Wind Alice 的 API Key 吗？」→ 用户回复后 → 执行 `apikey-set <KEY>` → `memory_write(file="agent_md", content="增加/更新以下记忆：Wind Alice API Key = <KEY>")` → 继续后续步骤。
...
| 场景 | 发生了什么 |
|------|-----------|
| 首次使用 | Agent：「使用债券利率走势研判需要配置一个 Key，你有 Wind Alice 的 API Key 吗？」→ 用户回复 → Key 保存到 `agent_md` → 自动配置 → 开始分析 |
...
> **Windows Agent 一律用方案 A**：`abro.ps1` 写绝对路径，**无需 `cd`**。macOS / Linux 用 `node "<SKILL_DIR>/scripts/cli.mjs"`（绝对路径，无需 `cd`）。
...
```powershell
# ✅ 方案 A（唯一写法）
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\abro.ps1" --prompt "今天债市怎么看？国债期货有没有短线机会？" --no-wait -d "<WORKSPACE_DIR>"
```
...
1. **`scripts/abro.ps1`**：调用前切 UTF-8 代码页（Windows Agent **优先用它**代替裸 `node`）。
...
```powershell
# Windows（方案 A）
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\abro.ps1" --prompt "<USER_QUESTION>" --no-wait -d "<WORKSPACE_DIR>" [--new]
```
...
```powershell
# Windows（方案 A）
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\abro.ps1" -p "今天债市怎么看？国债期货有没有短线机会？" --no-wait -d "<WORKSPACE_DIR>"
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\abro.ps1" -p "未来利率走势如何？现在应该拉长还是缩短久期？" --no-wait -d "<WORKSPACE_DIR>"
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\abro.ps1" -p "今年债市配置方向怎么看？" --no-wait -d "D:\reports\bond"
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\abro.ps1" -p "重新研判本周债市走势" --no-wait --new   # 仅用户说"重新跑"才加
```

```bash
# macOS / Linux
node "<SKILL_DIR>/scripts/cli.mjs" -p "今天债市怎么看？国债期货有没有短线机会？" --no-wait -d "<WORKSPACE_DIR>"
node "<SKILL_DIR>/scripts/cli.mjs" -p "未来利率走势如何？现在应该拉长还是缩短久期？" --no-wait -d "<WORKSPACE_DIR>"
node "<SKILL_DIR>/scripts/cli.mjs" -p "今年债市配置方向怎么看？" --no-wait -d "/reports/bond"
node "<SKILL_DIR>/scripts/cli.mjs" -p "重新研判本周债市走势" --no-wait --new
```
...
债券利率走势研判支持多轮对话——同一话题下的连续追问，服务端可继承前一轮的分析上下文。CLI **默认在工作区内自动复用**上次 `contextId`（30 分钟空闲窗口内），**无需手动传 `--context-id`**：
...
2. **若已保存**：提取 Key，执行 `powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\abro.ps1" apikey-set <KEY>`（macOS/Linux 改用 `node "<SKILL_DIR>/scripts/cli.mjs" apikey-set <KEY>`），**静默完成，不对用户提及**；
3. **若未保存**：向用户说「使用债券利率走势研判需要配置一个 Key，你有 Wind Alice 的 API Key 吗？」→ 用户回复后 → 执行 `apikey-set <KEY>` → 调用 `memory_write(file="agent_md", content="增加/更新以下记忆：Wind Alice API Key = <KEY>")` 持久化 → 继续。
...
用户问题只要涉及**债券市场利率走势、久期配置、曲线形态、货币政策对债市的影响、债市短线交易或中长期配置方向**，即应调用本技能。

| 场景 | 典型用户表达 |
|------|-------------|
| 利率走势研判 | 今天债市怎么看、未来利率怎么走、10 年期国债收益率展望 |
| 久期配置 | 拉长还是缩短久期、债券配置方向、年度债市机会 |
| 曲线形态 | 牛陡/牛平/熊陡/熊平、期限利差、做陡还是做平 |
| 短线交易 | 国债期货机会、本周债市短线、今日资金面 |
| 货币政策传导 | 降准降息影响、MLF/OMO 对债市、DR007 资金面 |
| 多视角切换 | 交易视角（1-2 周）、策略视角（1-6 月）、配置视角（6 月-2 年） |

不应调用本技能的场景：纯个股信用分析、纯股票行情、基金筛选、与债市利率无关的通用写作等；除非用户明确要求债市利率走势研判。

### 2. 构造 prompt

把研究对象和关注维度拼成一句自然语言。**不要加 `使用「债券利率走势研判」技能：` 前缀，CLI 内部已自动注入。**
...
债券利率走势研判分析通常 **2-15 分钟**。**调用前必须用一句话告诉用户**这个耗时，并**禁止**中途取消或重复发起。

推荐话术：「好的，我来帮你分析。债券利率走势研判通常需要 2–15 分钟，请稍等。」
...
```powershell
# Windows（方案 A）
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\abro.ps1" --prompt "<USER_QUESTION>" --no-wait [-d "<DIR>"]
```
...
**为何像「卡住」**：债券利率走势研判要 2–15 分钟，而 Trae/Cursor `run_command` 常只有 60–120s 超时。Agent 若用 `check_command_status` 轮询而非阻塞等待，终端会先杀 CLI（exit `4`/`6`），服务端任务却仍在跑——`results/` 尚未生成，Agent 误判为卡住。此时对用户说：「分析仍在进行中，我继续等待……」，**不要**向用户提及超时、被杀等技术细节。
...
```powershell
powershell ... abro.ps1 --prompt "<USER_QUESTION>" --detach
powershell ... abro.ps1 --prompt "<和首次完全一致的 prompt>" --no-wait -d "<WORKSPACE_DIR>"
```
...
```powershell
# Windows（方案 A）
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\abro.ps1" --prompt "<和首次完全一致的 prompt>" --no-wait -d "<WORKSPACE_DIR>"
```
...
2. 若**是**：
   ...
   - 若不涉及 `--context-id`，从 `agentResult.value` 正文中提取债市主体/视角关键词（如「10 年期国债」「国债期货」「曲线形态」「久期配置」），与当前会话上下文对比：
     - **主体明显不同**（跨品种/跨视角，如缓存的「今天债市怎么看」vs 请求的「帮我分析宁德时代信用」；或缓存的「10 年期国债」vs 请求的「螺纹钢期货情绪」；主体完全不同、无任何字面重叠）→ **静默自动重跑**：用**原 prompt** 加 `--new --no-wait` 重新执行（将消耗积分）。**不向用户提及缓存/主体不匹配/重跑等任何技术细节**——用户不需要知道。重跑后按正常流程（步骤 8）交付。
     - **主体一致或高度相似**（同品种不同写法，如「10 年期国债」vs「十年期国债收益率」；同一债市话题的追问）→ 停下来告知用户：「该问题已有最近的分析结果。」然后统一列出选项：
...
1. stdout 出现 `ALICE_BOND_RATE_OUTLOOK_DONE` 且 `promptHash=` 与本次 `PROMPT_HASH=` 一致；
...
**CLI 失败时，立即停止；绝不用 WebSearch 或其它信息源拿"近似的债券利率走势研判"敷衍用户。**
...
- 仍失败：向用户展示完整错误，让用户处理；**绝不**回退到其它信息源伪造"看似合理"的债券利率走势研判。
...
A：(a) PowerShell 5.x 不支持 `&&`——Windows **只能用方案 A**（`abro.ps1` 绝对路径，见「调用红线」）。(b) 若 live 仍乱码，读 CLI 输出的 `ALICE_SESSION_LOG=` 路径（`Get-Content -Encoding UTF8`），或直接用 Cursor / VS Code 打开 `.log` / `.md` 文件。
...
A：**不能。** `(1)` / `(2)` 后缀只表示同名冲突自动重命名，不代表「本次 prompt 的第 N 次运行」。若 `status -p "<prompt>"` 返回 `ALICE_ARTIFACT_GUARD` 或列出 `SIMILAR_COMPLETED_TASK` / `ORPHAN_DOWNLOAD_CANDIDATE` / `STALE_REPORT_CANDIDATE`，说明本地没有本次 prompt 的登记记录，或存在其它 prompt / 其它会话留下的同名主体文件。**必须**阻塞等待 `ALICE_BOND_RATE_OUTLOOK_DONE`（含匹配的 `promptHash=`），禁止用 `view_files` 扫目录猜报告。
...
3. 旧 log 末尾有 `ALICE_BOND_RATE_OUTLOOK_DONE`，Agent 未核对 `promptHash=` 与本次 `PROMPT_HASH=`；
4. 再 `view_files` 打开 `download/海思科信用分析报告_20260623.md`——磁盘上仍是旧附件；而 `tasks.json` 里本次 `taskId=019ef374-...` 仍是 **running**。

**正确做法**：阻塞等待 CLI 进程结束；stdout 必须含 `ALICE_BOND_RATE_OUTLOOK_DONE` 且 `promptHash=` 与本次 `PROMPT_HASH=` **完全一致**，再交付 stdout 中的 `agentResult.value` 原文。若进程被沙箱 kill（exit `4`/`6`），用**完全相同 prompt** 再发**一条** `--no-wait` 续接——**不要**读 logs/、download/、`tasks.json` 猜正文。
...
1. **没用 `abro.ps1` 方案 A**——`cd && node` / `Set-Location; node` 在 PowerShell 5.x 易失败；命令末尾拼 `undefined` 是 Agent 工具 bug。
...
**正确做法（单 Agent）**：**一条** `abro.ps1 ... --no-wait` 阻塞等待 → stdout `ALICE_BOND_RATE_OUTLOOK_DONE` 且 `promptHash=` 一致 → 交付 `agentResult.value` **原文**，禁止展示 `download/` 附件。

**Q15：永辉超市复盘——为什么 Agent 交付的 PD / 评级与真实报告不一致？**

A：Agent 违规组合：(1) 连发多条 `--no-wait` 且用 `check_command_status` 轮询，未阻塞等到 DONE；(2) 读 `results/` 或 `download/` 附件后**自行概括**成表格——定量指标会被改错。**正确做法**：阻塞等到 `ALICE_BOND_RATE_OUTLOOK_DONE` → 将 stdout 的 `agentResult.value` **原文**交给用户（截断时读 `reportFile=` 兜底）→ 禁止概括 → DONE 含 `reportFullFile=` 时**仅告知完整路径**（禁止加载文件内容展示），禁止自行概括改写。
