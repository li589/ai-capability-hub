...
// 本 CLI 仅承载万得 Alice 的「债券利率走势研判」技能。
// 服务端通过 prompt 文本前缀 `使用「<Skill 名>」技能：` 识别 Skill。
const ALICE_SKILL_NAME_ZH = "债券利率走势研判";
...
//        ALICE_BOND_RATE_OUTLOOK_DONE taskId=<id> promptHash=<hash> reportFile=<abs> reportFullFile=<abs>
//      Agent 只需匹配该行就能确认成功，比解析 ALICE_BOND_RATE_OUTLOOK_STATUS=COMPLETED
...
/**
 * 输出 ALICE_BOND_RATE_OUTLOOK_DONE 行。幂等：同一进程内只输出一次。
...
function emitAnalysisDone({ taskId, promptHash } = {}) {
  ...
  const parts = ["ALICE_BOND_RATE_OUTLOOK_DONE"];
  ...
  // 🔴 交付提醒框：在 DONE 行之前、agentResult.value 正文已全部流式打印之后输出。
  // 卡在「正文打完、即将判定完成」的决策点，提醒 Agent 必须把 agentResult.value
  // 逐字复述成正文回复，否则结果停留在工具 stdout 里会被宿主（WorkBuddy 等）折叠
  // 进「已完成」块、用户看不到。顶部文档规则离执行点太远、长流程里易被稀释；此处
  // 为最不易漏的即时提醒（Agent 必然读 stdout）。ALICE_DELIVER_AS_TEXT=1 供自检引用。
  for (const line of [
    "================ ⚠️ 交付提醒（必须执行）================",
  ...
  console.log(
    "[CLI] 上一行 ALICE_BOND_RATE_OUTLOOK_DONE 是本进程任务"
      + "真正完成"
...
function installStrictExitHook() {
  ...
  process.on("exit", () => {
    ...
    try {
      console.error(
        "[CLI][strict] 进程退出但未输出 ALICE_BOND_RATE_OUTLOOK_DONE；强制退出码="
          + EXIT_STRICT_NO_DONE
          + "。",
      );
      console.error(
...
// strict 模式专用退出码：
// - 6 = 进程退出时未输出 ALICE_BOND_RATE_OUTLOOK_DONE，且 exitCode 仍为 0；
 ...
 * 始终使用「债券利率走势研判」技能：
 *   - 文本前缀：`使用「债券利率走势研判」技能：<原 prompt>`
 *   - chatMode "12" + originalChatMode "4"，不携带 agentCard。
 *   - contextId：若调用方通过 `reuseContextId` 传入（同一会话续问），复用该 ID；
 *                否则生成新 UUID。taskId 始终新生成。
 *
 * @param {string} prompt                    用户原始问题
 * @param {object} [opts]
 * @param {string} [opts.reuseContextId]     同一会话复用的 contextId
 */
...
function clearCompletedRecordForResubmit(registry, promptHash, { logPrefix = "[CLI]" } = {}) {
  ...
  console.log(
    `${logPrefix} 若只需查询历史落盘路径，请用：alice-bond-rate-outlook status -p "<同一问题>"`,
  );
...
function printAttachBanner({ taskId, contextId, startedAt }) {
  ...
  console.log("");
  console.log("ALICE_BOND_RATE_OUTLOOK_STATUS=RUNNING");
...
function printCliRunStartNotice() {
  const lines = [
    "ALICE_BOND_RATE_OUTLOOK_STATUS=RUNNING",
    "[CLI] 债券利率走势研判已启动：本进程将阻塞运行直至成功（agentResult + [完成]）或失败，通常需 2-15 分钟。",
    "[CLI] [等待中] 心跳期间无新输出属正常；禁止用 check_command_status 轮询替代等待本进程结束。",
    "[CLI] 见到 [任务已受理] 仅表示已提交，不等于报告完成；必须等到 ALICE_BOND_RATE_OUTLOOK_STATUS=COMPLETED。",
  ];
  for (const line of lines) {
    console.error(line);
  }
...
function persistAgentResultIfAny({ registry, taskId, promptHash, prompt }) {
  ...
  if (!text) return null;
  const header = [
    `<!-- alice-bond-rate-outlook：本次 Alice agentResult.value 已落盘 -->`,
    `<!-- 重要：本文件是 Alice 响应正文（agentResult.value），并不一定等于"完整报告"。 -->`,
...
function printCliRunEndNotice({ success, exitCode, elapsed }) {
  const lines = success
    ? [
        "ALICE_BOND_RATE_OUTLOOK_STATUS=COMPLETED",
    ...
    : [
        "ALICE_BOND_RATE_OUTLOOK_STATUS=FAILED",
...
function usage() {
  return [
    "alice-bond-rate-outlook — 调用万得 Alice Agent「债券利率走势研判」技能，系统化研判债券市场利率走势（交易/策略/配置视角）并流式输出",
    "",
    "Usage:",
    "  alice-bond-rate-outlook --prompt <QUESTION> [--download-dir <DIR>] [--new]",
    "  alice-bond-rate-outlook --prompt <QUESTION> --no-wait                       # 沙箱推荐：CLI 内部自旋 probe 直到完成（默认）",
    "  alice-bond-rate-outlook --prompt <QUESTION> --no-wait --once                # 单次探针：几秒内返回（脚本/调试）",
    "  alice-bond-rate-outlook --prompt <QUESTION> --no-wait \\",
    "      --watch-interval 60 --watch-timeout 1800                              # 自定义轮询节奏",
    "  alice-bond-rate-outlook status --prompt <QUESTION>                        # 查询落盘路径（不访问服务端）",
    "  alice-bond-rate-outlook check-conflict --prompt <QUESTION>                # [已废弃] 主调用 --no-wait 已内置去重，无需预检",
    "  alice-bond-rate-outlook apikey-set <KEY>",
    "  alice-bond-rate-outlook apikey-get",
    "  alice-bond-rate-outlook apikey-clear",
    "  alice-bond-rate-outlook --help",
    "",
    "Options:",
    "  --prompt,       -p <QUESTION>   用户提问，例如\"今天债市怎么看？国债期货有没有短线机会？\"（必填）",
    "  --download-dir, -d <DIR>        保留兼容；不影响下载目录——附件统一落当前工作空间 process.cwd()（见下文）。仅影响报告附件落盘，不影响 tasks.json 等状态（状态固定走 ~/.wind-alice/）",
    ...
    "                                  ALICE_BOND_RATE_OUTLOOK_DONE 行，会把 exitCode=0 强制改成 6，",
    ...
    "",
    "退出码:",
    "  0   成功：必定伴随 stdout 含 ALICE_BOND_RATE_OUTLOOK_DONE 行（Agent 判定任务完成的唯一确定信号）",
    "  1   一般失败 / 任务终态为 failed",
    "  2   参数错误 / KEY_MISSING",
    "  4   --no-wait 专用：已达全程自旋上限（默认 60min）仍未完成；Agent 再执行一次相同 --no-wait 续接",
    "  5   --no-wait 专用：--once 且本地无任务记录（默认 --no-wait 会自动提交）",
    "  6   strict 模式兜底：进程退出但未输出 ALICE_BOND_RATE_OUTLOOK_DONE（通常表示进程被沙箱终止）；",
...
async function downloadCollectedFiles(apiKey, override, ctx = {}) {
  ...
  for (const { url, filename } of items) {
    ...
    if (result?.ok) {
      console.log(`- ${filename}`);
      console.log(`  已保存：${formatLocalFileLink(result.path)}`);
 ...
 * `ALICE_BOND_RATE_OUTLOOK_STATUS=...`，由调用方（单次 probe / watch 轮询）在合适
...
async function runProbeStep({ registry, taskId, promptHash, prompt, url, headers, apiKey, downloadDir }) {
  ...
  if (probe.kind === "working") {
    ...
    console.log(
      "[CLI] 本进程尚未输出 ALICE_BOND_RATE_OUTLOOK_DONE；logs/ 下其它 .session.log 内的 DONE 行与本 PROMPT_HASH 不一致则无效。",
    );
 ...
 * 调用方读到的 `ALICE_BOND_RATE_OUTLOOK_STATUS=...` 语义一致。
 *
...
function publishFinalStatus(outcome, { suggestExternalPoll = false } = {}) {
  switch (outcome) {
    case "completed":
      console.log("ALICE_BOND_RATE_OUTLOOK_STATUS=COMPLETED");
      emitAnalysisDone();
      break;
    case "failed":
      console.log("ALICE_BOND_RATE_OUTLOOK_STATUS=FAILED");
      break;
    case "no-task":
      console.log("ALICE_BOND_RATE_OUTLOOK_STATUS=NO_TASK");
      break;
    case "working":
    default:
      if (suggestExternalPoll) {
        ...
        console.log(
          "[探针] 禁止读取 download/ 同名报告或 STALE_REPORT_CANDIDATE；必须见到 ALICE_BOND_RATE_OUTLOOK_DONE 且 promptHash= 一致。",
        );
      }
      console.log("ALICE_BOND_RATE_OUTLOOK_STATUS=RUNNING");
      break;
...
async function runStatusCommand({ prompt, downloadDir }) {
  ...
  if (!record) {
    ...
    if (replayCandidates.length > 0) {
      ...
      console.log(
        "[status] 请把下列候选项列给用户后，用已有任务的**原 prompt** 重放，或阻塞等待本次 --no-wait 的 DONE 行。",
      );
...
async function main() {
  ...
  // analysis 主路径：默认开启 strict 模式（未输出 ALICE_BOND_RATE_OUTLOOK_DONE 即视为
  ...
  if (noWait) {
    ...
    console.log("ALICE_AGENT_REDLINE=禁止在 stdout 出现 ALICE_BOND_RATE_OUTLOOK_DONE 之前 view_files/read download/ 主体报告");
      ...
      mainLoop: while (true) {
        for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
          ...
          if (!startBannerPrinted) {
            ...
            console.log(
              `[开始] 已向 Alice「${ALICE_SKILL_NAME_ZH}」发起请求；债券利率走势研判通常耗时 2-5 分钟，期间会定期播报等待进度，请勿中断。`,
            );
          }
