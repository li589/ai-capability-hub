...
if (isHelp) {
  console.log(
    [
      'alice-bond-rate-outlook — 调用万得 Alice Agent「债券利率走势研判」，覆盖交易/策略/配置多视角与宏观流动性/曲线/技术情绪五大维度，用户原话直接透传给服务端',
      "",
      "Usage:",
      "  alice-bond-rate-outlook --prompt <QUESTION> [--download-dir <DIR>] [--new]",
      "  alice-bond-rate-outlook --prompt <QUESTION> --detach                        # 后台跑：立刻返回 PID 与日志路径，进程脱离当前会话",
      "  alice-bond-rate-outlook --prompt <QUESTION> --no-wait                       # 沙箱推荐：CLI 内部自旋直到完成（默认）",
      ...
      "",
      "Windows UTF-8:",
      "  powershell -NoProfile -ExecutionPolicy Bypass -File scripts/abro.ps1 --prompt <Q> --no-wait",
      ...
      "  --session-scope <ID>            （兼容，已不推荐）旧式宿主隔离标识；--context-id 已按会话隔离、不再依赖",
      "                                  共享文件，故通常无需传。仅旧式 --continue-session 路径才用到它；也可用环境变量",
      ...
      "  --no-strict                     关闭 strict 模式（默认开启）；未输出 ALICE_BOND_RATE_OUTLOOK_DONE 时",
      ...
      "  会话状态文件（仅旧式 --continue-session 路径用到，--context-id 不读不写共享文件）：",
      ...
      "",
      "退出码:",
      "  0   成功：必定伴随 stdout 含 ALICE_BOND_RATE_OUTLOOK_DONE 行（含 promptHash= / reportFullFile=；Agent 判定任务完成的唯一确定信号）",
      "  1   一般失败 / 任务终态为 failed",
      ...
      '  Set-Location "D:\\path\\to\\alice-bond-rate-outlook"; node scripts/cli.mjs --prompt "今天债市怎么看？国债期货有没有短线机会？"',
      '  node "D:\\path\\to\\alice-bond-rate-outlook\\scripts\\cli.mjs" --prompt "未来利率走势如何？现在应该拉长还是缩短久期？"',
      '  alice-bond-rate-outlook -p "当前曲线形态如何？适合做陡还是做平？"',
      '  alice-bond-rate-outlook -p "货币政策对债市影响怎么判断？" --download-dir "D:\\reports\\bond"',
      '  alice-bond-rate-outlook -p "重新分析10年期国债走势" --new',
      '  alice-bond-rate-outlook -p "今年债市配置方向怎么看？" --no-wait              # 沙箱：CLI 内部自旋直到完成',
      '  alice-bond-rate-outlook -p "10年期国债收益率未来一个月会怎么走？" --no-wait --once     # 脚本：单次探针',
      '  alice-bond-rate-outlook -p "今天债市怎么看？" --detach           # 后台跑，立即返回，IDE 杀终端也不影响',
      "",
    ...
    if (!prompt || typeof prompt !== "string" || !prompt.trim()) {
      ...
      console.log(
        [
          ...
          `  · 轮询结果:    alice-bond-rate-outlook -p "${prompt}" --no-wait`,
          `  · 查落盘路径:  alice-bond-rate-outlook status -p "${prompt}"`,
          "",
          "完成后报告位置：",
