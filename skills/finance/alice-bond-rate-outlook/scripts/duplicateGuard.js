...
/** 债券利率走势研判 replay 预检用的通用词（不作为主体 token）。 */
...
/** 常见债券/利率标的（按匹配优先级排列）。 */
const BOND_INSTRUMENT_PATTERNS = [
  /国债期货/g,
  /\d+年期国债/g,
  /\d+年国债/g,
  /10年期国债/g,
...
/**
 * 从 prompt 提取可用于 replay 判定的主体 token（标的片段 / 证券代码）。
...
function extractReplaySubjectTokens(text) {
  ...
  for (const re of BOND_INSTRUMENT_PATTERNS) {
    re.lastIndex = 0;
...
function hasBondRateOutlookIntent(text) {
  const t = String(text);
  return /债市|利率|收益率|久期|曲线|国债|国开|政金|地方债|货币政策|降准|降息|DR007|资金面|期限利差|债券利率走势研判/.test(
    t,
  );
}

function extractBondInstrumentName(normalized) {
  for (const re of BOND_INSTRUMENT_PATTERNS) {
    re.lastIndex = 0;
...
function isLikelyBondInstrumentName(name) {
  if (!name || name.length < 2) return false;
  if (/^[和与及]/.test(name)) return false;
  if (/和|与|及/.test(name)) return false;
  if (/^(分析|对比|报告|利率|走势|研判|市场|配置)/.test(name)) return false;
  return true;
}

/**
 * 从债券利率走势研判 prompt 提取稳定的主体键（跨措辞 submit 锁 / 调度续接用）。
 * 返回 null 表示无法可靠提取（如对比多个标的）。
 *
 * @param {string} prompt
 * @returns {string|null}
 */
export function computeSubjectKey(prompt) {
  ...
  if (!normalized || !hasBondRateOutlookIntent(normalized)) return null;

  if (/对比|比较|二者|两家|两者/.test(normalized)) return null;
  if (/(?:对比|比较).{0,24}[\u4e00-\u9fff]{2,8}和[\u4e00-\u9fff]{2,8}/.test(normalized)) return null;

  const instrument = extractBondInstrumentName(normalized);
  if (instrument) return `name:${instrument}`;
  ...
  for (const m of normalized.matchAll(/[\u4e00-\u9fff]{4,16}/g)) {
    ...
    if (stripped && isLikelyBondInstrumentName(stripped) && !REPLAY_SUBJECT_STOPWORDS.has(stripped)) {
  ...
  if (/今天|今日|当天|盘后/.test(normalized)) return "bond-rate-outlook:today";
  if (/本周|这周|一周/.test(normalized)) return "bond-rate-outlook:this-week";
  if (/本月|这个月|一个月|未来一个月/.test(normalized)) return "bond-rate-outlook:this-month";
  if (/今年|年度|全年|中长期|配置/.test(normalized)) return "bond-rate-outlook:this-year";
  return "bond-rate-outlook:general";
}
...
/**
 * completed 任务 replay 预检用的相似度（比 running 防护略宽，捕获「利率走势」vs「利率走势研判报告」等措辞差异）。
 *
...
export function comparePromptsForReplay(a, b) {
  ...
  if (!hasBondRateOutlookIntent(a) || !hasBondRateOutlookIntent(b)) {
...
export function findSimilarRunning(records, prompt, opts = {}) {
  ...
  for (const r of records) {
    ...
    // 与 completed replay 共用 comparePromptsForReplay，捕获「利率走势」vs「利率走势研判报告」等同主体措辞差异
