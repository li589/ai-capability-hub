---
name: alice-stock-dd-list
description: 一键生成买方视角上市公司调研问题清单，含看多/看空逻辑摘要、3-5 个深度议题及管理层调研问题，支持 A
  股、港股及海外公司。适用于生成调研问题清单、投资尽调问题、DD checklist 等场景。
description_zh: 万得 Alice 上市公司调研问题清单 CLI：一键生成买方视角结构化调研问题清单，含看多/看空逻辑摘要、3-5
  个深度议题及管理层调研问题，支持 A 股、港股及海外上市公司。
description_en: Wind Alice Stock DD List CLI - generate a buy-side due diligence
  question list for any listed company in one step. Retrieves financial data,
  broker research, industry news, and consensus estimates to produce a
  structured investment memo with bull/bear thesis, 3–5 deep-dive topics, and
  pointed questions for management meetings. Works for A-shares, Hong Kong, and
...
# Stock DD List（万得AI-上市公司调研问题清单）

通过 `alice-stock-dd-list` 调用万得 Alice Agent，为上市公司一键生成买方视角的结构化调研问题清单（看多/看空逻辑 + 3–5 个深度议题 + 管理层调研问题），支持 A 股、港股及海外上市公司。

> **核心原则**：基础设施交给脚本；Agent 只负责把用户想调研的公司拼成 `--prompt` 传入（可直接粘贴原话）。**默认就执行上市公司调研问题清单，无需也不要再指定其它 Skill。** 任务完成后**只将 CLI stdout 中的 `agentResult.value` 原文原样交给用户**；CLI 静默下载的附件（`reportFullFile=`）**不要**读给用户看。
>
> **宿主 Agent：执行任何 shell 命令前必须先读同目录 [`AGENT.md`](./AGENT.md)**（执行契约：七步流程 / 八条红线 / 完成判定 / replay / 退出码 / 交付 / 会话续接 / 命令模板 / 自检清单）。本文只讲**何时调用 + CLI 参数 + 环境配置**。

## 何时调用本技能

用户问题只要涉及**上市公司调研问题清单、投资尽调问题、DD checklist、管理层调研准备、首次覆盖/跟踪调研**，即应调用。

| 场景 | 典型用户表达 |
|------|-------------|
| 首次覆盖调研 | 帮我生成比亚迪的调研问题清单 |
| 跟踪调研准备 | 调研宁德时代，重点关注海外业务进展 |
| 投资尽职调查 | 做一份腾讯的投资尽调问题 |
| 事件驱动研究 | 针对并购/业绩异常生成针对性调研问题 |
| 投资委员会备忘 | 输出看多/看空逻辑与待验证问题 |

**不应调用**：纯宏观数据解读、基金筛选、市场规模测算、与上市公司调研无关的通用写作等（除非用户明确要求调研问题清单）。

**构造 prompt**：把研究对象（公司名称或股票代码，如"比亚迪"或"002594.SZ"）与关注维度拼成一句自然语言。**不要加 `使用「上市公司调研问题清单」技能：` 前缀，CLI 内部已自动注入。** 原文越完整、公司越明确，调研问题越精准。
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\asdl.ps1" --prompt "<USER_QUESTION>" --no-wait -d "<WORKSPACE_DIR>" [--new]
```
**macOS / Linux**：
```bash
...
```powershell
# Windows（方案 A）
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\asdl.ps1" apikey-set <KEY>   # KEY 裸值不加引号
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\asdl.ps1" apikey-get         # 查看是否已配置 + 脱敏末四位
```
```bash
# macOS / Linux
...
1. **`scripts/asdl.ps1`**：调用前切 UTF-8 代码页（Windows Agent **优先用它**代替裸 `node`）。
