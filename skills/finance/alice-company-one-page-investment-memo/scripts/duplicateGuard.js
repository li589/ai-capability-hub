...
/** 公司一页纸 replay 预检用的通用词（不作为主体 token）。 */
...
function hasInvestmentMemoIntent(text) {
  const t = String(text);
  if (/一页纸|投资报告|投资价值|投资备忘录|值得投资|投资分析|investment\s*memo|worth\s*invest/i.test(t)) {
    return true;
  }
  if (/[\u4e00-\u9fff]{2,}|\d{6}/.test(t)) {
    return /分析|报告|投资|估值|怎么样|价值|值不值得|memo/i.test(t);
  }
  return false;
}
...
function isLikelyCompanyCoreName(name) {
  ...
  if (/^(财务|投资|估值|风险|分析|对比|报告|一页纸|价值)/.test(name)) return false;
  return true;
}

/**
 * 从公司一页纸 prompt 提取稳定的主体键（跨措辞 submit 锁 / 调度续接用）。
...
export function computeSubjectKey(prompt) {
  ...
  if (!normalized || !hasInvestmentMemoIntent(normalized)) return null;
 ...
 * 同时，对较长段去掉前缀/后缀的停用词后提取核心主体名（如「给我一份比亚迪的一页纸投资报告」→「比亚迪」）。
...
/**
 * completed 任务 replay 预检用的相似度（比 running 防护略宽，捕获「投资报告」vs「一页纸投资报告」等措辞差异）。
 *
...
export function comparePromptsForReplay(a, b) {
  ...
  if (!hasInvestmentMemoIntent(a) || !hasInvestmentMemoIntent(b)) {
...
export function findSimilarRunning(records, prompt, opts = {}) {
  ...
  for (const r of records) {
    ...
    // 与 completed replay 共用 comparePromptsForReplay，捕获「信用报告」vs「一页纸投资报告」等同主体措辞差异
