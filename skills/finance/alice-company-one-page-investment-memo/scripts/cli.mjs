...
if (isHelp) {
  console.log(
    [
      'alice-company-one-page-investment-memo — 调用万得 Alice Agent「公司一页纸」，一键生成公司投资价值一页纸报告（投资逻辑/催化剂/风险），用户原话直接透传给服务端',
      "",
      "Usage:",
      "  alice-company-one-page-investment-memo --prompt <QUESTION> [--download-dir <DIR>] [--new]",
      "  alice-company-one-page-investment-memo --prompt <QUESTION> --detach                        # 后台跑：立刻返回 PID 与日志路径，进程脱离当前会话",
      "  alice-company-one-page-investment-memo --prompt <QUESTION> --no-wait                       # 沙箱推荐：CLI 内部自旋直到完成（默认）",
      ...
      "",
      "Windows UTF-8:",
      "  powershell -NoProfile -ExecutionPolicy Bypass -File scripts/acom.ps1 --prompt <Q> --no-wait",
      "  管道捕获 live 中文乱码时，读 stdout 的 ALICE_SESSION_LOG= 路径（UTF-8 BOM 会话日志）。",
      "",
      "Options:",
      '  --prompt,       -p <QUESTION>   用户提问，例如"请分析一下比亚迪的投资价值？腾讯有没有短线机会？"（必填）',
      ...
      "  --no-strict                     关闭 strict 模式（默认开启）；未输出 ALICE_COMPANY_ONE_PAGE_INVESTMENT_MEMO_DONE 时",
      ...
      "",
      "退出码:",
      "  0   成功：必定伴随 stdout 含 ALICE_COMPANY_ONE_PAGE_INVESTMENT_MEMO_DONE 行（含 promptHash= / reportFullFile=；Agent 判定任务完成的唯一确定信号）",
      "  1   一般失败 / 任务终态为 failed",
      ...
      '  Set-Location "D:\\path\\to\\alice-company-one-page-investment-memo"; node scripts/cli.mjs --prompt "请分析一下比亚迪的投资价值？腾讯有没有短线机会？"',
      '  node "D:\\path\\to\\alice-company-one-page-investment-memo\\scripts\\cli.mjs" --prompt "宁德时代目前估值贵不贵？投资逻辑是什么？"',
      '  alice-company-one-page-investment-memo -p "贵州茅台的护城河和盈利能力如何？"',
      '  alice-company-one-page-investment-memo -p "隆基绿能基本面怎么样？" --download-dir "D:\\reports\\company"',
      '  alice-company-one-page-investment-memo -p "重新分析比亚迪投资价值" --new',
      '  alice-company-one-page-investment-memo -p "今年医药板块有哪些值得关注的公司？" --no-wait              # 沙箱：CLI 内部自旋直到完成',
      '  alice-company-one-page-investment-memo -p "中芯国际的投资逻辑和风险点？" --no-wait --once     # 脚本：单次探针',
      '  alice-company-one-page-investment-memo -p "请分析一下比亚迪的投资价值" --detach           # 后台跑，立即返回，IDE 杀终端也不影响',
      "",
    ...
    if (!prompt || typeof prompt !== "string" || !prompt.trim()) {
      ...
      console.log(
        [
          ...
          `  · 轮询结果:    alice-company-one-page-investment-memo -p "${prompt}" --no-wait`,
          `  · 查落盘路径:  alice-company-one-page-investment-memo status -p "${prompt}"`,
          "",
          "完成后报告位置：",
