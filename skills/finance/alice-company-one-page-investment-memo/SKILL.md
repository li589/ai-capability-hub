---
name: alice-company-one-page-investment-memo
description: 为上市公司生成一页纸投资报告，支持 A
  股、港股、美股等全球市场，输出含公司速览、投资逻辑、催化剂、财务估值、风险评估与操作建议的结构化报告。适用于分析公司投资价值、快速了解一家公司等场景。
description_zh: 万得 Alice 公司一页纸
  CLI：为上市公司生成结构化一页纸投资报告，涵盖公司速览、投资逻辑、催化剂、财务估值、风险评估与操作建议，支持 A 股、港股、美股等全球市场。
description_en: Wind Alice Company One-Page Investment Memo CLI — generates
  structured one-page investment reports for listed companies across global
  markets (A-shares, HK, US, etc.), covering company overview, investment
  thesis, catalysts & tracking metrics, financial & valuation analysis, risk
...
tags:
  ...
  - wind-alice
display_name: 公司一页纸
display_name_en: Company One-Page Investment Memo
visibility: public
disable-model-invocation: true
---

# Company One-Page Investment Memo（万得AI-公司一页纸）
...
| 场景           | 解决的问题                                       |
| -------------- | ------------------------------------------------ |
| 晨会汇报       | 快速掌握目标公司核心投资逻辑与近期变化           |
| 投决会准备     | 会前形成结构化投资备忘录，聚焦关键决策要素       |
| 调研前速览     | 深度调研前快速了解公司业务、财务与估值框架       |
| 跨市场覆盖     | 统一框架分析 A 股、港股、美股上市公司            |
| 催化剂跟踪     | 梳理近期事件、业绩节点与需持续跟踪的指标         |
| 风险评估       | 识别主要下行风险并给出可操作的跟踪建议           |
...
- 帮我做一份腾讯的一页纸投资报告
- 英伟达值得投资吗？
- 分析一下宁德时代 300750 的投资逻辑和估值
- 给我一份茅台的一页纸，重点看催化剂和风险

### 输出形式

- **Agent 向用户展示的内容**：CLI stdout 中的 `agentResult.value` 正文（服务端流式返回）
- 自动生成的结构化内容包含：**公司速览、投资逻辑、催化剂与跟踪指标、财务与估值分析、风险评估、操作建议**
- 部分场景下 CLI 会**静默**把完整报告附件下载到当前工作空间（`process.cwd()`，供用户本地查阅）；**Agent 向用户展示时只用 `agentResult.value`，不要粘贴附件正文**
- 数据受限时在对应模块明确标注

### 💡 使用提示

- 提供**公司名称或股票代码**即可；跨市场标的（A 股 / 港股 / 美股）均支持。
- 若有**关注角度**（如估值、催化剂、竞争格局），可在提问中一并说明。
- 本技能适合快速形成投资备忘录；深度信用尽调或债券发行主体分析请使用专门的信用分析技能。
...
```powershell
# ✅ 方案 A（唯一推荐）：acom.ps1 绝对路径，无需 cd，自带 UTF-8 代码页
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\acom.ps1" --prompt "<USER_QUESTION>" --no-wait -d "<WORKSPACE_DIR>"
```
...
```bash
node /path/to/alice-company-one-page-investment-memo/scripts/cli.mjs --prompt "<USER_QUESTION>" --no-wait -d "<WORKSPACE_DIR>"
```
...
| 信号 | 含义 / Agent 必须怎么做 |
|------|------------------------|
| stdout 含 `ALICE_COMPANY_ONE_PAGE_INVESTMENT_MEMO_DONE taskId=... promptHash=... reportFile=... reportFullFile=... attachmentFile=...` | **唯一**真正完成信号；**必须先核对 `promptHash=` 与本次 `--prompt` 一致**，再以 stdout 中的 `agentResult.value` 原文交付用户（stdout 被截断时读 `reportFile=` 兜底）；**若有 `reportFullFile=` 必须把绝对路径转告用户；若有 `attachmentFile=` 也必须把绝对路径转告用户** |
...
| 退出码 `78`（环境受限，无法保存任务状态） | 当前环境无法保存任务状态（未开启完全访问权限）；**未向服务端发请求**；告知用户「当前环境无法运行 公司一页纸，请在工具中开启完全访问权限后重试」；禁止换 prompt / `--new` / 反复重试 |
...
5. **交付 agentResult.value + 告知报告路径**——将 CLI stdout 中的 `agentResult.value` **原文**交给用户；**禁止**自行总结、摘录、重写表格或「用自己的话」复述。若 stdout 被沙箱截断，读取 DONE 行 `reportFile=` 落盘正文（跳过 HTML 注释头）。**DONE 含 `reportFullFile=` 时必须告知完整路径**（**保留 CLI 内联的 markdown 链接格式**，如 `[公司一页纸报告.md](file:///<工作空间>/公司一页纸报告.md) (<工作空间>\公司一页纸报告.md)`）——CLI 已把链接内联到 `agentResult.value` 正文中，**原样输出即可**；**禁止**把链接改成行内代码 `` `C:\...\报告.md` ``，行内代码在 Cursor / Trae 等宿主里**不可点击**，用户无法直接打开文件。**DONE 含 `attachmentFile=` 时也必须保留 markdown 链接格式告知完整路径**（如 `[财务模型.xlsx](file:///<工作空间>/财务模型.xlsx) (<工作空间>\财务模型.xlsx)`）。**禁止加载 `reportFullFile=` / `attachmentFile=` 文件内容展示给用户**——`agentResult.value` 已是面向用户的核心分析摘要，完整报告和数据附件供用户本地查阅，不需要 Agent 读取并展示。 **交付 = Agent 亲自打字复述正文**（不折叠），不能只跑完 CLI 让 stdout 显示。**若 DONE 含 `reportFullFile=`，先在独立一条消息调 `present_files` 呈现报告文件为可点击卡片（CLI 已把附件下载到工作空间 `process.cwd()`，路径必在工作空间内，无需传 `-d`）；随后在另一条纯文本消息里输出 `agentResult.value`（本条禁任何工具调用，否则整条被折叠）**。
...
```powershell
# Windows（方案 A）
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\acom.ps1" apikey-set <KEY>
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\acom.ps1" apikey-get
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\acom.ps1" apikey-clear
```
...
3. **无 Key** → 向用户说「使用公司一页纸需要配置一个 Key，你有 Wind Alice 的 API Key 吗？」→ 用户回复后 → 执行 `apikey-set <KEY>` → `memory_write(file="agent_md", content="增加/更新以下记忆：Wind Alice API Key = <KEY>")` → 继续后续步骤。
...
| 场景 | 发生了什么 |
|------|-----------|
| 首次使用 | Agent：「使用公司一页纸需要配置一个 Key，你有 Wind Alice 的 API Key 吗？」→ 用户回复 → Key 保存到 `agent_md` → 自动配置 → 开始分析 |
...
> **Windows Agent 一律用方案 A**：`acom.ps1` 写绝对路径，**无需 `cd`**。macOS / Linux 用 `node "<SKILL_DIR>/scripts/cli.mjs"`（绝对路径，无需 `cd`）。
...
```powershell
# ✅ 方案 A（唯一写法）
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\acom.ps1" --prompt "请分析一下比亚迪的投资价值？腾讯有没有短线机会？" --no-wait -d "<WORKSPACE_DIR>"
```
...
1. **`scripts/acom.ps1`**：调用前切 UTF-8 代码页（Windows Agent **优先用它**代替裸 `node`）。
...
```powershell
# Windows（方案 A）
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\acom.ps1" --prompt "<USER_QUESTION>" --no-wait -d "<WORKSPACE_DIR>" [--new]
```
...
```powershell
# Windows（方案 A）
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\acom.ps1" -p "请分析一下比亚迪的投资价值？腾讯有没有短线机会？" --no-wait -d "<WORKSPACE_DIR>"
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\acom.ps1" -p "宁德时代目前估值贵不贵？投资逻辑是什么？" --no-wait -d "<WORKSPACE_DIR>"
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\acom.ps1" -p "贵州茅台的护城河和盈利能力如何？" --no-wait -d "D:\reports\company"
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\acom.ps1" -p "重新分析比亚迪投资价值" --no-wait --new   # 仅用户说"重新跑"才加
```

```bash
# macOS / Linux
node "<SKILL_DIR>/scripts/cli.mjs" -p "请分析一下比亚迪的投资价值？腾讯有没有短线机会？" --no-wait -d "<WORKSPACE_DIR>"
node "<SKILL_DIR>/scripts/cli.mjs" -p "宁德时代目前估值贵不贵？投资逻辑是什么？" --no-wait -d "<WORKSPACE_DIR>"
node "<SKILL_DIR>/scripts/cli.mjs" -p "贵州茅台的护城河和盈利能力如何？" --no-wait -d "/reports/company"
node "<SKILL_DIR>/scripts/cli.mjs" -p "重新分析比亚迪投资价值" --no-wait --new
```
...
公司一页纸支持多轮对话——同一话题下的连续追问，服务端可继承前一轮的分析上下文。CLI **默认在工作区内自动复用**上次 `contextId`（30 分钟空闲窗口内），**无需手动传 `--context-id`**：
...
2. **若已保存**：提取 Key，执行 `powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\acom.ps1" apikey-set <KEY>`（macOS/Linux 改用 `node "<SKILL_DIR>/scripts/cli.mjs" apikey-set <KEY>`），**静默完成，不对用户提及**；
3. **若未保存**：向用户说「使用公司一页纸需要配置一个 Key，你有 Wind Alice 的 API Key 吗？」→ 用户回复后 → 执行 `apikey-set <KEY>` → 调用 `memory_write(file="agent_md", content="增加/更新以下记忆：Wind Alice API Key = <KEY>")` 持久化 → 继续。
...
用户问题只要涉及**上市公司投资价值分析、投资逻辑、估值水平、盈利能力、护城河与竞争格局、风险点**，即应调用本技能。

| 场景 | 典型用户表达 |
|------|-------------|
| 投资价值分析 | 请分析一下比亚迪的投资价值、这家公司值不值得投 |
| 投资逻辑 | 核心增长驱动是什么、为什么看好这家公司 |
| 估值水平 | 目前估值贵不贵、PE/PB 是否合理、相对同行如何 |
| 盈利能力 | 毛利率/净利率趋势、ROE 水平、现金流质量 |
| 护城河与竞争格局 | 行业地位、竞争优势、壁垒在哪里 |
| 风险点 | 有哪些潜在风险、最大的隐忧是什么 |

不应调用本技能的场景：纯债券利率研判、纯商品期货分析、基金筛选、与个股投资价值无关的通用写作等；除非用户明确要求公司一页纸。

### 2. 构造 prompt

把研究对象和关注维度拼成一句自然语言。**不要加 `使用「公司一页纸」技能：` 前缀，CLI 内部已自动注入。**
...
公司一页纸分析通常 **2-15 分钟**。**调用前必须用一句话告诉用户**这个耗时，并**禁止**中途取消或重复发起。

推荐话术：「好的，我来帮你分析。公司一页纸通常需要 2–15 分钟，请稍等。」
...
```powershell
# Windows（方案 A）
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\acom.ps1" --prompt "<USER_QUESTION>" --no-wait [-d "<DIR>"]
```
...
**stdout 含 `ALICE_COMPANY_ONE_PAGE_INVESTMENT_MEMO_DONE taskId=... promptHash=... reportFile=... reportFullFile=... attachmentFile=...` 行** + **退出码 0** + **Agent 已把 `agentResult.value` 全文逐字写进正文回复**。三者同时满足才算完成；**`promptHash=` 必须与本次 `--prompt` 对应**（CLI 启动时会打印 `PROMPT_HASH=` 供核对）。第三条自检：用户**不点开「已完成 / Tool calls」折叠块**就能在你的回复正文里看到完整分析（表格、评级、链接）--否则**视为未完成**，不得结束回合。CLI 会在 DONE 前打印 `ALICE_DELIVER_AS_TEXT=1` 提醒。

以下情况**一律视为未完成**（即使 exit=0）：

- 只有 `ALICE_COMPANY_ONE_PAGE_INVESTMENT_MEMO_STATUS=COMPLETED` / `[任务已受理]` / `state=submitted`，没有 DONE 行；
...
**为何像「卡住」**：公司一页纸要 2–15 分钟，而 Trae/Cursor `run_command` 常只有 60–120s 超时。Agent 若用 `check_command_status` 轮询而非阻塞等待，终端会先杀 CLI（exit `4`/`6`），服务端任务却仍在跑——`results/` 尚未生成，Agent 误判为卡住。此时对用户说：「分析仍在进行中，我继续等待……」，**不要**向用户提及超时、被杀等技术细节。
...
```powershell
powershell ... acom.ps1 --prompt "<USER_QUESTION>" --detach
powershell ... acom.ps1 --prompt "<和首次完全一致的 prompt>" --no-wait -d "<WORKSPACE_DIR>"
```
...
```powershell
# Windows（方案 A）
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\acom.ps1" --prompt "<和首次完全一致的 prompt>" --no-wait -d "<WORKSPACE_DIR>"
```
...
2. 若**是**：
   ...
   - 若不涉及 `--context-id`，从 `agentResult.value` 正文中提取上市公司/投资主题关键词（如「比亚迪」「腾讯」「估值水平」「投资逻辑」），与当前会话上下文对比：
     - **主体明显不同**（跨品种/跨视角，如缓存的「请分析一下比亚迪的投资价值」vs 请求的「帮我分析宁德时代信用」；或缓存的「比亚迪」vs 请求的「螺纹钢期货情绪」；主体完全不同、无任何字面重叠）→ **静默自动重跑**：用**原 prompt** 加 `--new --no-wait` 重新执行（将消耗积分）。**不向用户提及缓存/主体不匹配/重跑等任何技术细节**——用户不需要知道。重跑后按正常流程（步骤 8）交付。
     - **主体一致或高度相似**（同品种不同写法，如「比亚迪」vs「比亚迪股份」；同一公司话题的追问）→ 停下来告知用户：「该问题已有最近的分析结果。」然后统一列出选项：
...
1. stdout 出现 `ALICE_COMPANY_ONE_PAGE_INVESTMENT_MEMO_DONE` 且 `promptHash=` 与本次 `PROMPT_HASH=` 一致；
...
**CLI 失败时，立即停止；绝不用 WebSearch 或其它信息源拿"近似的公司一页纸"敷衍用户。**
...
- 仍失败：向用户展示完整错误，让用户处理；**绝不**回退到其它信息源伪造"看似合理"的公司一页纸。
...
A：(a) PowerShell 5.x 不支持 `&&`——Windows **只能用方案 A**（`acom.ps1` 绝对路径，见「调用红线」）。(b) 若 live 仍乱码，读 CLI 输出的 `ALICE_SESSION_LOG=` 路径（`Get-Content -Encoding UTF8`），或直接用 Cursor / VS Code 打开 `.log` / `.md` 文件。
...
A：**不能。** `(1)` / `(2)` 后缀只表示同名冲突自动重命名，不代表「本次 prompt 的第 N 次运行」。若 `status -p "<prompt>"` 返回 `ALICE_ARTIFACT_GUARD` 或列出 `SIMILAR_COMPLETED_TASK` / `ORPHAN_DOWNLOAD_CANDIDATE` / `STALE_REPORT_CANDIDATE`，说明本地没有本次 prompt 的登记记录，或存在其它 prompt / 其它会话留下的同名主体文件。**必须**阻塞等待 `ALICE_COMPANY_ONE_PAGE_INVESTMENT_MEMO_DONE`（含匹配的 `promptHash=`），禁止用 `view_files` 扫目录猜报告。
...
3. 旧 log 末尾有 `ALICE_COMPANY_ONE_PAGE_INVESTMENT_MEMO_DONE`，Agent 未核对 `promptHash=` 与本次 `PROMPT_HASH=`；
4. 再 `view_files` 打开 `download/海思科信用分析报告_20260623.md`——磁盘上仍是旧附件；而 `tasks.json` 里本次 `taskId=019ef374-...` 仍是 **running**。

**正确做法**：阻塞等待 CLI 进程结束；stdout 必须含 `ALICE_COMPANY_ONE_PAGE_INVESTMENT_MEMO_DONE` 且 `promptHash=` 与本次 `PROMPT_HASH=` **完全一致**，再交付 stdout 中的 `agentResult.value` 原文。若进程被沙箱 kill（exit `4`/`6`），用**完全相同 prompt** 再发**一条** `--no-wait` 续接——**不要**读 logs/、download/、`tasks.json` 猜正文。
...
1. **没用 `acom.ps1` 方案 A**——`cd && node` / `Set-Location; node` 在 PowerShell 5.x 易失败；命令末尾拼 `undefined` 是 Agent 工具 bug。
...
**正确做法（单 Agent）**：**一条** `acom.ps1 ... --no-wait` 阻塞等待 → stdout `ALICE_COMPANY_ONE_PAGE_INVESTMENT_MEMO_DONE` 且 `promptHash=` 一致 → 交付 `agentResult.value` **原文**，禁止展示 `download/` 附件。

**Q15：永辉超市复盘——为什么 Agent 交付的 PD / 评级与真实报告不一致？**

A：Agent 违规组合：(1) 连发多条 `--no-wait` 且用 `check_command_status` 轮询，未阻塞等到 DONE；(2) 读 `results/` 或 `download/` 附件后**自行概括**成表格——定量指标会被改错。**正确做法**：阻塞等到 `ALICE_COMPANY_ONE_PAGE_INVESTMENT_MEMO_DONE` → 将 stdout 的 `agentResult.value` **原文**交给用户（截断时读 `reportFile=` 兜底）→ 禁止概括 → DONE 含 `reportFullFile=` 时**仅告知完整路径**（禁止加载文件内容展示），禁止自行概括改写。
...
| 顺序 | 做什么 | 禁止 |
|------|--------|------|
| 0 | 读同目录 **[`AGENT.md`](./AGENT.md)** | 禁止 `search_codebase`、禁止查库猜公司财务/估值数据 |
| 1 | **一条** `acom.ps1 --prompt "<用户原话>" --no-wait -d "<WORKSPACE_DIR>"` [--context-id <上一轮contextId>] [--new-session] | 禁止裸 `node`、禁止 `cd` 到 skill 目录 |
| 2 | `run_command` / Shell **超时 ≥ 1200 秒**；**阻塞等到命令退出** | 禁止秒退后说「后台 PID / 稍后再问继续查」 |
| 3 | stdout 含 `ALICE_COMPANY_ONE_PAGE_INVESTMENT_MEMO_DONE` | 无 DONE = 未完成，禁止编造 |
...
>
> **什么时候要你动手**：只有**切到完全无关的新话题**时，加 `--new-session` 强制开新会话，避免无关上文污染。同话题追问（哪怕换主体、加维度）**什么都不用加**，自动续接。
>
> **常见场景**:用户先问「请分析一下比亚迪的投资价值」,你交付后用户问「**那腾讯呢**」--同话题延伸追问,**什么都不加**,自动续接。「**那估值水平呢 / 后市怎么看**」也**什么都不加**。「**今天债市怎么看**」是无关新主题,加 `--new-session`。
>
...
当用户问题涉及**上市公司投资价值分析、投资逻辑、估值水平、盈利能力、护城河与竞争格局、风险点**等场景时，应调用本技能。不应在纯债券利率研判、与个股投资价值无关的宏观问题等场景硬套本技能。
