...
>
> 因此,CLI 通过 `acra.ps1` 跑完后,服务端返回的 `agentResult.value` 完整结果**会停留在 Bash 工具的 stdout 输出里,被折叠进「已完成」块**。**你不能指望用户去点开那个折叠块**。
>
...
>
> **常见场景**:
> - 用户先「沪铜日报」-> 交付 -> 「**那铁矿石呢**」：同话题延伸，**什么都不加**，自动续接。
> - 「**那库存压力呢 / 后市怎么看**」：同话题追加维度，**什么都不加**。
...
| 步 | 动作 | 对用户怎么说 |
|----|------|-------------|
| 0 | **API Key 配置**：从 `agent_md` 读取已保存的 Wind Alice API Key，若存在则执行 `apikey-set`；若不存在则询问用户提供 Key，保存到 `agent_md` 后再执行 `apikey-set` | （已有 Key 时静默执行）；无 Key 时：「使用商品智研助手需要配置一个 Key，你有 Wind Alice 的 API Key 吗？」 |
| 1 | 把用户问题拼成一句自然语言 `--prompt`（**不要**加 `使用「商品智研助手」技能：` 前缀）；**同步判断是否切到完全无关的新话题**（见 [「会话续接判断」专节](#会话续接判断agent-自动执行无需用户显式说)）：同话题追问**什么都不加**（CLI 工作区内自动续接）；完全无关新话题才加 `--new-session` | （无需说话） |
| 2 | 告知用户：通常需 **2–15 分钟** | 「好的，我来帮你分析。商品智研助手通常需要 2–15 分钟，请稍等。」 |
...
| 5b | **图表图片 HTML 报告（当服务端返回走势图 PNG 时）**：① 检查**工作空间（`process.cwd()`）中是否存在走势图 PNG**（命名含 `_price.png` / `_basis.png` / `_forward_curve.png` / `_sunk_capital.png` / `_warehouse_receipt.png`）；② 若存在 → **先保存 `agentResult.value` 为临时 MD 文件** → 用 `generate_chart_html.py` 的 `-r` 参数传入报告文本 + `-d` 指定走势图 PNG 所在目录（工作空间） + `--text-output` 指定修复后文本输出路径，生成**文字+图片混排**完整 HTML 文件输出到工作空间，**同时输出修复了 `/project/` 引用的纯文本**（`--text-output`）；③ 在一条**独立消息**里调 `present_files` 呈现该 HTML（可点击卡片+浏览器预览）。**若同时有步骤 5 的 `reportFullFile=`，本步骤的 `present_files` 可与步骤 5 合并为同一调用（一次 present_files 传多个文件）**。无走势图 PNG 时跳过本步 | （静默调用，无需对用户解释） |
| 6 | 🔴 **现在立即**：在**另一条只含文本、不含任何工具调用**的消息里，把 `agentResult.value` **全文逐字交付**（去掉 `agentResult.value:` 行首前缀）。**若步骤 5b 生成了 `--text-output` 修复后文本，则交付修复后文本（图片引用已替换为本地可点击链接）**；否则交付原始 agentResult.value。**本条消息禁止调用任何工具**（present_files / Bash / Read 等）--含工具调用的整条消息会被 WorkBuddy 折叠、藏住正文。**禁止**只说「分析完成」、**禁止**概括/改写。CLI 会在 DONE 前打印 `ALICE_DELIVER_AS_TEXT=1` 提醒框，见到即执行 | 交付报告（图片引用已修复为可点击本地路径） |

### 步骤 5b 详细说明：图表图片 HTML 报告生成（文字+图片混排）

当 CLI 完成分析后，CLI 会**把服务端返回的走势图 PNG 兜底强制下载到工作空间（`process.cwd()`，即 `<WORKSPACE_DIR>`）**（即使 `-d` 指向别处）；报告附件（`reportFullFile=`）仍落 `-d` 目录。这些图片文件命名规则为：
...
1. **检查走势图是否存在**：在 DONE 行解析后，检查 `-d` 目录中是否有上述命名的 PNG 文件（`ls` / `Get-ChildItem` 或 `Glob` 查找 `*_price.png` `*_basis.png` `*_forward_curve.png` `*_sunk_capital.png` `*_warehouse_receipt.png`）；

2. **🔴 保存 agentResult.value 为临时 MD 文件**（必须！纯图片模式已废弃）：
   - 将 `agentResult.value` 的完整正文保存到工作空间临时文件（如 `_report_text.md`）
   - **使用 Write 工具写入**（不是 FileWrite），编码 UTF-8

3. **生成文字+图片混排 HTML 报告 + 修复图片引用纯文本**：

```powershell
# Windows — 用 -r 参数传入报告文本，生成文字+图片混排的完整 HTML
# --text-output 同时输出修复了 /project/ 引用后的纯文本（供步骤 6 交付用）
<python_path> "<SKILL_DIR>\scripts\generate_chart_html.py" -d "<图表PNG所在目录>" -r "<临时MD文件>" -o "<工作空间>\<标题>.html" -t "<标题>" --text-output "<工作空间>\_report_fixed.md"
```

   - `<python_path>`：确保有 Pillow 库的 Python 解释器（当前环境需先 `pip install Pillow`）
   - 图表 PNG 所在目录：**工作空间（`process.cwd()`，即 `<WORKSPACE_DIR>`）**（CLI 兜底把走势图 PNG 强制下载到工作空间，即使 `-d` 指向别处）
   - **`-r` 必传**：报告 Markdown 文本文件路径，脚本按章节（价格分析/基差分析/远期曲线/沉淀资金/注册仓单）自动匹配并内嵌对应走势图
   - **`--text-output` 必传**：输出修复后纯文本路径，脚本将 `![...](project/xxx.png)` 替换为 `[文件名](file:///绝对路径) (绝对路径)` 可点击链接，本地无对应文件的行自动删除。**此文件供步骤 6 交付用**
   - 不传 `-r` 时脚本退回纯图片模式（向后兼容，**但新建分析禁止使用纯图片模式**）

4. **present_files 呈现**：在一条**独立消息**里调 `present_files`，传入生成的 HTML 路径。如果步骤 5 也有 `reportFullFile=`，可将两个文件路径合并为一次 `present_files` 调用（减少用户看到的卡片数量）。

**完整代码示例**（步骤 5 + 5b 合并，DONE 后执行）：

```powershell
# ① 先用 Write 工具把 agentResult.value 保存为 _report_text.md（UTF-8）
# ② 再用 generate_chart_html.py 生成文字+图片混排 HTML + 修复引用纯文本
python "<SKILL_DIR>\scripts\generate_chart_html.py" -d "<WORKSPACE_DIR>" -r "<WORKSPACE>\_report_text.md" -o "<WORKSPACE>\NYMEX原油日报.html" -t "NYMEX原油日报" --text-output "<WORKSPACE>\_report_fixed.md"
# ③ Agent 交付步骤 6 时，用 Read 工具读 _report_fixed.md 获取修复后文本
```

**合并 present_files 示例**（一次调用呈现所有文件）：

```
present_files([reportFullFile工作空间副本, <标题>.html工作空间路径])
```
...
> 3. **顺序**：① 静默跑完主调用 -> ②（若 `reportFullFile=` 或存在走势图 PNG）在**独立消息**调 `present_files`（**本回合只能有 present_files 一个工具调用（禁 Bash/Read/Edit/Write/文件操作）**--否则卡片被折叠隐藏；这条可折叠成卡片；若两者都有一并传入）-> ③ 在**纯文本消息**里输出 `agentResult.value`（**本条禁任何工具调用**，否则整条被折叠）。**若步骤 5b 生成了修复后文本（`--text-output`），步骤 ③ 交付修复后文本（图片引用已替换为可点击本地路径，/project/ 破图引用已删除）**。
> 4. 🔴 **`reportFullFile=` 路径已在工作空间**：CLI 已把所有附件**统一下载到当前工作空间**（`process.cwd()`），`reportFullFile=` 路径必在工作空间内，**无需 `cp` 复制**——直接用该路径调 `present_files` 即可。
...
3. **若未保存**：
   - 向用户说：「使用商品智研助手需要配置一个 Key，你有 Wind Alice 的 API Key 吗？」
...
| 「请提供 Wind Alice API Key 以便调用商品智研助手服务端接口。」 | 「使用商品智研助手需要配置一个 Key，你有 Wind Alice 的 API Key 吗？」 |
...
宿主执行主调用 / status 等 Bash 工具调用时，会把你填在工具调用的 `description` 字段里的文字，作为**权限确认框的标题**展示给用户。请把该字段填**简短中文**，不要填英文（如「Call Wind Alice for financial analysis」→ 改填「调用万得 Alice 进行商品智研助手」）。

对应七步流程的推荐 `description` 取值：

| 步骤 | 命令 | `description` 填写 |
|------|------|--------------------|
| 3 | 主调用 `--no-wait` | 调用万得 Alice 进行商品智研助手 |
...
>
> 1. **Windows 必须用 `acra.ps1`，禁止裸 `node scripts/cli.mjs`**（裸 node 在 PowerShell 5.x 下中文乱码 + 沙箱兼容性问题）。
> 2. **禁止 `Set-Location`/`cd` 到 `<SKILL_DIR>`**。`acra.ps1` 用绝对路径调用，**不 cd**，让 `process.cwd()` 保持当前工作区。数据目录(tasks.json/session.log/submit-locks)**固定在 `~/.wind-alice/`**（跨会话稳定，不跟 cwd 走），故 cd 不影响状态目录；但仍禁止 cd 到 skill 目录：避免污染 skill 目录、避免 `present_files` 路径错乱。
...
> 4. **禁止用 `Out-File`/`>`/`2>&1 | Out-File` 把 stdout 重定向到文件**。CLI 的完成信号(`ALICE_COMMODITY_RESEARCH_ASSISTANT_DONE`、`agentResult.value`)在 stdout，Agent 必须**直接捕获 stdout**，重定向到文件既把文件写到工作区外(弹窗)、又可能导致 Agent 读不到完成信号。
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\acra.ps1" --prompt "<USER_QUESTION>" --no-wait -d "<WORKSPACE_DIR>" [--context-id <上一轮contextId>] [--new-session]
```
...
| **上一轮返回了错误主体**（replay 命中但主体不匹配） | 加 `--new-session`（session 已污染）；用明确 Prompt（含品种名）新开会话 |
...
- **代词/指代**:「它 / 这个品种 / 上面那份能不能加 XX」。
- **相同主体追加维度**:上次分析 XX 的 A 维度,本次问 XX 的 B 维度(同一主体、不同角度)。
- **同一商品话题延伸**:上文分析「沪铜日报」,本次问「那铁矿石呢 / 远期曲线呢 / 后市怎么看」--同属该商品话题,**必续接**。

**案例（必读）**:
- ✅ 用户先「沪铜日报」-> 你交付 -> 用户「**那铁矿石呢**」：同话题延伸追问，**什么都不加**，自动续接。
- ✅ 用户「沪铜未来走势」-> 「**那库存压力呢**」：同话题追加维度，**什么都不加**。
- ❌ 用户「沪铜日报」-> 「**宁德时代信用怎么样**」：完全无关新主题，加 `--new-session`。

**判定为切换（加 `--new-session`）**:
- 用户明说:「换个话题 / 忘掉刚才 / 新问题」。
- 主体/主题明显无关:上文聊商品期货,本次问个股信用或债券利率。
...
6. **禁止在无完成信号时交付** — 无 `ALICE_COMMODITY_RESEARCH_ASSISTANT_DONE` 与 `agentResult.value` 时**不得交付**任何分析结论。
...
- stdout 含 **`ALICE_COMMODITY_RESEARCH_ASSISTANT_DONE`**，且其中 **`promptHash=`** 与本次 CLI 打印的 **`PROMPT_HASH=`** 一致；
- 退出码 **`0`**；
...
2. 若**是**：
   ...
   - 若不涉及 `--context-id`，从 `agentResult.value` 正文中提取商品品种/维度关键词，与当前会话上下文对比：
   - **主体明显不同**（跨品种/跨视角，如缓存的「沪铜日报」vs 请求的「宁德时代信用分析」；或缓存的「沪铜」vs 请求的「螺纹钢期货情绪」）→ **静默自动重跑**：用**原 prompt** 加 `--new --no-wait` 重新执行（将消耗积分）。**不向用户提及缓存/主体不匹配/重跑等任何技术细节**——用户不需要知道自己引来了别的主题缓存。重跑后按正常流程交付。
   - **主体一致或高度相似**（同品种不同写法，如「沪铜」vs「铜期货」；同一商品话题的追问）→ 停下来告知用户：「该问题已有最近的分析结果。」然后列出：(A) 查看已有结果 / (B) 重新分析 / (C) 取消。**禁止**未经用户明确选择就交付或 `--new`。
   - **拿不准时**（主体模糊、仅泛化关键词如「商品」「期货」、缩写无法判定）→ 按「主体一致」处理，停下来询问用户。
3. 若**否**：正常交付，按步骤 5 处理。

**关键判断标准**：
- 明显不同：两个商品主体/话题完全不同、无任何字面重叠（如「沪铜日报」→「宁德时代信用」、「沪铜」→「螺纹钢期货情绪」）。这类情况直接静默重跑，**绝不打扰用户**。
- 需要停下来问：同一品种不同写法（「沪铜」→「铜期货」）、同一商品话题的细化追问、或 Agent 无法判断是否同一主题时。
...
| `78` | 环境受限，无法保存任务状态 | **未向服务端发请求**；告知用户「当前环境无法运行 商品智研助手，请在工具中开启完全访问权限后重试」；禁止换 prompt / `--new` / 反复重试 |
...
商品智研助手通常 **2–15 分钟**。若宿主 `run_command` 只有数十秒～几分钟超时，**第一条 `--no-wait` 会被强杀**（exit `4`/`6`），看起来像「卡住」——其实是终端杀了 CLI，服务端往往仍在跑。此时对用户说：「分析仍在进行中，我继续等待……」，**不要**说「被杀」「超时」等技术细节。
...
powershell ... acra.ps1 --prompt "<USER_QUESTION>" --no-wait
```
...
| 信号 | 含义 |
|------|------|
| `ALICE_COMMODITY_RESEARCH_ASSISTANT_DONE` | **唯一**完成标记 |
...
# ❌ 用裸 node cli.mjs 代替 acra.ps1（Windows 上会导致中文乱码、沙箱兼容性问题）
...
□ stdout 有 ALICE_COMMODITY_RESEARCH_ASSISTANT_DONE？
...
□ **步骤 5b**：已检查**工作空间（`process.cwd()`）**中是否存在走势图 PNG（`*_price.png` / `*_basis.png` / `*_forward_curve.png` / `*_sunk_capital.png` / `*_warehouse_receipt.png`）？若有 → 已保存 `agentResult.value` 为临时 MD → 已用 `generate_chart_html.py -r` + `--text-output` 生成**文字+图片混排** HTML 和**修复引用后纯文本**并通过 `present_files` 呈现？
□ 步骤 6 交付时，若有 `--text-output` 修复文本，已交付修复文本（图片引用替换为可点击本地路径）而非原始 agentResult.value？
□ 附件路径已内联在正文中，不在末尾再追加？
