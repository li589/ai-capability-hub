...
// 本 CLI 仅承载万得 Alice 的「商品智研助手」技能。
// 服务端通过 prompt 文本前缀 `使用「<Skill 名>」技能：` 识别 Skill。
const ALICE_SKILL_NAME_ZH = "商品智研助手";

// ChatAgent 请求渠道标识，供服务端识别调用来源（Tencent SkillHub）。
...
//        ALICE_COMMODITY_RESEARCH_ASSISTANT_DONE taskId=<id> promptHash=<hash> reportFile=<abs> reportFullFile=<abs>
//      Agent 只需匹配该行就能确认成功，比解析 ALICE_COMMODITY_RESEARCH_ASSISTANT_STATUS=COMPLETED
...
/**
 * 输出 ALICE_COMMODITY_RESEARCH_ASSISTANT_DONE 行。幂等：同一进程内只输出一次。
...
function emitAnalysisDone({ taskId, promptHash } = {}) {
  ...
  const parts = ["ALICE_COMMODITY_RESEARCH_ASSISTANT_DONE"];
  ...
  console.log(
    "[CLI] 上一行 ALICE_COMMODITY_RESEARCH_ASSISTANT_DONE 是本进程任务"
      + "真正完成"
...
function installStrictExitHook() {
  ...
  process.on("exit", () => {
    ...
    try {
      console.error(
        "[CLI][strict] 进程退出但未输出 ALICE_COMMODITY_RESEARCH_ASSISTANT_DONE；强制退出码="
          + EXIT_STRICT_NO_DONE
          + "。",
      );
      console.error(
...
// strict 模式专用退出码：
// - 6 = 进程退出时未输出 ALICE_COMMODITY_RESEARCH_ASSISTANT_DONE，且 exitCode 仍为 0；
 ...
 *
 * 背景：此前 message/stream 的非 200 拒绝只裸打 errorText + exit=1，Agent 看不到
 * 「等该账号下其它任务完成后再用相同 prompt 重试」的指引，易误判为普通失败而换 prompt
 * 重试（再占并发槽）。
 */
...
function clearCompletedRecordForResubmit(registry, promptHash, { logPrefix = "[CLI]" } = {}) {
  ...
  console.log(
    `${logPrefix} 若只需查询历史落盘路径，请用：alice-commodity-research-assistant status -p "<同一问题>"`,
  );
...
function printAttachBanner({ taskId, contextId, startedAt }) {
  ...
  console.log("");
  console.log("ALICE_COMMODITY_RESEARCH_ASSISTANT_STATUS=RUNNING");
...
function printCliRunStartNotice() {
  const lines = [
    "ALICE_COMMODITY_RESEARCH_ASSISTANT_STATUS=RUNNING",
    "[CLI] 商品智研助手已启动：本进程将阻塞运行直至成功（agentResult + [完成]）或失败，通常需 2-15 分钟。",
    "[CLI] [等待中] 心跳期间无新输出属正常；禁止用 check_command_status 轮询替代等待本进程结束。",
    "[CLI] 见到 [任务已受理] 仅表示已提交，不等于报告完成；必须等到 ALICE_COMMODITY_RESEARCH_ASSISTANT_STATUS=COMPLETED。",
  ];
  for (const line of lines) {
    console.error(line);
  }
...
function persistAgentResultIfAny({ registry, taskId, promptHash, prompt }) {
  ...
  if (!text) return null;
  const header = [
    `<!-- alice-commodity-research-assistant：本次 Alice agentResult.value 已落盘 -->`,
    `<!-- 重要：本文件是 Alice 响应正文（agentResult.value），并不一定等于"完整品种日报"。 -->`,
...
function printCliRunEndNotice({ success, exitCode, elapsed }) {
  const lines = success
    ? [
        "ALICE_COMMODITY_RESEARCH_ASSISTANT_STATUS=COMPLETED",
    ...
    : [
        "ALICE_COMMODITY_RESEARCH_ASSISTANT_STATUS=FAILED",
...
function usage() {
  return [
    "alice-commodity-research-assistant — 调用万得 Alice Agent「商品智研助手」技能，输入期货品种名称或 Wind Code 流式输出商品智能研报",
    "",
    "Usage:",
    "  alice-commodity-research-assistant --prompt <QUESTION> [--download-dir <DIR>] [--new]",
    "  alice-commodity-research-assistant --prompt <QUESTION> --no-wait                       # 沙箱推荐：CLI 内部自旋 probe 直到完成（默认）",
    "  alice-commodity-research-assistant --prompt <QUESTION> --no-wait --once                # 单次探针：几秒内返回（脚本/调试）",
    "  alice-commodity-research-assistant --prompt <QUESTION> --no-wait \\",
    "      --watch-interval 60 --watch-timeout 1800                              # 自定义轮询节奏",
    "  alice-commodity-research-assistant status --prompt <QUESTION>                        # 查询落盘路径（不访问服务端）",
    "  alice-commodity-research-assistant check-conflict --prompt <QUESTION>                # [已废弃] 主调用 --no-wait 已内置去重，无需预检",
    "  alice-commodity-research-assistant apikey-set <KEY>",
    "  alice-commodity-research-assistant apikey-get",
    "  alice-commodity-research-assistant apikey-clear",
    "  alice-commodity-research-assistant --help",
    "",
    "Options:",
    "  --prompt,       -p <QUESTION>   用户提问，例如\"沪铜日报\"（必填）",
    ...
    "                                  ALICE_COMMODITY_RESEARCH_ASSISTANT_DONE 行，会把 exitCode=0 强制改成 6，",
    ...
    "",
    "退出码:",
    "  0   成功：必定伴随 stdout 含 ALICE_COMMODITY_RESEARCH_ASSISTANT_DONE 行（Agent 判定任务完成的唯一确定信号）",
    "  1   一般失败 / 任务终态为 failed",
    "  2   参数错误 / KEY_MISSING",
    "  4   --no-wait 专用：已达全程自旋上限（默认 60min）仍未完成；Agent 再执行一次相同 --no-wait 续接",
    "  5   --no-wait 专用：--once 且本地无任务记录（默认 --no-wait 会自动提交）",
    "  6   strict 模式兜底：进程退出但未输出 ALICE_COMMODITY_RESEARCH_ASSISTANT_DONE（通常表示进程被沙箱终止）；",
...
// SSE 先行路径在 agentResult 到达瞬间即下载，服务端可能尚未把附件 / 走势图 PNG 上传到
// 文件存储，此时返回 404（RESOURCE_NOT_FOUND / 文件不存在）。短暂等待重试给服务端上传
// 留出时间（tasks/get 轮询路径因天然延迟几乎不会命中此竞态）。仅对 404 重试，其它错误
// 立即失败。
...
async function downloadCollectedFiles(apiKey, override, ctx = {}) {
  ...
  // 所有下载件（报告附件 / 数据附件 / 走势图）统一落工作空间 process.cwd()，
  // 避免 -d 传成工作空间外目录或未传 -d（回落 ~/Downloads）时文件落外部。
  // generate_chart_html.py 也扫描此目录。
 ...
 * `ALICE_COMMODITY_RESEARCH_ASSISTANT_STATUS=...`，由调用方（单次 probe / watch 轮询）在合适
...
async function runProbeStep({ registry, taskId, promptHash, prompt, url, headers, apiKey, downloadDir }) {
  ...
  if (probe.kind === "working") {
    ...
    console.log(
      "[CLI] 本进程尚未输出 ALICE_COMMODITY_RESEARCH_ASSISTANT_DONE；logs/ 下其它 .session.log 内的 DONE 行与本 PROMPT_HASH 不一致则无效。",
    );
 ...
 * 调用方读到的 `ALICE_COMMODITY_RESEARCH_ASSISTANT_STATUS=...` 语义一致。
 *
...
function publishFinalStatus(outcome, { suggestExternalPoll = false } = {}) {
  switch (outcome) {
    case "completed":
      console.log("ALICE_COMMODITY_RESEARCH_ASSISTANT_STATUS=COMPLETED");
      emitAnalysisDone();
      break;
    case "failed":
      console.log("ALICE_COMMODITY_RESEARCH_ASSISTANT_STATUS=FAILED");
      break;
    case "no-task":
      console.log("ALICE_COMMODITY_RESEARCH_ASSISTANT_STATUS=NO_TASK");
      break;
    case "working":
    default:
      if (suggestExternalPoll) {
        ...
        console.log(
          "[探针] 禁止读取 download/ 同名报告或 STALE_REPORT_CANDIDATE；必须见到 ALICE_COMMODITY_RESEARCH_ASSISTANT_DONE 且 promptHash= 一致。",
        );
      }
      console.log("ALICE_COMMODITY_RESEARCH_ASSISTANT_STATUS=RUNNING");
      break;
...
async function main() {
  ...
  // analysis 主路径：默认开启 strict 模式（未输出 ALICE_COMMODITY_RESEARCH_ASSISTANT_DONE 即视为
  ...
  if (noWait) {
    ...
    console.log("ALICE_AGENT_REDLINE=禁止在 stdout 出现 ALICE_COMMODITY_RESEARCH_ASSISTANT_DONE 之前 view_files/read download/ 主体报告");
      ...
      mainLoop: while (true) {
        for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
          ...
          if (!startBannerPrinted) {
            ...
            console.log(
              `[开始] 已向 Alice「${ALICE_SKILL_NAME_ZH}」技能发起请求；商品智研助手通常耗时 2-5 分钟，期间会定期播报等待进度，请勿中断。`,
            );
          }
