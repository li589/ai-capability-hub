...
if (isHelp) {
  console.log(
    [
      'alice-commodity-research-assistant — 调用万得 Alice Agent「商品智研助手」，覆盖交易/策略/配置多视角与宏观流动性/曲线/技术情绪五大维度，用户原话直接透传给服务端',
      "",
      "Usage:",
      "  alice-commodity-research-assistant --prompt <QUESTION> [--download-dir <DIR>] [--new]",
      "  alice-commodity-research-assistant --prompt <QUESTION> --detach                        # 后台跑：立刻返回 PID 与日志路径，进程脱离当前会话",
      "  alice-commodity-research-assistant --prompt <QUESTION> --no-wait                       # 沙箱推荐：CLI 内部自旋直到完成（默认）",
      ...
      "",
      "Windows UTF-8:",
      "  powershell -NoProfile -ExecutionPolicy Bypass -File scripts/acra.ps1 --prompt <Q> --no-wait",
      "  管道捕获 live 中文乱码时，读 stdout 的 ALICE_SESSION_LOG= 路径（UTF-8 BOM 会话日志）。",
      "",
      "Options:",
      '  --prompt,       -p <QUESTION>   用户提问，例如"沪铜日报？铁矿石有没有短线机会？"（必填）',
      ...
      "  --no-strict                     关闭 strict 模式（默认开启）；未输出 ALICE_COMMODITY_RESEARCH_ASSISTANT_DONE 时",
      ...
      "",
      "退出码:",
      "  0   成功：必定伴随 stdout 含 ALICE_COMMODITY_RESEARCH_ASSISTANT_DONE 行（含 promptHash= / reportFullFile=；Agent 判定任务完成的唯一确定信号）",
      "  1   一般失败 / 任务终态为 failed",
      ...
      "  12  [已废弃] check-conflict 子命令专用：24h 内已有相似 completed 任务可重放",
      ...
      '  Set-Location "D:\\path\\to\\alice-commodity-research-assistant"; node scripts/cli.mjs --prompt "沪铜日报？铁矿石有没有短线机会？"',
      '  node "D:\\path\\to\\alice-commodity-research-assistant\\scripts\\cli.mjs" --prompt "螺纹钢期货近期走势如何？库存压力大不大？"',
      '  alice-commodity-research-assistant -p "当前原油远期曲线如何？库存怎么变化？"',
      '  alice-commodity-research-assistant -p "黄金近期价格驱动因素有哪些？" --download-dir "D:\\reports\\commodity"',
      '  alice-commodity-research-assistant -p "重新分析沪铜走势" --new',
      '  alice-commodity-research-assistant -p "今年商品市场配置方向怎么看？" --no-wait              # 沙箱：CLI 内部自旋直到完成',
      '  alice-commodity-research-assistant -p "铁矿石未来一个月会怎么走？" --no-wait --once     # 脚本：单次探针',
      '  alice-commodity-research-assistant -p "沪铜日报" --detach           # 后台跑，立即返回，IDE 杀终端也不影响',
      "",
    ...
    if (!prompt || typeof prompt !== "string" || !prompt.trim()) {
      ...
      console.log(
        [
          ...
          `  · 轮询结果:    alice-commodity-research-assistant -p "${prompt}" --no-wait`,
          `  · 查落盘路径:  alice-commodity-research-assistant status -p "${prompt}"`,
          "",
          "完成后报告位置：",
