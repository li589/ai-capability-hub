...
class OrderStore:
    ...
    def save(self, order: dict):
        with self._conn() as conn:
            conn.execute("""
                ...
                (out_trade_no, payment_code, amount, description, answers,
            ...
            """, (
                ...
                json.dumps(order.get('answers', []), ensure_ascii=False),
    ...
    def update_progress(self, out_trade_no: str, progress: dict):
        with self._conn() as conn:
            conn.execute(
    ...
    def update_result(self, out_trade_no: str, result: str):
        import time
        with self._conn() as conn:
            conn.execute(
                "UPDATE orders SET result = ?, completed_at = ? WHERE out_trade_no = ?",
                (result, int(time.time()), out_trade_no)
            )
