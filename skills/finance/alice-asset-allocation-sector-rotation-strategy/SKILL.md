---
name: alice-asset-allocation-sector-rotation-strategy
description: 用于分析未来1-6个月股票行业相对强弱与行业内部权重倾斜，结合动量、资金面、估值和景气度因子输出超配/中性/低配清单、行业评分、约束检查和可选行业目标权重；仅调整股票内部结构，不改变股债等大类资产总仓位。
description_zh: 万得 Alice 资产配置-行业轮动策略 CLI：分析未来 1-6 个月各行业相对强弱与权重倾斜，结合动量、资金面、估值与景气度因子输出超配/中性/低配清单、行业评分、约束检查与可选行业目标权重。
description_en: Use when analyzing 1-6 month equity sector rotation and sector-level tilts, combining momentum, flows, valuation, and fundamentals to produce overweight/neutral/underweight lists, sector scores, constraint checks, and optional sector targets while keeping total equity exposure unchanged.
version: 1.0.0
author: WindAlice
tags: [sector-rotation, asset-allocation, industry-rotation, overweight-underweight, sector-tilt, wind-alice]
---
# Asset Allocation - Sector Rotation Strategy（资产配置-行业轮动策略）

通过 `alice-asset-allocation-sector-rotation-strategy` 调用万得 Alice Agent 的「资产配置-行业轮动策略」专业技能，分析未来 1-6 个月各行业相对强弱，生成行业配置建议与权重优化方案（超配/中性/低配清单 + 行业评分 + 目标权重）。

> **核心原则**：基础设施交给脚本；Agent 把用户问题拼成一句自然语言作为 `--prompt` 传入（**不要**加 `使用「资产配置-行业轮动策略」技能：` 前缀，CLI 内部已自动注入）。任务完成后**只将 CLI stdout 中的 `agentResult.value` 原文原样交给用户**；CLI 静默下载的附件（`reportFullFile=`）**仅告知路径，禁止加载内容展示**。**关键（避免结果被折叠）**：`agentResult.value` 必须由 Agent **作为自己的文本回复正文逐字打出来**（`type=text` 才不折叠）；**若 DONE 含 `reportFullFile=`，先在独立一条消息调 `present_files`，随后在另一条纯文本消息里逐字输出 `agentResult.value`（本条禁任何工具调用）**。
>
> **宿主 Agent：执行任何 shell 命令前必须先读同目录 [`AGENT.md`](./AGENT.md)**（执行契约：七步流程 / 八条红线 / 完成判定 / replay / 退出码 / 交付 / 会话续接 / 命令模板 / 自检清单）。本文只讲**何时调用 + CLI 参数 + 环境配置**。

## 何时调用本技能

用户问题涉及**行业轮动、行业超配/低配/标配、行业权重调整、行业景气度与相对强弱、资金流向行业、板块轮动、风格切换**等场景，即应调用。

| 场景 | 典型用户表达 |
|------|-------------|
| 行业配置决策 | 未来 3 个月哪些行业值得超配？ / 当前市场环境下应该侧重哪些行业？ |
| 行业对比 | 科技和消费行业哪个更有配置价值？ / 医药和电子行业哪个景气度更高？ |
| 权重优化 | 如何调整我的行业配置权重？ / 帮我分析下周期行业的轮动机会 |
| 景气度/资金 | 近期资金流向哪些行业板块？ / 新能源行业还值得继续持有吗？ |
| 风格轮动 | 未来半年行业轮动的主线是什么？ |

**试试这样问**：「未来 3 个月哪些行业值得超配？」「科技和消费行业哪个更有配置价值？」「帮我分析下周期行业的轮动机会」「近期资金流向哪些行业板块？」「金融行业和低估值板块的配置建议」。

**输入**：时间窗口（如「未来 3 个月」「未来半年」，1-6 个月）。时间窗口是主体维度，不同窗口视为不同主体。

**输出**：超配/中性/低配清单 + 行业评分 + 约束检查 + 可选行业目标权重区间。聚焦**股票资产内部的行业结构调整**，不改变股债等大类资产总仓位；覆盖申万一级/二级行业、中信行业分类等全市场行业；多维分析框架（宏观周期、景气度、估值、资金、政策）；可与战术资产配置（TAA）配合--TAA 决定股债比例，行业轮动决定股票内部结构。

**不应调用**：纯个股信用分析、纯债券利率研判、与行业轮动无关的通用金融问答--除非用户明确要求行业轮动分析。
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\asrs.ps1" --prompt "<USER_QUESTION>" --no-wait -d "<WORKSPACE_DIR>" [--new]
```
**macOS / Linux**：
```bash
...
```powershell
# Windows（方案 A）
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\asrs.ps1" apikey-set <KEY>   # KEY 裸值不加引号
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\asrs.ps1" apikey-get         # 查看是否已配置 + 脱敏末四位
```
```bash
# macOS / Linux
...
1. **`scripts/asrs.ps1`**：调用前切 UTF-8 代码页（Windows Agent **优先用它**代替裸 `node`）。
