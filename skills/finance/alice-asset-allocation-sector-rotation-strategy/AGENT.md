# Alice 资产配置-行业轮动策略 - 宿主 Agent 执行契约
...
宿主 UI（WorkBuddy 等）的折叠规则：**你亲自打字输出的文本正文（`type=text`）永远正常显示；任何工具调用的输出（含 Bash stdout、read_file 内容、present_files 卡片）回合结束后会被折叠进「已完成」块，用户必须点开才看得到。** CLI 通过 `asrs.ps1` 跑完后，`agentResult.value` 完整结果**会停留在 Bash stdout 里被折叠**，你不能指望用户去点开。
...
| 步 | 动作 | 对用户怎么说 |
|----|------|-------------|
| 0 | **API Key 配置**：从 `agent_md` 读取已保存的 Wind Alice API Key，若存在则执行 `apikey-set`；若不存在则询问用户提供 Key，保存到 `agent_md` 后再执行 `apikey-set` | （已有 Key 时静默执行）；无 Key 时：「使用资产配置-行业轮动策略需要配置一个 Key，你有 Wind Alice 的 API Key 吗？」 |
| 1 | 把用户问题拼成一句自然语言 `--prompt`（**不要**加 `使用「资产配置-行业轮动策略」技能：` 前缀）；同步判断是否切到完全无关的新话题：同话题追问**什么都不加**（CLI 工作区内自动续接）；完全无关新话题才加 `--new-session` | （无需说话） |
| 2 | 告知用户：通常需 **2–15 分钟** | 「好的，我来帮你分析。资产配置-行业轮动策略通常需要 2–15 分钟，请稍等。」 |
...
**流程**：1. `memory_recall(action="load", scope="agent_md")` 检查是否已保存 Key；2. **若已保存**：执行 `apikey-set <KEY>`（Windows: `powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\asrs.ps1" apikey-set <KEY>`；macOS/Linux: `node "<SKILL_DIR>/scripts/cli.mjs" apikey-set <KEY>`），**静默完成，不对用户提及**；3. **若未保存**：向用户说「使用资产配置-行业轮动策略需要配置一个 Key，你有 Wind Alice 的 API Key 吗？」-> 用户回复后 -> 执行 `apikey-set <KEY>` -> `memory_write(file="agent_md", content="增加/更新以下记忆：Wind Alice API Key = <KEY>")` 持久化 -> 继续。
...
宿主执行 Bash 工具调用时，会把 `description` 字段文字作为**权限确认框标题**展示给用户。填**简短中文**：步骤 3 主调用填「调用万得 Alice 进行资产配置-行业轮动策略」；`status` 填「查询本地任务落盘路径」。不要把 CLI / task_id / exit code 等技术细节写进 `description`。
...
> 1. **Windows 必须用 `asrs.ps1`，禁止裸 `node scripts/cli.mjs`**（裸 node 在 PowerShell 5.x 下中文乱码 + 沙箱兼容性问题）。
> 2. **禁止 `Set-Location`/`cd` 到 `<SKILL_DIR>`**。`asrs.ps1` 用绝对路径调用，不 cd，让 `process.cwd()` 保持当前工作区。状态目录(tasks.json/session.log/submit-locks)**固定在 `~/.wind-alice/`**（跨会话稳定，不跟 cwd 走）。
...
> 4. **禁止用 `Out-File`/`>`/`2>&1 | Out-File` 把 stdout 重定向到文件**。完成信号(`ALICE_ASSET_ALLOCATION_SECTOR_ROTATION_STRATEGY_DONE`、`agentResult.value`)在 stdout，Agent 必须直接捕获 stdout。
> 5. **`-d` 路径含空格必须加双引号**。

**Windows（PowerShell 5.x 不支持 `&&`，一律用方案 A）**：
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File "<SKILL_DIR>\scripts\asrs.ps1" --prompt "<USER_QUESTION>" --no-wait -d "<WORKSPACE_DIR>" [--context-id <上一轮contextId>] [--new-session]
```
**macOS / Linux**：
```bash
...
| 上一轮返回错误分析主体（replay 命中但主体不匹配） | 加 `--new-session`（session 已污染），用含时间窗口/行业的明确 Prompt 新开 |
| 距上次超 30 分钟 | CLI 自动按新会话处理（窗口过期） |

**判定为续接（任一命中即什么都不加）**：追问上文结论/原因（「是XX引起的吗」「为什么上面说XX」）；省略式追问（「XX 呢」）；显式承接（「接着刚才/再看下XX方面」）；代词指代（「它/上面那份」）；相同主体追加维度；同一分析话题延伸（如「未来3个月哪些行业值得超配」->「那科技行业呢 / 资金流向呢 / 景气度呢」）。

**判定为切换（加 `--new-session`）**：用户明说「换个话题/忘掉刚才」；主体明显无关（如行业轮动 -> 个股信用 / 债券利率）；用户明说要「重新分析」（同时加 `--new`，语义正交）。案例：上文「未来3个月哪些行业值得超配」->「宁德时代信用怎么样」= 完全无关新主题，加 `--new-session`。
...
4. **禁止手动翻目录猜报告** - 禁止扫 `Downloads/`、`results/`、`logs/` 按时间窗口/行业或修改时间挑文件；`ALICE_ARTIFACT_GUARD`、`ORPHAN_DOWNLOAD_CANDIDATE`、`ALICE_FORBIDDEN_READ_UNTIL_DONE` 是**警告**，不是可交付路径。
5. **交付 `agentResult.value` 原文（附件路径已内联）** - 禁止自行总结、改表格、重写行业评分/超配低配；附件路径已由 CLI 内联在正文中，不需末尾再追加；**必须保留 CLI 生成的 markdown 链接格式**（`[文件名](file:///绝对路径) (绝对路径)`）--禁止改成行内代码 `` `路径` ``，行内代码不可点击；**禁止加载 `reportFullFile=` / `attachmentFile=` 文件内容展示给用户**（`agentResult.value` 已是面向用户的核心分析摘要，完整报告和数据附件供用户本地查阅）。
6. **禁止在无完成信号时交付** - 无 `ALICE_ASSET_ALLOCATION_SECTOR_ROTATION_STRATEGY_DONE` 与 `agentResult.value` 时**不得交付**任何分析结论。
...
- stdout 含 **`ALICE_ASSET_ALLOCATION_SECTOR_ROTATION_STRATEGY_DONE`**，且其中 **`promptHash=`** 与本次 CLI 打印的 **`PROMPT_HASH=`** 一致；
- 退出码 **`0`**；
- **Agent 已把 `agentResult.value` 全文逐字写进正文回复**（CLI 会在 DONE 前打印 `ALICE_DELIVER_AS_TEXT=1` 提醒）。自检：用户**不点开「已完成 / Tool calls」折叠块**就能在你的回复正文里看到完整分析（含表格、行业评分、链接）--否则**视为未完成**，不得结束回合。
...
2. 若**是**：
   ...
   - 若不涉及 `--context-id`，从 `agentResult.value` 提取分析主题/时间窗口关键词（如「未来3个月」「科技」「资金流向」「景气度」），与当前会话上下文对比：
     - **主体明显不同**（跨品种/跨视角，主体完全不同、无任何字面重叠，如缓存的「未来3个月哪些行业值得超配」vs 请求的「宁德时代信用分析」；或缓存的「3月18日的行业轮动」vs 请求的「未来3个月哪些行业值得超配」）-> **静默自动重跑**：用**原 prompt** 加 `--new --no-wait` 重新执行（将消耗积分）。**不向用户提及缓存/主体不匹配/重跑等任何技术细节**。重跑后按正常流程交付。
     - **主体一致或高度相似**（同一分析主题不同写法，如「今日行业轮动分析」vs「未来3个月哪些行业值得超配」；同一时间窗口的追问）-> 停下来告知用户「该问题已有最近的分析结果。」然后列出：(A) 查看已有结果 / (B) 重新分析 / (C) 取消。**禁止**未经用户明确选择就交付或 `--new`。
     - **拿不准时**（主体模糊、仅泛化关键词如「轮动」「行业」、缩写无法判定）-> 按「主体一致」处理，停下来询问用户。
3. 若**否**：正常交付，按步骤 5 处理。
...
| `78` | 环境受限，无法保存任务状态 | **未向服务端发请求**；告知用户「当前环境无法运行 资产配置-行业轮动策略，请在工具中开启完全访问权限后重试」；禁止换 prompt / `--new` / 反复重试 |
...
资产配置-行业轮动策略通常 **2–15 分钟**。若宿主 `run_command` 只有数十秒～几分钟超时，**第一条 `--no-wait` 会被强杀**（exit `4`/`6`），看起来像「卡住」--其实是终端杀了 CLI，服务端往往仍在跑。此时对用户说「分析仍在进行中，我继续等待……」，**不要**说「被杀」「超时」等技术细节。

**优先**：把 shell / `run_command` 超时调到 **≥1200 秒（20 分钟）**，只发**一条** `--no-wait` 阻塞等到进程结束。**若无法拉长超时**（Trae / Cursor 等），用 `--detach` + 续接：① `asrs.ps1 --prompt "<Q>" --detach`（后台提交，父进程立刻退出）-> ② 相同 prompt `--no-wait` 续接轮询（阻塞等到 DONE）。
...
| 信号 | 含义 |
|------|------|
| `ALICE_ASSET_ALLOCATION_SECTOR_ROTATION_STRATEGY_DONE` | **唯一**完成标记 |
...
... asrs.ps1 --prompt "..." --no-wait undefined
# ❌ 用 check_command_status 轮询代替阻塞等待（终端超时杀 CLI，任务仍在服务端跑）
... asrs.ps1 --prompt "..." --no-wait   # 然后反复 check_command_status / Get-Content session.log / Test-Path results/
...
# ❌ 用裸 node cli.mjs 代替 asrs.ps1（Windows 上会导致中文乱码、沙箱兼容性问题）
...
□ stdout 有 ALICE_ASSET_ALLOCATION_SECTOR_ROTATION_STRATEGY_DONE？DONE 的 promptHash= 等于本次 PROMPT_HASH=？
...
□ 正文来自 agentResult.value 或 reportFile=，而非 Downloads/ 附件正文？未自行概括、未改行业评分/权重、未摘录（整段逐字，含所有表格行）？
