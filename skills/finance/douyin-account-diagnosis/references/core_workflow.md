...
2. 内置默认密钥缺失且未配置 → 引导用户前往 [红狐hub](https://redfox.hk/settings/api-keys?source=workbuddy) 获取并配置
3. 鉴权就绪 → 进入 Step 1
...
> ⚠️ **禁止输出 HTML 文件**。报告仅以 Markdown 格式输出：先在对话中完整展示，再将相同内容保存为 `.md` 文件交付用户。文件命名格式：`{账号}_健康诊断报告_{时间戳}.md`（时间戳格式：`YYYYMMDD_HHMMSS`，通过 `date +%Y%m%d_%H%M%S` 获取）。文件保存到当前工作目录下。
...
报告在对话中完整展示后，将相同内容保存为 Markdown 文件交付给用户：

1. 获取时间戳：执行 `date +%Y%m%d_%H%M%S`
2. 文件命名：`{账号昵称}_健康诊断报告_{时间戳}.md`（如 `今夜诡诡_健康诊断报告_20260805_154659.md`）
3. 保存路径：当前工作目录
4. 通过 `present_files` 工具将文件卡片交付给用户
5. **禁止生成 HTML 文件**
