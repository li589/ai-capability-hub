...
display_name: 抖音账号诊断
display_name_en: Douyin Account Diagnosis
description_zh: 一键诊断抖音账号运营健康度，从六个维度量化评估，输出100分制评分与优化建议
description_en: One-click Douyin account health diagnosis with six-dimension
  scoring and optimization advice
...
>
> 如需使用自己的额度，可前往 [红狐hub](https://redfox.hk/settings/api-keys?source=workbuddy) 获取 API KEY，并通过以下任一方式配置：
>
...
诊断完成后**直接在对话中展示**完整的 Markdown 结构化报告，同时将报告保存为 `.md` 文件交付给用户。文件命名格式：`{账号}_健康诊断报告_{时间戳}.md`（时间戳格式：`YYYYMMDD_HHMMSS`）。

> ⚠️ **禁止输出 HTML 文件**。报告仅以 Markdown 格式输出，对话中完整展示 + 生成 .md 文件卡片交付。
...
**Q：提示积分不足怎么办？**
A：前往 [红狐hub](https://redfox.hk/settings/api-keys?source=workbuddy) 充值后重试即可。
