...
display_name: A股公众号大V
display_name_en: A-Share WeChat KOLs
description_zh: 获取A股领域公众号大V账号30个及账号数据，含最新文章及阅读量
description_en: Fetch top 30 A-share WeChat KOL accounts with latest article data and read counts.
category: finance
version: 1.0.0
author: 红狐数据
---
...
> API 请求均携带 `A股公众号大V-WorkBuddy` 标识。使用环境变量 `REDFOX_API_KEY` 鉴权。
...
请前往 [红狐hub](https://redfox.hk/settings/api-keys?source=workbuddy) 获取 API Key。
