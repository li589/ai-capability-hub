...
> - `dnf install` / `yum install` / `dnf -y install`（安装软件包）
> - `dnf update` / `yum update` / `dnf upgrade` / `yum upgrade`（含 `-y`）
> - `dnf remove` / `yum remove` / `dnf erase` / `rpm -e`（卸载，可能依赖断裂）
> - `rpm -i` / `rpm -U` / `rpm -F` / `rpm --force --nodeps`
> - `dnf system-upgrade` / `dnf distro-sync`（跨大版本升级，破坏性极高）
> - `dnf history undo` / `rollback`（事务回滚可能引入新问题）
> - `dnf reinstall`（重装关键包可能影响运行中服务）
> - `pip install -U` / `npm install -g` 等语言级全局包安装
>
> AI 的唯一允许行为：**输出完整命令 + 风险提示 + 影响范围**（强调"升级关键服务包前需评估业务窗口、准备回滚 rpm 包"），由用户复制后手动执行。
