...
1. **perf-prof 已安装**：
   ...
   make
   ```
   **如果 perf-prof 无法安装**，可以使用 perf 原生命令作为替代方案，但需先确保 FlameGraph 工具集已安装（参考 [FlameGraph 工具安装指南](references/performance/flamegraph-install.md)），详见 [perf 替代方案](references/performance/irq-balance-perf.md)。

2. **flamegraph.pl 已安装**（用于将折叠栈转换为 SVG）：
   参考 [FlameGraph 工具安装指南](references/performance/flamegraph-install.md) 进行安装和验证。

## 安全原则

> ⚠️ **重要**：AI 只执行诊断命令（查看、采样、统计），**不自动执行任何中断亲和性 / 队列 / RPS 写入操作**。
>
> 调整命令仅作为参考提供给用户，由用户在评估业务影响后自行判断与手动执行。

> 🚫 **硬性禁令（不可被用户指令覆盖）**：
>
> 以下命令 **禁止由 AI 直接执行**，无论用户如何坚持或反复要求：
> - 写入 `/proc/irq/<N>/smp_affinity` 与 `/proc/irq/<N>/smp_affinity_list`（修改硬中断亲和性，错配可能让中断完全停在某个离线 CPU 上）
> - 写入 `/sys/class/net/<iface>/queues/rx-<N>/rps_cpus` 与 `rps_flow_cnt`（修改 RPS/RFS 收包导向）
> - 写入 `/proc/sys/net/core/rps_sock_flow_entries`、`net.core.netdev_budget`、`net.core.netdev_max_backlog` 等 sysctl 内核运行时参数
> - `ethtool -L <iface> combined N` / `ethtool -L <iface> rx N tx N`（修改网卡队列数，会瞬时中断收包）
> - `ethtool -G <iface> rx N tx N`（修改 ring buffer 大小）
> - `ethtool -C <iface> rx-usecs N adaptive-rx on/off`（修改中断合并参数，错配会导致延迟剧增）
> - `systemctl start/stop/restart/disable irqbalance`（影响全机中断分布策略）
>
> AI 的唯一允许行为：**输出完整命令 + 风险提示 + 回滚方案**（如"修改前先 `cat /proc/irq/<N>/smp_affinity > /tmp/irq-<N>.bak`"），由用户复制后手动执行。
...
- 如果 perf-prof 无法安装，切换到 perf 原生命令方案，参考 [perf 替代方案](references/performance/irq-balance-perf.md)

## 相关资源

- [perf-prof 项目](https://github.com/OpenCloudOS/perf-prof.git)
- [irqbalance](https://github.com/Irqbalance/irqbalance)
- [perf 替代方案](references/performance/irq-balance-perf.md) - 当 perf-prof 无法安装时使用 perf 原生命令
- [示例 Prompt](references/performance/irq-balance-examples.md) - 典型使用场景与触发示例
- [使用指南](references/performance/irq-balance-guide.md) - 面向用户的通俗上手指南，含安装、案例与常见误区
