...
> - `rm` / `rm -rf` / `rm -f`（任何删除文件或目录的操作）
> - `find ... -delete` / `find ... -exec rm` / `find ... -exec unlink`（批量删除）
> - `truncate -s 0` / `: > <file>` / `> <file>`（清空 / 截断已有文件，会破坏正在写入的日志）
> - `journalctl --vacuum-time` / `journalctl --vacuum-size` / `journalctl --vacuum-files`
> - `yum clean all` / `dnf clean all`（清空软件包缓存，下次 update 会全量重拉）
> - `package-cleanup --oldkernels`（删除旧内核，错配可能丢失唯一可启动镜像）
> - `systemctl restart <service>` 仅为"释放已删除文件占用"的目的执行（重启业务服务有副作用）
>
> AI 的唯一允许行为：**输出完整命令 + 风险提示 + 预览步骤**（如 `find ... -ls` 先列出再让用户确认 `-delete`），由用户复制后手动执行。
