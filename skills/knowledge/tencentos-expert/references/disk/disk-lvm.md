...
> - **PV 层**：`pvcreate` / `pvremove` / `pvresize` / `pvmove`
> - **VG 层**：`vgcreate` / `vgremove` / `vgreduce` / `vgextend` / `vgmerge` / `vgsplit`
> - **LV 层**：`lvcreate` / `lvremove` / `lvextend` / `lvreduce` / `lvresize` / `lvconvert` / `lvrename`
> - **快照与精简卷**：除查看外的快照写操作、`lvconvert --merge`
> - **配套破坏性操作**：`mkfs.*` 在 LV 上格式化、`resize2fs` / `xfs_growfs` 调整 fs 大小
>
> AI 的唯一允许行为：**输出完整命令 + 风险提示**（特别强调"`lvreduce` 缩容前必须先缩 fs，顺序错误会丢数据"），由用户复制后手动执行。
