...
> 💡 **推荐使用 nettrace 替代 perf 跟踪 kfree_skb**：`nettrace --drop` 可直接监控丢包并显示丢包原因，`nettrace --diag` 可自动诊断网络故障并给出修复建议。详见 `references/network/network-nettrace.md`。
