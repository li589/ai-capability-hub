...
| 用户问题匹配关键词 | 加载文件 | 说明 |
|-------------------|---------|------|
| 丢包、drop、packet loss、rx error、rx dropped、Ring Buffer、conntrack 满、ethtool -S 丢包、重传 | `network-packet-loss.md` | 丢包诊断完整流程 |
| 延迟、RTT、慢、latency、jitter、抖动、拥塞、带宽慢、队列延迟、传输慢 | `network-latency-diag.md` | 延迟诊断完整流程 |
| 同时涉及丢包和延迟（如"丢包且延迟高"） | 按**主要症状**先加载一个文件，另一个作为辅助参考 | — |

> **📂 文件加载指引**：
> - **丢包排查** → 请读取本文件同目录下的 **`network-packet-loss.md`** 文件，获取完整的丢包诊断流程（Phase 0 问诊 + Module 1~7 诊断步骤）。
> - **延迟排查** → 请读取本文件同目录下的 **`network-latency-diag.md`** 文件，获取完整的延迟诊断流程（Phase 0 问诊 + Module 1~6 诊断步骤）。
...
> - nettrace 的安装方式、BTF 兼容性、与传统工具的对比详见 `references/network/network-nettrace.md`
