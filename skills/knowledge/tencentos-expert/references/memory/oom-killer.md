...
├── SKILL.md                          # 本文件
├── references/
│   ├── oom-kernel-patterns.md        # 内核 OOM 日志特征库 & 根因分类
│   └── oom-metrics-guide.md          # 内存相关监控指标清单（外部参考）
├── scripts/
...
```bash
python3 <SKILL_DIR>/scripts/parse_oom_events.py \
  --log-file /path/to/messages \
  --output /tmp/oom_events.json --pretty
```

解析重点参考 `references/memory/oom-kernel-patterns.md` 中的正则模式和字段说明。
...
结合解析出的 OOM 事件字段，按根因分类树判定（详见 `references/memory/oom-kernel-patterns.md` 第二节）：
...
置信度评估规则参见 `references/memory/oom-kernel-patterns.md` 第二节各类型的"置信度规则"。
