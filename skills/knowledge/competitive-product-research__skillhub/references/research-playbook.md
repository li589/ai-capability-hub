...
- **四层流程**：信息采集 → 问题对齐 → 证据建库 → 双轨分析 → 决策输出（HTML / Markdown）
- **双轨分工**：
  - 体验对标轨（八维）：回答"怎么做、差在哪"
...
**交互形态优先级**：必须复用 `assets/intake-form.html`；宿主支持时内嵌，否则在浏览器打开。只有 HTML 无法使用时才退回文本清单。表单覆盖全部 10 项，输出格式不得预选；提交后的结构化参数才是确认结果。

**最低可启动输入**：调研目标 + 至少 1 个明确竞品。只有 1 个竞品时，先询问是否补第 2 个；用户跳过时，可用我方方案、行业常见做法或历史版本作为参照，并在报告中标注限制。
...
|---------|---------|
| 逐项补充 | 更新所有字段后开始分析 |
| "跳过，直接生成" | 跳过业务信息补充，用推断值启动；若格式未明确，仍须单独确认 HTML / Markdown |
| 只补部分 | 已补字段用实值，其余用推断值 |
| 反问/要求调整 | 修改采集清单后重新确认 |
| 只给 1 个竞品 | 询问补充第 2 个；若跳过，则降级为单竞品对照 |
| 只给截图/链接 | 先建立 Source Index，再补调研参数 |
...
- 只有 1 个竞品时可以启动，但报告标题区必须标注"单竞品对照"
...
- 网上检索只可作为线索，不得把搜索摘要直接当事实；关键事实优先交叉核对官网、年报、公开文档、权威媒体或用户材料
- 区分三类表述：已核实、官方口径、推断/行业常识；推断不得伪装成已核实事实
- 若对标物是社区方案、示意图、多产品组合，必须写明“非单一 SKU”或“无统一官网定价”，避免与正式产品作不对等比较
- HTML 报告中，所有结构化 Evidence 字段里的公开来源 `SRC-Pxxx` 必须使用 `<a>` 直接链接其来源 URL，并设置 `target="_blank" rel="noopener"`
- 一个 Evidence 单元格引用多个来源时，每个 `SRC-xxx` 必须拆成独立标签；不得把 `P002/U001` 等多个编号合并为一个不可点击 pill
- 没有外部 URL 的 `SRC-U`、`SRC-H`、`SRC-GAP` 使用页内锚点跳转到 Source Index 对应条目；Source Index 条目必须有与证据编号一致的 `id`
...
- 每条结论挂 `SRC-xxx` 证据编号
- HTML 中所有 Evidence 标签可点击：公开来源直达外部 URL，无外链来源跳到 Source Index；链接文本保留完整 `SRC-xxx`
...
- 单段以 3-5 行为上限；三方/多方对比优先用一张总表呈现核心差异
...
- 是否避免体验轨与策略轨重复
- 对外版是否完成脱敏
- 每条带数字的陈述是否能在 Source Index 找到对应来源
- 是否删除了无法核实的夸张对比
...
7. 链接一致：HTML 的公开证据标签均可直接打开来源 URL；多来源已拆分；无 URL 来源可跳到 Source Index
