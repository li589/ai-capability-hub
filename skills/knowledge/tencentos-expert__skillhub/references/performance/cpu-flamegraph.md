...
1. **perf-prof 已安装**：
   ...
   make
   ```
   **如果 perf-prof 无法安装**，可以使用 perf 原生命令作为替代方案，但需先确保 FlameGraph 工具集已安装（参考 [FlameGraph 工具安装指南](references/performance/flamegraph-install.md)），详见 [perf 替代方案](references/performance/cpu-flamegraph-perf.md)。
...
- 如果 perf-prof 无法安装，切换到 perf 原生命令方案（需先安装 FlameGraph 工具集，参考 [FlameGraph 工具安装指南](references/performance/flamegraph-install.md)），参考 [perf 替代方案](references/performance/cpu-flamegraph-perf.md)

## 相关资源

- [FlameGraph 工具集](https://github.com/brendangregg/FlameGraph)
- [perf-prof 项目](https://github.com/OpenCloudOS/perf-prof.git)
- [perf 替代方案](references/performance/cpu-flamegraph-perf.md) - 当 perf-prof 无法安装时使用 perf 原生命令
- [示例 Prompt](references/performance/cpu-flamegraph-examples.md) - 典型使用场景与触发示例
- [使用指南](references/performance/cpu-flamegraph-guide.md) - 面向用户的通俗上手指南，含安装、案例与常见误区
