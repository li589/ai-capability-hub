...
> - `systemctl restart sshd` / `stop sshd`（断 SSH 通道，远程操作直接失联）
> - `systemctl restart NetworkManager` / `network`（瞬断网络，可能导致连接失败）
> - `systemctl restart firewalld` / `iptables`（防火墙规则瞬时丢失风险）
> - `systemctl disable` / `mask`（禁用/屏蔽服务，可能导致系统功能丢失）
> - `systemctl daemon-reload`（reload 异常会影响所有 service）
> - `systemctl enable` 修改开机自启状态
> - `systemctl start/stop/restart` 任何业务关键服务（mysql/nginx/docker/kubelet 等）
> - 覆写 `/etc/systemd/system/*.service` / `/usr/lib/systemd/system/*.service`
>
> AI 的唯一允许行为：**输出完整命令 + 风险提示 + 回滚方案**（强调"重启 sshd/NetworkManager 前必须确认 console/VNC 通道可用"），由用户复制后手动执行。
