...
> - `date -s`（直接设置系统时间，业务断点）
> - `hwclock --systohc` / `hwclock -w` / `hwclock --hctosys`（硬件时钟读写）
> - `timedatectl set-time` / `set-timezone` / `set-ntp false`
> - 覆写 `/etc/chrony.conf` / `/etc/ntp.conf`（NTP 源/makestep/允许网段配置）
> - 覆写 `/etc/sysconfig/chronyd` / `/etc/sysconfig/ntpd`
> - `systemctl restart chronyd` / `ntpd` / `systemd-timesyncd`
> - `chronyc makestep` / `chronyc -a 'burst 4/4'`（强制时间跳变）
>
> AI 的唯一允许行为：**输出完整命令 + 风险提示**（强调"生产环境时间跳变必须先评估对数据库/证书/Kerberos/cron 的影响"），由用户复制后手动执行。
