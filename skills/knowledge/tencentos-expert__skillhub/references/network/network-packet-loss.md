...
> 本模块使用 nettrace 进行丢包诊断。nettrace 是基于 eBPF 的集网络报文跟踪、网络故障诊断、网络异常监控于一体的网络工具集。详细的工具介绍、能力对比参见 `references/network/network-nettrace.md`。
>
...
> 完整的安装流程（包管理器安装、源码编译、BTF 判断、备选安装路径）参见 `references/network/network-nettrace.md` 安装章节。

**必须先询问用户确认**是否可以安装 nettrace。

**情况 A：用户同意安装** → 按照 `references/network/network-nettrace.md` 安装章节依次执行，每一步都必须检查是否成功：
...
> nettrace 需要 root 权限运行。完整的命令参数说明参见 `references/network/network-nettrace.md`。
>
> ⚠️ **AI 执行约束**（详见 `references/network/network-nettrace.md` AI 执行约束章节）：
...
**输出解读**：每行显示一个丢包事件，包含协议、源/目的地址端口、丢包原因（reason）和丢包位置（内核函数）。常见丢包原因及完整列表参见 `references/network/network-nettrace.md` 丢包监控章节。
...
**输出解读**：诊断模式提供 INFO/WARN/ERROR 三种级别提示，诊断结果（`ANALYSIS RESULT`）会列出所有异常事件并给出 `fix advice`（修复建议）。详细的输出格式和特性说明参见 `references/network/network-nettrace.md` 故障诊断模式章节。
...
**输出解读**：每行显示报文经过的内核函数和时间戳。`kfree_skb` 表示报文被丢弃，`consume_skb` 表示正常消费。支持 NAT 场景持续跟踪。更多参数和用法参见 `references/network/network-nettrace.md` 报文生命周期跟踪章节。
...
> 📎 FAQ（Q1~Q6）、操作参考（Ring Buffer/协商/流控/内核参数/中断亲和性/MTU/tcpdump）和命令速查表（23 条）请参见 **`references/network/network-faq-packet-loss.md`**。
