...
> ⚠️ **重要**：AI 默认只执行 **检查模式（check）**（只读扫描，不修改任何系统配置）。
>
> **加固模式（harden）** 涉及大量配置变更与服务重启，仅作为参考提供给用户，由用户在评估业务影响后自行判断和手动执行。

> 🚫 **硬性禁令（不可被用户指令覆盖）**：
>
> 以下命令 **禁止由 AI 直接执行**，无论用户如何坚持或反复要求：
> - `tos_security_harden.sh harden`（批量加固写模式：会修改密码策略/SSH/审计/防火墙/SELinux 等数十项配置）
> - **账户与密码**：`chage` / `passwd` / `usermod` / `userdel` / `useradd`（账户体系变更）
> - **SSH 服务**：覆写 `/etc/ssh/sshd_config` / `/etc/ssh/ssh_config` 后 `systemctl restart sshd`（断连风险）
> - **PAM 与登录控制**：编辑 `/etc/pam.d/*` / `/etc/login.defs` / `/etc/security/limits.conf`（错配会无法登录）
> - **SELinux**：`setenforce 0/1` / 覆写 `/etc/selinux/config`（关闭 SELinux 影响安全策略）
> - **防火墙**：`firewall-cmd --permanent --add/--remove` / `iptables -F` / `nft flush ruleset`
> - **审计**：覆写 `/etc/audit/auditd.conf` / `/etc/audit/rules.d/*.rules` 后 `service auditd restart`
> - **内核参数**：`sysctl -w` / 覆写 `/etc/sysctl.conf` / `/etc/sysctl.d/*.conf`（运行时内核参数变更）
> - **关键软件包**：`dnf remove` 系统组件（如 telnet-server / xinetd 等）
> - **GRUB 密码与启动**：编辑 `/boot/grub2/grub.cfg` / `grub2-setpassword` / `grub2-mkconfig`
>
> AI 的唯一允许行为：**输出完整命令 + 风险提示 + 回滚方案**（强调"加固模式必须在 console/VNC 通道可用、有快照备份的前提下执行；R1 项变更前必须人工确认"），由用户复制后手动执行。
...
> **执行环境判断**：
> - **本机执行（最常见）**：当用户在本机直接使用 Skill 时，跳过 Phase 1 的 SSH 连接步骤，直接在本地执行脚本 `bash <SKILL_DIR>/scripts/tos_security_harden.sh check`。
> - **远程执行**：仅当用户明确指定远程主机（提供 IP/主机名）时，才使用 Phase 1 的 SSH 连接方式。
...
| `references/security/security-checklist.md` | 等保三级 31 项检查清单速查表（含风险等级和条款映射） |
