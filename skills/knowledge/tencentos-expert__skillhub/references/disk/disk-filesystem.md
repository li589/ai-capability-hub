...
> - `mkfs.*`（任何格式化：mkfs.ext4 / mkfs.xfs / mkfs.btrfs ...）
> - `fsck` / `e2fsck` / `xfs_repair`（含 `-L` 丢弃日志选项，可能丢数据）
> - `tune2fs` / `resize2fs` / `xfs_growfs` / `xfs_admin`（文件系统元数据写入）
> - 编辑 `/etc/fstab`（任何 sed/cat>/echo>> 写入：错配可能导致系统无法启动）
> - `mount -a` 在 fstab 改动后立即生效的操作
>
> AI 的唯一允许行为：**输出完整命令 + 风险提示 + 回滚方案**（如 fstab 改动需提供 `cp /etc/fstab /etc/fstab.bak` 备份步骤），由用户复制后手动执行。
