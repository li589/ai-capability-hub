...
> - `echo c > /proc/sysrq-trigger`（强制内核 panic，业务立即中断）
> - `kexec -e` / `kexec -l`（直接切换运行内核）
> - 编辑 `/etc/default/grub` 改 `crashkernel=` 参数 + `grub2-mkconfig` / `update-grub`
> - `grubby --update-kernel=ALL --args="..."`（持久化内核命令行）
> - `dracut -f` / `update-initramfs -u`（重建 initramfs，错误会无法启动）
> - 编辑 `/etc/kdump.conf` 后 `systemctl restart kdump`（重启会丢失下一次 panic 捕获机会）
>
> AI 的唯一允许行为：**输出完整命令 + 风险提示 + 验证步骤**（强调"任何 grub/initramfs 修改都需 console/VNC 通道可用，避免改完无法启动"），由用户复制后手动执行。
