...
Use this recipe only when the requested deliverable is a design system, style
guide, token catalog, or component specimen. Do not use it merely because the
source is styled HTML, a screenshot, a mockup, or a product prototype. When the
user asks to recreate such a source and preserve its style, preserve the source
page, app, or workflow shape instead.
...
- Prefer `Stack`, `Row`, `Grid`, `H1`, `H2`, `H3`, `Text`, `Card`,
  ...
  `useHostTheme`.
- Use `style` for static root overrides and a declared `className` for scoped
  selectors, pseudo states, or media queries.
- Use native `main`, `nav`, `section`, `div`, `h1`, `h2`, `button`, `input`,
  and `textarea` when exact source markup or nested styling is not exposed by a
  primitive.
